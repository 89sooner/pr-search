...
> 상태: review | 버전: v6.24 | 갱신일: 2026-09-01
...
| DEV-522 | 2026-09-01 | **한 상태 칸이 `open`으로 시작해 `resolved`로 끝났다.** `DEV-502`를 닫으며 판정을 **덧붙이기만 하고** 칸 머리의 `open`을 고치지 않았다. 그러면 사람은 끝을 읽고 `resolved`로 보는데 **계수 명령(`grep '| open'`)은 머리를 읽어 open으로 센다** — 이 세션의 최종 실측이 실제로 16건을 답했고 참값은 15건이었다. `CR-058`이 `DEV-477`에서 세운 것과 같은 종류다: **정본 표에 없는 것은 아무도 세지 않고, 두 가지를 말하는 칸은 세는 쪽과 읽는 쪽이 갈린다** | WP-070 / DEV-502 | 문서 편차 | 불필요 (설계 변경 없음) | **resolved (2026-09-01)** — 칸 머리를 `resolved`로 바꾸고 당시 기록은 "당시 기록:"으로 표시해 **보존했다** — 역사를 지우지 않는다. 같은 모순이 다른 행에 없다는 것을 전수로 확인했다(`grep -E '^\| DEV-[0-9]{3} .*\| open .*resolved'` 0건) |
