...
| WP-080 | REL-007 R2 — 검증된 레지스트리 운영 승인·R0 실행 정책 | REL-007 (**CR-090**) | **in_progress** | 에이전트 | feature/rel007-registry-approval | 6.87장 — gh-cli 단위(판정·자격·해석기·신선도 22), DB 통합(권한 우회·승인·근거·차단·감사 롤백·동시성·가드·030 왕복 22), 실행기 흐름 통합(실제 gh 2.97.0·HTTPS 목 GHE, claim 경합·정책 확인 불가 포함 14), search-api HTTP 통합 7, web lib·a11y QA-GH-47~49·Chromium e2e 5, 회귀 CR-090 셋, 변이 26, 이미지 다섯·smoke, 옛 앱 롤백 실측 | 운영 승인·철회(A-006)와 `pr.list` 차단·재개(A-005 최소), 배포 범위별 정책 revision·append-only 이력·같은 트랜잭션 감사, 요청 수락과 claim의 같은 판정·수락 revision 비승계·claim 가드(030), `DEV-685` 종결. 새로 실행 가능한 명령 0 — 실행 허용 `pr.list` 하나. 사내 GHES `NOT RUN`(`DEV-674`) |
...
예상과 달랐던 점: `FOR SHARE`에 UPDATE 권한이 필요해 행 잠금 설계를 버렸다 · `prs_app`이 TEMP 표를 만들 수 있었다 · 풀 연결 세션에 남은 TEMP 표가 뒤 시험의 한정하지 않은 조회를 가로챘다(`ON COMMIT DROP`으로 고쳤다) · 실제 프록시가 중복 방지 키를 버리고 있었다(`DEV-690`) · 문서의 CSRF 토큰이 코드에 없었다(`DEV-691`) · CR-088 회귀가 옛 배선을 문자열로 걸었다 · 조립 시험의 인증 누락이 변이를 살렸다(T14).
