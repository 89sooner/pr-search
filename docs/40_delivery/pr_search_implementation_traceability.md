...
> 상태: review | 버전: v6.66 | 갱신일: 2026-09-11
...
| 단위 | 2,193 통과 · 1 skip (122 파일) | 기준선 2,099에서 94건 증가 |
| 회귀 | 427 통과 (8 파일) | |
| 통합 | 1,608 통과 (100 파일) | 실제 PostgreSQL·Redis. 기준선 1,602에서 6건 증가 |
| 접근성 | 379 통과 (17 파일) | |
| e2e | 180 통과 · **1 실패** | `flow-003.spec.ts`의 `DEV-377` — 병렬 부하에서 렌더가 늦는 알려진 편차다. 단독 실행은 14건 전부 통과했다 |
| 빌드 | 통과 | |
...
**변이 시험.** 새 시험이 실제로 무엇을 지키는지 네 곳에서 되돌려 확인했다. **열여섯이 전부 kill됐다.**
