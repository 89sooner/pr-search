...
> 상태: review | 버전: v3.5 | 갱신일: 2026-08-27
...
| WP-035 | 무중단 재색인 | REL-004 | done | 에이전트 | PR #52 | DoD 21항 통과 (6.39장). 통합 10건(실 PG·ES) · 회귀 +30 · 단위 +13 · 핵심 변이 6종 킬. **WP-032 선행 조건 증명 완료** | FR-ING-008 |
...
| DEV-306 | 2026-08-26 | **`authz` 역할이 배포되지 않는다 — 접근 통제 축이다.** `startAuthzWorker`(JOB-AUTH-001, WP-012 / FR-AUTH-003)를 세우는 갈래가 있는데 manifest가 없다. 그 워커가 `prs:permission`을 소비해 `EVT-AUTH-001`을 실제 사용자 집합으로 펼치고 캐시를 무효화한다. 배포되지 않으면 **팀에서 회수된 사용자의 권한 캐시가 무효화되지 않고** `access_scope_version`도 오르지 않아, TTL이 만료될 때까지 회수된 범위로 계속 조회한다 | WP-012 / FR-AUTH-003 AC-2, JOB-AUTH-001, EVT-AUTH-001 | 구현 결함 | - | open (2026-08-26) — **이 CR에서 고치지 않는다.** CR-035·036이 같은 축(팀 접근 범위)에서 유출을 고쳤고, 이것은 그 결정들이 실제로 반영되는 경로다 — **별도 CR로 위협 모델과 함께 다루는 것이 옳다.** TTL 만료로 결국 갱신되므로 영구 유출은 아니지만 FR-AUTH-003 AC-2가 승인한 것은 **이벤트 기반 즉시 무효화**다. 회귀 예외 목록에 등재해 다음 사람이 반드시 보게 했다 |
...
**WP-035의 본체는 구현하지 않았다.** 이 CR이 한 것은 계약 경화와, 감사가 드러낸 **배포 도달성 결함의 정정**이다. WP-035는 여전히 `todo`이며 REL-004 진행률도 그대로 **3/8**이다.
...
**다음은 WP-035 구현이다.** 계약은 6.38장과 비동기 계약 3.5장에 서 있고, 러너가 살 자리(`batch` 배포)도 생겼다. 남은 것: 이중 쓰기 seam(열일곱 경로) · 버전 인덱스 생성 · 원자 별칭 전환 · 정본 재구축 · JOB-ING-006 러너 · API-ADM-004 라우트 · CLI · 보관 스윕 · 적대적 변이.

**그리고 별도 CR이 셋 열려 있다** — 감사가 드러낸 미배포 역할이다. `authz`(DEV-306, 권한 캐시 무효화 — **접근 통제 축**), `release`(DEV-305, 미러 PVC 배치가 함께 필요), `backfill`(DEV-304, `enrich` 파드의 역할 구성). 셋 다 **이 WP의 소산이 아니고** 각자 다른 WP가 만든 기능이 배포에서 죽어 있는 것이다. 운영 도달성 회귀의 예외 목록에 사유·DEV와 함께 등재해 다음 사람이 반드시 보게 했다.
