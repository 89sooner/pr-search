...
5. ES 컨테이너 3개의 Docker host 배치 확정 — 마스터 자격 노드 3개가 서로 다른 호스트에 놓이는지 확인 (인프라 4.1장). CPU·RAM·디스크 사양과 호스트 수는 CR-004에서 결정하지 않았다
6. WP-001부터 순서대로 착수. WP-001 완료 후 WP-002·WP-003·WP-005·WP-006은 병렬 가능
