...
> 상태: review | 버전: v2.1 | 갱신일: 2026-08-26
...
| REL-001 | - | - | - | - | - |
| REL-002 | - | - | - | - | - |
| REL-003 | **미통과 (2026-08-26 판정)** | **NOT RUN** — 1000만 문서 합성 데이터셋도 `test:perf` 스크립트도 이 환경에 없다 (DEV-058). 예산을 지키는 **구조**만 검증됐다 | **부분** — 역할 6종 × 역할 요구 경로는 API 계층에서 통과. 화면 13종 축은 화면이 다 서지 않아 걸 수 없다 | **부분** — 시퀀스 정확성(git 대조)은 회귀 계층이 매 실행 판정한다. ACC-06(관계 간선 정확도)은 REL-004 소관이라 아직 대상이 없다 | 성능 미측정, 실제 GHE·Kubernetes 미실행, `flow-001` 로컬 간헐 실패 |
| REL-004 | - | - | - | - | - |
| REL-005 | - | - | - | - | - |
| REL-006 | - | - | - | - | - |

#### REL-003 마감 판정 (2026-08-26)
...
**REL-004 진입 조건.** WP 기준으로는 열렸다. 다만 **릴리스 게이트가 통과하지 않은 상태이므로 베타 공개(검증 계획 §배포 단계)는 이 판정으로 승인되지 않는다** — 성능 측정과 실환경 smoke가 남아 있다.
...
**REL-003의 WP가 전부 닫혔다** (WP-020~028, WP-067, WP-068). 릴리스 게이트는 6.31장에서 **미통과로 판정했다** — 성능(QA-PERF-01~10)이 측정된 적이 없고(DEV-058) 실제 GHE·Kubernetes smoke가 미실행이다. WP 완료와 게이트 통과는 다른 판정이며 하나가 다른 하나를 대신하지 않는다.
