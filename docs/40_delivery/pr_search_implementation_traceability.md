...
| DEV-068 | 2026-08-21 | **`Role` 타입이 클라이언트에 닿지 않는다.** C-002가 `roles: Role[]`을 요구하는데 `@prs/authz`의 진입점이 `@prs/es`(`applyMandatoryScopeFilter`)를 재수출한다. 클라이언트 컴포넌트에서 import하면 Node 전용 `@elastic/elasticsearch`가 브라우저 번들로 끌려와 빌드가 깨진다 | WP-015 / C-002, NFR-005 | 범위 공백 | **CR-018** | **resolved (2026-08-21)** — `roles.ts`는 import가 **0개**라 그 자체로 클라이언트 안전하다. **서브패스 export `@prs/authz/roles`**를 열어 그것만 가져간다. web에 역할 목록을 복제하지 않는다 — 역할이 늘면 두 곳이 갈라지고, 갈라진 쪽이 운영 내비게이션을 잘못 거른다 |
