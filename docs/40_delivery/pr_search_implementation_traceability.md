...
`WP-075` 구현: PR 제목 M 넘버 표기(`FR-SEQ-009`)가 들어왔다 — **이 제품이 사람의 지시 없이 GHE를 고치는 최초의 경로다**(`CR-084`, `ADR-022`). 전역 스위치 `MNUMBER_ANNOTATE_ENABLED`의 기본값이 **꺼짐**이라 이 변경을 받는 것만으로는 아무 PR 제목도 바뀌지 않는다. 검증은 6.81장이며 **사내 실제 GHE 표기는 `NOT RUN`이다** — 외부에 GHE가 없어 쓰기 흐름 전부를 로컬 HTTP 목으로 검증했고, public 저장소의 PR 제목도 건드리지 않았다.

`0.1.0-pilot.5` 발행: 사내 요청 두 건(`CR-082` git CA · `CR-083` GHE 직접 로그인)을 담아 발행했다. 태그는 `2795666`을 가리키고 자산 SHA-256은 `aa83abf9…6afd`이며 `immutable`이라 잠겼다. 값과 사내 전달 사항은 6.80장이다. **사내 반입과 실제 GHE OAuth App 검증은 `NOT RUN`이다.**

CR-083 후속(`DEV-615`): **계약을 바꾸면서 그 계약을 강제하는 자리 하나를 놓쳤다.** 릴리스 산출물 게이트(`smoke-images.sh`)가 옛 기대를 들고 있어 번들 빌드가 「이 번들을 반입하지 않는다」로 멈췄고 `0.1.0-pilot.5`가 나가지 못했다. **그때 회귀는 초록이었다** — 검사의 존재를 문자열로만 보았기 때문이다. 게이트의 기대를 실제 계약 함수에 넣어 대조하는 시험을 세웠고, 옛 기대로 되돌리는 변이가 그 시험을 죽이는 것을 확인했다. 검증은 6.79장이다.

CR-083 독립 검토 기록: 작성과 검토를 나눠 두 판을 돌렸고 **blocker 1건**을 찾았다 — `DEV-614`. 세션 쿠키가 응답에서 유실되어 **로그인에 성공해도 브라우저가 세션을 받지 못했다.** 이 결함은 `CR-083` 이전부터 있었고 OIDC 경로도 같았다. 콜백에 라우트 시험이 없어 드러나지 않았을 뿐이며, 검토가 그 공백을 지적해 시험을 세우자마자 첫 실행에서 잡혔다. 그 밖에 major 셋(콜백 무로그, 신원 키 네임스페이스 충돌, 조립 배선 무검증)을 닫았다.

CR-083 구현 기록: 사내 GHE 계정으로 직접 로그인하는 경로를 열었다(`AUTH_PROVIDER=github`). 기존 OIDC 배포는 바뀌지 않는다. 함께 닫은 것이 둘이다 — `.env.example`의 그룹 매핑 안내가 파서와 어긋나 운영자를 거부당하는 값으로 보내던 것(`DEV-612`), 그리고 **로그인이 `app_user` 행을 만들지 않아 로그인 직후 첫 조회가 503이 되던 것**(`DEV-613`)이다. 뒤의 것은 OIDC 경로에도 있던 기존 결함이며, 닫지 않으면 이 CR이 여는 경로가 성립하지 않는다. 검증 결과는 6.78장이고 **실제 GHE OAuth App을 쓰는 사내 검증은 `NOT RUN`이다.**

CR-082 구현 기록: 사내 `0.1.0-pilot.4` 반입이 보고한 `DEV-561`을 닫았다. `git` 서브프로세스가 사내 CA를 신뢰하지 못해 `JOB-MIR-001`이 실패하던 것을 배포 정의에서 고쳤고, 회귀가 **미러를 다루는 서비스마다 최종 환경 키 집합을 계산해** 그 값이 실제로 닿는지 묻는다. 검증 결과는 6.77장이며 사내 재적용은 `NOT RUN`이다.
...
| WP-042 | 이분 탐색 보조 | REL-006 | done | 에이전트 | fix/pr150-bisect-export | 통합 10 · a11y 2 · E2E 3 통과 | CR-071 / DEV-560 / 6.74장 |
| WP-043 | 관계 그래프 API와 W-007 | REL-006 | todo | - | - | - | 조건부 — **착수하지 않는다.** ACC-06이 NOT RUN이라 활성화 조건이 성립하지 않는다 (6.46장) |
| WP-044 | 검색 결과 내보내기 | REL-006 | done | 에이전트 | fix/pr150-bisect-export | ES 단위 11 · 통합 6 · a11y 1 · E2E 3 통과 | CR-072 / DEV-570 / 6.75장 |
...
| WP-045 | gh capability 레지스트리와 parity 검증기 | REL-007 (CR-086 · CR-088 · CR-089 · CR-090 부분) | **in_progress** | 에이전트 | #181 · c5c8aea / #184 · a996540 / #186 · 99d53d3 / #188 · 9c8a781 | 6.83장·6.85장 — 인벤토리·manifest r0.2·분류 표·규칙·검증기(단위 110, 변이 10)·드리프트(통합 4, 비동기 판 포함)·029 · 6.86장 — 결과 계약 차원(`GATE-GH-01d`) · 6.87장 — 검증 기록의 배포 범위·보고서 판 해석기 | 들어온 것: 인벤토리·manifest(`WP-078` 당시 `r0.2`, 지금 `r0.3`)·`API-GH-001`, leaf 196·flag 1,034·positional 164·`--json` 707 분류(`NFR-009` 본표 100%), `gh:validate-capabilities`·`gh:diff-capabilities`·`gh:inventory`, `gh_capability_snapshot`, CI 게이트(단위 시험 + integration 드리프트 시험), `GATE-GH-01d` 통과(`CR-089`, `DEV-675` 종결), 검증 기록의 배포 범위와 등록된 보고서 판 해석기(`CR-090`). 남은 것: 호스트 지원 판정(`DEV-674`)·parity 최종(WP-060). 이 행은 `CR-089` 반영에서 빠져 있었다(`DEV-693`) |
...
| WP-047 | 격리 gh 실행기와 실행 수명주기 | REL-007 (CR-086 · CR-090 부분) | **in_progress** | 에이전트 | #181 · c5c8aea / #188 · 9c8a781 | 6.83장 — 실행기 통합 10/10(실제 gh + HTTPS 목), spawn 단위, smoke 6절 · 6.87장 — 운영 정책 흐름 통합(claim 경합·정책 확인 불가 포함) | 들어온 것: `gh-executor`(spawn·재검증·argv 대조·취소·상한·하트비트·고아 회수·잔여 스윕·헬스)·`prs:gh:executions`·028, 큐의 운영 정책 판정과 정책 공유 잠금 claim·claim 가드 030(`CR-090`). 남은 것: 잠금·승인 전이·출력 스트리밍(`DEV-651`) |
| WP-048 | W-010 GitHub Command Center 수직 슬라이스 | REL-007 (CR-086 · CR-090 부분) | **in_progress** | 에이전트 | #181 · c5c8aea / #188 · 9c8a781 | 6.83장 — lib/gh 41·a11y 24·콜백 라우트 8·e2e 8 · 6.87장 — W-010 실행 판정 표시(a11y QA-GH-49·e2e) | 들어온 것: W-010 최소·W-021 최소(`WP-077`), W-010 실행 판정 표시(`CR-090`). 남은 것: 생성형 폼 일반화·Recipe·A-007 (A-006은 `WP-078`~`WP-080`이 들였다) |
...
| WP-059 | capability 드리프트와 정책 관리 (A-005, A-006) | REL-011 (CR-088 · CR-090 부분) | **in_progress** | 에이전트 | #184 · a996540 / #188 · 9c8a781 | 6.85장 — A-006 a11y 8·e2e 2, registry 통합 6, 실행기 registry-check 통합 5 · 6.87장 — 운영 승인·차단 (WP-080) | 들어온 것: A-006 읽기(`/ops/gh-registry`, `operator`·`security_officer`), 드리프트 → `registry_stale` 거절(`CR-088`), 운영 승인·철회와 스냅숏 활성화·A-005 최소(`/ops/gh-policy`, `pr.list` 차단·재개)·정책 변경 감사(같은 트랜잭션)·`admin_action_required` 흐름(`CR-090`, `DEV-685` 종결). 남은 것: A-005 나머지(허용 목록 편집·위험도 재정의·승인 지정·엔드포인트·확장 허용 목록과 그 실행)·A-006의 GHES 버전·parity 최종(WP-060) |
...
| WP-080 | REL-007 R2 — 검증된 레지스트리 운영 승인·R0 실행 정책 | REL-007 (**CR-090**) | **done** | 에이전트 | #188 · 9c8a781 | 6.87장 — gh-cli 단위(판정·자격·해석기·신선도 23), DB 통합(권한 우회·승인·근거·차단·감사 롤백·동시성·가드·030 왕복 23), 실행기 흐름 통합(실제 gh 2.97.0·HTTPS 목 GHE, claim 경합·정책 확인 불가·잠금 대기 초과 포함 15), search-api HTTP 통합 8, web lib·a11y QA-GH-47~49·Chromium e2e 5, 회귀 CR-090 셋, 변이 31, 필수 배터리(최종 `9bb981d` 전 단계 통과), 독립 검토 두 관점과 재검토(blocker·major 0), 이미지 다섯·smoke(검토 반영 뒤 재검증), 옛 앱 롤백·백업 복원 실측 | 운영 승인·철회(A-006)와 `pr.list` 차단·재개(A-005 최소), 배포 범위별 정책 revision·append-only 이력·같은 트랜잭션 감사, 요청 수락과 claim의 같은 판정·수락 revision 비승계·claim 가드(030), `DEV-685` 종결. 새로 실행 가능한 명령 0 — 실행 허용 `pr.list` 하나. 사내 GHES `NOT RUN`(`DEV-674`) |
...
| FR-SRCH-012 | WP-044 | `apps/search-api/src/export/routes.ts`, `apps/pipeline-worker/src/export.ts`, `packages/es/src/export.ts`, `packages/db/src/repositories/search-export.ts`, `apps/web/components/ExportDialog.tsx` | ES 단위 11, 실 PG·ES·Redis 통합 6, a11y 1, E2E 3 | verified |
...
| FR-SEQ-007 | WP-042 | `packages/db/src/repositories/bisect-session.ts`, `apps/search-api/src/sequence/routes.ts`, `apps/web/components/BisectPanel.tsx` | 실 PG·Redis 통합 10, a11y 2, E2E 3 | verified |
...
| FR-AUTH-001 | WP-012, WP-015 | `packages/authz/src/{oidc,pkce,id-token,jwks,session,session-store,roles,config}.ts`, `apps/search-api/src/auth/*`, `apps/web/app/auth/{login,callback,logout}/route.ts`, `apps/web/app/api/[...path]/route.ts`, `apps/web/lib/{oidc-state,proxy}.ts`, **CR-091**: `apps/search-api/src/auth/{registration,role-command}.ts`·`src/role-cli.ts`(실효 역할·관리자 지정), `apps/web/lib/server/{effective-roles,config,page-guard}.ts`·`apps/web/instrumentation.ts`, `deploy/single-host/prsctl`(`role`·`auth_token_conflict`), **CR-092**: `apps/web/lib/{redirect,auth-paths}.ts`(경로 리다이렉트), `apps/web/app/auth/{callback,logout,signed-out}`·`apps/web/app/gh/identity/callback/route.ts`, `apps/web/components/{UserMenu,SignedOutView,AppTopBar}.tsx` | `packages/authz/src/{id-token,oidc,session,session-store,roles}.test.ts`, `apps/search-api/integration/authz/enforcement.test.ts`, `apps/web/lib/{oidc-state,proxy}.test.ts`, `apps/web/e2e/shell.spec.ts`, **CR-091**: `packages/authz/src/config.test.ts`·`apps/search-api/src/auth/registration.test.ts`·`integration/authz/{assigned-roles,role-command}.test.ts`·`apps/web/lib/server/effective-roles.test.ts`·`apps/web/app/auth/callback/route.test.ts`·`regression/cr091-auth-feedback.test.ts`, **CR-092**: `apps/web/lib/{redirect,architecture}.test.ts`·`apps/web/app/auth/{callback,logout}/route.test.ts`·`apps/web/app/gh/identity/callback/route.test.ts`·`apps/web/a11y/shell.test.tsx`·`apps/web/e2e/shell.spec.ts` | done (AC-1~AC-5. WP-015가 브라우저 왕복을 채웠다 — 인가 리다이렉트·PKCE·`state`/`nonce` 검증·세션 발급·원래 경로 복귀·로그아웃. 왕복 상태는 짧은 수명 HttpOnly 쿠키로 나른다(DEV-071). **실제 IdP 왕복은 자격 증명이 없어 NOT RUN** — e2e는 OIDC 미구성 배포의 503과 라우트 계약까지 건다) **CR-091: AC-2의 평문 HTTP 파일럿 면제(`DEV-694`)와 AC-10 실효 역할·관리자 지정(`DEV-695`) — 검증 6.89장, 사내 확인 NOT RUN** **CR-092: 복귀 주소를 경로로(`DEV-699`)와 AC-5의 화면 진입점·완료 화면(`DEV-700`) — 검증 6.91장, 사내 확인 NOT RUN** |
| FR-AUTH-002 | WP-012 | `packages/authz/src/{scope,scope-source,scope-database}.ts`, `packages/es/src/scoped-query.ts`, `apps/search-api/src/auth/{principal,errors,me,context}.ts` | `packages/es/integration/scope-enforcement.test.ts`, `packages/es/src/architecture.test.ts`, `packages/authz/integration/scope.test.ts`, `apps/search-api/integration/authz/enforcement.test.ts` | done (AC-1~AC-6. 단, 검색·집계 API 자체는 WP-013·WP-014가 세운다 — 여기서는 필터와 강제 지점을 세우고 ES에 직접 물어 결과 집합을 검증했다) **CR-092: 조직·팀은 `org_team` 범위에서만 GHE에서 읽고 실패는 단계·상태·필요 권한으로 로그에 남긴다(`DEV-698`) — `packages/authz/src/scope-source.test.ts`, `apps/search-api/integration/authz/scope-diagnostics.test.ts`, 검증 6.91장, 사내 확인 NOT RUN** |
...
| FR-AUTH-004 | WP-002, WP-039, WP-041, WP-044, WP-080 | `packages/domain/src/audit.ts` (정본 어휘), `packages/db/src/repositories/auth.ts`의 `changeAssignedRole`(관리자 지정 역할의 부여·회수 감사 — 같은 트랜잭션, CR-091), `apps/search-api/src/audit/{recorder,routes,cursor}.ts`, `apps/search-api/src/export/routes.ts`, `apps/pipeline-worker/src/retention.ts`, `packages/db/src/{partitions,pool,config}.ts`, `packages/db/src/repositories/audit.ts`, `packages/db/migrations/{004,005,018}_*.sql`, `packages/db/migrations/030_gh_operations_policy.up.sql`(운영 정책 변경의 적용 감사 — 같은 트랜잭션, CR-090), `apps/search-api/src/gh/routes.ts`(운영 정책 변경의 거절 감사), `apps/web/{lib/audit.ts,components/Audit*.tsx,app/ops/audit/page.tsx,lib/nav.ts}` | `integration/audit/{audit-records,failure-isolation,action-coverage}.test.ts`, `apps/search-api/integration/export/export.test.ts`, `pipeline-worker/integration/retention/*.test.ts`, `packages/db/integration/audit-grants.test.ts`, `apps/web/{lib/audit.test.ts,a11y/audit.test.tsx,e2e/audit.spec.ts}`, `regression/runtime-reachability.test.ts`, `packages/db/integration/gh-policy.test.ts`(감사 실패 시 변경 롤백), `apps/search-api/integration/gh/policy-routes.test.ts`(거절 감사·`replayed` 0행) | done — AC-1~8. `safe_marker.set`과 `export.create`까지 소유 WP가 활성화해 미활성 액션은 0건이다 (CR-054·057·072). CR-090: `gh_registry.approve`·`gh_registry.revoke`·`gh_capability.block`·`gh_capability.resume` 활성 — 적용은 AC-6 예외(같은 트랜잭션), 거절은 AC-6 원칙대로 |
...
| FR-GH-001 | WP-045, WP-060, WP-077, WP-078, WP-079 | `packages/gh-cli/src/{inventory,manifest,capabilities,validate,drift,resource-ref,binding,graph}.ts`, `packages/gh-cli/src/classification/{commands,rules,classify,dimensions,results,ports,contract-checks}.ts`, `apps/search-api/src/gh/{routes,registry}.ts`(API-GH-001·013·014), `apps/web/components/GhRegistryView.tsx`(A-006) | gh-cli 단위(help-parse·manifest·classify·commands·validate 변이·results·binding·graph·resource-ref), 드리프트 통합(동기·비동기), `integration/result-contract.test.ts`(실제 gh), registry 통합, a11y QA-GH-01·32·45, e2e | done — AC-1~AC-9 구현(분류 100%·근거·차원별 커버리지·미분류 0, `DEV-657` 종결), AC-10 core/extension 분리 보고, AC-11·AC-12 구현(CR-089 — leaf 196의 결과 계약과 typed 입출력 port, manifest에서 계산한 그래프, `GATE-GH-01d` 통과, `DEV-675` 종결). 대상 GHES에서의 지원 확인은 `FR-GH-011` AC-4·AC-5의 몫이며 `NOT RUN`(`DEV-674`) |
| FR-GH-002 | WP-047, WP-048, WP-077, WP-079 | `apps/search-api/src/gh/executions.ts`, `packages/gh-cli/src/{argv,result}.ts`, `apps/gh-executor/src/runner.ts` | 실행기 통합(argv 대조·golden argv·`pr_list_v2` 참조·`invalid_identifier`), routes 통합, e2e | partial — AC-1·3·4·7·8·10 구현(R0 `pr.list`). AC-10의 자원 참조는 `pr_list_v2.references`(CR-089, 검증된 실행 컨텍스트의 host·repository). 승인·확인(AC-5·6)은 R0 밖 |
...
| FR-GH-005 | WP-058, WP-079 | `packages/gh-cli/src/{binding,json-pointer,graph}.ts`(평가·판정·그래프만, CR-089) | binding·json-pointer·graph 단위, `integration/result-contract.test.ts`(실제 gh 결과 → 명시 선택 → `pr view` 입력) | partial — AC-8의 구조화 바인딩 평가와 제한 JSON Pointer, AC-11의 호환 판정(순수 함수, 저장·실행 없음, 성공해도 `executable: false`)만. Recipe 정의·저장·실행·순환 검증·fan-out·W-023은 미구현(`WP-058`) |
...
| FR-GH-009 | WP-048, WP-054, WP-059, WP-080 | `packages/gh-cli/src/registry-policy.ts`(`decideExecution`의 운영자 차단 단계), `packages/db/migrations/030_gh_operations_policy.up.sql`(`block`·`resume`, claim 가드의 차단 확인), `apps/search-api/src/gh/{policy,routes,executions}.ts`, `apps/web/components/GhPolicyView.tsx`(A-005 최소) | gh-cli 단위, DB 통합(차단·재개·가드), 흐름 통합(차단 → 거절·대기 요청 닫힘 → 재개), HTTP 통합(403 `GH_POLICY_BLOCKED`), a11y·e2e QA-GH-48·QA-GH-49 | partial — AC-8(운영자의 capability 차단·재개, 대상 `pr.list`)만 구현(CR-090). AC-1~7(위험도·확인·승인)은 R0 밖 |
| FR-GH-010 | WP-056 | - | - | not_started |
| FR-GH-011 | WP-045, WP-059, WP-077, WP-078 | `packages/gh-cli/src/{pin,drift}.ts`, `apps/gh-executor/src/{index,registry-check,runner}.ts`(기동 대조 + JOB-GH-003 + `registry_stale`), `packages/db/migrations/029_gh_capability_registry.up.sql`, `Dockerfile` | 회귀(pin.ts = Dockerfile = smoke = fake-docker, JOB-GH-003 배선), 실행기 통합(registry_stale·registry-check 5), 드리프트 통합 3, smoke 6절 | partial — AC-1~3 구현(버전·해시·manifest 해시 대조 → 기동 거부; 드리프트 검사 기동 시·주기 → 검증 기록 + `registry_stale` 거절 = `execution_disabled`). **AC-6~AC-10 구현(CR-090 / WP-080)** — 운영 승인 필요(`admin_action_required`)·승인 자격(서버 적재 정의·범위의 최신 실행기 근거·보고서 판 해석기·재현 해시·게이트·신선도)·정책 revision(충돌·멱등·불변 이력)·수락과 claim의 같은 판정과 비승계·승인 없이 동작하는 검사·조회·사유 표시(`packages/gh-cli/src/{registry-policy,registry-cadence}.ts`, `packages/db/migrations/030_gh_operations_policy.*`, `packages/db/src/repositories/gh-policy.ts`, `apps/search-api/src/gh/policy.ts`, `apps/gh-executor/src/{registry-check,runner}.ts`, `apps/web/components/{GhRegistryApprovalPanel,GhExecutionGateBanner}.tsx`; 시험 6.87장). AC-4·AC-5 호스트 판정은 미구현 — 전부 `unverified`(`DEV-674`) |
| FR-GH-012 | WP-048, WP-077 | `packages/db/migrations/028_gh_operations.up.sql`, `packages/db/src/repositories/gh-execution.ts`, `executions.ts`(`toExecutionView`), `apps/web/components/GhHistoryView.tsx` | gh-schema 통합 13(멱등 경합), routes 통합(가시성·중복 키), e2e(이력·재실행) | partial — AC-1~5 구현(감사 정본은 `gh_execution`). CR-090: 실행 기록에 수락한 운영 정책 revision(`policy_revision`)이 붙고, 대기 요청은 과거 승인을 승계하지 않는다(AC-4의 취지). AC-6 상충 잠금은 R0 밖 |
| FR-GH-013 | WP-055, WP-059 | - | - | not_started |
| NFR-009 | WP-045, WP-060, WP-078, WP-079 | `packages/gh-cli/manifest/gh-2.97.0.json`(r0.3), `packages/gh-cli/src/{validate.ts,classification/{dimensions,contract-checks}.ts}`, `scripts/gh-capabilities.mjs` | `validate.test.ts`(커밋된 manifest — 본표·결과 계약 차원 100%·변이, CR-089 결과 계약·port 위조), `integration/drift.test.ts`(실제 바이너리 = GATE-GH-02), registry 통합 | partial — `GATE-GH-01`(command·alias·positional·flag·inherited·short·반복·interaction·입출력·`--json`) 100%, `GATE-GH-01b` 분리 보고, `GATE-GH-02` 드리프트 검출, `GATE-GH-01d` 여섯 차원 통과(CR-089 — `DEV-675` 종결, port 분모 명시 `DEV-688`). 실측 기준 열의 `--json` command는 40으로 정정(`DEV-676` 종결). 그래프 간선·opaque 결과 수는 정보 차원으로 보고한다 |
...
| DEV-703 | 2026-09-17 | CR-096 영문 전환으로 web 기동 경고와 거부 헤더가 `Warning: …`·`Cannot start because …`가 됐지만 release smoke와 fake-docker fixture가 옛 한국어 표식을 찾아, 실제 로그가 정확한데도 pilot.10 번들을 발행 전에 두 차례 거부했다 | NFR-007 / WP-084·WP-085 | 검증 계약 불일치 | CR-096 | resolved — smoke와 fixture가 실제 영문 경고·거부 헤더를 대조한다. 두 pilot.10 실행 모두 태그·릴리스를 만들기 전 중단됨을 확인했고 같은 main 계보에서 재시도한다 |
...
| DEV-697 | 2026-09-15 | **`prsctl smoke`가 정상 `search-api`를 `✗ /healthz → HTTP/1.1`로 실패시켰다.** 헬스체크를 `compose exec -T <svc> wget -qO- --server-response … 2>&1 \| awk '/HTTP\//{print $2}' \| tail -1`로 읽었다. `docker compose exec`는 컨테이너의 stdout과 stderr를 따로 나르므로 도착 순서가 보장되지 않고, 개행 없는 JSON 본문(stdout)이 헤더(stderr)보다 먼저 오면 한 줄이 `{"status":"ok",…}  HTTP/1.1 200 OK`가 되어 두 번째 필드가 `HTTP/1.1`이다. pilot.7 이미지에 `docker exec`로 20회 걸어 8회 재현했다(`-O/dev/null`이면 20회 모두 200). 사내는 「pilot.7에서 파싱 방식이 바뀌었다」고 적었지만 그 파이프는 `CR-060`(pilot.2)부터 같다 — 간헐이라 이전 반입에서 드러나지 않았다. 5xx에서는 busybox가 덧붙이는 `wget: server returned error: HTTP/1.1 500 …` 줄 때문에 같은 자리가 `server`를 읽었다 | DEV-515 · DEV-577 / WP-070 · WP-071 | 구현 결함 | CR-092 | resolved — `http_status`가 본문을 버리고(`-O/dev/null`) `HTTP/x.y` 바로 뒤의 세 자리 토큰을 첫 응답·마지막 응답으로 고른다. 헬스체크 둘과 진입 화면이 이 함수를 지난다. 회귀가 최악의 도착 순서(본문 먼저)를 흉내 내는 가짜 `compose`와 busybox 실측 출력(5xx·리다이렉트·연결 거부)으로 strict 모드에서 판정을 건다. 실제 pilot.7 이미지에서도 20회 모두 200을 확인했다 |
...
| DEV-690 | 2026-09-14 | **web 프록시가 `Idempotency-Key` 헤더를 버려, 실제 프록시를 지난 W-010 실행 요청이 400(`INVALID_PARAMETER`)이었다.** 프록시의 요청 헤더 허용 목록(`apps/web/lib/proxy.ts`)이 신원 헤더 차단(`DEV-067`) 뒤 `accept`·`accept-language`·`content-type`만 넘기고 중복 방지 키를 빠뜨렸다. W-010은 키를 헤더로 보내고 search-api는 키가 없으면 400이다. 드러나지 않은 이유: e2e는 브라우저에서 `/api/gh/**` 응답을 목킹하고 통합 시험은 search-api를 직접 부른다 — 둘 다 실제 프록시를 지나지 않는다. `CR-090` 착수 조사에서 코드 판독으로 발견해 `proxy.test.ts`로 재현했다(허용 목록에서 빼면 실패 — 변이 T10). 처방: 허용 목록에 `idempotency-key`를 더했다. 신원을 주장하지 않으며 search-api가 세션 사용자별로 묶는다. 운영 정책 변경(`API-GH-008`)도 같은 헤더를 쓴다 | FR-GH-012 AC-5 · API-GH-002 · API-GH-008 · WP-077 | 기술 제약 | CR-090 | resolved |
| DEV-689 | 2026-09-14 | **전량 e2e를 기본 작업자(12코어에서 6)로 돌리면 `flow-003.spec.ts:176`(뒤로가기로 검색에 돌아온다)이 이 판의 gh 명세와 함께 돌 때 자주 실패했다.** 실패 시점 스냅숏은 URL이 이미 `/search`인데 `main`에 커밋 상세가 그대로 서 있는 모양이다 — `/search`가 `force-dynamic`이라 뒤로가기의 RSC 왕복이 서버 부하만큼 늦는, `DEV-376`이 적은 렌더 지연이다. 기능 결함이 아니다(단독 실행 2/2·gh 명세 제외 1/1·작업자 1개 1/1 통과). 관련성 실측: 같은 앱 빌드에서 gh e2e 명세만 origin/main 판으로 바꾸면 전량 4회 중 실패 0, 이 판의 명세로는 5회 중 3회(교차 배치) — A-006 e2e가 결과 계약 표·간선 표를 더 그리며 병렬 부하가 늘어 5초 기본값을 넘기는 확률이 올랐다. 기준 커밋 `f3cd00f`의 main CI(run `34791175999`, e2e 포함)는 옛 명세로 success였다. 처방: 판정할 계약은 「검색 화면과 원래 입력이 돌아온다」이지 5초가 아니므로 **그 도착 판정 하나만** 15초까지 기다린다 — 엉뚱한 곳에 도착하면 바로 앞의 URL 판정이 먼저 잡는다. 수정 뒤 같은 조건(기본 작업자·이 판의 gh 명세)으로 전량 5회 모두 191/191 통과했다 | FLOW-001 · WP-018 · WP-079 | 기술 제약 | CR-089 | resolved |
| DEV-688 | 2026-09-14 | **SRS `NFR-009`의 입력 port 분류율 기준 열이 「바인딩 가능한 capability 기준」이었고 출력 port는 「위와 동일」이었다.** 글자 그대로 읽으면 입력 port의 분모도 composability가 바인딩 가능한 capability(출발이 될 수 있는 것)인데, 입력 port는 **도착**의 성질이라 그렇게 세면 `pr merge`·`run rerun`처럼 결과가 끝나는 command의 입력 자리가 게이트에서 빠진다. 이 판은 출력 port 분모를 바인딩 가능한 capability 32, 입력 port 분모를 대상 자원 자리를 가진 capability 80으로 세고, 둘 다 port 수가 아니라 capability 수이며 분모 0을 통과로 보지 않는다. SRS 기준 열을 그 뜻으로 고쳤다(v2.27) | NFR-009 · srs_final.md | 문서 오류 | CR-089 | resolved |
| DEV-687 | 2026-09-14 | **CR-087·CR-088이 바꾼 문서의 상태 머리글을 올리지 않았고, 두 CR이 「원장에 적는다」고 한 문서 검사기 결과가 원장 6.84·6.85장에 없다.** CR-088 커밋 `a996540`이 내용을 바꾼 문서 중 머리글이 있는 13개(작업 패키지 v2.29·원장 v6.73·검증 계획 v0.10·로드맵 v0.19·API 계약 v0.28·데이터 모델 v0.22·비동기 v0.12·백엔드 v0.8·프런트엔드 v0.5·인프라 v0.16·관측성 v0.4·와이어프레임 v0.17·QA 체크리스트 v0.16)가 판·갱신일 그대로이고, CR-087 `ce5a4b0`도 원장 머리글을 그대로 두었다. 작업 패키지 4장 커버리지 표에는 `WP-078`이 빠져 있었다. 처방: 과거 커밋의 판을 소급해 만들지 않고, 이 CR에서 해당 문서의 판을 한 번씩 올려 머리글이 내용과 맞게 했다(갱신일은 이 CR의 날짜). 빠진 검사기 결과는 재구성하지 않는다 — 그 시점의 출력이 남아 있지 않으며, 이 CR의 before/after를 6.86장에 적는다. 커버리지 표는 `WP-078`·`WP-079`를 넣어 10·합계 71로 고쳤다 | change_control.md · 원장 · 작업 패키지 | 문서 오류 | CR-089 | resolved |
| DEV-686 | 2026-09-14 | **도메인 회귀 시험(`pnpm test:regression`)이 CI에서 돌지 않았다.** `vitest.config.ts`의 include에 `regression/**`이 없고 `.github/workflows/ci.yml`에는 그 단계가 있었던 적이 없는데, 검증 계획 8장은 회귀 픽스처를 「CI에서 반복 검증한다」고 적고 3장 표는 주기를 「주 1회 + 릴리스」로 적었다 — 실행 허용 1건을 고정하는 `runtime-reachability.test.ts`의 REL-007 블록도 로컬에서만 돌았다. 처방: integration 잡의 `test:integration` 다음에 같은 서비스·환경 변수로 `test:regression`을 순차 실행하는 단계 하나를 더했다(러너·트리거·잡 구조·포트 불변). DEV-664 시험은 러너의 실제 `docker compose`를 부르므로 도구가 없으면 skip이 아니라 실패로 드러난다. 회귀가 그 단계의 존재와 순서를 건다 | NFR-008 · 검증 계획 3·8장 · WP-077 | 범위 공백 | CR-089 | resolved |
| DEV-685 | 2026-09-14 | **스냅숏 활성화 CHECK(029)가 `GATE-GH-01d`를 보지 않는다.** CHECK는 `activated_at`이 있으면 미분류 집계 열이 0이어야 한다는 것뿐이라, 결과 계약 차원이 미달인 manifest(예: `r0.2`)의 스냅숏도 CHECK만으로는 활성화될 수 있다. 결과 계약 차원은 열이 아니라 검증기 보고서가 계산하는 사실이어서 CHECK 식으로 옮기려면 열을 더해야 한다. 이 판은 활성화 절차 자체가 없고(`WP-059`) 어느 행도 활성화하지 않으므로 마이그레이션을 더하지 않았다 — 활성화 절차를 여는 판이 「최신 검증 기록의 보고서 판 `r2` 이상·status `passed`」를 조건으로 삼아야 한다 | WP-059 · ENT-GH-006 · NFR-009 | 범위 공백 | CR-089 · CR-090 | resolved — CR-090: 활성화 절차가 운영 승인으로 들어왔다. 조건은 이 DEV가 적은 「보고서 판 `r2` 이상·`passed`」보다 좁다 — 판 번호의 크기를 비교하지 않고 등록된 해석기가 읽는 판(`r2`)만 받으며, 이 배포 범위의 가장 최근 실행기 기록·저장 해시와 재현 해시·`GATE-GH-01`·`01b`·`01d`·신선도 한도를 함께 요구하고 DB 함수가 잠금 아래 다시 확인한다. `activated_at`은 최초 승인 시각으로 한 번 채우고 현재 승인 여부의 정본은 `gh_operations_policy`다. 029 CHECK는 고치지 않았다(6.87장) |
| DEV-684 | 2026-09-14 | **SRS 9.8 4항과 `WP-066` 완료 기준의 예시 연쇄 `gh search prs` → `gh pr checks` → `gh run rerun`이 고정 gh 2.97.0에서 성립하지 않는다.** `pr checks`의 JSON 필드(name·state·startedAt·completedAt·link·bucket·event·workflow·description)에는 check run·workflow run 식별자가 없고(`pkg/cmd/pr/checks/checks.go:22-32`), SRS 3항의 `GhResourceRef.kind` 15종에도 check run이 없다 — 예시의 `CheckRunRef[]`는 만들 수 없는 참조다. link URL을 식별자로 재해석해 간선 수를 채우지 않았다. 실제로 성립하는 연결은 `search prs`의 `PullRequestRef[]` → `pr checks`의 입력(명시 선택 조건부)과 `run list`의 `WorkflowRunRef[]`(`databaseId`) → `run rerun`의 입력이다. SRS 예시와 WP-066 완료 기준을 그 연결로 고쳤다 | FR-GH-001 AC-12 · WP-066 · srs_final.md | 문서 오류 | CR-089 | resolved |
| DEV-683 | 2026-09-14 | **CR-088 분류 표의 주 결과 종류 63건과 출력 모드 29건이 고정 gh의 비TTY 동작과 달랐다.** 주 결과 종류: 이름에서 짐작한 `resource`가 실제로는 stdout이 비는 명령(`pr close`·`pr merge`·`issue close`·`label edit`·`repo rename` 등 12건 → `exit_status`), comment·릴리스·run URL을 찍는 명령(7건 → `url`), 문구·이름 한 줄을 찍는 명령(5건 → `text`), `--format json` exporter가 있는 project 명령(13건 → `json`)이었다. exporter가 있는 `attestation verify`·`auth status`·`release verify`·`release verify-asset`·`repo read-file`(5건)은 `text`가 아니라 `json`이고, 자원을 식별하는 list·view·search 20건과 `issue edit`(URL 줄) 1건은 `json`·`resource`가 아니라 `resource_list`·`resource`다. 출력 모드: `workflow run --json`(입력 flag)을 JSON 출력으로, `pr create`·`issue create`·`repo create`·`repo edit`의 `--template`(입력)을 템플릿 출력으로 셌고, `--format json`만 있는 16개 명령의 JSON 모드, project 목록·조회 4개의 기본 텍스트 모드, 파일 출력 3개와 `--watch` 1개의 모드를 빠뜨렸다. 규칙(`outputFormatsOf`)과 표를 고치고 주 종류가 이름과 다른 행마다 소스 줄 근거(`RESULT_KIND_EVIDENCE`)를 붙였다 — 규칙 판 `rules-2026-09-14.3`, manifest `r0.3` | WP-045 · FR-GH-001 AC-9·AC-11 | 기술 제약 | CR-089 | resolved |
| DEV-682 | 2026-09-14 | **`pr_list_v1` 파서가 `number`를 필수로 읽어, 실행 정의가 허용한 「JSON 필드 하나만 고르기」가 실패했다.** 실행 정의는 `--json` 필드를 1~10개 고르게 열어 두었고 gh는 **고른 필드만** 찍는다(실측: `--json title` → `[{"title":…}]`). 그래서 `title`만 고른 정상 조회가 `result_parse_failed: row_shape`로 끝났다(CR-086 이후 잠복한 실제 사용자 흐름). 또 GraphQL이 번호를 `null`로 주면 gh는 Go zero value `0`을 찍는다(실측) — v1은 0을 번호로 받았다. 처방: 허용한 선택을 줄이지 않고 결과 스키마를 `pr_list_v2`로 올렸다 — `number`를 고르지 않았으면 행의 번호는 `null`이고 참조는 `unavailable`(`identity_field_not_selected`), 골랐는데 양의 안전한 정수가 아니면 `result_parse_failed: invalid_identifier`다. 과거 `pr_list_v1` 기록은 다시 해석하지 않는다 | WP-077 · FR-GH-002 AC-10 · QA-GH-24 | 기술 제약 | CR-089 | resolved |
| DEV-681 | 2026-09-14 | **인벤토리 정렬이 `localeCompare`라 프로세스 로케일에 묶여 있었다.** 커밋된 순서는 기본(en-US)·코드포인트 순서와 같았지만 `th` 로케일에서는 공백을 무시하는 대조로 `pr view`가 `preview` 뒤로 가(독립 검토 나 실측) 같은 바이너리에서 인벤토리 해시가 달라지고 실행기가 diff는 빈 채 영구 `drift`가 된다. compose는 LANG을 정하지 않아 현실 확률은 낮았다. 코드 단위 비교로 바꿨다 — 커밋된 manifest의 순서는 그대로다 | WP-045 · NFR-009 | 기술 제약 | CR-088 | resolved |
| DEV-680 | 2026-09-14 | **주기 레지스트리 검사가 `spawnSync`로 이벤트 루프를 20~30초 막았다.** `checkDrift` → `extractInventory`가 command마다 동기 spawn(229회)이라 그동안 실행 중인 gh의 하트비트(5초)·취소 폴링(500ms)·stdout 소비·`/healthz`가 멈추고, 파이프 버퍼(64KB)를 넘는 출력의 실행은 정지 동안 write에서 멈춰 R0 상한 30초를 넘겨 `timed_out`될 수 있었다(독립 검토 나, major). 기동 검사는 구독 전이라 무해했다. 처방: 인벤토리 추출·드리프트 검사의 **비동기 판**(`extractInventoryAsync`·`checkDriftAsync`, `spawn` 동시 4)을 두고 실행기가 그것을 쓴다 — 같은 정렬·같은 해시를 내고(통합 시험이 동기 판과 대조), 도는 동안 타이머 tick이 산다(시험). 동기 판은 CLI·시험용으로 남는다. 회귀가 실행기에 `checkDrift(`가 없음을 건다 | JOB-GH-003 · FR-GH-006 · NFR-011 | 기술 제약 | CR-088 | resolved |
| DEV-679 | 2026-09-14 | **드리프트·구조 실패를 확인하고도 DB 기록이 실패하면 `stale`이 서지 않아 실행이 계속됐다.** `runRegistryCheck`가 판정 뒤 기록(`record`)을 같은 함수에서 기다리다 기록이 던지면 판정을 담은 결과가 반환되지 않았고, 호출부는 로그만 남기고 상태를 바꾸지 않았다(독립 검토 가·나 둘 다 지적, major). 대표 조건은 마이그레이션 029가 아직 적용되지 않은 DB에 새 실행기 이미지를 먼저 올린 경우 — `relation does not exist`가 프로세스 수명 내내 반복되어 드리프트가 영영 stale로 서지 않았다. 처방: 판정을 먼저 확정하고 기록은 try/catch로 감싸 `recordError`로 돌려주며, 호출부는 기록 실패와 무관하게 판정으로 `stale`·지표를 갱신한다. 기동 시 기록 실패는 029 적용 여부를 확인하라는 로그를 남긴다. 시험: DB가 죽은 풀 + 드리프트 → `stale=true`·`verificationId=null`·`record_failed` 지표 | JOB-GH-003 · FR-GH-011 AC-3 | 기술 제약 | CR-088 | resolved |
| DEV-678 | 2026-09-14 | **gh 2.97.0의 help가 bool flag 셋의 값 자리에 다른 flag 이름을 적는다** — `auth setup-git -f, --force --hostname`, `cache delete --succeed-on-no-caches --all`, `project item-edit --value --field`. 파서 결함이 아니라 원문이 그렇다(실측, `cat -A`). 값이 아니라 「함께 써야 하는 flag」의 표기이므로 규칙 `help-placeholder-artifact`가 bool로 읽고 그 사실을 note에 남긴다 | WP-045 · FR-GH-001 AC-7 | 기술 제약 | CR-088 | resolved |
| DEV-677 | 2026-09-14 | **실행 가능한 그룹 command가 있다** — `codespace ports`는 하위 command(`forward`·`visibility`)를 가진 그룹이면서 자기 flag와 `--json` 필드를 가진다. 분류 모델은 leaf만 분류·실행 대상으로 보므로 그룹의 분류는 `null`이다. 다만 flag·inherited·short alias·`--json` 필드 차원의 **분모에는 그룹의 flag를 포함**했다 — SRS 실측 기준 1,034·312·625·707이 그 분모이고, 빼면 숫자가 다른 뜻이 된다. 그룹의 flag 컨트롤은 같은 규칙을 그 자리에서 적용해 센다(결정적) | WP-045 · NFR-009 | 기술 제약 | CR-088 | resolved — 문서화 |
| DEV-676 | 2026-09-14 | **SRS `NFR-009` 실측 기준 열의 `--json` 지원 command 41이 이 판의 추출 40과 다르다.** 이 판: JSON FIELDS 절이 있는 command 40 = leaf 39 + 실행 가능한 그룹 `codespace ports` 1(필드 707은 같다). 다른 열은 뜻이 맞는다 — command node 228 = 그룹 32 + leaf 196(별칭 전용 `co`를 더하면 229), short alias 625 = 고유 542 + inherited 출현 83, inherited 312, 반복 37, placeholder 230은 SRS의 세는 법(대안별)과 이 판의 자리 164(묶음별)가 다른 분모다. 41의 출처를 재구성하지 못했다. SRS는 baseline이라 이 CR에서 고치지 않는다 — 정정 여부는 다음 CR이 검증기 보고서(정본)를 근거로 정한다 | NFR-009 · srs_final.md | 문서 오류 | CR-088 · CR-089 | resolved — CR-089: 41은 `--json` flag를 가진 command 수였고 그중 `workflow run --json`은 입력 flag다. SRS v2.27이 40으로 정정했다 |
| DEV-675 | 2026-09-14 | **결과 계약 차원(`GATE-GH-01d`)이 미달이다** — bindability·자원 타입은 실행을 여는 정의(`GhCapabilityDefinition`, 1건)에만 있고 입출력 port는 bindable capability가 없어 분모 0이다. leaf 196의 결과 **종류**와 **민감도**는 표로 분류했다(secret 2 · sensitive 4 · public 8). 검증기 보고서는 `GATE-GH-01d`를 FAIL로 적고 전체 상태를 `incomplete`로 두며, CLI는 기본(strict)에서 종료 1이다. 「정의됨」으로 세지 않았다 | WP-066 · NFR-009 · FR-GH-001 AC-11·AC-12 | 범위 공백 | CR-088 · CR-089 | resolved — CR-089: 여섯 차원 통과(6.86장). 본문의 민감도 수치는 독립 검토 반영 전 값이며 확정 값은 secret 4·sensitive 7이다(6.85장) |
| DEV-674 | 2026-09-14 | **대상 GHES에서 지원을 확인한 command가 0건이다** — 모든 leaf의 `hostSupport`가 `unverified`이고 `unsupported_by_host`는 지원 상태에도 interaction에도 0건이다. 확인하지 않은 것을 미지원으로 적지 않았다(FR-GH-011 AC-5). 호스트 판정(AC-4: gh 버전·GHE 버전·App 권한·사용자 권한·정책의 함수)은 사내 GHES가 있어야 한다. A-006은 이 사실을 「미확인」으로 그대로 보인다 | WP-045 · WP-059 · FR-GH-011 AC-4·AC-5 | 범위 공백 | CR-088 | open — 사내 확인 NOT RUN |
| DEV-673 | 2026-09-14 | **`EVT-GH-006`(`gh.capability.drift_detected`)을 버스 이벤트로 발행하지 않는다.** 드리프트는 `gh_capability_verification` 행(status `drift`, diff JSON)·실행기 로그(`레지스트리 검사 실패`)·지표 `gh_registry_stale`·헬스 `registry.stale`·A-006으로 드러난다. 구독자로 적힌 「알림」 채널이 아직 없어 이벤트를 만들 소비자가 없다 — 알림 채널을 붙이는 판이 발행 여부를 정한다 | WP-059 · EVT-GH-006 · FR-GH-011 | 범위 공백 | CR-088 | open |
| DEV-672 | 2026-09-14 | **`gh_capability_snapshot`의 계획 DDL과 다르게 만들었다.** (1) `CHECK (unclassified_count = 0)`를 `CHECK (activated_at IS NULL OR (네 차원의 미분류 = 0))`으로 — 미분류가 남은 manifest도 **진단용으로 기록**할 수 있어야 A-006이 「무엇이 미완인가」를 보인다. 막는 것은 활성화뿐이며, 활성화 조건은 command만이 아니라 interaction·flag·positional 미분류도 0이다(독립 검토 나). (2) `inventory_hash`·차원별 카운트·`coverage` JSONB·`first_seen_at`을 더하고 `activated_at`을 NULL 허용으로. (3) 검증 회차의 기록은 스냅숏에 덮어쓰지 않고 신설 `gh_capability_verification`(ENT-GH-012, append-only — UPDATE·DELETE는 트리거가 거부)에 더한다. (4) 마이그레이션 번호는 계획의 009가 아니라 029다 | ENT-GH-006 · ENT-GH-012 · WP-045 | 기술 제약 | CR-088 | resolved — 데이터 모델 3.5장 정정 |
| DEV-671 | 2026-09-14 | **`JOB-GH-003`(드리프트 점검)의 자리와 실패 처분을 확정했다.** 잡은 `pipeline-worker`가 아니라 **`gh-executor`**에 있다 — gh 바이너리와 DB를 둘 다 가진 프로세스가 그것뿐이다. 기동 시 검사를 **기다린 뒤** 구독을 세우므로 켜진 실행기의 첫 실행은 검사 뒤에 온다(비동기 판·동시 4로 10초 안팎, command마다 `--help`). 헬스 서버는 검사보다 먼저 열려 `registry.status: unchecked`로 답한다. 드리프트·구조 실패의 처분은 프로세스 종료가 아니라 **실행마다 `registry_stale` 거절**이다 — 종료하면 헬스가 사라져 「왜」를 잃는다. 일시 오류(시간 초과)는 3회 재시도 뒤 기록만 남기고 `stale`을 바꾸지 않는다 — 시간 초과 하나로 실행을 멈추지 않는다. 주기는 `GH_EXECUTOR_REGISTRY_CHECK_MS`(기본 1일, 최소 1분) | JOB-GH-003 · FR-GH-011 AC-3 · WP-045 | 기술 제약 | CR-088 | resolved — 비동기 9.4·인프라 12장 정정 |
...
| DEV-657 | 2026-09-13 | **manifest 분류가 `support: 'unknown'` 195건(leaf 196 중 1건만 분류)이라 `NFR-009` 게이트를 통과하지 못한다.** 분류하지 않은 것을 분류했다고 적지 않았고 `gh_capability_snapshot`(CHECK unclassified=0)도 만들지 않았다. 화면의 커버리지 문구가 그 수를 그대로 보인다. REL-007 완료 조건. **종결(CR-088)**: leaf 196개 전부를 근거 있는 표로, flag 1,034·inherited 312·positional 164·`--json` 707을 규칙으로 분류해 본표 차원(`GATE-GH-01`) 전부 100%·미분류 0이 됐고 검증기가 그것을 CI에서 건다. 남은 미달은 다른 사실이라 따로 적었다 — 결과 계약 차원(`DEV-675`)·호스트 지원 확인(`DEV-674`) | WP-045 · NFR-009 | 범위 공백 | CR-086 · CR-088 | resolved — CR-088 (잔여는 DEV-674·DEV-675) |
...
| DEV-646 | 2026-09-13 | **해제 표시가 실행자 락 밖에서 돌았다.** 잔여 스윕이 회차를 시작하기 전에 부르는 `markDisabledRepositoryTargets`는 GHE를 부르지 않는 순수 DB 쓰기라 락 없이 두었는데, 그러면 「실행자는 하나다」가 **회차에만 참인 말**이 된다. 두 프로세스가 같은 행을 함께 표시해 `disabled` 지표가 부풀 수 있다(GHE 쓰기는 없으므로 외부 영향은 없다). 독립 검토가 note로 지목했다. 이 호출도 락으로 감쌌다 — 회차와 별도로 잠그는 것은 둘이 독립적이기 때문이며, 그 사이 다른 프로세스가 회차를 가져가도 잃는 것이 없다 | JOB-SEQ-005 / WP-075 | 범위 공백 | CR-085 | resolved |
| DEV-645 | 2026-09-13 | **운영자가 재개한 행이 대기열 뒤로 밀렸다.** `resumeAnnotateTargets`가 상태를 `NULL`로 되돌리면서 `annotated_at = now()`도 썼는데, 대상 정렬이 `annotated_at ASC NULLS FIRST`라 **방금 연 행이 가장 늦게** 처리된다. 사람이 확인하고 다시 하라고 지시한 행이 제일 뒤로 가는 것은 그 지시의 뜻과 어긋난다. 독립 검토가 note로 지목했다. `annotated_at`도 비워 재개한 행이 앞으로 오게 했다 — 근거 세 열은 지우지 않으므로 「한 번도 시도하지 않은 행」과는 여전히 구별된다 | FR-SEQ-009 AC-9 / WP-075 | 범위 공백 | CR-085 | resolved |
| DEV-644 | 2026-09-13 | **감사 crash 창의 서술이 한쪽만 적고 있었다.** 감사를 정본 표시 앞에 둔 주석이 「남는 것은 감사는 있는데 표시가 없는 행이고 다음 회차가 마무리한다」고만 적었는데, **반대쪽 창**(PATCH 뒤·감사 전에 죽으면 「썼는데 감사가 없는 행」이 남고, 상태가 비어 있어 다음 회차가 복구로 보지 않고 감사 없이 `done`으로 끝낸다)은 적지 않았다. 독립 검토가 「축소해 적고 있다」고 지목했다. 창은 어느 순서로 두어도 남으므로 **둘 다 적고**, 감사를 앞에 둔 이유를 「앞의 창이 더 짧다」로 정확히 좁혔다 — 「감사가 절대 빠지지 않는다」가 아니다 | FR-AUTH-004 AC-6 / WP-075 | 문서 간 모순 | CR-085 | resolved |
| DEV-643 | 2026-09-13 | **사전 점검 CLI의 주석이 없는 기능을 서술했다.** 파일 상단이 「GHE에는 조회 요청만 보낸다(그것도 `--probe`를 줄 때만)」고 적었는데 인자 파서에 `--probe`가 없어 그 값을 주면 거부된다. 실제 동작은 주석보다 **더 안전한 쪽**(GHE 요청이 한 건도 없음)이지만, 문서가 실재하지 않는 경로를 안내하는 것은 `CR-082`가 같은 종류로 겪은 자리다(런북이 존재하지 않는 변수를 안내했다). 독립 검토가 note로 지목했다. 주석을 실제 동작에 맞추고, 쓰기 권한을 미리 확인해 주지 않는 이유를 함께 적었다 | WP-075 | 문서 간 모순 | CR-085 | resolved |
| DEV-642 | 2026-09-13 | **사내 GHES의 한도 수치를 Cloud 문서로 가정할 수 없다 — 관측만 기록한다.** 자가 연구가 GHES 문서에서 「Rate limits are disabled by default for GitHub Enterprise Server. Contact your site administrator to confirm the rate limits for your instance.」를 확인했다. 관리자가 켜더라도 Management Console이 설정하는 단위는 「인증된 요청/인증되지 않은 요청」별 수치이며 Cloud 문서가 말하는 installation 단위와 같다는 보장이 없다. 이 판의 설계는 그 사실에 영향을 받지 않는다 — 간격과 유예는 **문서가 보장하는 수치가 아니라 우리가 정한 하한**을 지키고, 한도 신호가 오면 그때 그 값을 따르기 때문이다. 다만 「한도는 5,000/시간이니 이 정도면 안전하다」는 식의 계산을 문서 어디에도 적지 않는다 | JOB-SEQ-005 / WP-075 | 문서 공백 | CR-085 | **open** — 사내가 실제 한도 설정을 확인해 주면 그때 런북에 적는다 |
| DEV-641 | 2026-09-13 | **`pull_request.annotate`의 감사 `result_code` 어휘가 어느 계약에도 없었다.** 이 저장소의 관례는 액션마다 자신의 어휘를 계약 문서가 소유하는 것이고(`safe_marker.set`이 API 계약에 여섯 값을 표로 못박아 둔 선례), `audit_record.result_code`는 DB에 제약이 없는 `TEXT`다. `CR-084`는 값을 코드에만 두어 **감사 로그를 읽는 사람이 목록 없이 사유를 복원할 수 없는 상태**였고, `CR-085`가 `annotated_observed`를 더하면서 그 공백이 실제 문제가 됐다. 보안 문서 13.2절에 두 값(`annotated`·`annotated_observed`)과 각각이 남는 조건을 표로 적고, 회귀가 계약과 코드를 **양방향으로** 대조한다 — 코드에 세 번째 값이 조용히 생기면 실패한다 | FR-AUTH-004 / WP-075 | 문서 공백 | CR-085 | resolved |
| DEV-640 | 2026-09-13 | **한도 유예가 한 이벤트에만 걸렸다.** `429`나 대기 신호가 있는 `403`을 받으면 그 회차를 멈추고 `defer`했지만, 유예는 **그 이벤트 하나**에만 적용됐다. 다음 이벤트나 잔여 스윕이 곧바로 같은 한도를 두드려 유예가 사실상 아무것도 아니었다 — 공식 문서가 「한도에 걸린 채 요청을 계속하면 integration이 차단될 수 있다」고 경고하는 바로 그 형태다. 한도 신호를 실행자 전체의 게이트에 걸어 **그 시각까지 모든 변경 요청**을 멈춘다. 한도로 멈춰 있는 동안에는 정본을 읽지도 않는다 | JOB-SEQ-005 / WP-075 | 범위 공백 | CR-085 | resolved |
| DEV-639 | 2026-09-13 | **결과를 모르는 요청을 실패로 기록했다.** 시한 초과·연결 끊김·게이트웨이 5xx로 끝난 변경 요청은 서버가 처리했을 수 있는데 `failed`로 적었다. 공식 API가 멱등성 키도 요청 조회도 주지 않으므로 클라이언트는 확정할 수 없다 — 「실패했다」가 거짓일 수 있는 기록이며, 그 기록을 보고 운영자가 다른 판단을 한다. 마이그레이션 027이 `unknown` 상태를 더해 「모른다」를 그대로 적고, 다음 회차가 제목을 다시 읽어 확인한다. 복구 중 접두를 관측한 경우의 감사 결과 코드도 `annotated_observed`로 갈랐다 — 다른 주체가 같은 접두를 붙였을 가능성을 API가 가려 주지 않기 때문이다 (`AC-10`) | FR-SEQ-009 AC-10 / WP-075 | 범위 공백 | CR-085 | resolved |
| DEV-638 | 2026-09-13 | **프로세스 종료가 상한 없는 스윕을 기다렸다.** `startAnnotateSweeper`의 `stop()`이 `await loop`로 진행 중인 회차의 완료를 기다렸는데, 그 회차는 최대 200건 × (쓰기 간격 + 왕복)이라 몇 분이 걸릴 수 있다. `SIGTERM`을 받은 컨테이너가 그동안 죽지 못하고 오케스트레이터의 강제 종료를 기다린다. 종료 신호를 회차로 전달해 **진행 중인 HTTP 왕복까지** 끊고, 끊긴 행은 정본에 남아 다음 기동이 이어받게 했다 | JOB-SEQ-005 / WP-075 | 범위 공백 | CR-085 | resolved |
| DEV-637 | 2026-09-13 | **조회 성공만으로 권한 차단을 풀었다 — `DEV-626`의 처방을 뒤집는다.** `DEV-626`은 「남은 대상이 모두 이미 표기된 저장소가 영영 안 풀린다」를 고치려고 `GET` 200에서 차단을 풀게 했다. 그런데 공식 API는 **읽기와 쓰기를 다른 권한으로 나눈다** — 조회만 가능한 설치에서도 200이 오므로 그 200은 제목을 고칠 수 있다는 증거가 아니다. 그 결과 차단이 풀렸다가 다음 회차가 다시 막는 순환이 생기고, 그동안 운영자는 「풀렸다」는 상태를 본다. 차단은 **실제 쓰기 성공**이나 운영자의 명시적 재개로만 푼다. **대가를 적는다**: `DEV-626`이 지적한 지연이 일부 돌아온다 — 쓸 것이 없는 저장소는 쿨다운만큼 차단이 남는다. 새 PR이 생기면 그때 쓰기가 성공해 풀리므로 영구적이지 않다 | FR-SEQ-009 / WP-075 | 문서 간 모순 | CR-085 | resolved |
| DEV-636 | 2026-09-13 | **실패한 변경 요청 사이에 간격이 없었다.** `DEV-622`가 넣은 1초 간격은 **성공한 행 뒤에만** 적용됐다. 5xx가 이어지면 남는 것은 재시도 백오프(200ms)뿐이라 PATCH가 0.2초 간격으로 나간다 — 공식 문서의 권고는 성공 여부와 무관한 **변경 요청 사이**의 것이고, 부 한도를 부르는 형태가 정확히 이것이다. 실제 목 서버의 수신 시각으로 재현했다(간격 0). 간격을 **모든 변경 요청 앞**으로 옮기고 보낸 시각을 보내기 전에 기록해 실패한 요청도 간격에 들게 했다 | JOB-SEQ-005 / WP-075 | 범위 공백 | CR-085 | resolved |
| DEV-635 | 2026-09-13 | **한 행의 처리가 회차 예산을 넘겼다.** 예산 검사가 행과 행 **사이**에만 있어, 조회·재시도·쓰기가 모두 느린 한 행이 예산을 통째로 넘겨도 멈추지 않았다. 그동안 버스는 이벤트를 회수한다. 예산의 중단 신호를 HTTP 왕복에 걸어(`AbortSignal.any`) 진행 중인 요청까지 끊고, 만료 판정을 타이머가 아니라 **시각 비교**로 바꿨다 — 타이머만 믿으면 해상도만큼 늦게 알아챈다 | JOB-SEQ-005 / WP-075 | 범위 공백 | CR-085 | resolved |
| DEV-634 | 2026-09-13 | **줄을 기다리는 동안 예산이 집행되지 않았다 — `DEV-628`이 고치려던 자리가 절반만 닫혀 있었다.** `DEV-628`이 예산을 줄 서기 전부터 재게 했지만, 회차는 여전히 앞 회차가 **끝난 뒤에야** 예산 초과를 알아챘다. 앞 회차가 200초를 쥐면 뒤에 선 이벤트는 200초 동안 반환하지 않고, 그 사이 버스가 회수한다. 실시간으로 재현했다(예산 150ms를 준 회차가 앞 회차의 게이트가 열릴 때까지 반환하지 않음). 줄 서기 자체를 중단 신호와 경주시켜 **예산이 다하면 줄에서 빠져나오게** 했다. 빠져나온 자리는 즉시 비워 뒤에 선 회차가 기다리지 않는다 | JOB-SEQ-005 / WP-075 | 범위 공백 | CR-085 | resolved |
| DEV-633 | 2026-09-13 | **확인된 본문 불일치를 다음 회차가 성공으로 덮었다.** `DEV-630`이 「서버가 저장한 제목이 다르면 실패로 남긴다」를 만들었는데, `failed`는 다음 회차의 대상이다. 그 회차는 제목을 다시 읽고 **접두가 있다는 것만으로** `already_annotated`를 판정해 `done`으로 덮는다 — 원래 제목이 잘린 사실이 한 회차 만에 성공으로 사라진다. `DEV-630`의 수정이 **자기가 만든 상태를 다음 회차가 지우는 것까지는 보지 않았다.** 마이그레이션 027이 `body_changed` 상태를 더하고 대상 질의에서 빼, 운영자가 확인하고 재개할 때까지 멈춘다 (`AC-9`) | FR-SEQ-009 AC-1·AC-9 / WP-075 | 범위 공백 | CR-085 | resolved |
| DEV-632 | 2026-09-13 | **변경 요청의 재시도가 첫 시도의 제목을 그대로 다시 보냈다.** `withRetry(() => updateTitle(decision.nextTitle))`는 같은 closure를 최대 다섯 번 실행한다. 첫 PATCH가 5xx로 실패하거나 응답을 잃은 뒤 재시도까지의 사이에 **사람이 제목을 고쳤거나, 다른 M 접두가 생겼거나, 운영자가 저장소를 껐거나, 재채번으로 번호가 무효가 됐어도** 첫 시도에 계산한 문자열이 그대로 나갔다. 시험으로 셋 다 재현했다 — 특히 「다른 M 접두를 덮지 않는다」(`ADR-022` 결정 5)가 재시도 경로에서 무너졌다. 재시도의 단위를 요청에서 **판단 전체**로 넓혔다: 다시 보낼 때마다 정본 확인 → 제목 재조회 → 순수 판정 → 간격 → 울타리 재확인 → PATCH를 다시 지난다 (`AC-8`) | FR-SEQ-009 AC-1·AC-8 / WP-075 | 범위 공백 | CR-085 | resolved |
| DEV-631 | 2026-09-12 | **토큰 발급의 인증 실패를 재시도 가능으로 분류했다.** `DEV-619`가 발급기 오류를 표기 모델로 옮기며 `auth`를 그대로 `auth`에 대응시켰는데, 표기 모델의 `auth`는 재시도 가능이다(요청 경로의 `401`은 만료라 다시 받으면 되기 때문). 그러나 **발급 자체가 인증에 실패했다면 App ID나 개인 키가 틀린 것**이고 다섯 번을 두드려도 답이 같다 — 행마다 5회면 한 회차 200행에서 최대 1,000회다. 재검토가 잡았다. 발급 경로의 `auth`만 권한 계열로 옮겨 저장소를 막고 쿨다운 뒤에 다시 보게 한다. 요청 경로의 `401` 처리는 그대로다 | JOB-SEQ-005 / WP-075 | 범위 공백 | CR-084 | resolved |
| DEV-630 | 2026-09-12 | **`DEV-623`의 수정이 성공 판정을 지나치게 느슨하게 만들었다.** 완전 일치가 서버의 공백 정리에 거짓 실패를 내는 것을 고치며 「기대한 접두가 있는가」로 바꿨는데, 그 판정은 **맨 앞 대괄호만 보고 본문은 대조하지 않는다.** 공식 문서가 제목 길이 상한을 밝히지 않으므로 서버가 본문을 조용히 자르는 경로가 있을 수 있고, 그때 **원래 제목이 잘린 것을 성공으로 기록한다** — 「원래 제목의 나머지 부분은 바꾸지 않는다」(`AC-1`)가 무너지는 자리다. 재검토가 잡았다. 뒤 공백만 걷어 내고 나머지는 글자 그대로 대조하도록 좁혔다: 완전 일치와 접두 확인 사이가 옳은 자리였다 | FR-SEQ-009 AC-1 / WP-075 | 범위 공백 | CR-084 | resolved |
| DEV-629 | 2026-09-12 | **회차 겹침을 막는 장치가 프로세스 안에만 있다.** `DEV-620`이 닫은 이중 쓰기는 모듈 수준 `passChain`으로 막는데 그것은 프로세스 국소 상태다. 파드가 둘이면 한쪽이 붙들고 있는 이벤트를 **30초 뒤 다른 쪽이 `XCLAIM`으로 가져가** 같은 행을 동시에 처리한다 — 제목은 멱등이라 망가지지 않지만 요청과 감사가 두 벌이 되고 `DEV-620`이 그 자리에서 다시 열린다. 잔여 스윕도 리더 선출이 없어 같은 문제를 갖는다. 재검토가 「지금은 minor이지만 스케일아웃 시 major」로 지목했다. **지금은 두 배포 프로파일이 모두 인스턴스 하나로 못 박혀 있고 회귀가 그것을 강제하므로 성립하지 않는다.** 분산 claim을 이 WP에서 만들지 않고, 대신 두 배포 정의에 **replica 1이 성능 조정이 아니라 정확성 조건이며 늘리려면 분산 claim이 먼저**라고 적었다 — 다음 사람이 값만 올리는 것을 막는 것이 지금 할 수 있는 전부다 | JOB-SEQ-005 / WP-075 | 기술 제약 | CR-084 · CR-085 | **resolved (2026-09-13, CR-085)** — `annotate:runner` advisory 세션 락으로 같은 정본 DB를 보는 프로세스 중 하나만 쓰게 했다. 통합 시험이 **실제 자식 프로세스**를 띄워 그 경계를 지난다. **보장 범위는 그 DB를 공유하는 프로세스들뿐이다** — 서로 다른 DB를 쓰는 두 배포나 네트워크 분할의 exactly-once는 여전히 막지 못하고, 락을 잃기 전에 이미 보낸 요청은 서버에 닿는다. 처리량을 늘리려면 분산 claim이 여전히 먼저다 |
| DEV-628 | 2026-09-12 | **`DEV-627`의 수정이 대기 시간을 재지 않았다 — 같은 자리에서 두 번째다.** 이벤트 회차에 예산을 두었으나 그것을 **락을 잡은 뒤부터** 쟀다. 잔여 스윕은 예산이 없어 상한만큼(최대 200건 × 1초) 회차를 쥘 수 있고, 그 뒤에 선 이벤트는 **기다리기만 하다** 버스의 30초 회수 시한을 넘긴다 — 예산을 넣은 이유가 바로 그 회수인데 대기가 예산 밖이라 막지 못했다. 독립 검토가 「수정이 부른 새 절충」으로 지목한 자리다. 예산을 **줄을 서기 전부터** 재고, 대기만으로 다하면 정본을 읽지도 않고 `defer`로 돌아간다 | JOB-SEQ-005 / WP-075 | 범위 공백 | CR-084 | resolved |
| DEV-627 | 2026-09-12 | **내 수정이 다음 결함을 만들었다.** `DEV-622`가 변경 요청 사이에 1초를 넣자 한 회차가 상한(200건)에서 200초까지 늘어났다. 그런데 이벤트 버스는 **30초 방치된 항목을 회수한다**(`claimIdleMs`) — `JOB-SEQ-005`가 적은 「30초」가 바로 그 값이다. 회차가 그보다 오래 붙들면 처리 중인 이벤트를 회수가 가로채 같은 행을 다시 집는다. 멱등이라 제목이 망가지지는 않지만 요청과 감사가 두 벌이 되고 미처리 목록이 쌓인다. 이벤트 경로에 시간 예산(25초, 회수 시한 아래)을 두고 예산이 다하면 **한 것까지 남기고 `defer`로 곧 다시 받는다** — `ack`하면 남은 PR이 잔여 스윕까지 최대 하루를 기다리고, `retry`면 정상 동작이 재시도 횟수를 태워 dead letter로 간다. 잔여 스윕에는 예산을 두지 않는다: 회수 시한의 제약을 받지 않으며 예산을 두면 하루에 몇 건만 메운다 | JOB-SEQ-005 / FR-SEQ-009 AC-3 / WP-075 | 범위 공백 | CR-084 | resolved |
| DEV-626 | 2026-09-12 | **권한 차단이 쓰기 성공에서만 풀렸다.** 저장소가 한 번 차단된 뒤 남은 대상이 모두 이미 표기돼 있으면 GHE가 `200`을 주는데도 차단이 남는다. 그 사이 새로 머지된 PR은 이벤트 경로가 차단 저장소를 제외하므로 **권한이 복구됐는데도 표기가 다음 스윕(기본 하루)까지 밀린다.** 독립 검토 두 판이 같은 자리를 지적했다. 조회가 성공한 시점이 곧 접근이 증명된 시점이므로 `already_annotated`·`mismatch`에서도 차단을 푼다 — 권한 오류 경로에서는 풀지 않는다 | FR-SEQ-009 예외 처리 / JOB-SEQ-005 / WP-075 | 범위 공백 | CR-084 | resolved |
| DEV-625 | 2026-09-12 | **영원히 실패하는 행이 잔여 스윕을 굶길 수 있었다.** 저장소 이름으로 코드를 정할 수 없으면(`no_digits`·`multiple_digit_runs`) 그 행은 몇 번을 다시 봐도 `failed`인데, 목록 정렬이 `repository_id`로 시작하고 상한(기본 200)이 걸려 있었다. **번호가 낮은 그런 저장소가 상한을 통째로 차지하면 높은 번호 저장소의 행에 스윕이 영영 닿지 않는다** — 복구 안전망이 그 순간 무력해진다. 독립 검토가 잡았다. 저장소 코드 규칙을 SQL에 다시 쓰는 대신(정본은 `repositoryCodeOf` 하나여야 한다) **한 번도 시도하지 않은 행을 먼저, 그다음은 가장 오래전에 손댄 것부터** 보도록 정렬을 바꿨다 — 공평한 순서는 영구 실패의 종류를 몰라도 성립하며 다른 영구 실패(권한·검증)에도 같이 듣는다. **남는 것은 순환 자체다** — 그 행들은 스윕마다 다시 뽑혀 `failed`로 재기록된다. GHE 호출은 한 번도 없고(코드 판정이 요청보다 앞이다) 지표 `mnumber_annotate_total{result="code_unavailable"}`가 그 사실을 계속 드러내므로, 저장소 이름을 고칠지 받아들일지는 운영자가 그 지표를 보고 정한다. 규칙을 SQL로 복제해 후보에서 빼는 방법도 있으나 저장소 코드의 정본이 둘이 되는 대가가 더 크다 | FR-SEQ-009 AC-3 / JOB-SEQ-005 / WP-075 | 범위 공백 | CR-084 | resolved |
| DEV-624 | 2026-09-12 | **API 계약 문서가 새 설정 항목을 적지 않았다.** 구현이 `annotate_enabled`를 `PATCH /api/v1/admin/repositories/{id}`로 열었는데 `pr_search_api_contracts.md`는 그 요청이 `sequence_branches`·`mirror_enabled`만 받는다고 적고 있었다. 문서를 보고 구현하는 클라이언트는 **저장소별 비상 스위치의 존재 자체를 알 수 없다** — `THR-046`이 실제로 일어났을 때 가장 먼저 써야 하는 것인데 그렇다. 자동 리뷰가 잡았다 | FR-SEQ-009 AC-6 / API-ADM-001 / WP-075 | 문서 오류 | CR-084 | resolved |
| DEV-623 | 2026-09-12 | **성공 판정이 공식 문서가 보장하지 않는 것을 전제했다.** 응답의 `title`이 보낸 문자열과 **정확히 같을 때만** 성공으로 보았는데, 공식 문서는 응답 `title`이 요청 값과 일치한다고 보장하지 않는다(제목 길이 상한과 공백 처리도 마찬가지로 문서에 없다). 서버가 앞뒤 공백을 다듬기만 해도 **성공한 표기가 `failed`로 기록되고** 지표가 거짓을 말한다. `AC-1`이 요구하는 것은 접두가 붙었는가이므로 같은 순수 판정기에 응답을 넣어 그것을 묻도록 바꿨다 | FR-SEQ-009 AC-1 / WP-075 | 기술 제약 / 계약 공백 | CR-084 | resolved |
| DEV-622 | 2026-09-12 | **변경 요청의 속도 지침을 따르지 않았다.** 공식 문서가 「`POST`·`PATCH`·`PUT`·`DELETE` 요청을 많이 보낼 때는 각 요청 사이에 최소 1초를 기다리고, 동시에 보내지 말고 직렬로 보내라」고 정하며 「한도에 걸린 채 요청을 계속하면 integration이 차단될 수 있다」고 경고하는데, 회차가 상한(기본 200건)만큼을 간격 없이 연달아 보내고 있었다. 직렬이기는 했으나 간격이 없었다. 실제로 쓴 뒤에만 `MNUMBER_ANNOTATE_WRITE_SPACING_MS`(기본이자 하한 1초)를 쉬도록 고쳤다 — 조회만 한 회차까지 늦추면 스윕이 이유 없이 길어진다 | FR-SEQ-009 / JOB-SEQ-005 / WP-075 | 범위 공백 | CR-084 | resolved |
| DEV-621 | 2026-09-12 | **에폭 울타리가 목록 질의 시점에만 있었다.** `listAnnotateTargets`의 조인은 **그 질의가 도는 순간**의 에폭만 증명한다. 목록을 뽑아 한 건씩 처리하는 동안 재채번이 들어오면 그 뒤의 행은 이미 무효가 된 M 번호를 들고 GHE로 나가고, **제목은 되돌릴 수 없다.** 자동 리뷰가 잡았다. 외부 쓰기 바로 앞에서 정본을 다시 묻는 `isAnnotationCurrent`를 두고, 재채번 중(`reassigning`) 공간은 목록에서 제외했다. **경합이 사라지지는 않는다** — 그 질의와 PATCH 사이의 간격은 남으며, 창이 목록 전체의 처리 시간에서 질의 한 번으로 줄었을 뿐이다 | FR-SEQ-008 AC-4 / FR-SEQ-009 AC-7 / WP-075 | 범위 공백 | CR-084 | resolved |
| DEV-620 | 2026-09-12 | **이벤트 회차와 잔여 스윕이 같은 프로세스에서 겹칠 수 있었다.** 둘은 서로를 모르는 채 돌므로 아직 표시되지 않은 같은 행을 둘 다 골라, 둘 다 제목을 읽고 둘 다 PATCH를 보낼 수 있었다. 결과 제목은 같아도 **요청과 감사 기록이 두 벌**이 되어 「쓴 횟수」가 거짓이 된다. 배포가 replica 1이라는 사실은 파드 **사이**를 막을 뿐 이 경합을 막지 못한다. 자동 리뷰가 잡았다. 프로세스 안에서 회차를 하나씩 돌도록 직렬화했다 — 공식 문서의 「변경 요청은 직렬로」와도 같은 방향이다 | FR-SEQ-009 AC-2 / JOB-SEQ-005 / WP-075 | 범위 공백 | CR-084 | resolved |
| DEV-619 | 2026-09-12 | **토큰 발급 실패가 표기의 재시도 모델을 우회했다.** `InstallationTokenProvider`는 `GitHubApiError`를 던지는데 재시도와 한도 유예는 `AnnotateApiError`만 알아본다. 그래서 **토큰 엔드포인트의 일시 장애가 영구 실패로 기록되고** 이벤트는 ack되어 회복이 다음 스윕(기본 하루)까지 밀렸다. 자격이 멀쩡한데도 그렇다. 자동 리뷰가 잡았다. 발급기의 오류를 표기 오류 모델로 옮기되 `not_found`는 권한으로 본다 — 공식 문서가 **비공개 자원에 대한 인증 부족을 `403`이 아니라 `404`로 답한다**고 적기 때문이다 | FR-SEQ-009 AC-3 / JOB-SEQ-005 / WP-075 | 범위 공백 | CR-084 | resolved |
| DEV-618 | 2026-09-12 | **제목 쓰기의 read-then-write 경합을 공식 API로 막을 수 없다.** 제목을 읽은 뒤 PATCH하기까지의 사이에 사용자가 제목을 고치면 그 편집이 덮인다. 조건부 쓰기로 막으려 했으나 공식 문서가 **「`POST`·`PUT`·`PATCH`·`DELETE` 같은 비안전 메서드의 조건부 요청은 특정 엔드포인트에서 따로 명시하지 않는 한 지원하지 않는다」**고 적고, PR 갱신 엔드포인트에는 그런 명시가 없다. **없는 완화를 지어내지 않는다**(`THR-046`의 규율). 대신 창을 좁히고 피해 반경을 적는다 — 쓰기는 제목 한 필드뿐이고, 한 PR에 **평생 한 번만** 쓰므로(그 뒤 `done`) 노출은 PR당 한 번의 수백 밀리초이며, 덮인 제목은 사용자가 다시 고치면 다음 회차가 접두를 보고 아무것도 하지 않는다. 자동 리뷰가 지적했고 완화 불가를 문서 근거로 확인했다 | FR-SEQ-009 AC-1 / THR-046 / WP-075 | 기술 제약 / 계약 공백 | CR-084 | **open** — 공식 API가 조건부 쓰기를 지원하면 재검토한다. 지금은 창을 좁히는 것 외에 할 수 있는 것이 없다 |
| DEV-617 | 2026-09-12 | **감사 어휘의 정본과 코드가 어긋나 있었다.** `srs_final.md` 10.2절 감사 대상 액션 표는 `CR-077` 이후 `pull_request.annotate`를 **미활성(WP-075가 소유)**으로 적고 있었는데, `packages/domain/src/audit.ts`의 `NOT_ACTIVATED_AUDIT_ACTIONS`는 **빈 배열**이었다 — 그 파일의 주석이 "WP-044가 export.create를 배선하여 미활성 액션이 남지 않았다"고 적는데 그 문장은 `CR-077`이 표에 새 행을 더하기 전에 쓰인 것이다. 그래서 **미활성 액션이 코드 어디에도 표현되지 않은 채** SRS만 그것을 말하고 있었고, `DEV-403`이 세운 "기능 활성화와 함께 이동한다"는 규칙이 걸릴 대상이 없었다. 활성 목록·미활성 목록·SRS 표가 같은 말을 하는지를 회귀가 묻지 않은 것이 원인이다. `WP-075`가 이 액션을 실제로 배선하면서 활성으로 옮겼고, **SRS 표의 상태 칸과 코드의 두 목록을 함께 대조하는 회귀**를 세웠다 | FR-AUTH-004 AC-1 / FR-SEQ-009 AC-4 / WP-075 | 문서 오류 | CR-084 | resolved |
| DEV-616 | 2026-09-12 | **`403`의 두 원인을 구분하는 방법을 공식 문서가 보장하지 않는다.** `FR-SEQ-009`의 예외 처리는 「GHE가 403·404를 반환하면 그 저장소의 표기를 중단한다」고 적고 사유로 「권한이 없는데 재시도를 반복하면 rate limit만 소모한다」를 든다. 그런데 GitHub 공식 문서는 **주 한도와 부 한도 모두 `403` 또는 `429`로 온다**고 적으며, 한도로 인한 403과 권한 거부 403을 응답에서 가르는 방법은 문서에 없다. 상태 코드만 보고 전부 권한으로 단정하면 **한도 소진 한 번이 그 저장소의 표기를 영구히 멈추고**, 전부 한도로 보면 권한 없는 저장소에 계속 요청해 FR이 막으려던 바로 그 소모가 일어난다. **문서가 보장하는 것만 쓴다** — 같은 문서가 정한 재시도 순서(「`retry-after`가 있으면 그 시간만큼, 없고 `x-ratelimit-remaining`이 0이면 `x-ratelimit-reset`까지, 그 밖에는 최소 1분」)를 그대로 구현하고, **대기 신호가 하나도 없는 403만 권한으로 다룬다**. 기다릴 근거가 없으면 재시도가 의미를 갖지 못하기 때문이다. `429`는 문서가 한도 전용으로 정의하므로 신호가 없어도 한도로 본다. 실제 로컬 HTTP로 네 경우(대기 신호 없는 403 · 잔여 0인 403 · `retry-after` 있는 403 · 신호 없는 429)를 갈라 확인했다. **공식 계약이 넓어지면 이 판정을 다시 본다** | FR-SEQ-009 예외 처리 / JOB-SEQ-005 / WP-075 | 기술 제약 / 계약 공백 | CR-084 · CR-085 | resolved — 문서가 보장하는 신호만으로 판정하며, 구분 방법이 공식화되면 재검토한다. **추가 근거 (2026-09-13, `CR-085` 자가 연구).** 공식 문서의 Troubleshooting 절이 권한 부족 `403`에 「Resource not accessible by integration」 메시지와 `X-Accepted-GitHub-Permissions` 헤더라는 **적극적 식별 신호**를 부여한다는 것을 확인했다. 지금의 소거법(대기 신호가 없으면 권한)은 그대로 유효하고, 그 신호를 더하면 판정이 소거가 아니라 식별이 된다. **이번 판은 코드를 바꾸지 않았다** — 문서가 세 신호(주 한도·부 한도·권한)의 상호 배타성까지 보장하지는 않으므로 그 조합의 설계는 별도 판단이 필요하다 |
| DEV-615 | 2026-09-11 | **바뀐 쿠키 계약과 릴리스 산출물 게이트가 어긋났다.** `CR-083`이 `AUTH_ENABLED=false`를 명시한 배포의 insecure 쿠키를 허용하도록 계약을 넓혔는데, `deploy/single-host/smoke-images.sh`는 **정확히 그 조합이 거부되어야 한다**고 요구하고 있었다. 계약을 강제하는 자리가 셋(단위 시험·기동 검증·배포 게이트)인데 앞의 둘만 갱신했다. 결과는 번들 빌드가 「번들 이미지가 런타임 검사를 통과하지 못했다」로 멈춘 것이고, **릴리스가 나가지 못했다.** 그 시점에 회귀는 초록이었다 — `runtime-reachability.test.ts`가 스모크에 그 검사가 **있는지**를 문자열로 보므로, 검사가 그대로 있고 **기대만 반대**인 상태를 볼 수 없었다. 게이트의 기대 조합을 소스에서 뽑아 `webConfigFailure`에 넣어 대조하는 시험을 `apps/web/lib/server/config.test.ts`에 세웠고, 거부뿐 아니라 **허용해야 할 형상이 실제로 서는지**도 게이트가 검사하게 했다 — 한 방향만 거는 게이트는 계약이 넓어질 때 조용히 반대 방향으로 거짓말한다 | FR-AUTH-001 AC-2 / WP-072 · WP-076 | 범위 공백 | CR-083 | resolved |
| DEV-614 | 2026-09-11 | **로그인 콜백이 세션 쿠키를 응답에서 잃어버렸다.** `NextResponse.cookies.set`은 자기가 아는 목록으로 `set-cookie` 헤더를 **다시 쓴다.** 콜백은 세션 쿠키를 `headers.append`로 달고 그 뒤에 왕복 쿠키 삭제를 `cookies.set`으로 했으므로, **세션 쿠키가 그 한 번에 사라졌다.** 로그인은 성공하고 리다이렉트도 정상인데 브라우저는 세션을 받지 못해 사용자가 로그인 화면으로 되돌아온다. **`CR-083` 이전부터 있었고 OIDC 경로도 같았다** — 라이브러리 시험이 `serializeSessionCookie`가 옳은 문자열을 만드는지만 묻고 그 문자열이 응답에 남는지는 아무도 묻지 않았으며, 콜백에는 라우트 시험 자체가 없었다. 독립 검토가 그 공백을 지적해 시험을 세우자 첫 실행에서 잡혔다. 두 쿠키를 같은 방법(`headers.append`)으로 달아 고쳤다 — 순서를 맞추는 것으로 고치면 다음 사람이 줄을 옮길 때 같은 자리가 다시 열린다 | FR-AUTH-001 AC-2 / WP-015 · WP-076 | 범위 공백 | CR-083 | resolved |
| DEV-613 | 2026-09-11 | **로그인이 사용자 정본에 행을 만들지 않는다.** `AccessScopeResolver`는 `app_user` 행이 없으면 `ScopeUnavailableError`를 던지고 그 자리의 주석이 「로그인이 만들었어야 하므로 정합성 문제다」라고 적는데, `upsertUserOnLogin`을 부르는 **프로덕션 코드가 어디에도 없었다** — 시험 픽스처만 그 함수를 썼다. 그래서 로그인은 성공하고 세션도 만들어지는데 **첫 조회가 503 `permission_unavailable`이 된다.** 사내가 아직 OIDC를 켜 본 적이 없어 드러나지 않았고, `CR-083`이 GHE 로그인을 여는 순간 처음 밟게 되는 자리다. `web`은 `@prs/db`에 의존하지 않으므로 콜백에서 직접 만들 수 없어, 세션을 읽는 자리(`RegisteringSessionStore`)에서 upsert한다 | FR-AUTH-001 · FR-AUTH-002 / WP-012 · WP-076 | 범위 공백 | CR-083 | resolved |
| DEV-612 | 2026-09-11 | **`.env.example`의 그룹 매핑 안내가 파서와 어긋났다.** 「`그룹=역할` 쌍을 쉼표로 잇는다. 역할은 보안 문서 5.1의 여섯 중 하나다」라고 적혀 있으나, `parseGroupRoleMap`은 **콜론**으로 가르고 (`group:role`) **`manager`·`qa` 둘만** 부여를 허용한다(`CR-015`·`DEV-049`). 안내대로 적으면 형식 오류로 던지고, 역할 여섯 중 나머지 넷을 적어도 던진다. `DEV-577`이 `SESSION_COOKIE_SECURE`에서 겪은 것과 같은 모양 — 문서가 운영자를 거부당하는 값으로 보낸다 | FR-AUTH-001 AC-8 / WP-012 | 문서 오류 | CR-083 | resolved |
| DEV-561 | 2026-09-11 | **`git` 서브프로세스가 사내 CA를 신뢰하지 않아 `JOB-MIR-001`이 실패했다.** 사내 `0.1.0-pilot.4` 업그레이드에서 미러 초기화가 `SSL certificate problem: unable to get local issuer certificate`로 끝나고 미러 볼륨이 빈 채로 남았다. `NODE_EXTRA_CA_CERTS`는 **Node 런타임만** 읽는다 — 미러 초기화와 미러 fetch는 `git`을 서브프로세스로 부르므로 `GIT_SSL_CAINFO`를 따로 받아야 한다. **런북 6장은 그 사실을 이미 적고 있었으나** `compose.yml`에 그 변수를 넘기는 자리가 없어 문서가 존재하지 않는 경로를 안내했다. 다른 서비스는 Node가 CA를 읽어 정상이었으므로 증상이 CA로 보이지 않았고, 그것이 진단 비용을 만들었다 | JOB-MIR-001 · WP-070 / FR-SEQ-001 | 범위 공백 | CR-082 | resolved |
...
| DEV-590 | 2026-09-11 | **`mnumber_evidence`의 `ON DELETE RESTRICT`가 기존 통합 시험의 정리를 막았다 — CI가 잡았다.** `apps/search-api/integration/release/{containment,timeline}.test.ts`가 `beforeAll`에서 조건 없이 `DELETE FROM merge_sequence`를 부르는데, **앞서 끝난 다른 파일**(`sequence/mnumber.test.ts`, 저장소 7431)이 남긴 증거가 그 행을 참조해 `23503 … is still referenced from table "mnumber_evidence"`로 죽는다. **외부 전량 실행 3회는 통과했다** — 통합 시험은 `fileParallelism: false`로 **순차 실행**이므로 경합이 아니라 **앞선 파일이 정리하지 않고 남긴 잔여 행**이 원인이고, 파일 실행 순서가 로컬과 CI에서 달라 CI에서만 났다. 처음에는 이것을 병렬 경합으로 적었는데 사실이 아니었고, 독립 리뷰가 `vitest.integration.config.ts:39`를 들어 바로잡았다 — 재현 조건을 잘못 적으면 같은 부류를 다음에 조사할 때 엉뚱한 곳을 보게 된다. **제약을 풀지 않았다**: 증거가 행과 함께 조용히 사라지면 "무엇을 근거로 확정했는가"가 흔적 없이 없어지므로 `RESTRICT`가 옳다. **지우는 쪽을 고쳤다** — `clearMergeSequence(pool, where?, params?)`를 `packages/db/integration/helpers.ts`에 두고 앱 헬퍼 둘이 그대로 재수출하며, `DELETE FROM merge_sequence`를 부르던 15개 시험 파일이 전부 이 헬퍼를 쓴다. 순서가 한 곳에만 있어야 한 곳이 빠진 날 그 파일에서만 깨지는 일이 없다. **반대 방향 단언을 시험으로 세웠다** — 증거가 있으면 정본 행 삭제가 거절되고 순서를 지키면 지워진다(`merge-number-schema.test.ts`) | FR-SEQ-008 / WP-074 / ENT-SEQ-005 | 시험 기반 결함 (신규 제약이 기존 정리를 막음) | CR-080 | **resolved** — 스키마는 그대로, 정리 순서를 공유 헬퍼로 모았다 |
| DEV-598 | 2026-09-11 | **재채번 중인 공간에서 durable 러너가 백오프 없이 무한 재시도했다 — 독립 리뷰가 실행으로 재현했다.** 강제 푸시가 오면 `markReassigning`이 본 트랜잭션과 **별개로 먼저 커밋**하고(재채번 중 조회가 마지막 확정 값을 보게 하는 기존 설계다), 그 뒤 본 트랜잭션이 전체 체인을 다시 걷는다. 그 동안 `reconcileMergeNumbers`는 `space_reassigning`을 돌려주는데 러너가 그것을 `delayMs: 0`으로 놓았고, `claimed > 0`이 `pollMs` 대기를 건너뛰어 **회차마다 다섯 문장이 나가는 바쁜 루프**가 됐다. 리뷰가 가짜 pool로 재현해 3초에 claim 545회를 측정했다 — 대조군(집을 work 없음)은 3회다. 복제본 하나가 초당 약 900문장을 보낸다. 큰 저장소의 재채번이 수 분이면 그 내내 그렇고, 워커가 `markReassigning` 커밋 뒤 죽으면 상태가 남아 **사람이 개입할 때까지 끝나지 않는다**(`reassignSequence`에는 `restoreSequenceState`가 없다). **세 `retry` 사유를 갈랐다** — `checkpoint_moved`·`epoch_moved`는 남이 값을 옮긴 것이라 다음 회차가 앞으로 가므로 즉시 재시도가 옳고, `space_reassigning`은 기다림이므로 락 경합과 같은 5초 defer를 쓴다. 설계 6.3절이 정한 것도 백오프이지 지연 0이 아니다 | JOB-SEQ-004 / WP-074 / 설계 6.3 | 구현 결함 (자원 소진) | CR-080 | **resolved** — 사유별로 처분을 가른다 |
| DEV-599 | 2026-09-11 | **`formatMergeNumber`가 되읽을 수 없는 문자열을 만들 수 있었다 — 두 번째 독립 검토가 찾았다.** `number`는 막으면서 `code`는 전혀 보지 않아 `''`가 `M--3`을, `'19-00'`이 `M-19-00-3`을 만들고 둘 다 `parseMergeNumber`가 `null`을 낸다. **이 파일이 내건 왕복 불변식이 한쪽에서만 지켜졌다.** 지금은 호출부가 `repositoryCodeOf`의 결과만 넘겨 도달하지 않지만, 불변식은 함수가 스스로 지켜야 한다 | FR-SEQ-008 / OD-009 / WP-074 | 방어 누락 | CR-080 | resolved — 코드가 숫자만으로 이루어지지 않으면 던지고, 만든 것이 언제나 되읽히는 것을 시험이 건다 |
| DEV-600 | 2026-09-11 | **화면의 입력 판정 셋이 계약보다 느슨했다.** (1) `numberOrNull`이 `Number.isFinite`만 봐서 `pr_number: 1.5`가 `/pr/acme/pay/1.5`를, `-3`이 `/pr/acme/pay/-3`을 만들었다 — 설계 9절은 `1..2147483647`과 정수를 정한다. (2) 중복 query key를 **조용히 첫 값으로 접었다** — 서로 다른 두 번호를 담은 링크가 오류 없이 하나로 이동하고, 사용자는 자기가 무엇을 열었는지 모른 채 그 답을 인용한다. 설계 9절은 중복 key를 400으로 정하는데 클라이언트가 접으면 서버가 그 답을 낼 기회 자체가 없다. (3) `hasPendingMergeNumber`가 배지가 **숨긴 행**(커밋)까지 세어, 보이지 않는 것을 기다리느라 60초 동안 재검증이 헛돌 수 있었다 — 같은 파일의 두 함수가 "이 행에 M이 있는가"에 다른 답을 했다 | FR-SEQ-008 / 설계 9절 / WP-074 | 계약 미이행 | CR-080 | resolved — 셋을 고치고 `duplicated` 갈래를 화면에 붙였다 |
| DEV-601 | 2026-09-11 | **한 PR에 정본 행이 둘일 때 같은 입력이 두 답을 냈다 — 두 번째 독립 검토가 실행으로 보였다.** `indexCanonical`이 PR 번호를 키로 `rows.set()`을 돌려 **나중 행이 앞 행을 덮는데**, 어느 행이 이길지는 `lookupMergeNumbers`가 돌려준 순서가 정하고 그 질의에 `ORDER BY`가 없었다. 도달 경로는 **같은 PR의 이중 squash SHA**다(설계 3절이 명시적으로 상정한다). `merge_sequence_numbered_pr_uk`가 `WHERE merge_number IS NOT NULL` 부분 인덱스라 미채번 행의 중복을 막지 않고, 그 상태는 planner가 `mapping_conflict`로 멈추는 바로 그 상태이므로 두 행이 정본에 영구히 함께 남는다. 검토가 배열 순서만 바꿔 `M-1900-42`와 `pending / mapping_conflict`가 번갈아 나오는 것을 보였다 — **데이터가 전혀 바뀌지 않았는데 새로고침마다 답이 달라진다.** `FR-SEQ-008 AC-3`("부여된 M 넘버는 어떤 경우에도 다른 항목으로 옮겨 가지 않는다")과 정면으로 어긋난다. 부여된 번호를 가진 행이 언제나 이기게 하고, 둘 다 없으면 작은 서수를 고르며, 질의에도 `ORDER BY`를 두어 조회 순서 자체를 고정했다 | FR-SEQ-008 AC-3 / API-SEQ-007 / WP-074 | 구현 결함 (비결정성) | CR-080 | **resolved** — 순서를 뒤집는 변이를 적용해 시험이 잡는 것을 확인했다 |
| DEV-602 | 2026-09-11 | **blocker 사유를 enum 검증 없이 응답에 그대로 실었다.** `mnumber_blocked_reason`은 DTO의 `merge_number_reason`으로 나가 화면이 "사유 코드: …"로 렌더하는데, 읽는 쪽이 도메인의 `isMergeNumberBlockReason`을 쓰지 않았고 DB 제약도 길이뿐이다. 검토가 그 열에 `connect ECONNREFUSED 10.0.0.4:5432`를 넣어 **접속 주소가 화면에 그대로 실리는 것**을 실행으로 보였다. 쓰기 쪽이 타입으로 막고 있어 현재 유출 경로는 없지만, 설계 9절이 "내부 예외 메시지를 reason으로 전달하지 않는다"를 정했으므로 **읽는 쪽도 한 번 더 본다** | FR-SEQ-008 / 설계 9절 / THR-006 | 방어 누락 | CR-080 | resolved — 모르는 값은 일반 대기로 접고, 아는 사유는 그대로 전한다 |
| DEV-603 | 2026-09-11 | **planner의 멱등 재실행 분기가 생산 호출부에서 도달할 수 없다.** `existingNumber`가 있는 행을 통과시키는 갈래는, 같은 행이 `isAlreadyNumbered`도 참으로 만들어 그 아래 `mapping_conflict`에 먼저 걸린다(`listNumberedPullRequests`가 `merge_number IS NOT NULL AND pull_request_number = ANY(...)`로 조회하므로 반드시 그렇다). 기존 시험이 `isAlreadyNumbered: () => false` 대역을 써서 이 모순을 덮었고, **그 시험은 멱등을 증명하지 못한다.** 정상 경로에서는 번호와 checkpoint가 한 트랜잭션이고 공간 락이 동시 기록을 막아 이 상태가 생기지 않으므로 남긴다 — 다만 이 갈래가 대비하려던 상황이 실제로 오면 의도한 통과가 아니라 공간 전체를 멈춘다. AC-7의 실질적 보증은 `assignMergeNumbers`의 `AND (ms.merge_number IS NULL OR ms.merge_number = a.merge_number)`와 checkpoint가 하고 있다 | FR-SEQ-008 AC-7 / WP-074 | 도달 불가 코드 | CR-080 | **open** — 이 판에서 고치지 않았다. 고치려면 두 방어 중 어느 쪽이 정본인지 정해야 하고 그것은 계약 판단이다 |
| DEV-604 | 2026-09-11 | **`DEV-594`를 고치면서 계약에 없는 키를 이벤트 payload에 실었다 — 재검증이 잡았다.** 상관 ID를 발행 시점에 복원하려고 `announce` work의 payload에 남겼는데, 그 payload를 그대로 `EVT-SEQ-004`로 내보내 **일곱 번째 키**가 실렸다. 봉투에 이미 있는 값이 payload에 중복되고 `MergeNumberAssigned` 타입이 말하는 여섯 필드와 실제로 나가는 것이 달라진다. 기존 시험이 `toMatchObject`라 추가 키를 잡지 못했다 — 재검증이 발행을 가로채 키 목록을 세어 찾았다. 발행 직전에 상관 ID를 분리해 **봉투에만** 싣는다. 오늘 깨지는 소비자는 없다(힌트 구독이 `mnumber.assigned`를 무시한다) | EVT-SEQ-004 / WP-074 | 수정이 만든 결함 | CR-080 | resolved — 키 집합 자체를 세는 시험을 세웠다 |
| DEV-605 | 2026-09-11 | **`DEV-597`의 재색인 복구가 PR마다 왕복을 하나씩 만들었고, 꺼진 배포에 잔여 행을 남겼다 — 재검증이 잡았다.** 5만 PR 저장소면 재색인 경로에 왕복이 5만 번 들어간다. 그리고 `MNUMBER_ENABLED=false`인 배포에서는 러너가 `refresh`만 집고 `cleanupDoneWork`가 `done`만 지우므로 그 `materialize` 행들이 **영영 `ready`로 남아** 대기 지표에 잡힌다. `requestWorkBatch`로 페이지 하나를 문장 하나로 묶고, 꺼진 배포에서는 의도를 아예 만들지 않는다 — 아무도 처리하지 않을 일을 표에 쌓지 않는다. 배치가 단건과 같은 뜻인지(키·payload·generation, 진행 중 lease 보존)를 시험이 걸고, 두 `ON CONFLICT` 절이 **문자 그대로 같음**을 정규화 대조로 확인했다. 재색인이 설정을 다시 읽는 것은 그 함수가 순수하기 때문이며 그 전제도 시험으로 고정했다 | JOB-SEQ-004 / WP-074 | 수정이 만든 결함 | CR-080 | resolved |
| DEV-606 | 2026-09-11 | **`DEV-605`의 가드가 `DEV-597`을 배포에서 조용히 무효로 만들었다 — 재검증이 잡았다.** 꺼진 배포에 잔여 행을 남기지 않으려고 재색인에 `MNUMBER_ENABLED` 가드를 뒀는데, **재색인은 `batch` 역할에서 돌고 그 컨테이너에는 이 값이 전달되지 않는다.** compose의 `app-env` 앵커에도, K8s의 batch 매니페스트에도 그 이름이 없어 `resolveMergeNumberConfig()`가 언제나 `false`를 답한다. 결과로 M을 켠 배포에서도 재색인 뒤 복구 의도가 **한 건도 생기지 않고**, 새 색인의 M이 영영 비어 `merge_number_projection_state`가 영구 `pending`이 된다 — 오류도 로그도 없이 그렇게 된다. **내가 major로 고쳤다고 적은 것이 되돌아온 것이며, 그 기록이 사실과 달라지는 것이 이 편차의 무게다.** 값을 `worker-batch`와 K8s batch에 전달했다. **회귀가 개수가 아니라 서비스 이름으로 센다** — 개수만 세면 한 곳을 빼고 다른 곳에 둘을 둬도 통과한다. `web`에 주지 않는다는 `DEV-589`의 결정은 그대로다 | JOB-SEQ-004 / WP-074 / DEV-597 | 수정이 만든 결함 (배포 배선) | CR-080 | **resolved** — 플래그를 빼는 변이를 적용해 시험이 잡는 것을 확인했다 |
| DEV-607 | 2026-09-11 | **`DEV-606`을 고치면서 `batch` 역할이 채번 설정 전체를 검증하게 됐다 — 내가 리뷰어에게 물으며 직접 찾았다.** 재색인이 플래그를 읽도록 `resolveMergeNumberConfig()`를 불렀는데, 그 함수는 `MNUMBER_BATCH_SIZE`·`MNUMBER_POLL_MS` 같은 **채번 전용 값까지 검증하고 범위를 벗어나면 던진다**(실측: `MNUMBER_BATCH_SIZE=9999` → `1..1000의 정수여야 한다`). 그러면 채번과 무관한 `batch` 역할이 채번 설정 때문에 기동하지 못하고 **정리·보존·재색인이 함께 죽는다.** 그 검증은 그 값을 실제로 쓰는 `sequence` 역할의 몫이다. 재색인은 **플래그 한 값만** 읽게 했고, 켜짐의 정의(`'true'` 하나)가 두 곳에서 같다는 것을 회귀가 고정한다. **주석이 아니라 실제 코드를 본다** — 왜 부르지 않는지 설명하는 주석에 그 함수 이름을 적는 것은 막을 이유가 없다 | JOB-SEQ-004 / WP-074 / DEV-606 | 수정이 만든 결함 (기동 결합) | CR-080 | **resolved** — 되돌리는 변이를 적용해 시험이 잡는 것을 확인했다 |
| DEV-608 | 2026-09-11 | **M 켜짐의 정의가 세 곳에 흩어져 이미 갈라져 있었다 — 독립 검토가 실측 표로 보였다.** 세 구현(`search-api`의 `resolveMergeNumberEnabled`, 워커의 `resolveMergeNumberConfig`, `DEV-607`이 만든 인라인 판정)을 같은 입력으로 부르니 **일곱 중 셋에서 답이 달랐다.** 빈 문자열에 `search-api`는 `false`인데 워커는 던졌고(`??`가 `''`를 잡지 않아 그대로 내려간다), `yes`·`TRUE`에 두 곳은 던지는데 `batch`만 **조용히 꺼졌다** — 그것이 `DEV-606`에서 고친 실패 모양 그대로다. K8s는 매니페스트가 역할마다 따로라 configmap의 빈 값 하나가 `worker-sequence`만 죽이는 경로가 실재한다. **단언이 아니라 구조로 막았다** — 워커에 `resolveMergeNumberEnabled`를 두고 `resolveMergeNumberConfig`와 batch 블록이 **둘 다 그것을 부른다.** 빈 문자열은 `search-api`와 같게 꺼짐으로 맞췄다(compose가 이미 빈 값을 `false`로 접는다). `DEV-607`의 목적은 그대로다 — 이 함수는 채번 전용 값을 건드리지 않는다 | JOB-SEQ-004 / WP-074 / DEV-607 | 구현 분기 (역할 사이) | CR-080 | **resolved** — 회귀가 두 앱의 함수를 실제로 불러 입력 표로 건다 |
| DEV-609 | 2026-09-11 | **대기가 끝나도 소진 배너가 남는다 — 병합 후 자동 리뷰가 잡았다.** `usePendingRevalidation`이 `pending`이 사라질 때 `startedAt`만 지우고 `exhausted`를 남긴다. 60초를 다 쓴 뒤 번호가 붙으면 **화면이 번호를 보이면서 동시에 "아직 확정되지 않았습니다"라고 말한다.** 게다가 `active`가 `!exhausted`에 막혀 다음 대기의 재검증이 아예 시작되지 않는다. 대기가 끝났다는 사실이 곧 그 표시를 지울 근거다. **처음 쓴 시험이 이것을 증명하지 못했다** — "다시 확인" 버튼을 눌렀는데 그 버튼은 `restart()`가 어차피 소진을 지우므로 변이를 적용해도 통과했다. 버튼 없이 `pending`만 거짓이 되는 경로를 훅으로 직접 관찰하도록 고쳐서야 잡혔다 | FR-SEQ-008 / WP-074 / 설계 9절 | 구현 결함 | CR-081 | **resolved** — 변이로 확인 |
| DEV-610 | 2026-09-11 | **관측 루프가 표본마다 정본을 물었다 — 병합 후 자동 리뷰가 잡았다.** 기본 한도 100에서 왕복이 100번이고 그것이 2초마다 돈다. `lookupMergeNumbers`는 이미 튜플 목록을 받으므로 묶는 비용이 없다 — 목록·상세가 페이지를 한 번에 묻는 것과 같은 규율이다(ADR-023 C5). **ES는 묶지 않았다** — 문서마다 조회가 필요하고, 여기서 보려는 것이 실제 검색 hit라 그 한 번을 줄이면 관측의 뜻이 달라진다. 정본 조회가 실패하면 이번 회차는 아무것도 관측하지 않고 다음 회차가 다시 본다 — 없는 사실을 남기지 않는다. **관측 루프에 시험이 아예 없었다** | FR-SEQ-008 AC-14 / WP-074 | 효율 결함 | CR-081 | resolved — 표본 수와 무관하게 조회가 1회임을 시험이 건다 |
| DEV-611 | 2026-09-11 | **`DEV-610`을 고치며 `DEV-601`에서 세운 규칙을 워커 쪽에 다시 어겼다 — 자동 리뷰가 잡았다.** 관측 루프의 배치 조회를 `Map`으로 색인하면서 **나중 행이 앞 행을 덮게** 했다. `lookupMergeNumbers`는 같은 PR의 두 행(이중 squash SHA)을 `merge_seq` 순서로 둘 다 돌려주므로, 번호 있는 행이 앞이고 충돌 행이 뒤면 관측이 번호 없는 쪽을 보고 **영영 `visible`이 되지 않는다.** `merge-number-view.ts`의 `preferRow`가 API 쪽에서 세운 것과 같은 규칙을 적용했다 — **같은 정본을 읽는 두 곳이 다른 행을 고르면 안 된다.** 한 번 고친 종류의 결함이 다른 파일에서 되살아난 것이며, 그 규칙이 한 곳에만 적혀 있던 것이 원인이다 | FR-SEQ-008 AC-3 / WP-074 / DEV-601 | 수정이 만든 결함 | CR-081 | **resolved** — 충돌 행이 뒤에 있어도 번호를 가진 행으로 관측하는 것을 시험이 건다 |
| DEV-591 | 2026-09-11 | **목록·상세·범위 DTO가 `branch_not_tracked`를 한 번도 만들지 않았다 — 독립 리뷰가 찾았다.** `mergeNumberFieldsOf`의 2단계가 `context.trackedBranches`를 보는데 그 입력을 채우는 호출부가 하나도 없었다(`grep`으로 `src` 사용처 0건). 그래서 채번 대상이 아닌 브랜치에 머지된 PR이 `not_applicable / branch_not_tracked`가 아니라 `pending / not_sequenced`가 되고, 화면은 **"시퀀스 채번 대기"라고 말한다 — 그 PR은 영원히 채번되지 않으므로 거짓 안내다.** 같은 PR을 `API-SEQ-007`로 물으면 409 `branch_not_tracked`를 정확히 답하므로 **두 표면의 답이 갈렸다.** 화면의 "채번 비대상 브랜치" 갈래는 도달 불가능한 죽은 코드였다. `lookupMergeNumbers`가 같은 스냅숏에서 저장소의 `sequence_branches`를 함께 읽어 넘기게 했다 — 왕복이 하나 늘지만 행 수와 무관한 고정 1회다 | FR-SEQ-008 / WP-074 / 설계 9절 | 구현 누락 | CR-080 | **resolved** — 배치가 대상 브랜치를 함께 읽고, 두 표면이 같은 답을 하는 것을 시험이 건다 |
| DEV-592 | 2026-09-11 | **해석·목록 API가 단일 read 스냅숏이 아니었다 — 한 응답이 두 세대를 섞을 수 있었다.** 계약이 "REPEATABLE READ read-only transaction 또는 단일 SQL snapshot"을 명시했는데(설계 9절·API-SEQ-007) 구현은 `Pool`에 문장을 따로 보냈고, `grep`으로 확인한 `REPEATABLE READ` 사용처가 **0건**이었다. 공간 조회가 에폭 4를 읽어 대조를 통과한 뒤 재채번이 커밋하면, 에폭 인자를 받지 않는 `lookupMergeNumbers`가 현재 공간을 따라가 에폭 5 행을 돌려준다 — 응답은 `seq_epoch: 4, epoch_stale: false`인데 `merge_seq`와 `merge_number_epoch`는 5다. 화면은 에폭 4를 적으면서 에폭 5의 링크를 만든다. `withReadSnapshot`을 두어 해석의 2~6단계와 목록 배치를 각각 한 스냅숏에 묶었다. **스냅숏은 정본 읽기에서 끝난다** — ES 조회는 네트워크 시간만큼 걸리므로 그 동안 읽기 커넥션을 붙잡으면 부하가 몰릴 때 그것이 병목이 된다(미러 락이 fetch 동안 시퀀스 트랜잭션을 열지 않는 것과 같은 규율). 처음 고칠 때는 주석만 그렇게 적고 ES 호출이 스냅숏 안에 남아 있었고, 자체 점검에서 그것을 잡아 밖으로 뺐다. 시험이 `ROLLBACK`이 ES 조회보다 먼저 나오는 것을 단언하며, 되돌리는 변이를 적용해 실제로 잡는 것을 확인했다 | FR-SEQ-008 / API-SEQ-007 / ADR-007 | 계약 미이행 | CR-080 | **resolved** — 두 경로가 `BEGIN ISOLATION LEVEL REPEATABLE READ READ ONLY` 안에서 돌고, 시험이 그 문장의 존재까지 단언한다 |
| DEV-593 | 2026-09-11 | **한 push가 표본 두 행을 만들어 정상 채번에서도 영구 대기가 쌓였다.** 수신 시점에는 PR을 몰라 `pr_number IS NULL`로 행을 만들고, M 번호가 붙으면 `pr_number`를 채운 **새 행**을 만들었다. unique 키에 `pr_number`가 들어 있어 둘이 서로를 덮지 않으므로 push 행은 `mnumber_assigned_at` 없이 남아 `pending`으로 집계되고, `received_to_sequence`는 같은 push를 두 번 셌다. 운영자는 아무 문제 없는 배포에서 push 수만큼 대기가 쌓인 표를 본다 — **측정 도구가 실제 상태가 아니라 자기 구조를 보고한 것이다.** `promotePushSample`이 비어 있는 `pr_number`를 채워 같은 행을 잇게 했다. 한 push가 PR 여럿을 실어 오면 첫 PR이 그 행을 가져가고 나머지는 각자 새 행을 만든다 — 행 수가 PR 수와 같아진다 | FR-SEQ-008 AC-14 / WP-074 | 구현 결함 | CR-080 | **resolved** — 승격과 다중 PR 갈래를 실제 PostgreSQL로 건다 |
| DEV-594 | 2026-09-11 | **`EVT-SEQ-004`만 상관 ID 없이 발행됐다.** `announce` work의 payload에 상관 ID가 없어 발행 시점에 복원할 수단이 없었고, 빈 문자열이 나갔다. `sequence.assigned`는 이미 싣고 있어 **한 push의 추적이 채번 직전에서 끊겼다.** 채번을 유발한 work의 상관 ID를 회차에 넘기고 `announce` payload에 실어 잇는다. 유발 요청이 없는 조정 스윕에는 그 회차의 새 ID를 만든다 — 빈 문자열보다 낫다 | EVT-SEQ-004 / WP-074 | 관측 누락 | CR-080 | resolved |
| DEV-595 | 2026-09-11 | **`epoch_stale` 응답의 키 부재 단언이 계약이 정한 여덟 중 다섯만 셌다.** `merge_number_epoch`·`merge_number_reason`·`merge_number_projection_state`가 빠져 있었다. **구현은 올바르다** — 그 응답은 키 넷만 만든다. 단언이 좁으면 새 키 하나가 조용히 새어 나가도 시험이 통과한다 | API-SEQ-007 / WP-074 | 시험 공백 | CR-080 | resolved — 계약이 없다고 정한 키 전부를 센다 |
| DEV-596 | 2026-09-11 | **`API-SEQ-007`의 검사 순서를 고정하는 시험이 없었다.** 계약이 보안 근거로 "권한 밖 repository의 실제 코드를 비교한 오류를 먼저 내지 않는다"를 정했는데, 기존 시험은 권한 밖 저장소를 **`pr_number`로만** 물었다. 코드 대조는 M 방향에서만 도므로 접근 범위 검사를 코드 대조 뒤로 옮겨도 시험이 전부 초록이었다. 그 순서가 뒤집히면 권한 밖 저장소에 M을 물었을 때 404 대신 400 `repository_code_mismatch`가 나오고 **그 차이만으로 비공개 저장소의 실재와 이름의 숫자를 알아낼 수 있다.** 구현은 현재 올바르다. 새 시험이 그 변이를 실제로 잡는 것을 변이 적용·원복으로 확인했다 | API-SEQ-007 / ADR-008 / WP-074 | 시험 공백 (보안 계약) | CR-080 | resolved |
| DEV-597 | 2026-09-11 | **재색인이 M 값을 다시 채우지 않아 전환 뒤 관측이 영원히 대기로 남았다.** 재구축은 PostgreSQL 스냅숏에서 문서를 만드는데 스냅숏에는 M 필드가 없다(투영이 싣지 않으며 그것이 옳다). `merge_seq`는 다음 채번이 `applySequenceToDocuments`로 다시 쓰지만 **M에는 그에 해당하는 "재구축 뒤에 도는 경로"가 없어**, 전환 전에 확정된 번호는 새 색인에 하나도 없었다. **화면의 번호는 틀리지 않는다** — API가 언제나 정본을 답한다. 손상되는 것은 `merge_number_projection_state`이며 그 값에 기대는 운영 watch가 관측 완료를 판정하지 못한다. PR 재구축 뒤 채번된 PR마다 `materialize` 의도를 예약해 러너가 **payload가 아니라 그때의 정본을** 읽어 채우게 했다 — 에폭이 그 사이 오르면 obsolete로 접힌다 | FR-SEQ-008 / WP-074 / 설계 8절 | 구현 누락 | CR-080 | **resolved** — 재구축이 복구 의도를 남긴다 |
...
| DEV-583 | 2026-09-11 | SRS·UI는 W-004 M 병기를 요구하는데 WP-074 DoD는 W-001/002만 포함하고 새 M 구현을 3 merge 방식으로 요구했다. 사용자 squash-only 프로파일에 맞춰 W-004 PR 행·상태/링크·기존 merge_seq 회귀 보존과 실행 범위를 정정했다 | FR-SEQ-008 AC-9·13 / WP-074 | 문서 간 모순 | CR-079 | resolved — 문서 정정, UI 구현은 todo |
...
| DEV-560 | 2026-09-08 | API-SEQ-005가 카탈로그에만 있어 세션 생성·표시·재개·초기화 요청 및 에폭·동시성 처리 계약이 비어 있었다. 상세 계약과 수직 구현을 세웠다 | WP-042 / FR-SEQ-007 | 문서 오류 | CR-071 | resolved |
| DEV-570 | 2026-09-08 | API-SRCH-006이 카탈로그에만 있고 동기 파일·비동기 잡·다운로드 및 artifact 저장과 접근 범위 재검증의 상세 계약이 없었다. 상세 계약과 실행 경로를 세웠다 | WP-044 / FR-SRCH-012 | 문서 오류 | CR-072 | resolved |
| DEV-558 | 2026-09-08 | **제품 CSS 검사가 직계 자식만 훑었다.** `DEV-555`가 세운 회귀는 `apps/web/app`의 직계 자식만 세면서 주석에는 "파일이 늘어나도 이 시험은 따라간다"고 적었다 — **시험이 자기 범위를 과장한 것이다.** 라우트 지역 스타일시트(`app/search/results.css`)나 `components/` 아래의 것은 `transition`을 선언해도 초록이었다. 재현했다: 두 자리 모두 통과했고, `apps/web` 전체를 재귀로 훑게 고치자 둘 다 잡혔다. `node_modules`와 점으로 시작하는 디렉터리는 제품 소스가 아니므로 제외한다 | WP-073 / DEV-555 | 시험 결함 | CR-069 | resolved |
| DEV-557 | 2026-09-08 | **정정이 또 다른 틀린 참조를 만들었다.** `DEV-555`를 고치며 `workbench.css` 계약을 "§10"으로 가리켰는데 실제 `## 10`은 「차트 색상」이고, 그 계약은 **번호 없는 절**에 있었다. 틀린 참조가 파생 문서·`CR-068`·원장 세 곳에 복제됐다. 번호 없는 절이 둘(「GitHub Operations 토큰 매핑」·「P4 작업대 소비 규칙」) 있어 생긴 일이라 **그 둘에 번호를 붙이고**(13·14) 참조를 실제 번호로 고쳤다. 절 번호를 인용할 때는 그 번호의 제목을 먼저 읽는다 | WP-073 / DEV-555 | 문서 오류 | CR-069 | resolved |
| DEV-556 | 2026-09-08 | **`ADMIN_DATABASE_URL`이 강제 경로에 없어 새 설치가 수신 전면 거부로 간다.** `DEV-553`의 처방은 `.env.example`의 주석 "처음부터 채운다"였는데 **주석은 강제하지 않는다.** `prsctl`의 `require_env`가 세는 필수 키 아홉에 이 값이 없어, 표본을 복사해 런북의 정본 목록을 그대로 따른 새 설치는 그 값이 **빈 채로 모든 관문을 통과한다.** 마이그레이션은 `raw_event` 파티션을 만들지 않고(실측) 그것을 만드는 것은 `JOB-AUD-001` 하나뿐이므로, 파티션이 소진되는 순간 **모든 웹훅이 `store_failed`로 떨어진다** — 첫 반입에서 실제로 일어난 일이다. **게이트의 존재가 아니라 위치가 실패의 대가를 정한다**는 것을 이 저장소가 세 번째로 겪었다(`DEV-524` → `DEV-544` → 여기). `require_env`에 넣고, `install`·`upgrade`·`restore`가 `ADMIN_DATABASE_URL`의 롤을 **접속 주체와 같은 자리에서** 만들게 했다(`provision_retention_role`). URL 파싱은 마지막 `@` 앞까지를 자격으로 보아 비밀번호의 `@`를 견디고, 퍼센트 인코딩과 `prs_admin` 직접 접속은 거부한다 — 전자는 앱이 디코딩한 값으로 접속해 인증 실패를 내고 후자는 `NOLOGIN` 그룹 롤이다. SQL은 실제 PostgreSQL로 실증했고 멱등이다. **7장의 "복구 뒤 수동 재생성" 제한이 함께 없어진다** | WP-070 / WP-072 / JOB-AUD-001 | 구현 결함 | CR-069 | resolved |
| DEV-555 | 2026-09-08 | **파생 토큰 문서가 스스로와 어긋났다.** §9는 모션 금지 규칙의 근거로 "제품이 스타일시트를 두지 않는다"는 당시의 사실을 들고 있는데, `CR-067`이 같은 문서 §14에 `apps/web/app/workbench.css`를 **제품 배치의 단일 진입점**으로 선언해 그 근거를 깼다. `CR-067`의 cascade가 이 문서를 실제로 갱신했으면서 §9의 문장은 남긴 것이라, `DEV-546`이 런북에서 겪은 것과 같은 형태다 — **한 문서 안에서 두 문장이 어긋난다.** 실측하니 그 스타일시트에 `transition`·`animation`·`@keyframes`는 **0건**이어서 규칙 자체는 지켜지고 있었다. 그러므로 규칙을 완화하지 않고 근거만 현행화했다. 개별 문장이 아니라 **불변식**을 회귀로 고정했다 — 제품 CSS가 몇 개든 그 안에 전환·애니메이션 선언이 없어야 하고, 그 파일이 실재하는 동안 §9가 사라진 근거를 다시 주장하지 않아야 한다. 파일이 늘어나도 시험이 따라간다. **정정문을 쓰다 검사가 자기 검색어를 세는 함정을 한 번 밟았다** — 설명이 옛 문구를 그대로 인용해 시험이 그 문장을 셌고, 뜻을 유지하며 표현을 바꿔 해소했다(이 저장소에서 세 번째다) | WP-073 / CR-064 / CR-067 | 문서 결함 | CR-068 | resolved |
...
### 6.72 `0.1.0-pilot.3` 반입 준비 — 릴리스 후보 감사 (2026-09-08, CR-068)

**첫 반입 이후 처음 만드는 번들이다.** `0.1.0-pilot.2`(대상 `0a73065`)가 사내에 선 뒤
main이 스물여덟 커밋 움직였고, 그중 둘은 **사내에서 손으로 넘긴 일을 없앤다.**

#### 6.72.1 이 번들이 사내에 무엇을 가져가나

| 무엇 | 사내에 미치는 영향 |
| --- | --- |
| `DEV-551` 수정 (`CR-066`) | **web 컨테이너 안의 손 스텁이 필요 없어진다.** 이미지가 외부 모듈을 실체화하므로 `upgrade`·`restart` 뒤에도 500이 나지 않는다 |
| `DEV-548` 수정 (`CR-066`) | `GHE_API_URL`을 비워 두어도 `<base>/api/v3`로 되돌아간다. 사내 `.env`의 명시 설정은 그대로 유효하다 |
| `DEV-549`·`DEV-550`·`DEV-552`·`DEV-553` (`CR-066`) | 런북이 웹훅 경로·인바운드 차단·CA 마운트 여섯 자리·파티션 소진을 사실대로 안내한다 |
| `WP-073` 작업대 (`CR-067`) | 검색·상세·셸이 조사 작업대로 정돈된다. **API·인가·SRS 범위 변경 없음** |
| Conductor `0.3.0`→`0.3.1` | 앱 CSS가 새 버전을 담는다 |

**사내 형상은 그대로 다시 얹어야 한다.** 이 번들도 `compose.yml`을 덮으므로 CA 마운트
여섯 자리와 `.env` 여섯 값, `prs_retention` 롤은 런북 5장의 표대로 재적용한다.

#### 6.72.2 후보 감사에서 나온 것 — 문서가 스스로와 어긋났다 (DEV-555)

`WP-073`이 파생 토큰 문서 §14에 `workbench.css`를 제품 배치의 정본으로 들였는데, 같은
문서 §9는 **제품이 스타일시트를 두지 않는다는 사실을 모션 금지의 근거로** 계속 들고
있었다. `CR-067`의 cascade가 이 문서를 실제로 갱신했으면서 그 문장은 남긴 것이다.

**규칙 자체는 지켜지고 있었다.** 그 스타일시트에 `transition`·`animation`·`@keyframes`는
0건이다(실측). 그래서 규칙을 완화하지 않고 근거만 현행화했고, 개별 문장이 아니라
**불변식**을 회귀로 고정했다 — 제품 CSS가 몇 개든 그 안에 전환·애니메이션 선언이 없어야
하고, 그 파일이 실재하는 동안 §9가 사라진 근거를 다시 주장하지 않아야 한다.

**정정문을 쓰다 검사가 자기 검색어를 세는 함정을 밟았다.** 설명이 옛 문구를 그대로
인용해 시험이 그 문장을 셌다. 뜻을 유지하며 표현을 바꿔 해소했다 — 이 저장소에서
세 번째다.

#### 6.72.3 검증 — 릴리스 후보 `57da09f` 기준

| 계층 | 결과 |
| --- | --- |
| 단위 | 1919 통과 (1 skipped) |
| 통합 | 1476 통과 |
| 회귀 | 373 통과 (`runtime-reachability` 334 = 기존 + `DEV-555` 둘) |
| a11y | 358 통과 |
| 대비 | 232쌍, 실패 0 |
| e2e | **168 통과 · 1 실패** — `flow-003.spec.ts:176`이며 `DEV-377`이다 |
| `pnpm typecheck` · `pnpm lint` | 통과 |
| 변이 4종 (제품 CSS에 `transition`·`@keyframes` 주입, §9 근거 되살리기, 결론 문장 삭제) | **전부 잡았다** |

**e2e 실패를 플레이크로 단정하지 않고 귀속했다.** `--workers=1` 단독 실행 3회가 모두
통과했다(14/14). `DEV-377`은 "뒤로가기 뒤 검색 화면 렌더가 병렬 부하에서 5초를 넘는다"로
이미 열려 있고 `CR-067`도 main에서 재현해 열림을 유지했다. **새 회귀가 아니다** —
그러나 통과로 세지도 않는다.
...
| 항목 | 값 |
| --- | --- |
| 릴리스 후보 커밋 | `57da09f` + 이 절의 `CR-068` 정정 |
| 다음 버전 | `0.1.0-pilot.3` (`0.1.0-pilot.1`·`.2`의 명명 규칙) |
| 같은 버전의 릴리스·태그·초안 잔재 | **없다** (`gh release view`·`git ls-remote --tags` 실측) |
| `immutable releases` | **`true`** — `0.1.0-pilot.2` 발행 시점과 다르다 |
| 디스크 여유 | 668GB (번들 약 2.6GB + 아카이브 약 1.1GB) |
| `gh` 인증 · `docker` | 인증됨 · 29.3.1 |

**설정이 켜졌으므로 발행은 되돌릴 수 없다.** 발행한 릴리스를 지워도 같은 태그 이름을
다시 쓸 수 없다(GitHub 문서, `DEV-544`). `0.1.0-pilot.2`는 설정 전에 발행돼 소급되지
않지만, 이 버전부터는 **버전 이름이 한 번만 쓰인다.** 그래서 발행은 감사와 검증이 끝난
뒤 사람이 결정한다.

### 6.72.5 번들 생성과 검사 — `0.1.0-pilot.3` (2026-09-08, 발행하지 않았다)

**두 번 만들었다.** 첫 빌드는 `a17bb18`에서 나왔고(SHA-256 `3572da6c…`) 검사도 통과했으나,
**발행 전에 도착한 리뷰가 그 번들이 담은 `prsctl`의 P1을 잡았다**(`DEV-556`, 6.72.6장). 번들은
그 스크립트를 담으므로 폐기하고 `bbb7914`에서 다시 만들었다. 아래 값은 **두 번째 빌드**의 것이다.

**`--release` 없이 만들었다.** 결정자가 발행 범위를 "빌드까지"로
정했고, `immutable releases`가 켜진 뒤로는 버전 이름이 한 번만 쓰이므로 발행은 검사가
끝난 뒤 사람이 정한다.

| 항목 | 값 |
| --- | --- |
| 대상 커밋 | `bbb79142e68472b179ec49306637bae85077103d` (`main`) |
| 운반 아카이브 | `pr-search-0.1.0-pilot.3-offline.tar.gz` · 1,128,764,543바이트 |
| SHA-256 | `c47f40458b86aee8df6a8d72ace24ace96951dabdb52b36a80dcbe70e10fdc39` |
| 번들 디렉터리 | 약 2.6GB (`images` 2.75GB 중 앱 749MB · 백킹 2.0GB) |
| 마이그레이션 수준 | `023` — `0.1.0-pilot.2`와 같다 |
| ES 매핑 sha256 | `14225454d1a0c2f1312afa58cb4ce7998e6a751c6883ee12c8b1c7aaec2a8f34` |

**아카이브를 별도 위치에 풀어 사내 절차를 그대로 밟았다.** 문서를 읽고 통과를 주장하지
않는다 — 실행이 판정한다.

| 검사 | 결과 |
| --- | --- |
| `tar -xzf` → `prsctl verify` | **10개 파일 전부 일치** |
| 계보 (`git fetch` → `vendor/upstream`) | manifest의 `upstream.commit`과 **일치** |
| 그 커밋의 내용 | `WP-073`의 `workbench.css`와 `CR-068`의 `DEV-555` 기록이 들어 있다 |
| 앱 이미지 태그 | `web`·`search-api`·`ingest-gateway`·`pipeline-worker`·`db`·`es` 여섯이 `0.1.0-pilot.3` |
| 백킹 이미지 | `postgres:16-alpine`·`elasticsearch:8.19.0`·`redis:7-alpine`·`filebeat:8.13.4` 넷 |
| 시크릿 혼입 | 아카이브에 `.env`·키·인증서 **0건** (스크립트 자체 검사와 별도로 재확인) |

**web 이미지는 번들에 담기기 전에 따로 세워 봤다.** `WP-073`이 제품 스타일시트를 처음
들였으므로 `DEV-551`의 실체화가 새 기능과 함께 성립하는지 물어야 했다.

| 확인 | 결과 |
| --- | --- |
| `docker build --target web` | 빌드 로그에 `실체화: node_modules/pg-…` 한 줄 |
| `docker run` 뒤 `/`·`/search`·`/repositories`·`/ranges` | **네 경로 모두 200** |
| 외부 모듈 오류 | **0건** |

**사내가 받을 것과 다시 얹을 것.** 전달은 셋이다 — 버전 `0.1.0-pilot.3`, 이 저장소 한정
읽기 토큰, 위의 SHA-256. 그리고 이 번들도 `compose.yml`을 덮으므로 CA 마운트 일곱 자리와
`.env` 여섯 값, `prs_retention` 롤은 런북 5장의 표대로 재적용한다. **마이그레이션·환경
변수 키·compose 서비스 구성은 `0.1.0-pilot.2`와 같아** 갱신 자체는 단순하다.

**재빌드본은 고침이 실제로 담겼는지까지 물었다.** 번들 안의 `prsctl`을 직접 읽어
`require_env`의 필수 목록과 `provision_retention_role` 호출 셋을 확인했고, **아카이브를 풀어
그 스크립트를 실제로 돌렸다.**

| 검사 | 결과 |
| --- | --- |
| `prsctl verify` | 10개 파일 전부 일치 |
| 번들의 `prsctl` | 필수 목록에 `ADMIN_DATABASE_URL` 있음 · `provision_retention_role` 호출 셋 |
| 번들의 런북·환경 표본 | 「파티션 수명 주체」 절과 "필수 값이다" 표기 있음 |
| **`.env.example`만 채우고 `ADMIN_DATABASE_URL`을 비운 채 `load`** | **`오류: 필수 값이 비어 있다: ADMIN_DATABASE_URL`로 거부** — 첫 반입에서 조용히 통과했던 상태다 |
| 값을 채운 뒤 `load` | 통과 (이미지 적재까지) |

**아직 발행하지 않았다.** 태그도 릴리스도 만들지 않았으므로 이 산출물은 언제든 지우고
다시 만들 수 있다.

#### 6.72.6 발행 전에 리뷰가 셋을 잡았다 (CR-069 / DEV-556~558)

**번들을 만든 뒤, 발행하기 전에 머지 후 리뷰가 도착했다.** 이 저장소의 관례대로 전수로
세었고(`pr-search` 4건 중 셋이 이 라운드), 판정 없이 고치지도 닫지도 않았다.

| 지적 | 판정 | 무엇이었나 |
| --- | --- | --- |
| `PR #151` P1 | **실결함** | `ADMIN_DATABASE_URL`이 `require_env`에 없어 새 설치가 수신 전면 거부로 간다 (`DEV-556`) |
| `PR #152` P2 | **실결함** | `workbench.css` 계약을 "§10"으로 가리켰는데 그 번호는 「차트 색상」이다 (`DEV-557`) |
| `PR #152` P2 | **실결함** | 제품 CSS 검사가 직계 자식만 훑으면서 "파일이 늘어나도 따라간다"고 적었다 (`DEV-558`) |

**셋 다 내가 만든 것이다.** `DEV-553`을 고치며 처방을 주석에 두었고, `DEV-555`를 고치며
틀린 참조와 과장된 시험을 함께 넣었다. **고침 자체를 다시 감사하지 않으면 보이지 않는
자리**이며 이 저장소가 반복해 기록해 온 형태다.

`DEV-556`은 특히 값비쌌다. 첫 반입에서 이미 겪은 실패를 **문서로만 막아 두었으므로**,
다음 새 설치가 같은 자리에서 같은 증상을 냈을 것이다. 이번 번들이 향하는 곳이 바로 그
설치다.

| 검증 | 결과 |
| --- | --- |
| URL 파싱 경계 7종 (비밀번호의 `@`·`:`, 퍼센트 인코딩, 대문자 롤, 스킴·비밀번호 부재, `prs_admin`) | 전부 의도대로 |
| SQL 실증 (실제 PostgreSQL, `@`·`:` 포함 비밀번호) | 롤 생성·`GRANT prs_admin`·**멱등** 확인 |
| 재귀 탐색 (`app/search/results.css`·`components/probe.css` 주입) | **둘 다 잡았다** |
| 회귀 전량 | 338 통과 (`runtime-reachability`, `DEV-556` 넷 신규) |
| 변이 8종 | 6종이 잡혔고 **둘이 살아남았다** |

**살아남은 둘 가운데 하나는 등가, 하나는 시험 구멍이었다.** 등가 쪽은 치환이 주석만 더한
것이라 계약을 건드리지 않았고, 구멍 쪽은 `toContain`이 **부분 문자열로 통과**해
`ADMIN_DATABASE_URL_X`를 잡지 못했다. 줄 첫머리로 재도록 좁혀 다시 확인했다.

**그리고 내 회귀 하나가 틀린 계약을 굳히고 있었다.** `CR-066`에서 쓴 시험이 `.env.example`에
"마이그레이션이 만들지 않는다"가 **있어야 한다**고 요구했는데, `DEV-556`이 그것을 사실이
아니게 만들었다. 느슨하게 만들지 않고 새 계약("필수 값이다")으로 갱신했다.

### 6.72.7 `0.1.0-pilot.3` 발행 사실 (2026-09-08)

**결정자가 발행을 지시했고 성공했다.** 값은 기억이나 스크립트 출력이 아니라 **GitHub API로
다시 읽어** 적는다 — `6.66.6`이 세운 형식이다.

| 항목 | 값 |
| --- | --- |
| 버전 · 태그 | `0.1.0-pilot.3` (`refs/tags/0.1.0-pilot.3`) |
| 대상 커밋 | `139e818d53b209d7c19d62e9bf3dab6b07d02708` — 원격 태그가 실제로 이 커밋을 가리킨다(역참조 확인) |
| 릴리스 이름 | PR Search 0.1.0-pilot.3 |
| 상태 | `draft: false` · `prerelease: false` · **`immutable: true`** |
| 초안 생성 | 2026-09-08T07:43:52Z |
| 발행 | 2026-09-08T07:48:35Z |
| 자산 | `pr-search-0.1.0-pilot.3-offline.tar.gz` · 1,128,788,822바이트 · `state: uploaded` |
| 자산 digest | `sha256:b5889459a14aa0a68df4365ba490ce78ede533444007b888a3a1aa910f220893` |
| 로컬 `sha256sum` | 위와 **일치** |
| 내려받은 횟수 | 0 — 사내 반입이 아직 시작되지 않았다는 뜻이다 |

**`0.1.0-pilot.2`와 다른 점이 하나 있다.** 그 릴리스는 `immutable: false`였고 이것은
`true`다. 설정이 발행 시점에 켜져 있었으므로 **이 릴리스는 자산과 태그가 잠긴다** — 지워도
같은 버전 이름을 다시 쓸 수 없다.

**세 번 만들어 세 번째를 발행했다.** 첫 빌드(`a17bb18`)는 리뷰가 잡은 `DEV-556` 때문에,
둘째(`bbb7914`)는 그 기록을 담은 `#155`가 병합되면서 각각 대상 커밋이 옮겨졌다. 발행된 것은
**셋째뿐이며 앞의 둘은 태그도 릴리스도 만들지 않았다** — 그것이 `--release` 없이 만든 이유다.

#### 사내 운영자에게 전달할 것 셋

`DEV-530`이 정한 대로 **릴리스와 별도 채널로** 전달한다.

| 무엇 | 값 |
| --- | --- |
| 버전 | `0.1.0-pilot.3` |
| 읽기 토큰 | 결정자가 발급한다 — 대상 저장소 하나, 권한 Contents Read-only 하나, 만료 90일 이내. **값은 어디에도 적지 않는다** |
| 자산 SHA-256 | `b5889459a14aa0a68df4365ba490ce78ede533444007b888a3a1aa910f220893` |

#### 이 반입에서 달라지는 것

| 무엇 | 사내에 미치는 영향 |
| --- | --- |
| `DEV-551` | **web 컨테이너의 손 스텁이 필요 없어진다.** `upgrade`·`restart` 뒤에도 500이 나지 않는다 |
| `DEV-556` | `ADMIN_DATABASE_URL`이 필수가 되고 롤을 `install`·`upgrade`·`restore`가 만든다. **기존 설치는 그 값이 이미 있으므로 `upgrade`가 그대로 지나간다** |
| `DEV-548` | `GHE_API_URL`을 비워도 `<base>/api/v3`로 되돌아간다 |
| `WP-073` | 검색·상세·셸이 조사 작업대로 정돈된다 |

**마이그레이션 수준은 `023`으로 `0.1.0-pilot.2`와 같고**, 환경 변수 키와 compose 서비스 구성도
같다. 다만 이 번들도 `compose.yml`을 덮으므로 CA 마운트 일곱 자리는 `load` 완료 뒤, `upgrade`
전에 런북 5장 표대로 다시 얹는다 — `prs_retention` 롤은 이제 그 표에서 빠졌다(`DEV-556`, `DEV-571`).

### 6.72.8 `0.1.0-pilot.3` 사내 서버 업그레이드 실행 사실 (2026-09-09)

사내 서버에서 `verify` → `load` → CA 마운트 재적용(일곱 자리) → `upgrade` → `smoke`를 실행해 통과했다. public origin에는 사내 서버 주소를 기록하지 않는다. lineage는 `0.1.0-pilot.3` / commit `139e818d` / migration `023`과 일치했다.
...
| 타입 | `pnpm typecheck` | 통과 |
| 린트 | `pnpm lint` · `pnpm run lint:deps` | 통과 · 패키지 13개 위반 0건 |
| 단위 | `pnpm test` | 2099 통과 · 1 skip (119 파일) |
| 회귀 | `pnpm run test:regression` | 422 통과 (8 파일) |
...
#### 독립 리뷰가 여덟을 더 찾았다 (`DEV-591`~`DEV-598`)

작성과 검토를 나누어, 이 변경을 쓰지 않은 에이전트가 계약 문서와 코드를 1:1로 대조했다. **blocker 1건·major 4건·minor 10건**을 보고했고 그중 blocker는 실행으로 재현했다. 여기서 고친 것은 여덟이며, 나머지 minor는 아래 「고치지 않은 것」에 적는다.

| 등급 | 무엇 | 어디 |
| --- | --- | --- |
| blocker | 재채번 중 공간에서 러너가 지연 0으로 무한 재시도 | `DEV-598` |
| major | 해석·목록 API가 단일 read 스냅숏이 아님 | `DEV-592` |
| major | `branch_not_tracked`를 DTO가 한 번도 만들지 않음 | `DEV-591` |
| major | 표본 이중 행으로 정상 채번에서도 영구 대기 | `DEV-593` |
| major | 재색인 뒤 M 값 복구 경로 없음 | `DEV-597` |
| minor | `EVT-SEQ-004`의 상관 ID가 빈 문자열 | `DEV-594` |
| minor | `epoch_stale` 키 부재 단언이 좁음 | `DEV-595` |
| minor | 검사 순서를 고정하는 시험 없음 | `DEV-596` |

**넷은 시험이 전부 초록인 채로 존재했다.** `branch_not_tracked`는 입력을 채우는 호출부가 없어 갈래 자체가 죽어 있었고, 검사 순서는 권한 밖 저장소를 PR 번호로만 물어 뒤집혀도 통과했다. 초록이 안전을 뜻하지 않는 자리가 이 판에서만 셋째다(`DEV-588`·`DEV-590`에 이어).

**리뷰가 내 기록의 사실 오류도 잡았다.** `DEV-590`을 "병렬로 도는 시험의 경합"이라고 적었으나 통합 시험은 `fileParallelism: false`로 순차 실행이며, 실제 원인은 **앞서 끝난 파일이 남긴 잔여 행**이다. 재현 조건을 잘못 적으면 같은 부류를 다음에 조사할 때 엉뚱한 곳을 본다. 해당 문구를 고쳤다.

#### 두 번째 검토가 넷을 더 찾았다 (`DEV-599`~`DEV-603`)

범위를 **순수 판정 네 파일로 좁혀** 별도 에이전트에게 맡겼다. 첫 검토가 구조와 계약을 보았다면 이쪽은 경계값과 결정성을 보았고, 그래서 겹치지 않는 것이 나왔다.

**가장 무거운 것은 같은 입력이 두 답을 내는 자리였다**(`DEV-601`). 한 PR에 정본 행이 둘일 때 나중 행이 앞 행을 덮는데, 어느 행이 이길지는 조회 순서가 정하고 그 질의에 `ORDER BY`가 없었다. 검토가 배열 순서만 바꿔 `M-1900-42`와 "M 번호 대기"가 번갈아 나오는 것을 보였다 — **데이터가 전혀 바뀌지 않았는데 새로고침마다 답이 달라진다.** 도달 경로는 설계 3절이 명시적으로 상정한 이중 squash SHA이며, `AC-3`이 "부여된 번호는 옮겨 가지 않는다"고 정한 것과 정면으로 어긋난다.

나머지 셋은 방어가 한쪽에서만 지켜진 자리다. `formatMergeNumber`가 `code`를 보지 않아 되읽을 수 없는 문자열을 만들 수 있었고, 화면의 입력 판정이 정수·범위·중복 key에서 계약보다 느슨했으며, blocker 사유가 enum 검증 없이 응답에 실렸다 — 검토가 그 열에 접속 주소를 넣어 화면에 그대로 찍히는 것을 보였다.

**`merge-number-view.ts`에 단위 시험이 없었다.** 순수 판정인데 통합 시험으로만 덮여 있었다. 결정성·결정 순서·사유 검증을 거는 10건을 세웠고, 행 선택을 되돌리는 변이를 적용해 실제로 잡는 것을 확인했다.

`DEV-603`(planner의 멱등 갈래가 도달 불가)은 **열어 둔다.** 정상 경로에서는 그 상태가 생기지 않아 지금 해가 없지만, 고치려면 두 방어 중 어느 쪽이 정본인지 정해야 하고 그것은 계약 판단이다.

#### 재검증이 수정의 부작용 둘을 잡았다 (`DEV-604`·`DEV-605`)

고친 것을 같은 리뷰어가 다시 보게 했고, 그것이 값을 했다. **내 수정이 새 결함 둘을 만들었다.**

상관 ID를 이으려고 work payload에 남긴 값이 그대로 이벤트 payload로 나가 계약에 없는 일곱 번째 키가 실렸다. 기존 시험이 부분 일치라 잡지 못했고, 재검증이 발행을 가로채 **키 목록을 세어** 찾았다. 재색인 복구는 PR마다 왕복을 만들었고 기능이 꺼진 배포에 아무도 집지 않을 행을 남겼다.

**리뷰어가 자기 첫 재측정이 무효였다고 먼저 밝혔다.** 대역이 `available_at`을 무시해 defer가 기록돼도 지켜지지 않았고, 그래서 수정 뒤에도 547회가 나왔다. 대역을 실제 SQL처럼 고쳐 다시 재니 6초에 2회였고 기록된 지연이 정확히 `WORK_DEFER_MS`였다. 도구가 틀렸을 가능성을 먼저 의심한 것이 이 재검증을 믿을 수 있게 한다.

**재현되지 않은 관측 하나를 함께 남긴다.** 리뷰어의 회귀 연쇄 실행 첫 회차에서 `415 passed | 5 skipped`로 한 번 끝났고, 그 5가 `merge-number-vs-git.test.ts`의 시험 수와 같아 `beforeAll`의 git 픽스처 생성이 실패했을 가능성이 있다. 이후 10회가 전부 통과해 메커니즘을 특정하지 못했으므로 편차로 올리지 않는다. **CI에서 같은 모양이 나오면 flake로 덮지 않는다** — `createSquashFixture`의 실패 출력을 먼저 본다. `DEV-588`이 비슷한 부류다.

#### 마지막 재검증이 내 수정의 부작용 하나를 더 잡았다 (`DEV-606`)

**같은 리뷰어에게 세 번째로 보게 한 것이 값을 했다.** `DEV-605`에서 "꺼진 배포에 아무도 집지 않을 행을 쌓지 말자"고 가드를 뒀는데, **재색인이 도는 `batch` 역할에는 그 플래그가 전달되지 않는다.** 그래서 M을 켠 배포에서도 가드가 언제나 참이 되고, `DEV-597`이 고친 것이 통째로 되돌아온다. 오류도 로그도 없다.

내가 리뷰어에게 "플래그를 읽는 자리가 갈리지 않는지 봐 달라"고 물었고, 리뷰어가 **한 프로세스 안에서는 갈리지 않지만 그 일관성이 배포의 일관성을 뜻하지 않는다**고 답했다. 걱정한 방향은 맞았고 층이 틀렸다.

회귀를 **개수가 아니라 서비스 이름으로** 세도록 고쳤다. 이전 단언(`toHaveLength(2)`)은 한 곳을 빼고 다른 곳에 둘을 둬도 통과한다. 이제 어느 역할이 그 값을 받는지가 목록으로 고정된다.

**그 수정이 또 하나를 만들었고 이번에는 내가 찾았다**(`DEV-607`). 리뷰어에게 "환경 변수를 더한 것이 기존 배치 역할의 기동을 막지 않는지"를 물으면서 그 경로를 직접 실행해 보았는데, `resolveMergeNumberConfig()`가 채번 전용 값까지 검증하고 던진다. 재색인이 그것을 부르면 채번과 무관한 `batch`가 채번 설정 때문에 서지 않는다. **물어보려고 확인하는 과정에서 나온 것**이며, 묻기 전에 스스로 재현해 보는 편이 낫다는 사례다.

리뷰어가 함께 알린 minor 둘은 남긴다. `requestWorkBatch`가 타입이 허용하는 `availableAt`을 조용히 무시하는 것과, 한 배치에 같은 키가 들어가면 PostgreSQL이 거절하는 것이다. **현재 호출부는 둘 다 만들 수 없다** — 유일 인덱스가 한 페이지에 같은 PR이 두 번 나오는 것을 막고, 호출부가 예약 시각을 넘기지 않는다. 범용 헬퍼의 방어가 없다는 사실만 적어 둔다.

#### 철자를 세는 시험은 이미 갈라진 것을 보지 못한다 (`DEV-608`)

`DEV-607`을 고치며 두 곳이 같은 규칙으로 읽는다는 것을 **소스에 특정 문자열이 있는지로** 걸었다. 리뷰어에게 "그것으로 충분한가"를 물었고, **충분하지 않을 뿐 아니라 지금 이미 갈라져 있다**는 답을 실측 표와 함께 받았다.

| 입력 | search-api | worker sequence | worker batch |
| --- | --- | --- | --- |
| 빈 문자열 | `false` | **던짐** | `false` |
| `yes` | 던짐 | 던짐 | **조용히 `false`** |
| `TRUE` | 던짐 | 던짐 | **조용히 `false`** |

철자를 세는 시험이 이것을 못 보는 이유는 셋이다. 부분 문자열의 **존재**만 보므로 다른 규칙을 한 줄 앞에 넣어도 통과하고, 포매터가 따옴표를 바꾸면 동작이 같은데도 죽으며, **이미 갈라진 상태를 애초에 볼 수 없다.** 두 문자열은 그때도 둘 다 있었고 답은 이미 달랐다.

**단언을 고치는 대신 구조를 고쳤다.** 워커에 켜짐 판정을 하나 두고 설정 함수와 batch 블록이 둘 다 그것을 부른다. 회귀는 **두 앱의 함수를 실제로 불러** 입력 표를 건다 — 리팩터링에 깨지지 않고, 한쪽 규칙만 바뀌면 반드시 잡힌다. 규칙을 좁히는 변이와 넓히는 변이 둘 다로 확인했다.

#### 독립 리뷰 종결 (다섯 판)

작성과 검토를 나눈 것이 이 판에서 가장 값이 컸다. **blocker 1건과 major 6건이 닫혔고, 그중 여섯은 시험이 전부 초록인 채로 존재했다.**

| 판 | 무엇이 나왔는가 |
| --- | --- |
| 1 | blocker(재채번 중 바쁜 루프) 1 · major 4 · minor 10 |
| 2 | major(같은 입력이 두 답을 냄) 1 · minor 5 — 범위를 순수 판정 네 파일로 좁힌 별도 검토 |
| 3 | 내 수정이 만든 부작용 2 (`EVT-SEQ-004`의 여분 키, 재색인의 PR당 왕복) |
| 4 | 내 수정이 만든 부작용 1 (`batch`에 플래그 미전달로 M4가 조용히 되돌아감) |
| 5 | 이미 갈라진 켜짐 판정 (실측 표로 증명) |

**매번 증상이 아니라 그것이 들어온 경로를 함께 고쳤다.** 느슨한 `toMatchObject`가 여분 키를 통과시켰고, 개수 단언이 자리 바꿈을 통과시켰고, 역할 사이의 전달 누락이 배포에서만 드러났고, 철자를 세는 시험이 이미 갈라진 것을 보지 못했다. 그 넷을 각각 고쳤으므로 같은 부류가 다시 들어오면 잡힌다.

**세 판이 내 수정의 부작용이었다.** 고친 것을 다시 보게 하지 않았다면 셋 다 남았을 것이고, 그중 하나는 "major를 고쳤다"는 기록이 사실과 다른 상태였다.

**검증 도구 자체가 검증 대상이다.** 리뷰어가 `B1` 재측정에서 "고쳐지지 않았다"고 보고할 뻔했는데, 자기 대역이 `available_at`을 무시한다는 것을 스스로 의심하고 고쳐 다시 재고서야 맞는 값(6초에 2회, 기록된 지연이 정확히 `WORK_DEFER_MS`)이 나왔다. **틀린 도구는 조용히 틀린 답을 준다** — 이 판에서 대역이 거짓 통과를 만든 자리가 둘 더 있었다. `documents.test.ts`가 `isAlreadyNumbered: () => false` 대역으로 멱등을 증명하지 못했고(`DEV-603`), 통합 시험의 SQL 감시가 `pool.query`만 봐서 스냅숏 안의 문장을 통째로 놓쳤다.

**최종 확인**에서 세 구현이 일곱 입력 전부에서 일치함을 리뷰어가 같은 표로 다시 재어 확인했고, 블로킹 결함 없음으로 리뷰를 종료했다.
...
| 무엇 | 왜 남겼는가 |
| --- | --- |
| covered 판정이 앱 시계와 DB `created_at`을 비교 | 편차가 fetch 소요보다 크면 fetch가 못 본 push의 의도를 닫을 수 있다. **리뷰도 추측이라고 밝혔고 재현하지 않았다.** 편차가 그보다 작으면 안전 방향이다. 시계 의존을 없애려면 fetch 직전에 `clock_timestamp()`를 읽어야 하는데 그 변경의 영향 범위가 이 판보다 넓다 |
| `proof`에 `resume_page` 키 | 설계 6.2의 닫힌 목록에 없다. 값은 페이지 번호라 민감하지 않고 재개에 필요하다 — **구현이 아니라 계약을 좁게 고치는 쪽**이 맞고, 그것은 CR의 일이다 |
| `mnumber_blocked_reason`의 타입이 `string` | API로 그대로 나가는 값이라 enum으로 좁히는 편이 낫다. **지금 실제 유출은 없다** — 호출부가 고정 enum만 넘긴다. 타입과 CHECK를 함께 좁히는 것은 마이그레이션을 하나 더 만드는 일이다 |
| 증거 수집의 git 읽기가 미러 락 밖 | 설계 4.1과 문구가 다르다. 실패하면 `profile_unverified`로 **안전하게 퇴화하고** 락 순서(미러 → 시퀀스)를 어기지도 않는다 |

관측 루프의 종료 지연과 복제본 중복 관측은 `markObserved`가 최초 값만 남겨 데이터가 멱등하므로 비용만 든다. 이 판에서 고치지 않았다.
...
`mnumber_evidence`의 `ON DELETE RESTRICT`가 **기존 통합 시험의 정리를 막았다.** 두 릴리스 시험이 `beforeAll`에서 조건 없이 `merge_sequence`를 비우는데, **앞서 끝난** M 번호 시험이 남긴 증거가 그 행을 참조해 삭제가 거절된다. 통합 시험은 순차 실행(`fileParallelism: false`)이므로 경합이 아니라 잔여 상태가 원인이고, 파일 실행 순서가 로컬과 CI에서 달라 CI에서 처음 났다. 외부에서 초록이었다는 사실이 안전을 뜻하지 않는 자리가 또 하나 생겼다.
...
### 6.77 사설 CA가 git에도 닿는다 — 검증 결과 (2026-09-11, CR-082 / DEV-561)

**무엇을 고쳤나.** `deploy/single-host/compose.yml`의 `x-app-env` 앵커에 `GIT_SSL_CAINFO`를 더했다. 그 앵커를 `reindex`·`web`·`search-api`·`ingest-gateway`와 워커 아홉이 전부 병합하므로, `git`을 부르는 `worker-mirror`·`worker-sequence`·`worker-release` 셋에 한 자리로 닿는다. `.env.example`에 운영자가 채울 항목을 두고, 런북 6장에 4단계와 두 변수를 같은 값으로 둔다는 규칙을, 8장 문제 해결 표에 증상 행을 더했다.

**무엇을 검사하는가.** 회귀 `regression/runtime-reachability.test.ts`에 `describe('사설 CA가 git 서브프로세스에도 닿는다 (CR-082 / DEV-561)')`를 더했다. 파일 어딘가에 그 문자열이 있는지 묻지 않는다 — `compose.yml`을 파싱해 **서비스마다 병합 앵커를 펼친 최종 환경 키 집합**을 만들고, `MIRROR_ROOT`를 선언한 서비스(= 로컬 미러를 다루므로 `git`을 부르는 서비스)가 전부 `GIT_SSL_CAINFO`를 받는지 묻는다. 대상 목록을 손으로 적지 않으므로 새 역할이 미러를 쓰기 시작하면 자동으로 검사 대상이 된다. 두 CA 변수가 한 서비스에서 갈리지 않는지도 함께 본다.

**변이로 확인했다.** 시험이 무엇을 지키는지 되돌려 보았다.

| 변이 | 결과 |
| --- | --- |
| 앵커에서 `GIT_SSL_CAINFO` 줄 제거 | 2건 실패 |
| 앵커에서 빼고 `worker-mirror` 한 곳에만 둔다 | 2건 실패 — `DEV-552`가 가르친 함정을 실제로 잡는다 |
| `.env.example`의 항목 제거 | 1건 실패 |
| 런북 문제 해결 표의 증상 행 제거 | 1건 실패 |

**파서 자체도 검사 대상으로 두었다.** 이 판이 배운 것이다 — 대역이나 경로 선택 때문에 아무것도 증명하지 못하는 시험이 앞 라운드에 넷 있었다. 파서가 조용히 빈 집합을 만들면 나머지 단언이 전부 무의미하게 통과한다. 그래서 앵커 병합이 실제로 펼쳐졌는지(`NODE_ENV`), 명시 키가 함께 모이는지(`MIRROR_ROOT`), 받지 않는 서비스를 받는다고 말하지 않는지(`search-api`의 `SESSION_COOKIE_SECURE`)를 먼저 묻는다. **이 검사가 실제로 값을 했다** — 첫 구현의 파서가 앵커 블록을 닫으면서 `services:` 줄을 함께 버려 서비스를 하나도 읽지 못했고, 그 시험이 그것을 잡았다. 그 상태로 두었다면 나머지 단언이 빈 집합 위에서 통과했을 것이다.

**사내 적용은 `NOT RUN`이다.** 이 판은 배포 정의와 문서를 고쳤을 뿐이며, 실제 사내 호스트에서 미러 초기화가 성립하는지는 다음 반입이 확인한다. 사내 임시 조치(`.env`와 `compose.yml` 직접 수정)와 같은 값을 상류가 이제 기본으로 제공하므로, 다음 `upgrade`에서 그 수정본을 덮어써도 설정이 사라지지 않는다.

### 6.78 사내 GHE 직접 로그인 — 검증 결과 (2026-09-11, CR-083 / DEV-612 · DEV-613)

**무엇을 더했나.** `AUTH_PROVIDER`가 인증 공급자를 고른다. 값을 주지 않으면 `oidc`이고 기존 배포의 동작이 바뀌지 않는다. `github`이면 GHE OAuth2 Authorization Code + PKCE로 인가하고, 액세스 토큰으로 `/user`와 `/user/teams`를 읽어 신원과 역할을 확정한다.

**공식 문서로 확인한 것.** 추측하지 않았다. GHE의 인가 엔드포인트는 `code_challenge`/`code_challenge_method=S256`을 받으며 `plain`은 지원하지 않는다 — 그래서 `pkce.ts`를 공급자마다 새로 쓰지 않고 그대로 공유한다. 토큰 엔드포인트는 `Accept: application/json`이 없으면 form-urlencoded로 답한다. `/user/teams`에는 `read:org`가, `/user`에는 `read:user`가 최소 스코프다.

**문서가 답하지 않아 방어적으로 구현한 것 하나.** 토큰 교환이 거절될 때의 HTTP 상태 코드를 공식 문서가 명시하지 않는다. **GHE는 거절을 HTTP 200과 함께 본문의 `error`로 답한다**는 것이 통상 동작이므로, 상태 코드와 본문의 `error` 둘 다를 오류로 다룬다. 상태 코드만 보면 거절된 교환이 성공으로 읽히고 `access_token`이 없는 채로 다음 단계에 들어간다 — 이 판에서 가장 조용한 실패다.

**사내 제안을 그대로 쓰지 않은 자리 둘.** 근거는 `CR-083` cascade의 표에 있다. 요약하면 `admin`·`viewer`는 이 제품의 역할이 아니고(팀으로 부여 가능한 것은 `manager`·`qa` 둘), 쿠키 완화는 `AUTH_ENABLED=false`를 명시한 배포로 좁혔다.
...
| 단위 | 2,226 통과 · 1 skip (123 파일) | 기준선 2,099에서 127건 증가 |
| 회귀 | 427 통과 (8 파일) | |
| 통합 | 1,609 통과 (100 파일) | 실제 PostgreSQL·Redis. 기준선 1,602에서 7건 증가 |
| 접근성 | 379 통과 (17 파일) | |
| e2e | 181 통과 | 검토 반영 전 실행에서 `DEV-377`(병렬 부하의 알려진 편차)이 한 번 나왔고 단독 실행은 통과했다. 최종 실행은 전량 통과 |
| 빌드 | 통과 | |
| 문서 검사기 (`--strict`) | `main` 대비 신규 issue 0건 | |

**전량 통합 실행 첫 회에 `audit-records.test.ts`의 한 건이 깨졌고 재현되지 않았다.** 단독 실행(26건)과 두 번째
전량 실행(1,608건)이 모두 통과했다. 그 시험의 경로에 이 판의 변경이 닿지 않는 것을 확인했다 — 그 파일은
`new SessionStore(...)`를 직접 넣으므로 `RegisteringSessionStore`를 거치지 않는다. 숨기지 않고 적어 둔다.

**그 확인이 사각지대도 드러냈다.** 이 저장소의 통합 시험은 대부분 인증 컨텍스트를 손으로 조립하므로
`createAuthContext`의 실제 경로가 통합 계층에서 거의 걸리지 않는다. 그래서 `DEV-613`의 수정을 대역으로만
검증하지 않도록 **실제 PostgreSQL을 쓰는 통합 시험을 따로 더했다**
(`apps/search-api/integration/authz/registration.test.ts`). 대역은 `query`가 불렸는지만 보고 SQL이 실제로
도는지는 말하지 않는다 — 열 이름이나 `ON CONFLICT` 대상이 틀려도 통과시킨다.

**변이 시험.** 새 시험이 실제로 무엇을 지키는지 여섯 곳에서 되돌려 확인했다. **스물하나가 전부 kill됐다.**

| 대상 | 변이 | 결과 |
| --- | --- | --- |
| GHE OAuth 모듈 | 200-with-error 검사 제거 | 2건 실패 |
| | 사용자 `id`의 정수 검증 제거 | 3건 실패 |
| | 오류 메시지에 응답 본문 전체를 담음 | 1건 실패 (NFR-005) |
| | 팀 페이지 상한 무력화 | 1건 실패 |
| | 팀 중복 제거 무력화 | 1건 실패 |
| 정본 등록 (단위) | 등록 호출 제거 | 6건 실패 |
| | 실패한 등록도 캐시 | 2건 실패 |
| | 없는 GHE id를 `0`으로 | 1건 실패 |
| | 등록 캐시 무력화 | 2건 실패 |
| 정본 등록 (통합, 실제 DB) | 등록 호출 제거 | 6건 실패 |
| | GHE id를 상수로 | 2건 실패 |
| | `login`을 상수로 | 4건 실패 |
| 콜백 라우트 | 왕복 쿠키를 `cookies.set`으로 되돌림 (`DEV-614` 복원) | 1건 실패 |
| | 신원 키를 `login`으로 | 2건 실패 |
| | 공급자 접두 제거 | 2건 실패 |
| | 실패 로그 제거 | 1건 실패 |
| 조립 배선 | `createAuthContext`를 기반 저장소로 되돌림 | 1건 실패 |

**회귀가 배포 정의를 자동으로 요구했다.** `CR-078`이 만든 검사가 `packages/authz/src/config.ts`의 `required()` 호출을 소스로 읽어 `compose.yml`의 `web` 블록과 `.env.example`에 그 키가 있는지 본다. `GHE_BASE_URL`을 `required()`로 쓰자 그 검사가 먼저 죽었고, 배포 정의를 채우자 통과했다. **설계한 게이트가 다음 판에서 실제로 값을 한 첫 사례다.**

**계약을 의도적으로 바꾼 자리 하나.** `packages/authz/src/config.test.ts`의 「`AUTH_ENABLED=false`여도 운영의 insecure 쿠키는 거부한다」가 깨졌다. 시험을 느슨하게 하지 않고 **새 계약으로 갱신했다.** 그 시험의 주석이 적던 우려(「열어 두면 열린 채로 켜진다」)는 그대로 유효하므로, 면제를 계산값이 아니라 명시적 `AUTH_ENABLED=false` 선언에 걸고 「그 값을 남긴 채 인증만 켜면 거부한다」를 새 시험으로 세웠다.

**사내 검증은 `NOT RUN`이다.** 외부에는 실제 GHE OAuth App이 없어 토큰 교환과 사용자·팀 조회를 전부 대역으로 검증했다. 다음에 확인해야 할 것은 다음 넷이다.

- 실제 GHE OAuth App 등록과 callback URL 일치
- 실제 토큰 교환 응답의 모양 (특히 거절 시 상태 코드)
- `/user/teams`가 돌려주는 팀 슬러그가 `GHE_TEAM_ROLE_MAP`의 표기와 맞는지
- 로그인 후 첫 조회가 503이 아닌지 (`DEV-613`의 실제 폐쇄 확인)

**독립 검토 두 판이 이 판의 중심이었다.** 작성과 검토를 나눴고, 한 판은 보안·계약을,
한 판은 순수 판정의 경계값과 결정성을 보았다(후자는 함수를 실제로 실행해 입력별 답의
표를 만들었다).

| 판 | 무엇이 나왔는가 |
| --- | --- |
| 보안·계약 | **blocker 1** (`DEV-614` 세션 쿠키 유실) · major 3 · minor 다수 |
| 경계값 | blocker·major 없음 · minor 5 (그중 셋을 고쳤다) |

**blocker가 시험 공백에서 나왔다.** 검토가 「콜백 라우트에 시험이 없어 `userId`를
`login`으로 되돌려도 아무것도 죽지 않는다」를 지적했고, 그 공백을 메우려고 시험을
세우자 **첫 실행에서 전혀 다른 결함이 잡혔다** — 세션 쿠키가 응답에서 사라지고
있었다(`DEV-614`). 지적받은 것을 고치는 과정이 지적되지 않은 것을 드러낸 셈이다.

닫은 major 셋:

- **신원 키 네임스페이스** — GHE 숫자 id를 접두 없이 `app_user.user_id`에 넣으면, 사내
  IdP가 숫자 문자열을 `sub`로 쓰는 배포에서 다른 사람의 행을 덮는다. `github:` 접두를
  붙였다. **이 경로로 만들어진 데이터가 아직 없는 지금이 가장 싼 시점이다.**
- **콜백이 로그를 남기지 않았다** — 파일 머리글은 「로그에는 상관 ID와 함께 남는다」고
  적는데 그 코드가 없었다. 사용자가 들고 온 상관 ID로 운영자가 뒤질 곳이 없었다.
  적힌 의도와 실제가 다른 자리를 또 만든 것이라, 이행하는 쪽으로 고쳤다.
- **조립 배선 무검증** — `createAuthContext`의 한 줄을 되돌리면 모든 시험이 초록인 채
  `DEV-613`이 되살아났다. 고친 결함의 봉합점이 무방비였다.

경계값 검토에서 고친 셋:

- `joinUrl`을 문자열 이어 붙이기에서 `URL` 기반으로 바꿨다. 기반 주소에 쿼리가 섞이면
  실제 경로가 쿼리 값 안으로 삼켜져 **오류 없이 틀린 주소**로 요청이 나갔다.
- API 주소 조립을 같은 함수로 모았다. `trimSlash`는 슬래시를 하나만 떼고 `joinUrl`은
  전부 떼어, **한 값에 정규화 규칙이 둘**이었다 — 인가는 성립하고 사용자 조회만
  실패하는 부분 고장을 만든다.
- 스코프가 `',,,'`처럼 구분자만 있으면 빈 배열이 되어 무권한 토큰을 받았다. 분해 결과가
  비면 기본값으로 떨어지게 했다. OIDC 쪽도 같은 규칙을 쓴다.
- 등록 캐시가 `userId`만 키로 삼아 **개명이 정본에 반영되지 않았다.** `upsertUserOnLogin`이
  매 로그인 `login`을 갱신하도록 짜여 있는데 캐시가 그 설계를 무력화했다. 키를 정본에
  쓰는 값 전체로 바꿨다.

고치지 않은 minor와 그 근거:

- `allow_signup=false`는 재인증·계정 선택을 강제하지 않는다. **주석이 없는 보호를 있다고
  적고 있었으므로 주석을 고쳤다.** OIDC 경로도 `prompt`를 싣지 않아 같은 한계이며,
  공급자를 바꾼다고 달라지는 것이 아니다.
- `AUTH_ENABLED='False'`처럼 오탈자가 있으면 기동이 막히되 메시지가 진짜 원인을 가린다.
  방향은 fail-closed이고, 고치려면 「거짓의 표현」을 넓혀야 하는데 그것은 계약을 넓히는
  일이라 이 판에서 하지 않는다.
- `login` UNIQUE 충돌은 영구 실패인데 매 요청 재시도한다. 실패자 본인만 503이고 백오프가
  없다는 것이 대가다. 실패를 캐시하면 일시 오류가 영구 장애로 굳으므로 현재 선택을
  유지하고, 사람이 정본을 정리해야 한다는 것을 런북에 적었다.

### 6.79 릴리스 게이트와 계약의 어긋남 — 검증 결과 (2026-09-11, CR-083 / DEV-615)

**무엇이 있었나.** `CR-083`을 병합하고 `0.1.0-pilot.5` 번들을 만들자 빌드가 멈췄다.

```
오류: SESSION_COOKIE_SECURE=false 구성으로 띄웠는데 **죽지 않는다** (DEV-577)
오류: 번들 이미지가 런타임 검사를 통과하지 못했다 — 이 번들을 반입하지 않는다
```

게이트가 옳았다. 계약이 바뀐 것을 게이트가 몰랐을 뿐이다. `smoke-images.sh`는 `AUTH_ENABLED=false` + `SESSION_COOKIE_SECURE=false`가 **거부되어야 한다**고 요구했는데, `CR-083`이 사용자 결정으로 정확히 그 조합을 허용하도록 넓혔다.

**계약을 강제하는 자리가 셋이었다.**

| 자리 | 갱신했는가 |
| --- | --- |
| `packages/authz/src/config.test.ts` | 예 — 깨진 시험을 새 계약으로 갱신했다 |
| `apps/web/instrumentation.test.ts` | 예 — 면제와 그 경계 셋을 걸었다 |
| `deploy/single-host/smoke-images.sh` | **아니오** |

앞 라운드가 「프로세스 안의 일관성이 배포의 일관성이 아니다」(`DEV-606`)로 배운 것과 같은 층위다. 그때는 환경 변수가 한 컨테이너에 전달되지 않은 것이었고, 이번에는 **계약의 기대가 배포 게이트에 반영되지 않은 것**이다.

**그 시점에 회귀는 초록이었다.** `runtime-reachability.test.ts`는 스모크에 그 검사가 **있는지**를 문자열로 본다. 검사는 그대로 있었고 기대만 반대였으므로 그 시험이 볼 수 있는 것이 아니었다. 이 저장소가 「검사가 자기 검색어를 센다」로 여러 번 겪은 모양의 변형이다.

**무엇을 고쳤나.**

- 게이트의 기대를 새 계약에 맞췄다. 의도를 적지 않은 배포(`AUTH_ENABLED` 부재)와 그 값을 남긴 채 인증을 켠 배포를 각각 거부 기대로 세웠고, GHE 공급자의 자격 부재도 같은 계약을 받는지 더했다.
- **허용 기대를 처음으로 넣었다** (`expect_accepted`). 거부만 검사하는 게이트는 한 방향으로만 정직하다 — 계약이 넓어졌을 때 **서야 할 배포를 막고 있어도 아무것도 죽지 않는다.** 사내가 요청한 파일럿 형상이 바로 그 방향이므로 그것을 게이트에 넣었다.
- 회귀를 문자열에서 **실행 대조**로 바꿨다. 스모크에서 기대 조합을 뽑아 `webConfigFailure`에 넣는다. `.env.example`을 실제 계약 함수에 넣는 `config.test.ts`(`DEV-577`)와 같은 방식이다.

**변이로 확인했다.**

| 변이 | 결과 |
| --- | --- |
| 스모크를 `CR-083` 이전 기대로 되돌림 (실제로 발행을 막았던 상태) | 1건 실패 |
| 허용 기대를 제거 | 2건 실패 |

첫 변이가 이 시험의 값이다. **그 시험이 있었다면 번들 빌드에 30분을 쓰기 전에 잡혔다.**

**파서도 검사 대상으로 두었다.** 스모크의 호출 형태가 바뀌어 아무것도 뽑지 못하면 나머지 단언이 빈 목록 위에서 통과한다. 거부 기대 셋 이상과 허용 기대 하나 이상을 실제로 읽었는지 먼저 묻는다.

**빌드 환경에 관한 기록 하나.** 첫 시도는 호스트 메모리 부족으로 중단됐다 — 앞 라운드의 적재 검증이 남긴 `prs-pilot-*` 스택 13개가 약 2GiB를 쓰고 있었다. 사용자 승인을 받아 그 스택만 `stop`하고(볼륨은 보존) 다시 빌드했다. 통합 시험용 백킹은 건드리지 않았다.

### 6.80 `0.1.0-pilot.5` 발행 (2026-09-11, CR-082 · CR-083)

**무엇을 담았나.** 사내 `0.1.0-pilot.4` 반입이 보낸 요청 두 건이다.

| 요청 | 무엇 |
| --- | --- |
| `CR-082` / `DEV-561` | 사설 CA가 `git` 서브프로세스에도 닿는다. 미러 초기화가 SSL 오류로 실패하던 것을 닫았다 |
| `CR-083` / `WP-076` | 사내 GHE 계정으로 직접 로그인한다(`AUTH_PROVIDER=github`). 별도 OIDC IdP가 없는 배포용이다 |

그 과정에서 함께 닫은 편차가 넷이다 — `DEV-612`(문서가 파서와 어긋남), `DEV-613`(로그인이 `app_user` 행을 만들지 않아 첫 조회가 503), `DEV-614`(콜백이 세션 쿠키를 응답에서 잃어버림), `DEV-615`(릴리스 게이트가 바뀐 계약을 몰랐음). 뒤의 둘은 이 라운드의 독립 검토와 발행 시도가 각각 드러낸 것이다.
...
| --- | --- |
| 버전 | `0.1.0-pilot.5` |
| 태그 대상 | `2795666002cd2f60fe100460bf0d4955c38a36c2` |
| 자산 | `pr-search-0.1.0-pilot.5-offline.tar.gz` |
| 자산 SHA-256 | `aa83abf91fea7f0c2511cb412c4df843564f5a7dc1b035eecd0849de4b6a6afd` |
| 자산 크기 | 1,137,203,915 바이트 |
| `draft` · `prerelease` | 둘 다 `false` |
| `immutable` | `true` — 발행 뒤 자산과 태그가 잠긴다 |
| 마이그레이션 수준 | `025` (변경 없음) |

값은 `gh api repos/.../releases/tags/0.1.0-pilot.5`로 다시 물어 빌드 출력과 대조했다. `gh release view --json`에는 `digest`가 없으므로 그 경로로는 확인할 수 없다.

**태그 이후 `main`이 움직였으나 코드는 그대로다.** 발행 직후 `e7593cc`·`c01f926`이 들어왔고, `git diff 2795666 origin/main --name-only`로 확인하니 **`agent-context/` 밖의 변경이 0건**이다. 릴리스는 최신 코드 기준이다.

**사내 운영자에게 별도 채널로 전달할 것 셋** (`DEV-530`): 버전 `0.1.0-pilot.5` · 읽기 토큰 · 위 SHA-256. **토큰 값은 어디에도 적지 않는다.**

**사내에서 받는 명령** (런북 2.B 1단계):

```bash
GH_TOKEN=<읽기 토큰> gh release download 0.1.0-pilot.5 -R 89sooner/pr-search -p '*.tar.gz'
sha256sum pr-search-0.1.0-pilot.5-offline.tar.gz   # 위 SHA-256과 같아야 한다
tar -xzf pr-search-0.1.0-pilot.5-offline.tar.gz
```

**첫 발행 시도는 실패했고 그것이 값을 했다.** 게이트가 「이 번들을 반입하지 않는다」로 멈췄고, 원인은 바뀐 계약을 게이트가 몰랐던 것이었다(`DEV-615`). 게이트가 없었다면 **파일럿 형상이 기동하지 않는 번들이 사내로 갔을 것이다.** 6.79장이 그 판정이다.

**빌드 환경 기록.** 첫 빌드는 호스트 메모리 부족으로 중단됐다 — 앞 라운드의 적재 검증이 남긴 `prs-pilot-*` 스택이 약 2GiB를 쓰고 있었다. 사용자 승인을 받아 그 스택만 `stop`하고(볼륨 보존) 빌드한 뒤 되살렸다. 통합 시험용 백킹은 건드리지 않았다.

**다음 반입에서 확인할 것.**

- 임시 조치(`.env`·`compose.yml` 직접 수정)를 되돌리고 번들 기본값으로 미러 초기화가 성립하는지 (`DEV-561`)
- 실제 GHE OAuth App 등록과 callback URL 일치, 거절 응답의 상태 코드, 팀 슬러그 표기 (`CR-083`, 6.78장)
- 로그인 후 첫 조회가 503이 아닌지 (`DEV-613`), 세션 쿠키가 실제로 브라우저에 남는지 (`DEV-614`)
- `0.1.0-pilot.4` 재시험은 이 판으로 대체된다 — pilot.5가 그 수정을 전부 포함한다

### 6.73 PR #150 태그 복구 안내 정정 (CR-070 / DEV-559)

실제 로컬 bare 원격에서 태그 생성 A → 이동 B → 삭제 lease 실패 → 다른 주체의 이동 A를 재현했다. 이전 안내의 `--force-with-lease` 삭제가 성공해 SHA 일치가 소유를 증명하지 못함을 확인했다. 새 회귀는 수정 전 실패하고 수정 후 통과했다. `build-bundle.sh`의 복구 안내와 런북은 독립적인 소유 확인 전 태그를 보존하고 새 버전을 사용하도록 정정했다. 원자적 생성과 최초 자동 되돌리기의 lease는 유지한다.

검증: `pnpm run test:regression regression/release-tag-ownership.test.ts regression/runtime-reachability.test.ts` — 349건 통과. 실제 Git 사용, GitHub와 Docker는 대역이며 외부 릴리스는 변경하지 않았다.

### 6.74 WP-042 이분 탐색 보조 (CR-071 / DEV-560)

`API-SEQ-005`의 상세 계약을 채우고 기존 migration 002의 `bisect_session`을 실제 수직 경로에 연결했다. 후보는 `(good_seq, bad_seq]` 안의 실재 first-parent 커밋이며 수치 중앙에 가장 가까운 서수를 고른다. 사용자·시퀀스 공간 잠금, 현재 에폭 재검증, 세션 ID 세대 대조로 동시 표시와 초기화 뒤 늦은 요청이 범위를 넓히지 못한다. 재채번 중이거나 에폭이 바뀌면 후보를 무효화하며 직접 푸시 결과를 가짜 PR로 바꾸지 않는다.

W-004의 C-029는 서버 저장 상태를 복원하고 후보 수·예상 횟수를 `aria-live`로 알린다. 모순과 에폭 오류는 검사 버튼을 막고 명시적 초기화 경로를 남긴다. 초기 후보 분모를 저장하지 않으므로 추측한 백분율 대신 실제 후보 수를 표시한다.

검증: 실제 PostgreSQL·Redis 통합 10건, a11y 2건(axe 위반 0), Chromium FLOW-004 3건 통과. 프로덕션 웹 build, 전체 타입 검사와 lint가 통과했다.

### 6.75 WP-044 검색 결과 내보내기 (CR-072 / DEV-570)

`API-SRCH-006`을 W-001과 배치 러너까지 연결했다. 1000건 이하는 동기 CSV/JSON 파일, 1001~100000건은 `JOB-SRCH-001`, 그보다 많으면 400이다. 실행 당시 접근 범위 형태와 저장소 집합을 함께 고정하고 사용자·PG scope version·시퀀스 에폭을 동기 응답 전, 워커 페이지마다, 공개 직전과 다운로드 시점에 다시 확인한다. 권한 또는 에폭이 바뀌면 다시 요청하게 하며 기존 결과의 범위를 넓히지 않는다.
...
**검증과 발행.** Node 22.23.2 로컬 검증: typecheck·lint·lint:deps·전체 build, 단위 2,791(기존 skip 1), a11y 433, 대비 232쌍 실패 0, 회귀 503/503. PR #196 CI `35061974889` success(verify·integration), squash `a8796de`; main CI `35062529329` success. `0.1.0-pilot.8`은 이 커밋에서 7개 앱 이미지·4개 백킹 이미지를 포함한 2.7GiB 번들/1.1GiB 아카이브를 만들어 런타임·시크릿 검사와 GitHub 자산 digest 대조 뒤 발행했다. 자산 SHA-256은 `70b6070e6750c1f1a5c7f5df5624aef8d159f69fdca036a573648d6ab62831ff`; immutable releases가 켜져 있어 발행 뒤 자산·태그가 잠겼다. 사내 GHE 확인은 `NOT RUN`, 기존 미통과 게이트는 유지한다.
...
시작 `origin/main`은 `26e2627`(CR-089 병합 기록 뒤, main CI run `34816040697` success — verify·integration, integration 안의 `test:regression`도 실제 실행). 브랜치 `feature/rel007-registry-approval`(작업 트리 격리, 격리 서비스 `prs-approval-*` — PostgreSQL 55438·Redis 56383·ES 59204). **새로 실행 가능한 명령은 없다 — 실행 허용은 `pr.list` 하나이며, 운영 승인과 관리자 정책은 코드의 실행 목록을 줄일 수만 있다.** 사내 반입과 실제 GHES 시험은 `NOT RUN`이다.

**무엇을 증명했는가.** 「검사가 통과했다」는 화면 표시에서 끝나지 않고, 관리자의 명시적 운영 승인과 차단 결정이 실제 실행을 통제하며 그 근거와 변경 이력이 보존된다. 운영자는 A-006에서 현재 적재된 정의와 실행기 근거를 미리 보고 승인·철회하고, A-005(최소)에서 `pr.list`를 사유와 함께 차단·재개한다. API의 요청 수락과 실행기의 claim이 같은 판정 함수로 그 결정을 확인하고, 수락 뒤 정책이 바뀐 대기 요청은 과거 승인을 승계하지 않고 닫힌다.

**세 사실을 섞지 않는다 (S1).**

| 사실 | 어디에 있나 | 누가 쓰나 | 뜻 |
| --- | --- | --- | --- |
| 검증 기록 | `gh_capability_verification` (029, append-only, 030이 `scope`를 더함) | 실행기 `JOB-GH-003` (`prs_app` INSERT) | 어떤 바이너리·manifest를 검사했고 결과가 무엇이었나 |
| 운영 승인·차단 | `gh_operations_policy`(현재) · `gh_operations_policy_revision`(이력) (030) | `gh_operations_policy_apply` 함수만 | 운영자가 어느 정의를 이 배포 범위의 R0에서 쓰기로 했나, 어떤 capability를 막았나 |
| 실행 허용 | 저장하지 않는다 | 요청마다 `decideExecution`이 계산 | 이 요청이 지금 실행 가능한가 = 기능 켜짐 ∧ 코드·manifest 실행 목록 ∧ 정책 읽기 ∧ 현재 정의의 운영 승인 ∧ 운영자 차단 없음 ∧ (대기 요청이면) 수락 revision = 현재 ∧ 레지스트리 판정. 사용자 연결·권한·저장소는 그 뒤의 기존 재검증이 본다 |

「활성화」는 세 뜻으로 나눠 적는다. (1) **DB 기록 변경** — 정책 revision과 스냅숏 `activated_at`(최초 승인 시각, 한 번만). (2) **실제 배포 교체** — 이미지·manifest 교체이며 운영 승인은 아무것도 설치하지 않는다. (3) **실행 권한 변경** — 판정 입력 하나가 바뀔 뿐이고 상한은 코드의 `EXECUTABLE_CAPABILITIES`다.

**승인 기준과 반증 시험 (지시서 13장 A).** 서버가 적재한 정의·서버가 같은 정의로 만든 보고서·DB의 근거로만 판정하고 브라우저가 보낸 결과·해시를 믿지 않는다. 사유는 모두 모아 보인다(`evaluateApprovalEligibility`).

| 반증 입력 | 잡힌 사유 | 시험 |
| --- | --- | --- |
| 옛 판 `r1` 보고서 · 알 수 없는 판 `r999`·`r10`·`constructor` | `report_version_superseded` · `report_version_unsupported` — 판 문자열의 크기를 비교하지 않고 등록된 해석기(`r2`)만 받는다 | gh-cli 단위, search-api registry 통합(`unsupported_report_version`) |
| `passed` 문자열만 위조 | `report_hash_mismatch`·`report_not_passed` | gh-cli 단위 |
| 스냅숏·보고서·manifest 해시 불일치 | `evidence_other_definition`·`evidence_manifest_mismatch`·`report_hash_mismatch` | gh-cli 단위, DB 통합 |
| CI·CLI 기록 · 다른 배포 범위의 실행기 기록 | `evidence_missing` | gh-cli 단위, DB 통합(함수가 다시 거절) |
| 과거 통과 뒤 최신 드리프트·실패·일시 오류 | `evidence_not_passed`(API) · `evidence_not_latest` 충돌(DB 함수) | gh-cli 단위, DB 통합 |
| 오래된 근거 · 미래 시각 · 검사 주기를 모름 · 근거 없음 | `evidence_stale` · `evidence_cadence_unknown` · `evidence_missing` — 한도를 부풀린 호출은 `invalid_evidence_max_age` | gh-cli 단위, DB 통합, HTTP 통합(409 사유 목록) |
| API와 실행기의 적재 manifest 불일치 | `report_not_reproducible`(승인), `registry_stale`·`executor_definition_differs`(수락) | gh-cli 단위, 흐름 통합(정의가 바뀐 배포) |
| 바이너리·버전·인벤토리 관측 불일치 · 미분류 스냅숏 | `evidence_binary_mismatch`·`evidence_inventory_mismatch`·`snapshot_unclassified` | DB 통합 |

**신선도 한도.** 임의 상수를 두지 않고 검사 설정에서 계산한다: 한도 = 검사 주기(`GH_EXECUTOR_REGISTRY_CHECK_MS`, 기록의 `environment.registry_check_ms`) + 한 회차 최악 소요(시도 수 4 × (버전 10초 + 인벤토리 버전 20초 + 루트 help 20초 + ⌈비별칭 command 228 ÷ 동시 4⌉ × help 20초) + 재시도 대기 65초). 기본 91,225,000ms(25시간 20분 25초). 시간 상한·동시성·재시도 대기는 `registry-cadence.ts` 한 곳에서 검사 코드와 한도 계산이 같이 읽는다. 운영 영향: 주기를 줄이면 승인 근거의 유효 기간과 실행기의 새 실행권 만료도 줄고, 실행기가 한도 안에 통과를 남기지 못하면 새 실행은 `registry_evidence_expired`로 멈춘다(과거 통과 하나로 무기한 실행하는 경로를 없앴다). 승인 요청 경로는 인벤토리를 추출하지 않는다 — 근거가 없거나 오래됐으면 그 사실을 사유로 낸다.

**현재 상태·이력·revision 구조와 DB 역할 (지시서 8·9장).** 배포 범위(`GHE_BASE_URL`의 호스트, 클라이언트가 지정하지 않는다)마다 `gh_operations_policy` 한 행과 append-only `gh_operations_policy_revision`(`revision = previous_revision + 1`, UNIQUE `(scope, revision)`·`(scope, actor, idempotency_key)`). 쓰기는 `SECURITY DEFINER` 함수 하나(소유자 `prs_admin`, `search_path = pg_catalog, public, pg_temp`, 표 스키마 한정, `PUBLIC` 실행 회수, `prs_app`에 EXECUTE)이며 `prs_app`은 두 표를 읽기만 한다. search-api·web에 `prs_admin` 자격을 배포하지 않았다. 이 경계가 막는 것은 애플리케이션 롤의 우회이고 소유자·superuser는 경계 밖이다. 기존 `activated_at`이 있다고 승인을 만들지 않았다 — 마이그레이션은 정책 행을 하나도 만들지 않는다. 029 파일은 고치지 않았고 과거 스냅숏·검증 기록·실행 이력을 다시 쓰지 않았다.

**감사 원자성.** 적용된 결정의 감사 한 행은 함수 안에서 변경과 같은 트랜잭션에 쓴다(`FR-AUTH-004` AC-6 예외, SRS v2.28). `audit_record` INSERT가 실패하면 상태·이력·스냅숏 활성화가 모두 그대로다(DB 통합 — `audit_record`에 실패 트리거를 걸어 실측). 거절은 결과 코드(`forbidden`·`invalid`·`conflict`·`ineligible`·`key_reused`)로 best-effort 한 행, 같은 키·같은 내용의 재요청(`replayed`)은 0행이다(HTTP 통합). 거절 감사의 액션은 본문 `action`에서 정하므로 `action`이 없거나 네 값이 아니면 남기지 않고, 잠금 대기 초과 503은 결정의 거절이 아니므로 남기지 않는다 — 독립 검토 A의 지적에 동작은 두고 문서를 좁혔다(정본 어휘에 없는 액션을 만들지 않는다). `WP-075`의 원격 쓰기 best-effort 규칙은 복사하지 않았다. **이 내부 정책 감사 시험은 `GATE-GH-06`(실제 GHE 쓰기 실행의 감사)의 증거가 아니다.**

**동시 변경·응답 유실·큐 재검증 (지시서 13장 B·C).**

| 경합 | 결과 | 시험 |
| --- | --- | --- |
| 두 운영자가 같은 revision을 보고 승인 | 뒤 요청은 잠금을 기다린 뒤 `revision_changed` 충돌, 이력 1행 | DB 통합 |
| 승인과 차단 | 먼저 커밋한 쪽만 반영 | DB 통합 |
| 승인이 잠금을 쥔 동안 같은 범위의 새 검증 기록 | 기록은 승인 커밋까지 기다린다(검증 INSERT 트리거의 공유 잠금) | DB 통합 |
| 오래된 미리보기로 제출(그 사이 주기 검사가 새 기록을 남김) | 409 `evidence_changed` — 예전 확인을 적용하지 않는다 | 흐름 통합(실제 검사기) |
| 응답을 잃은 승인의 재요청 | 같은 키·같은 내용은 `replayed`(revision 추가 0), 다른 내용은 `GH_DUPLICATE_REQUEST` | 흐름·DB·HTTP 통합 |
| claim이 먼저 잠금을 쥠 → 차단 | 차단은 claim 커밋 뒤에 커밋되고, 차단 뒤의 claim은 가드가 PRS11로 거절 | DB 통합 |
| 재검증과 claim 사이에 차단 커밋 | claim 트랜잭션의 판정이 `policy_blocked`로 닫음(가드 대체 경로 `policy_changed`가 아니다), gh 호출 0 | 흐름 통합(`beforeClaim`) |
| 차단 전에 수락된 대기 요청 · 철회 전에 수락된 대기 요청 | claim에서 `policy_blocked`·`admin_action_required`로 닫힘, 재개 뒤에도 되살아나지 않음 | 흐름 통합 |
| 정책 표를 읽지 못함 | 수락 503 `GH_POLICY_UNAVAILABLE`, capability 목록·레지스트리·헬스는 200, 실행기는 대기 요청을 닫지 않고 남겼다가 복구 뒤 실행 | HTTP·흐름 통합 |
| 정책 변경이 배타 잠금을 10초 넘게 쥔 동안 claim | 55P03을 던지지 않고 `policy_unavailable`·`queued` 유지(이벤트는 `ack`, gh 호출 0), 잠금이 풀리면 실행 | 흐름 통합 (독립 검토 B 반영) |
| 다른 정책 변경이 배타 잠금을 10초 넘게 쥔 동안 정책 변경 | 503 `GH_POLICY_UNAVAILABLE`·`policy_lock_timeout`, revision 불변·감사 증가 0 | HTTP 통합 (독립 검토 B 반영) |
| 적용이 커밋된 뒤 응답용 재조회 실패 | 처리하지 않은 오류 500 — web은 503이 아닌 5xx에서 키를 유지하므로 같은 본문 재전송이 `replayed` 200(이중 적용·중복 감사 없음). 새 응답 모양을 시험 없이 들이지 않았다 | 같은 키 재요청은 흐름·HTTP 통합, 재조회 실패 자체는 정적 추적 (독립 검토 B) |

**실제 웹·gh 실행 증거.** 실행기 흐름 통합 시험(`apps/gh-executor/integration/policy-flow.test.ts`, 15건)이 실제 PostgreSQL·고정 gh 2.97.0(sha256 `141507c3…c409`)·HTTPS 목 GHE·실제 레지스트리 검사기·search-api의 실제 조회·변경·수락 함수·실제 러너로 결정자 지시서 1장의 흐름 1~9를 순서대로 보인다 — 승인 전 `GH_ADMIN_ACTION_REQUIRED`(GraphQL 0) → 미리보기(열리는 것 `pr.list` 하나, 열리지 않은 명령 195) → 승인(revision 1) → 실행(GraphQL 1, 결과 행 1) → 차단(revision 2) → 새 요청 403·대기 요청 닫힘(GraphQL 증가 0) → 재개(revision 3) → 새 요청 실행 → 이력·감사 행 셋. 그리고 다른 capability(`pr.view`) 거절·재시작한 실행기(새 검사기·새 러너)의 유지·정의가 바뀐 배포의 자동 승인 없음·철회. HTTP 계층(`policy-routes.test.ts`, 8건)은 실제 Fastify·세션(Redis)·PostgreSQL로 역할·JSON 전용·키·충돌·게이트를 본다. 화면은 실제 Chromium(`e2e/gh-policy.spec.ts`, 5건)이 **화면이 무엇을 보이고 무엇을 보내는가**만 증명한다(search-api 응답은 브라우저 경로에서 목킹) — 서버 판정의 증거는 위 두 통합 시험이다.

**이미지·migration·롤백 검증 (지시서 14장, 공식 발행 없음).** HEAD `f2e9a00`에서 `docker build --target {migrate,search-api,pipeline-worker,gh-executor,web}`를 태그 `cr090-verify`로 빌드했다(전부 종료 0). `./smoke-images.sh cr090-verify` 통과(web 기동·SSR·잠금 파일·운영 계약 거부 5종, pipeline-worker git, gh-executor 고정 gh 해시·꺼진 기동 `execution: disabled`·켜짐+봉인 키 없음 거부). 스모크의 SSR 목록에 없는 새 운영 화면은 같은 web 이미지를 네트워크 없이 띄워 `/ops/gh-policy`·`/ops/gh-registry`·`/gh`가 모두 200임을 따로 봤다. **롤백은 실제 옛 앱 코드로 쟀다**: `git archive 26e2627`로 풀어낸 옛 트리가 새 DB를 029까지 올리고 대기 요청 하나를 만든 뒤(A), 새 migrate 이미지 `prs/db:cr090-verify`가 그 DB에 030을 적용했고(B — 「적용: 030」, 함수 `SECURITY DEFINER`·소유자 `prs_admin`), 옛 코드의 `insertExecution`은 `PRS10`(행 증가 0), 옛 코드의 `claimExecution`은 030 이전 대기 요청에 `PRS11`(queued 유지)이었다(C). 029 → 030 → 029 → 030 왕복은 DB 통합 시험이 본다 — 과거 검증 기록 수 불변, `activated_at` 유지, 승인 비승계, 그 사이의 옛 대기 요청은 claim 거절. **030을 내리면 가드가 사라져 옛 앱의 실행이 다시 열린다** — 런북 7.C가 먼저 `GH_OPERATIONS_ENABLED=false`를 요구한다. 번들(`build-bundle.sh`)은 만들지 않았고 release·tag는 발행하지 않았다. **검토 반영(`9bb981d`) 뒤 다시 쟀다** — 030 가드·실행기·search-api가 바뀌었으므로 같은 다섯 대상을 태그 `cr090-final`로 다시 빌드했고(전부 종료 0), `./smoke-images.sh cr090-final` 통과, 새 운영 화면 SSR 200 셋, 새 migrate 이미지(`queued` 새 행 가드 포함)로 A·B·C를 되풀이해 같은 결과(옛 `insertExecution` PRS10, 옛 `claimExecution` PRS11)를 봤다.

**백업 복원 — 새 가드가 복구 경로를 막지 않는가 (독립 검토 A의 note를 실측으로).** 가드는 `queued`가 아니거나 revision이 없는 새 행을 거절하는데, 복원은 종료 상태와 revision 없는 과거 행을 다시 넣는다. `prsctl restore`와 같은 경로로 쟀다(HEAD `9bb981d`, 격리 PostgreSQL의 임시 DB, 일회성 스크립트 — 저장소에 넣지 않았다). DB 통합 시험(`gh-policy.test.ts` — 왕복 시험을 먼저, 나머지를 뒤에)이 남긴 정책 16행·이력 22행·감사 23행·스냅숏 28행·검증 기록 31행(범위 없는 실행기 기록 1 포함)과 실행 기록 14행(종료 상태 여섯, revision 없는 행 넷 포함)을 `pg_dump -Fc`로 떴다. 빈 DB에 `pg_restore --exit-on-error` 종료 0 → 마이그레이션 재적용 「적용할 마이그레이션이 없다」 → 표 7개의 행 수·내용 해시가 원본과 같다 → 카탈로그(가드 트리거는 부모와 월 파티션 셋에 복제, 적용 함수 `SECURITY DEFINER`·소유자 `prs_admin`·고정 `search_path`, NOT VALID CHECK, `prs_app` 권한)가 원본과 같다 → 복원된 DB에서도 `prs_app`의 `running` 직접 삽입과 revision 없는 삽입은 PRS10, 파티션 직접 삽입은 권한 거부. 029에서 뜬 백업은 복원 뒤 재적용이 030을 올리고, 과거 실행 행이 그대로이며 revision을 소급해 적지 않았다. **공식 근거와 관측을 나눈다**: pg_dump 문서는 트리거와 검증되지 않은 CHECK를 post-data 절이라고 적지만, 전체 복원이 데이터를 트리거보다 먼저 적재한다는 문장은 pg_restore 문서에 없다(14.4.9절은 데이터를 인덱스·외래 키보다 먼저 적재한다고만 적는다) — 그 순서는 덤프 목록(데이터 항목 뒤에 CHECK·트리거)과 이 복원으로 관측한 사실이다. 스크립트 1회차는 카탈로그 쿼리의 형 변환 오류로 비교가 비어 무효였고, 2회차는 가드 트리거가 파티션마다 복제되는 것을 모르고 기대 개수를 틀리게 적었다 — 둘 다 스크립트의 오류이며 3회차에서 모두 통과했다.

**새 시험을 믿기 전에 대상을 되돌렸다 — 변이 31종 (원문 보관 → 정확히 한 번 치환 → 시험 → 원문 복원과 바이트 대조, SQL 변이는 변이마다 새 DB에 001부터 적용).**

| ID | 제거한 방어 | 잡은 시험 | 결과 |
| --- | --- | --- | --- |
| T1 | 판정의 운영 승인 단계 | 판정 순서·다른 정의 거절 (단위) | killed |
| T2 | 대기 요청의 revision 비승계 | 판정 순서·비승계 (단위) | killed |
| T3 | 운영자 차단 단계 | 판정 순서 (단위) | killed |
| T4 | 보고서 판 해석기에 숫자 비교(`r999` 통과) | 해석기·자격 (단위) | killed |
| T5 | 실행기 마지막 통과의 신선도 만료 | 실행기 판정 입력 (단위) | killed |
| T6 | 재현 해시 대조 | 위조·재현 (단위) | killed |
| T7 | claim 트랜잭션 안의 재판정(가드만 남김) | claim 직전 차단 경합 (흐름 통합) | killed |
| T8 | API 수락의 판정 거절 | 승인 전·차단 뒤·철회 뒤 (흐름 통합) | killed |
| T9 | 변경 경로의 `operator` 판정 | 역할·모양·흐름 (HTTP 통합) | killed |
| T10 | 프록시의 `Idempotency-Key` 전달 | 프록시 허용 목록 (web 단위) | killed |
| T11 | 승인 요청의 미리보기 묶음 | 오래된 미리보기 (흐름 통합) | killed |
| T12 | API가 정책을 못 읽으면 `policy_unavailable`(빈 정책으로 읽지 않음) | 정책 확인 불가 (HTTP 통합) | killed |
| T13 | 실행기가 정책을 못 읽으면 대기 요청을 남김 | 정책 확인 불가 (흐름 통합) | killed |
| T14 | 기능 OFF면 gh 의존 없음 | 조립 (단위) | **첫 판 survived → 시험 보강 뒤 killed** |
| T15 | 러너가 claim의 잠금 대기 초과(55P03)를 던지지 않고 `queued`로 남김 | 잠금 대기 초과 claim (흐름 통합) | killed |
| T16 | 정책 변경의 잠금 대기 초과를 503으로(500이 아니라) | 잠금 대기 초과 변경 (HTTP 통합) | killed |
| S1 | 함수의 기대 revision 대조 | 충돌·동시 승인·승인 대 차단 (DB 통합) | killed |
| S2 | 감사 실패 시 롤백(예외를 삼킴) | 감사 실패 (DB 통합) | killed |
| S3 | 범위의 가장 최근 기록 확인 | 최신 드리프트 (DB 통합) | killed |
| S4 | 가드의 revision 대조 | 과거 revision claim (DB 통합) | killed |
| S5 | 가드의 차단 확인 | 차단 뒤 claim (DB 통합) | killed |
| S6 | 검증 기록 INSERT의 공유 잠금 | 직렬화 지점 (DB 통합) | killed |
| S7 | revision 없는 새 실행 기록 거절 | 옛 앱 INSERT (DB 통합) | killed |
| S8 | `SECURITY DEFINER` | 카탈로그·직접 쓰기·가로채기 등 18건 (DB 통합) | killed |
| S9 | 가드: 새 실행 기록은 `queued`만 | `running`·종료 상태 직접 삽입 (DB 통합) | killed |
| S10 | `prs_app`에 `public` CREATE 없음(GRANT를 더함) | 카탈로그 (DB 통합) | killed |
| U1 | DB 함수의 신선도 인자 상한(8일을 1일로 줄임) | 가장 긴 주기의 한도가 상한 안 (gh-cli 단위) | killed |
| R1 | 실행기가 검사기 상태를 그대로 넘김(가짜 신선 배선) | CR-088 배선 (회귀) | killed |
| R2 | claim의 정책 공유 잠금 | CR-090 판정·트랜잭션 (회귀) | killed |
| R3 | 정책 표 직접 쓰기 금지 | CR-090 쓰기 경로 (회귀) | killed |
| R4 | 변경 처리기의 `operator` 판정 | CR-090 역할·배선 (회귀) | killed |

T14가 처음 살아남은 이유: 조립 시험에 세션 인증이 없어 「인증 없음」 분기가 같은 `undefined`를 냈다 — 스위치를 무시해도 통과했다. 인증·Redis·Operations App 자격을 모두 주고 스위치만 바꾼 대조군을 넣어 다시 돌렸다. 정책 검사를 대역으로 바꾼 시험은 없다 — 승인은 실제 검사기 기록 → 실제 조회 → 실제 변경 함수 → DB 함수로만 만든다(`policy-helpers.ts`). 검토 반영 다섯(T15·T16·S9·S10·U1)은 반영 커밋 `9bb981d`에서 같은 실행기로 돌렸고, 변이마다 원문 복원을 바이트로 대조했다.

**이 판에서 실제로 관측한 실패와 처방 (예방적 수정 없음).** (1) 새 e2e W-010 시험 1건 — 저장소를 고르기 전에는 미리보기를 요청하지 않는 기존 계약을 시험이 빠뜨렸다 → 시험 절차를 고쳤다(화면 코드 불변). (2) web lint `no-control-regex` 1건 → 서버와 같은 예외 주석. (3) `tsc -p tsconfig.tests.json` 1건 — `GhExecutionRow`에 `policy_revision`이 생겨 기존 단위 시험 fixture가 타입을 어겼다 → fixture에 값. (4) 새 통합 시험 두 파일이 같은 DB에서 `acme/payments`를 다른 ID로 넣어 (owner, name) 고유 제약에 걸렸다(`repository_owner_name_key`) — CI는 파일 순서를 고정하지 않으므로 재현 가능 → 기존 gh 시험과 같은 TRUNCATE 정리. (5) `test:regression` 1건(481 중) — CR-088 회귀가 옛 실행기 배선을 문자열로 걸었다 → 의도를 유지한 새 배선으로 옮기고 CR-090 회귀 셋을 더했다. (6) 필수 배터리 1회차(HEAD `11254b3`)의 `pnpm lint` 2건 — `runner.ts`의 `import('@prs/db').PoolClient` 형 표기가 `consistent-type-imports`에 걸렸다 → type import로 바꿨다(`ecd7e31`, 형 표기만). (7) 검토 반영 문서를 쓰다가 API 계약이 정책 변경의 503에 「정책 조회 실패」를 넣은 것을 발견했다 — 코드는 변경 경로의 DB 오류를 처리하지 않은 오류(500)로 낸다. 동작은 두고 문서를 코드대로 고쳤다. (8) 문서 커밋 뒤 문서를 읽는 시험을 다시 돌리면서 한 번은 DB 환경 변수 없이 `vitest.regression.config.ts`를 돌려, DB가 필요한 두 파일(`range-vs-git`·`releases-vs-git`)이 실패했다(19건 skip) — 대상 결함이 아니라 실행 조건 누락이며, 배터리와 같은 환경의 `pnpm test:regression`은 484 통과했다. 재실행 성공으로 앞선 실패를 지우지 않고 여기에 남긴다.

**필수 배터리 (CI와 같은 순서 — verify: typecheck → lint → lint:deps → test → build → test:a11y → test:contrast → test:e2e, integration: 빈 `prs_test`·빈 Redis에서 test:integration → test:regression. 격리 서비스, Node v22.23.2).** 1회차 HEAD `11254b3`(문서 cascade 커밋): `pnpm lint` 종료 1(위 (6)), 나머지 종료 0 — 단위 2,695(1 skip — 실제 GHE 스모크)·a11y 424·e2e 196·통합 1,769(113 파일)·회귀 484(8 파일). **최종 HEAD `9bb981d`(검토 반영 뒤, 코드 트리 해시 `35fbb397b1cfdf75` — `docs/`·`agent-context/`·런북 제외, 시작·끝 동일)**: typecheck 0 · lint 0 · lint:deps 0 · 단위 2,696(1 skip) · build 0 · a11y 424 · 대비 0 · e2e 196 · 통합 1,772(113 파일) · 회귀 484(8 파일) — 전 단계 종료 0. 그 뒤의 커밋은 문서뿐이다.

**독립 검토 두 관점과 재검토 (지시서 16장, 읽기 전용 검토자 — 시험·DB·docker를 돌리지 않았다).** 1차 범위 `26e2627..11254b3`. **A(승인 권한·DB 권한·감사·실행 상한)**: blocker·major 0 · minor 2 · note 3. minor — 가드의 INSERT 분기가 `state`를 보지 않아 `prs_app`이 `running`·종료 상태로 바로 넣을 수 있었다(실행기는 `queued`만 집으므로 gh는 돌지 않지만 대조를 건너뛴다) → **반영**: 새 행은 `queued`만(PRS10), DB 통합 시험·변이 S9. minor — `action`이 없거나 알 수 없는 요청의 거절이 감사되지 않는다 → **동작 유지·문서 정정**: 정본 어휘에 없는 액션을 만들 수 없다. note — SQL 함수가 gh pin 상수를 대조하지 않는다 → **반영하지 않음**: pin은 코드·배포의 사실이라 불변 마이그레이션에 넣으면 다음 pin 교체 때 정당한 승인이 새 마이그레이션 전까지 막힌다. 실행은 `approvalMatchesManifest`와 가드가 pin된 정의로만 연다. note — 함수의 내장 호출이 스키마 한정이 아니다 → 전제(`prs_app`에 `public` CREATE 없음)를 카탈로그 시험·변이 S10으로 고정하고, 운영 DB 확인을 런북 7.C 4단계에 넣었다. note — 신선도 인자 상한(8일)과 TypeScript 계산의 결합 → 단위 시험·변이 U1. **B(동시성·최신 근거·큐 재검증·과거 기록·롤백)**: blocker·major 0 · minor 2 · note 3. minor — claim 트랜잭션의 55P03이 러너를 던지게 해 버스가 재시도로 돌리고 같은 파티션이 막힐 수 있었다 → **반영**: `policy_unavailable`·`queued` 유지, 흐름 통합·변이 T15. minor — 정책 변경의 55P03과 적용 뒤 재조회 실패가 500이다 → 55P03은 **반영**(503·감사 없음, HTTP 통합·변이 T16), 재조회 실패는 **동작 유지·문서화**(web이 같은 키로 다시 보내 `replayed`를 받는다). note — 정책 변경이 `audit_record` 파티션 가용성에 묶인다 → 관측성 `RB-26`·런북 증상 표. note — `checked_at`(트랜잭션 시작 시각) 정렬의 동률 역전 → 두 곳의 정렬이 같고 승인은 신선도·`passed`를 따로 요구해 실행을 여는 쪽으로 뒤집히지 않으므로 기록만 한다. note — 롤백 뒤 옛 앱의 재시도 반복 → 안전은 가드가 지키고 런북이 먼저 끄기를 요구하므로 기록만 한다. **재검토(같은 관점, `ecd7e31`·`9bb981d`)**: A·B 모두 새 blocker·major·minor 0이고, 처분의 근거(동작 유지·문서 정정, pin 미대조, 재조회 실패 500 유지)를 타당하다고 판정했다. 남은 note — (A) 55P03 판정이 claim 행 잠금 경합까지 `policy_lock_timeout`으로 이름 붙일 수 있다(처분이 같아 결함은 아니다) · (A) 복원 경로의 무회귀는 추론이다 → 위 복원 실측으로 확인했다 · (B) search-api는 정책 변경의 잠금 대기 초과를 로그·지표로 남기지 않는다 · (B) 실행기 지표 `policy_unavailable`이 정책 읽기 실패와 잠금 대기 초과를 합친다 · (B) 기존 스위퍼의 머리 막힘 — 계속 던지는 오류가 가장 오래된 대기 행에 있으면 그 회차가 멈춘다(이 판이 만든 것이 아니고, 55P03을 던지지 않게 해 오히려 줄였다). 로그·지표 이름·스위퍼의 세 note는 코드를 바꾸지 않고 문서(`RB-26`)와 이 기록에 남겼다.

**문서 cascade 독립 검토 (읽기 전용 검토자).** 범위는 `26e2627..bc41f2e`의 문서(SRS부터 런북까지)다. 첫 호출은 턴 한도에서 발견 사항 없이 멈췄고, 「지적 없음」으로 처리하지 않고 같은 검토자에게 남은 항목을 이어서 끝내게 했다. 확인한 것: 범위 규칙(통과로 바뀐 게이트 없음, `REL-007`·`WP-059` 전체 완료·사내 GHES 확인 주장 없음, SRS 변경이 CR-090 행의 범위와 같음), 코드 대조(오류 코드 넷·보강 셋의 HTTP 상태, 변경 경로의 503과 500의 경계, 판정 순서, 030의 DDL·권한·가드, 잠금 대기 10초, 신선도 91,225,000ms 산식, 지표·로그 문구), 원장·작업 패키지·CR 대장의 수치와 로그(배터리 두 판·이미지·롤백·복원 세 회차), 새 ID 17개의 충돌 없음, 내용을 바꾼 문서 19개의 판이 `26e2627` 대비 한 단계씩 올랐고 새 baseline이 없음, 표 칸 수, 과거 기록 보존(`WP-045` 정정), 지시서가 요구한 문장. **blocker·major·minor 0.** nit 1 — `QA-GH-49`의 FR 귀속이 같은 대상을 그리는 와이어프레임 배너 행보다 `FR-GH-009 AC-8` 하나가 적었다 → QA 체크리스트와 원장 4장 `FR-GH-009` 행의 QA 참조를 함께 맞췄다. 검토자가 확인하지 못했다고 적은 항목은 따로 대조했다: 변이 로그의 마지막 판정 31/31 killed, e2e 196·`gh-policy.spec.ts` 5건, 런북이 인용한 화면 문구 다섯이 web 소스에 있음, `apps/web/lib/proxy.ts`의 `idempotency-key`, `packages/domain/src/audit.ts`의 네 액션.

**문서 검사기 `--strict`.** 편집 전(`26e2627`과 같은 문서): 종료 1 · ERROR 4(정의되지 않은 ID `FR-CSS-005`·`D-002`, 자리표시어 13+8) · WARN 2(`risks.md` 경로). cascade 편집 뒤(`11254b3`), 검토 반영 편집 뒤(`bc41f2e`), 이 기록까지 넣은 최종 문서에서 모두 같은 6건이다 — **신규 0건**이며 strict 전체 통과는 아니다.

**능동 자가 연구 (지시서 5장).** 공식 근거는 PostgreSQL 16 문서, 로컬 관측은 격리 DB 실험(E1~E5)과 시험이다.

| 질문 | 공식 근거 | 로컬 관측 | 설계 판단 |
| --- | --- | --- | --- |
| 승인 판단과 저장 사이에 무엇이 바뀔 수 있나 | READ COMMITTED는 문장마다 새 스냅숏이고(13.2.1) VOLATILE 함수도 쿼리마다 새 스냅숏이다(38.7) | 잠금 없이 읽으면 다른 운영자의 변경·새 검증 기록·claim이 사이에 커밋될 수 있다 | 범위 단위 advisory lock — 변경은 배타(search-api가 판정 전에 잡고 함수가 다시 잡음), 검증 기록 INSERT와 claim은 공유 |
| 두 운영자의 동시 변경을 어떻게 충돌로 | 기다린 UPDATE는 갱신된 행으로 WHERE를 다시 평가한다(13.2.1) | E1: 뒤 트랜잭션은 앞 커밋까지 막히고 `WHERE revision = 기대` UPDATE는 0행 | 잠금 아래 기대 revision 대조(PRS01) + `WHERE revision = 현재` 이중 확인 |
| 행 잠금으로 claim과 직렬화할 수 있나 | `FOR SHARE`도 UPDATE 권한이 필요하다(SELECT 잠금 절) | E3: `prs_app`의 `FOR SHARE`·직접 UPDATE는 42501, advisory lock 호출은 가능 | 행 잠금 대신 advisory lock(표 권한 불필요, 트랜잭션 끝에 자동 해제 — 13.3.5). E2: 대기 중인 배타 뒤에 새 공유가 선다 |
| DB 소유자 자격을 주지 않고 필요한 변경만 | `SECURITY DEFINER` 함수는 `search_path`를 신뢰 스키마 뒤 `pg_temp`로 고정하고 `PUBLIC` 실행을 회수해 필요한 롤에만 EXECUTE를 준다(CREATE FUNCTION) | E4: `prs_app`이 함수로 변경 가능, `PUBLIC` 실행 42501, **`prs_app`은 TEMP 표를 만들 수 있다** | 표를 스키마 한정하고 `pg_temp`를 마지막에 두었다. 같은 이름의 임시 표 가로채기를 시험으로 건다 |
| 감사와 변경이 따로 성공하는 창이 없는가 | 트랜잭션은 전부 또는 전무이고(3.4) 잡히지 않은 `RAISE EXCEPTION`은 트랜잭션을 중단한다(43.9.1), `EXCEPTION` 블록은 그 블록 안만 롤백한다(43.6.8) | E4: 감사 INSERT 실패 시 상태·이력 불변. 변이 S2(예외를 삼킴)는 시험이 잡는다 | 감사를 함수 안 같은 트랜잭션에 두고 예외 블록으로 감싸지 않는다 |
| `report_hash`는 무엇까지 포함하나 | (코드) `reportHash` = 검증기 보고서 전체의 정규화 JSON sha256 — 판·검증기와 규칙 판·manifest 판과 해시·해시 검증·인벤토리 해시·차원·게이트·실행 허용·결과 계약 집계·findings·상태. 시각은 없다. 바이너리 해시·관측 gh 버전·실행기 환경·검사 시각은 기록의 별도 열이다 | E5: JSONB 왕복 뒤 해시 동일, 필드 하나를 바꾸면 달라진다 | 저장 해시 일치(무결성)와 서버가 같은 정의로 만든 보고서의 해시 일치(재현)를 따로 요구하고, 바이너리·버전·인벤토리는 관측 = 기대 열로 따로 본다 |
| 「최신 검사」와 「마지막 성공 검사」 | — | (기존 코드) 실행기 상태의 `checkedAt`은 결과와 무관한 마지막 회차였고 마지막 통과 시각이 없었다. API는 출처별 최신 1행만 있었다 | 실행기에 `lastPassedAt`, API에 범위의 최신 실행기 기록(일시 오류 포함·제외 두 뜻). 승인은 일시 오류까지 포함한 최신이 `passed`여야 한다 |
| 재시작·다른 배포·다른 호스트를 가를 근거 | — | (기존 코드) `executor_id` = 호스트명:pid:uuid 앞 8자로 재시작마다 바뀐다. 기록에 배포 범위가 없었다 | 배포 범위 = `GHE_BASE_URL`의 호스트를 기록에 적는다(030 NOT VALID CHECK). 실행기 신원은 근거로 쓰지 않는다 — 재시작이 승인을 무효로 만들지 않되, 승인은 정의와 그 범위의 최신 근거에 묶인다 |
| 기존 실행 기록에 승인 revision을 어떻게 잇나 | 기본값 없는 nullable `ADD COLUMN`은 표를 다시 쓰지 않는다(ALTER TABLE Notes — 요약본으로 확인) | 과거 행은 NULL | 과거 행을 다시 쓰지 않고 NULL로 둔다 — 그 행의 claim은 가드가 거절한다 |
| 함수의 한정하지 않은 내장 호출이 안전한가 (검토 뒤 추가) | PostgreSQL 15 릴리스 노트: `PUBLIC`의 `public` 스키마 CREATE 권한 제거 — 새 클러스터·새로 만든 DB에 적용하고, 업그레이드·덤프 복원은 기존 권한을 유지한다 | 시험 DB에서 `has_schema_privilege('prs_app', 'public', 'CREATE')`가 false(변이 S10이 GRANT를 더하면 실패). 복원 스모크의 원본·복원 DB도 false | 표만 스키마 한정하고 내장은 한정하지 않은 채 전제를 시험으로 고정했다. 옛 DB를 올렸거나 그 덤프를 복원한 운영 DB는 전제가 깨질 수 있어 런북 7.C 4단계에서 확인한다 |
| 새 가드가 백업 복원을 막지 않는가 (검토 뒤 추가) | pg_dump `--section`: 「Post-data items include definitions of indexes, triggers, rules, and constraints other than validated check constraints」. pg_restore 문서는 전체 복원의 적재 순서를 명시하지 않는다 | 덤프 목록에서 데이터 항목 뒤에 NOT VALID CHECK·가드 트리거가 온다. 빈 DB 복원 종료 0, 내용 해시 동일, 복원 뒤에도 가드가 산다. 가드는 월 파티션마다 복제된다 | 복원용으로 가드를 끄는 스위치를 두지 않는다 — `prsctl restore`(빈 DB 전체 복원 → 재적용)가 그대로 성립한다. 순서가 공식 문장이 아니므로 PostgreSQL 주 판이 바뀌면 같은 복원을 다시 잰다 |

예상과 달랐던 점: `FOR SHARE`에 UPDATE 권한이 필요해 행 잠금 설계를 버렸다 · `prs_app`이 TEMP 표를 만들 수 있었다 · 풀 연결 세션에 남은 TEMP 표가 뒤 시험의 한정하지 않은 조회를 가로챘다(`ON COMMIT DROP`으로 고쳤다) · 실제 프록시가 중복 방지 키를 버리고 있었다(`DEV-690`) · 문서의 CSRF 토큰이 코드에 없었다(`DEV-691`) · CR-088 회귀가 옛 배선을 문자열로 걸었다 · 조립 시험의 인증 누락이 변이를 살렸다(T14) · 가드의 INSERT 분기가 상태를 보지 않았다(검토 A) · claim 트랜잭션의 잠금 대기 초과가 러너를 던지게 했다(검토 B) · API 계약이 변경 경로의 DB 오류를 503이라고 적었지만 코드는 500이었다 · 가드 트리거가 월 파티션마다 복제된다(복원 카탈로그 비교) · 전체 복원에서 데이터가 트리거보다 먼저 적재된다는 것은 공식 문서의 문장이 아니었다.

**PR CI.** PR #188 — 첫 head `bc41f2e` run `34852128701` attempt 1 success, 문서 cascade 검토 기록을 더한 최종 head `43c24f4` run `34853418938` attempt 1 success. 최종 head의 `verify` 잡(14:06:19→14:09:42 UTC): typecheck·lint·lint:deps·단위 2,696(1 skip)·build·a11y 424·대비·e2e 196. `integration` 잡(14:06:19→14:12:56 UTC): 통합 1,772(113 파일) 다음 `test:regression` 484(8 파일) — 로컬 최종 배터리와 건수가 같다.

**병합.** squash `9c8a781` — 부모 `26e2627`, 트리 `f6170ce6`가 PR head `43c24f4`의 트리와 같다(병합 뒤 `origin/main` 실측). 병합 조건은 필수 check green과, 독립 검토 두 관점·재검토·문서 cascade 검토의 blocker·major 0이다.

**병합 뒤 main CI.** 병합 커밋 `9c8a781` — run `34854308135`(push) attempt 1 success. `verify` 잡(14:14:28→14:19:14 UTC): typecheck·lint·lint:deps·단위 2,696(1 skip)·build·a11y 424·대비·e2e 196. `integration` 잡(14:14:28→14:20:58 UTC): 통합 1,772(113 파일) 다음 `test:regression` 484(8 파일).

**사내에서만 확인할 수 있는 것 (NOT RUN).** 사내 반입 번들로 마이그레이션 030을 적용하고 운영 DB의 `public` CREATE 권한을 확인하는 것, 실제 GHES에서 실행기 기동 검사·A-006 근거 확인·최초 운영 승인·한 사용자·한 저장소의 `pr.list` 실행·차단·재개, 대상 GHES 지원 확인(`DEV-674`). 이 판은 release·tag를 발행하지 않았다.

### 6.86 REL-007 R1b — 결과 계약·typed port·순수 연결 판정·타입 그래프·A-006 조회 (2026-09-14, CR-089 / WP-079)

시작 `origin/main`은 `f3cd00f`(CR-088 병합과 handoff 기록 뒤, main CI run `34791175999` success). 브랜치 `feature/rel007-result-contracts`(작업 트리 격리, 격리 서비스 `prs-contracts-*`). **실행 허용은 `pr.list` 하나 그대로이고, 이 판에서 새로 실행되는 명령은 없으며, 릴리스는 발행하지 않았다.**

**무엇을 증명했는가.** 고정 gh 2.97.0의 leaf 196개 전부가 **근거를 가진 결과 계약**(출력 모드별 결과·adapter·스키마 또는 구조화 불가 사유, 자원 종류 또는 비적용 이유, composability, 민감도, typed 입출력 port와 조건)을 받았고, 독립 검증기가 인벤토리에서 계약을 다시 만들어 저장값과 대조한다. 실제 고정 gh의 `pr list` 결과가 PR 참조로 정규화되고, 목록에서 PR 하나를 명시적으로 고르면 `pr view`의 입력 계약과 호환으로 판정되지만 **실행은 열리지 않는다** — 목 서버가 받은 GraphQL은 `pr list`의 것 하나뿐이다. 그래프는 판정기의 답에서 계산되고(간선 398, 전부 조건부), 실행 가능한 간선·다단계 흐름은 0이다. 결과 계약이 생겨도 실행 준비·실행기 재검증은 그것을 읽지 않고, 결과 계약이 있는 `pr.view`를 `allowed`로 바꿔 적재해도 `GH_CAPABILITY_NOT_EXECUTABLE`이다. 도메인 회귀가 이 판부터 CI integration 잡에서 돈다.

**첫 계약 표(S1)는 change_control의 CR-089 cascade에 있다** — 결과 계약·port·bindable·구현 adapter·호환·실행 허용·다단계 흐름·대상 GHES 확인의 뜻과 분모.

**분모와 수치 — 검증기 보고서(판 `r2`, `validator-2026-09-14.2`, `rules-2026-09-14.3`, manifest `r0.3` hash `18589580…`, `pnpm gh:validate-capabilities` 종료 0) 실측.**

| 게이트 | 차원 | 분류/전체 | 분모를 센 법과 「분류됨」 |
| --- | --- | --- | --- |
| GATE-GH-01d | result_contract | 196/196 | leaf. 결과 계약이 있고 출력 모드마다 adapter와 스키마 또는 구조화 불가 사유가 있으며 주 종류가 구조화 모드와 맞다 |
| GATE-GH-01d | bindability | 196/196 | leaf. composability 8종 중 하나이고 bindable·출력 port·출력 모드와 어긋나지 않으며, port가 없으면 이유가 있다 |
| GATE-GH-01d | resource_type | 196/196 | leaf. `GhResourceRef.kind` 하나와 근거, 또는 자원 결과가 아닌 이유 — 가짜 종류를 채우지 않는다 |
| GATE-GH-01d | secret_output | 196/196 | leaf. secret 4개(`auth login`·`auth refresh`·`auth token`·`secret set`)는 `secret_non_bindable`·출력 port 0·모든 모드 비밀 |
| GATE-GH-01d | output_port | 32/32 | port 수가 아니라 바인딩 가능한 capability 수(출력 port 36). 분모 0이면 미달 |
| GATE-GH-01d | input_port | 80/80 | port 수가 아니라 대상 자원 자리를 가진 capability 수(입력 port 81). 분모 0이면 미달 |
| 정보 | graph_edges · opaque_text · adapter_implemented · executable_flows · host_support | 398 · 30 · 1 · 0 · 0/196 | 간선은 실행 승인이 아니다. opaque는 composability `opaque_result`. 구현 adapter는 `pr.list:pr_list_v2`. 사내 확인 0(`DEV-674`) |

분포: composability — partially_bindable 32 · terminal_result 115 · opaque_result 30 · policy_blocked 12 · secret_non_bindable 4 · artifact_result 3 · fully_bindable 0 · unsupported_by_host 0. 주 결과 종류 — exit_status 81 · json 40 · text 29 · resource 16 · resource_list 14 · url 9 · stream 4 · artifact 3. 자원 종류 — 없음 148 · project 8 · pull_request 7 · issue 7 · repository 6 · discussion 4 · release 4 · artifact 3 · codespace 3 · workflow_run 3 · commit 1 · workflow 1 · gist 1(project·gist·artifact는 종류만 있고 참조를 만들지 않는다). 간선 타입 — pull_request 144 · issue 102 · repository 84 · codespace 24 · release 16 · discussion 12 · workflow_run 12 · workflow 4. 같은 타입인데 이어지지 않는 짝 6(전부 목록 → 목록: `issue list`·`issue edit`·`issue status` 셋·`search issues` → `issue edit`). **직접 호환 0** — gh 결과에는 출력 모드·필드 선택·URL 대조 조건이 늘 있다. CR-088 대비 주 결과 종류 63건·출력 모드 29건 정정(`DEV-683`).

**실제 고정 gh 2.97.0으로 본 `pr list` 출력 (목 GHE, 실행기와 같은 argv·환경 허용 목록).**

| 입력 | stdout (종료 코드) | 이 판의 결과 |
| --- | --- | --- |
| `--json number` | `[{"number":12},{"number":11}]` (0) | 참조 둘, 호스트·저장소는 실행 컨텍스트 |
| `--json title` | `[{"title":…},{"title":"Add thing"}]` — 고른 필드만 (0) | 성공, 행 번호 `null`, 참조 `unavailable`(`identity_field_not_selected`). v1은 `row_shape`로 실패했다(`DEV-682`) |
| GraphQL `number: null` | `[{"number":0,…}]` — Go zero value (0) | `result_parse_failed: invalid_identifier` — 0을 번호로 받지 않는다 |
| GraphQL `number: "12"` | 빈 stdout, stderr `json: cannot unmarshal string …` (1) | 실행 실패(gh 자체) |
| GraphQL `number: -3` | `[{"number":-3,…}]` (0) | `invalid_identifier` |
| 빈 목록 | `[]` (0) | 성공, 참조 0개 — 없는 원소 `/0` 선택은 `index_out_of_range` |
| `author: null`·`headRefName` 없음 | `{"is_bot":true,"login":"app/"}`·`""` (0) | 행 값 그대로(식별과 무관) |

**검증기가 위조를 잡는가 — CR-089 변이 (`validate.test.ts`, 변이마다 해시를 다시 넣는다).**

| 위조 | 잡힌 코드 |
| --- | --- |
| `pr.list` 출력 port 삭제 | `bindable_port_desync` · `classification_recompute_mismatch` · `definition_adapter_port_missing` |
| 바인딩 가능한 계약 전부를 비바인딩으로(분모 0) | `classification_recompute_mismatch`, `GATE-GH-01d` 미달(`output_port 0/0`) |
| 재계산이 맞는 manifest의 port 분모 0 | 오류 0이어도 `GATE-GH-01d` 미달·상태 `incomplete`(판정 규칙 자체) |
| `auth.token` 출력을 `opaque_text`로(비밀이 흐르게) | `secret_flows` · `classification_recompute_mismatch` |
| `pr.view` port 중복·없는 타입·JSON에 없는 식별 필드·방향 위조 | `port_id_duplicate` · `port_type_invalid` · `port_identity_field_unknown` · `port_direction_mismatch` |
| `pr.view` 입력 port 삭제 | `input_ports_recompute_mismatch` · `input_ports_note_missing` |
| `pr.merge`를 대상 GHES 확인 없이 `unsupported_by_host`로 | `unsupported_without_host_evidence` |
| 구현 adapter를 옛 스키마·없는 port로 | `definition_drift` · `definition_adapter_unimplemented` · `definition_adapter_mismatch` · `definition_adapter_port_missing` |
| `pr.merge`·`pr.view`를 `allowed`로 | `execution_widened` · `executable_count_mismatch` |
| `pr.merge` 결과 계약 삭제 | `result_contract_missing`(gap) · `classification_recompute_mismatch` |

**새 시험을 믿기 전에 대상을 되돌렸다 — 변이 22종 (`mutate.mjs`, 기준선 11개 명령 전부 통과 뒤, 복원 후 작업 트리 해시 동일).**

| 변이 | 결과 |
| --- | --- |
| M01 `pr_list_v2` 파서가 번호 0을 받는다 | **생존 — 등가 변이**: 뒤이은 참조 검증(`validateResourceRef`)이 같은 규칙(양의 안전한 정수)으로 같은 결과(`invalid_identifier`)를 낸다. 관찰 가능한 차이가 없다 |
| M02 번호 미선택도 필수(v1 회귀) · M03 참조 호스트가 컨텍스트와 다름 | killed (result 단위) |
| M04 목록 → 단일 명시 선택 요구 제거 · M05 값 개수 검사 제거 · M06 목록 → 목록 불가 제거 · M07 비밀 source 불가 제거 | killed (binding 단위) |
| M08 배열 인덱스 선행 0 · M09 escape 해독 순서 · M10 prototype 토큰 | killed (json-pointer 단위) |
| M11 `secret_flows` 제거 · M12 port 분모 0 통과 · M13 구현 adapter 불일치 코드 제거 · M14 PR URL 근거를 모든 무리에 인용 | killed (validate 단위 — M14는 재계산 불일치로) |
| M15 A-006 상세에 `execution_id` · M16 `r1` 보고서를 검증됨으로 | killed (search-api registry 통합) |
| M17 간선에 「실행 가능」 · M18 참조 없음 문구 뒤집기 | killed (a11y) |
| M19 픽스처 간선 수 지어내기 | killed (픽스처 드리프트 가드) |
| M20 CI 회귀 단계 제거 · M21 실행 경로가 결과 계약을 읽음 | killed (회귀 CR-089) |
| M22 실행기 결과 kind를 `json`으로 | killed (실행기 통합) |

**실행 범위 유지 — 근거.** 실행 준비(`prepare`)는 manifest의 `allowed`와 코드 표 `findCapability`를 **둘 다** 요구하고 실행기 재검증도 코드 표를 다시 본다. 회귀 CR-089 블록이 실행 경로 두 파일에 결과 계약·port·`resultAdapter`·바인딩·그래프·composability 참조가 없음을, 앱 다섯의 제품 코드에 `evaluateBinding(` 호출이 없음을, A-006 조회가 실행 기록을 읽지 않음을(주석을 걷은 코드 기준), CI integration 잡의 `test:integration` → `test:regression` 순서를 건다. `evaluateBinding`은 성공해도 `executable: false`와 이유를 돌려준다. 스냅숏은 검사가 `passed`여도 활성화하지 않았다(`DEV-685`).

**과거 기록.** `pr_list_v1` 실행 결과는 `references`가 없어 W-010이 참조 문구를 그리지 않고 번호 칸도 기존대로 보인다. `r1` 보고서의 검증 기록은 API가 `contract_dimensions: not_in_report_version`으로 내고 A-006이 「결과 계약 미검증(옛 판)」으로 보인다 — `r10` 같은 판도 숫자로 비교한다. `r0.2` 스냅숏 행은 그대로 남고, 새 manifest는 해시가 달라 새 행으로 기록된다.

**능동 자가 연구 결과.**

| 질문 | 고정 버전의 근거 | 실제 관측 | 예상과 달랐던 점 | 바뀐 구현 판단 | 남은 미확인 |
| --- | --- | --- | --- | --- | --- |
| 같은 명령의 기본 출력과 JSON 출력이 같은 결과 계약인가 | `pkg/cmdutil/json_flags.go:225-257`(exporter는 `--json`일 때만 고른 필드를 인코딩), `--jq`·`--template`은 `--json`이 있어야 붙는다(:139-141·148-150) | 기본 출력은 표·요약이고 `--json`만 구조다 | 한 command를 계약 하나로 적으면 기본 출력에도 port가 있는 것처럼 보인다 | 계약을 **출력 모드마다** 둔다. jq·template 모드는 `opaque_text`·비바인딩 | — |
| 필드 선택에 따라 식별자가 없어질 수 있는가 | exporter가 고른 필드만 쓴다 | `--json title` → `[{"title":…}]` | 기존 v1 파서가 이 정상 조회를 실패로 만들었다(실결함) | `pr_list_v2` — 조회는 성공, 참조는 `unavailable`과 이유 | — |
| 목록·단일·nullable의 뜻 | `pr/list/list.go:203·213`(빈 결과도 `[]`), `pr/view/view.go:126`(객체 하나), `pr/status/status.go:98·168-176`(`--repo`면 `currentBranch` 없음) | 빈 목록 `[]` 종료 0 | `pr status`의 현재 브랜치 PR은 실행기에서 늘 null이다(작업 트리 없음) | cardinality·nullable·`workspace_required` 조건. 정적 간선으로만 남긴다 | — |
| PR·Issue·Check Run·Workflow Run 식별자 | issue 조회는 `issueOrPullRequest`로 PR과 같은 번호 공간(`issue/shared/lookup.go:156-163`), `pr checks` JSON 필드에 식별자 없음(`pr/checks/checks.go:22-32`), run은 `databaseId`(`run/shared/shared.go:175-176`, `run rerun`이 받는 값) | — | SRS 예시의 `CheckRunRef[]`는 만들 수 없고 check run은 자원 종류에도 없다 | 참조는 종류와 자리로 식별(같은 번호의 issue·PR이 다른 참조), `pr checks`는 출력 port 없음, 예시 정정(`DEV-684`) | — |
| URL·번호·node ID·저장소 컨텍스트 중 무엇을 근거로 | PR `<url>` 인자는 URL의 저장소·호스트로 `--repo`를 덮는다(`pr/shared/finder.go:117-120`), 출력 URL 문법은 gh 자신의 파서(`finder.go:301` 등), search 결과의 `repository.nameWithOwner`는 RepositoryURL의 마지막 두 조각(`pkg/search/result.go:431-437`) | 목의 행 `url`은 `https://127.0.0.1/…`인데 실행 호스트는 포트를 가진다 | URL 대안을 입력 자리로 받으면 실행 컨텍스트를 우회한다 | 입력 port는 번호·ID 대안만. host·repository는 검증된 컨텍스트, 출력 URL은 컨텍스트와 같을 때만 참조. node ID는 식별 자리로 쓰지 않는다 | 대상 GHES의 gist URL 경로 형식(gist create port 보류), 릴리스 URL을 읽는 gh 파서 부재 |
| JSON Pointer로 안전하게 표현할 수 있는 선택 | RFC 6901 3장(ABNF)·4장(`~1` 먼저 `~0` 나중)·7장(오류 조건), 배열 인덱스 선행 0 금지, `-`는 없는 원소 | — | — | 부분집합 + 제품 제한(128자·토큰 8·자기 속성만·prototype 토큰 거부·보정 없음) | — |
| 결과 스키마가 실제 adapter와 일치하는가 | 실행기는 `pr list`만 구조화한다 | — | 계약에 스키마 이름이 있다는 것과 실행기가 만든다는 것이 섞일 수 있다 | `IMPLEMENTED_RESULT_SCHEMAS`와 분리 집계 — 구현 adapter 1 | — |
| 비TTY stdout의 실제 모양 | `pr/close/close.go:111`·`pr/merge/merge.go:492·500` 등은 `IO.ErrOut`, `label create`·`repo edit` 등은 TTY에서만 stdout | stdout이 빈다 | 이름에서 짐작한 `resource`가 틀렸다(63건) | 주 종류를 비TTY 관측으로 정정하고 행마다 근거(`DEV-683`) | 사내 GHES에서의 동일성(`NOT RUN`) |
| `--web` 모드의 stdout | 안내는 stderr(28곳)나 `IsStdoutTTY()` 안의 stdout(7곳), `gist create`만 조건 없이 stdout(`gist/create/create.go:181-183`) | — | 일반 문장이 `pr list` 한 곳의 소스를 근거로 들고 있었다 | 「구조화된 결과가 없다」로 고치고 근거를 일반화 | — |
| `--json` 지원 command 41의 출처 | `workflow run --json`의 help 「Read workflow inputs as JSON via STDIN」 | `--json` flag를 가진 command 41 · JSON FIELDS를 가진 command 40 | 41은 입력 flag를 센 수였다 | SRS 정정(`DEV-676`), 출력 모드 판정에서 `workflow run --json` 제외 | — |

**배터리 (커밋 직전, 작업 트리 코드 해시 `325fa912e7f2dd77`(docs 제외 코드 트리, 시작·끝 동일), 격리 서비스).** `pnpm typecheck` 0 · `pnpm lint` 0 · `pnpm run lint:deps` 0 · `pnpm run test` 2,651 통과(1 skip — 실제 GHE 스모크) · `pnpm run test:integration` 1,725 통과 · `pnpm run test:regression` 481 통과(8 파일) · `pnpm build` 0 · `pnpm gh:validate-capabilities` passed(보고서 `686f4d6d…`) · `pnpm gh:diff-capabilities` match(바이너리 `141507c3…`, 인벤토리 해시 `83128580…`, added·removed·changed 0) · `pnpm run test:a11y` 413/414 · `pnpm run test:e2e` 190/191. 실패 두 건은 원인·관련성·재현으로 나눠 확인했다. a11y `ops-pipeline`의 5초 시간 초과는 단독 3/3·부하 없는 전체 414/414로 재현되지 않았고(배터리 도중 검토자의 시험과 CPU가 겹쳤다) 이 브랜치가 그 파일·컴포넌트를 바꾸지 않았다. e2e `flow-003:176`은 관련이 있었다(`DEV-689`) — 같은 앱 빌드에서 이 판의 gh 명세로는 전량 5회 중 3회 실패, origin/main 판 명세로는 4회 중 0회였고, 도착 판정만 15초로 늘린 뒤 같은 조건 전량 5회가 모두 191/191이다. 배터리 뒤 바뀐 코드는 회귀 시험의 주석 제거 정규식(검토 A의 nit — CR-089 블록 4건 재통과)과 `flow-003.spec.ts`(`DEV-689`)뿐이며, 커밋 직전 typecheck 종료 0 · lint 종료 0을 다시 확인했다.

**독립 검토 두 관점.** A(결과 의미·스키마·자원 식별·민감도·타입 호환): 결함 0. nit 3 — URL 번호 정규식의 오류 표지(도달 불가라 그대로), `pr status` `current_branch` 정적 간선(`workspace_required`로 문서화, 그대로), `sensitive` source를 막지 않는 이유(판정 자리에 조건을 주석으로 남김). 검토자는 manifest를 고쳐 해시를 다시 넣어도 위조가 전부 `failed`임을, 자동 호환 경로가 없음을, 인용 약 19곳이 v2.97.0 소스와 맞음을 dist로 재현했다. B(실행 권한 분리·CI 도달성·과거 기록·드리프트·사용자 흐름): blocker·major 0. minor 2 — 회귀 전체가 CI에 처음 편입된다(주석으로 명시, PR·main CI로 확인), `evaluateBinding` 부재 시험이 `ingest-gateway`를 걷지 않음(추가). nit 2 — 실행 경로 정규식에 `resultAdapter`·`outputPort` 추가, A-006 간선 문구를 「실행 미개방」으로 고정하고 타입을 `false`로 좁힘. 반영 중 회귀 CR-089의 A-006 조회 시험이 **처음 실행되며** 거짓 경보(문서 주석의 `gh_execution`)로 실패해 주석을 걷은 코드만 보도록 고쳤다. 재검토: 두 판 모두 새 발견 없음. A는 M01 등가 변이 판단과 nit 1·2를 고치지 않은 판단에 동의했고, 회귀 시험의 주석 제거 정규식을 저장소 표준 도우미(`(^|[^:])//` — `://` 보존)로 맞추라는 참고 nit을 남겨 반영했다. B는 반영분 네 건과 거짓 경보 수정을 코드로 다시 확인했다(회귀 CR-089 4건·단위 257건 통과).
...
### 6.85 REL-007 R1a — capability 분류·검증·드리프트·스냅숏·A-006 (2026-09-14, CR-088 / WP-078)

시작 `origin/main`은 `ce5a4b0`(CR-087 병합 뒤). 브랜치 `feature/rel007-capability-registry`. **실행 허용은 `pr.list` 하나 그대로이고 릴리스는 발행하지 않았다.**

**무엇을 증명했는가.** 고정 gh 2.97.0의 인벤토리(leaf 196·그룹 32·별칭 전용 1, flag 1,034·inherited 312, positional 164, `--json` 필드 707)가 **전부 근거를 가진 분류**를 받았고(`NFR-009` 본표 차원 100%·미분류 0), 그 분류를 **독립 검증기**가 인벤토리에서 다시 만들어 저장값과 대조하며, **실제 바이너리**의 인벤토리 해시가 manifest와 일치한다. 검증 결과는 실행기가 DB에 append-only로 남기고 API와 A-006이 같은 모델로 보인다. 분류가 늘어도 실행은 넓어지지 않았다 — manifest를 `allowed`로 바꿔 적재해도 준비 단계가 거절하고, 코드 표의 정의는 하나다.

**첫 계약 표(S1)는 change_control의 CR-088 cascade에 있다** — 분모, 「분류 완료」·「실행 허용」·「실제 GHES 지원 확인」의 뜻, `unknown`·`not_implemented`·`policy_blocked`의 차이, 스냅숏 기록과 활성화의 차이, 이번 완료 기준과 REL-007 완료 기준의 차이.

**분모와 수치 — 검증기 보고서(`pnpm gh:validate-capabilities --diagnostic`, manifest `r0.2` hash `c381880e…`, 규칙 `rules-2026-09-14.2`) 실측. 독립 검토 반영 뒤 재집계한 값이다.**

| 게이트 | 차원 | 분류/전체 | 분모를 센 법 |
| --- | --- | --- | --- |
| GATE-GH-01 | command_path | 196/196 | leaf(그룹 32·별칭 전용 1 제외). support ≠ unknown |
| GATE-GH-01 | command_alias | 45/45 | 별칭 가진 command 44 + 별칭 전용 노드 1 |
| GATE-GH-01 | positional | 164/164 | USAGE의 최상위 괄호 묶음(`[flags]` 제외, `-- <gitflags>` 포함). SRS의 placeholder 230은 대안별 셈 |
| GATE-GH-01 | command_flag | 1,034/1,034 | 모든 command의 FLAGS 절(실행 가능한 그룹 포함, `DEV-677`) — SRS와 같은 분모 |
| GATE-GH-01 | inherited_flag | 312/312 | INHERITED FLAGS 출현. 고유 4종 |
| GATE-GH-01 | short_alias | 625/625 | 고유 542 + inherited 출현 83 — SRS의 625 |
| GATE-GH-01 | repeatable_flag | 37/37 | cobra strings 계열 |
| GATE-GH-01 | interaction | 196/196 | web_native 160 · terminal_only 18 · policy_blocked 15 · web_equivalent 2 · sandbox_terminal 1 |
| GATE-GH-01 | io_mode | 196/196 | stdin·파일 입출력 flag·출력 형식·컨텍스트 |
| GATE-GH-01 | json_field | 707/707 | JSON FIELDS 절의 필드(command 40 = leaf 39 + 그룹 1, `DEV-676`) |
| GATE-GH-01d | result_kind · sensitivity | 196/196 · 196/196 | 결과 종류 8종, 민감도 4종(secret 4: `auth login`·`auth refresh`·`auth token`·`secret set` · sensitive 7: `api`·`auth status`·`config get`·`gist list`·`gist view`·`variable get`·`variable list`) |
| GATE-GH-01d | bindability · resource_type · io_port | 1/196 · 1/196 · 0/0 | 정의에만 있다 — **미달** (`DEV-675`) |
| GATE-GH-01b | extension_split | 9/9 | core 187 / extension plane 9(`extension *` 8 + `copilot`) |
| 정보 | host_support · graph_edges · opaque_text | 0/196 · 0 · 29 | 사내 확인 0(`DEV-674`), 간선 0, text 결과 29 |

support 분포: supported 144 · preview 16 · policy_blocked 15 · requires_extension 9 · terminal_only 8 · requires_local_workspace 4 · unsupported_by_host 0 · admin_only 0. 실행 차원: allowed 1 · not_implemented 180 · policy_blocked 15. 위험도: R0 80 · R1 65 · R2 38 · R3 13(`api`·`codespace ssh`·`extension exec`·`copilot`·`repo delete`·`secret set`·auth 넷·alias 둘). 민감도: internal 177 · public 8 · sensitive 7 · secret 4. 부작용: read 78 · write 68 · destructive 22 · local 24 · arbitrary 4. flag 컨트롤(leaf 기준 1,295 = 고유 1,015 + inherited 280): typed 873 · generic 326 · web 등가 38 · 승인 필요 40 · 터미널 전용 12 · 정책 차단 6, 비밀 값 표지 2(`secret set --body`·`--env-file`). 보고서 상태 `incomplete`(오류 0 · 미분류 0 · `GATE-GH-01d` 미달). 같은 입력의 보고서 해시가 같다(JSON 왕복 포함).

**검증기가 실제 결함을 잡는가 — 변이 10종과 실제 실수 하나.**

| 변이 (`validate.test.ts`) | 잡힌 코드 |
| --- | --- |
| command 삭제(`pr checkout`) | `coverage_mismatch` · `alias_target_missing` |
| flag 삭제(`pr list --state`) | `flag_classification_count` · `classification_recompute_mismatch` · `definition_option_missing` |
| 별칭 삭제 / 충돌 | `coverage_mismatch` / `alias_conflict` |
| 표에 없는 새 leaf 추가 | `support_unknown` · `interaction_unknown` · `positional_unknown`(gap) + `coverage_mismatch` |
| `pr.merge`를 `allowed`로 | `execution_widened` · `executable_count_mismatch` |
| `auth token`을 `not_implemented`로 / `pr merge`를 `policy_blocked`로 / 사유 null | `execution_derivation_mismatch` / `execution_derivation_mismatch` / `execution_reason_mismatch` (독립 검토 나) |
| command 중복 | `duplicate_command_path` · `duplicate_command_id` |
| 해시 변조 / gh 버전 / manifest 판 | `hash_mismatch` / `gh_version_mismatch` / `manifest_version_mismatch` |
| 민감도·결과 종류를 unknown으로 | `classification_recompute_mismatch` + `sensitivity_unknown` · `result_kind_unknown` |
| 정의 timeout 변경 / 정의 삭제 / 컨트롤 열거 밖 | `definition_drift` / `definition_missing` / `flag_control_invalid` |

실제 실수: 규칙(`rules.ts`의 통과 인자 순서)을 고친 뒤 manifest를 다시 만들지 않은 채 검증기를 돌리자 `classification_recompute_mismatch` 5건(`codespace cp`·`codespace ssh`·`gist clone`·`repo clone`·`repo fork`)으로 상태 `failed`가 났다 — 재생성 뒤 0건. 생성기의 결과를 다시 세어 성공하는 구조가 아니라는 것을 스스로 보였다.

**드리프트 (GATE-GH-02, `integration/drift.test.ts`, 실제 고정 gh 2.97.0).** `checkDrift` → `match`, 관측 버전 2.97.0, 바이너리 SHA-256 `141507c3…`, 인벤토리 해시 기대 = 관측, diff 0/0/0(동기 판 추출 27초). 비동기 판(`checkDriftAsync`·`extractInventoryAsync`, 실행기가 쓰는 것)이 동기 판과 같은 인벤토리·같은 해시를 내고 도는 동안 50ms 타이머가 계속 튄다(시험). 변이: manifest에서 `pr list --state`를 지우면 `changed ['pr list']`, `pr checkout`을 지우면 `added ['pr checkout']`, 없는 command를 더하면 `removed ['pr frobnicate']`; gh 버전이 다르면 인벤토리를 뽑지 않고 `version_mismatch`, 바이너리가 없으면 `error`. 3/3. CLI `pnpm gh:diff-capabilities` 종료 0.

**저장·기록 (029, `gh-registry-schema.test.ts` + `gh-schema.test.ts` + `migrate.test.ts`, 26/26).** 028 → 029 → 028 → 029 왕복(두 표·두 함수가 사라지고 028의 표는 남음, 재실행 멱등); `prs_app`은 두 표 SELECT·INSERT만; 검증 기록 UPDATE·DELETE는 소유자도 `restrict_violation`; 스냅숏 내용 UPDATE·DELETE 거부, `activated_at`은 한 번만; 미분류 195의 스냅숏은 기록되되 활성화는 `check_violation`; 같은 해시 두 번 → 한 행(내용 불변); 출처별 최신 하나; 실패 기록이 정상 기록을 덮지 않음; 기록 없음은 빈 배열.

**실행기 (JOB-GH-003, `registry-check.test.ts` 10 + `executor.test.ts` 11).** 검토 뒤 더한 시험: DB가 죽은 풀에서 드리프트 → `stale=true`·`verificationId=null`·`record_failed`(`DEV-679`); 5ms tick·100ms 검사 420ms 동안 회차 ≤ 6(쌓이지 않음); 30초 재시도 대기 중 `stop()`이 2초 안에 끝남; 해시 불일치 manifest + 일시 오류 → `failed`·stale(구조 실패가 가려지지 않음); 해시만 다른 드리프트의 detail. 실제 바이너리 기동 검사 → `incomplete`·stale=false·시도 1·스냅숏 1행(`manifest_hash`=적재 해시, unclassified 0, executable 1, leaf 196, flag 1,034, `activated_at` NULL)·검증 1행(`gh-executor`/`startup`, 관측 gh 2.97.0, 인벤토리 해시 일치, 환경에 바이너리 경로); 두 번째 회차는 같은 스냅숏에 기록만 더함. 주입: drift → `stale`·detail `added 1 · removed 0 · changed 1`·기록 status `drift`+diff·오류 로그 → 다음 match에서 해제; `binary_mismatch` → `failed`·stale; 오류 2회 뒤 match → 시도 3·`incomplete`; 오류 지속 → 시도 4·`error`·stale 유지·기록 error 문구; 주기 30ms로 돌린 검사기가 `periodic` 행을 남기고 `stop`이 회차를 기다림. 러너: `registry.isStale()`이 참이면 대기 실행이 `registry_stale`로 거절된다(기존 재검증 시험에 추가).

**API·화면.** `registry.test.ts` 6/6 — 세션 없음 401, developer 403(`FORBIDDEN_ROLE`), operator·security_officer 200; 기록 없음이면 신원·차원(≥15)·게이트(`01` true·`01b` true·`01d` false)·실행(`allowed`=`definitions`=`['pr.list']`)은 있고 검증·스냅숏은 `[]`이며 응답에 봉인 키가 없다; 기록이 생기면 출처별 최신(ci passed·gh-executor drift)과 `matches_served_manifest`, `executor_matches_served_manifest=false`, 최근 3, 스냅숏 `is_served`; 상세 `pr.list`(분류·flag 수 = 인벤토리 flag 수·근거 문장·정의)와 `auth.token`(policy_blocked·secret·정의 null), 없는 id 404, 형식 위반 400; **`pr.merge`를 `allowed`로 바꿔 적재한 서버의 미리보기는 409 `GH_CAPABILITY_NOT_EXECUTABLE`**; API-GH-001에 분류 요약. `gh-registry.test.ts` 12(순수 판정) · `a11y/gh-registry.test.tsx` 8(신원·차원 수치·게이트·기록 없음·드리프트 diff·403·404·500·검색·상세·`<script>` 텍스트·axe 0) · `e2e/gh-registry.spec.ts` 2(Chromium: 렌더·검색·상세 요청 1회·403·404) · `nav.test.ts`(두 역할에 보이고 나머지에 렌더링되지 않음).

**실행 범위 유지 — 근거.** `capabilities.ts`의 정의 배열은 `[PR_LIST_CAPABILITY]` 하나(회귀가 리터럴로 건다); manifest `coverage.executableCommands` 1, `allowed` = `['pr.list']`; 검증기 `execution.allowed` = `execution.definitions`; `prepare`는 코드 표에 없는 정의를 거절(통합 시험); 실행기 `findCapability`는 코드 표만 읽는다(변경 없음). 회귀 REL-007 블록 14/14(기존 10 + CR-088 4: 실행 허용 1·JOB-GH-003 배선·요청 경로에 검사 없음·A-006 역할과 라우트·배포 설정).

**능동 자가 연구 결과.**

| 질문 | 근거 | 관측 | 달라진 것 | 미확인 |
| --- | --- | --- | --- | --- |
| help의 값 자리표시자가 `--hostname`·`--all`·`--field`인 flag는 파서 결함인가 | 고정 바이너리 `--help` 원문(`cat -A`) | 원문이 그렇다 — `-f, --force --hostname   Force setup…` 등 3건 | 규칙 `help-placeholder-artifact`(bool + note), `DEV-678` | 없음 |
| SRS 실측 기준의 228·625·41은 어떤 분모인가 | manifest 재집계 | 228 = 그룹 32 + leaf 196(+별칭 전용 1 = 229) · 625 = 고유 542 + inherited 83 · `--json` command는 40(leaf 39 + 그룹 1) | 차원 note에 분모 명시, flag 분모에 그룹 포함(`DEV-677`), 41 vs 40은 `DEV-676` | 41의 출처 |
| USAGE의 `<remote-port>:<local-port>...`·`<port>:{public|private|org}...`는 자리 몇 개인가 | usage 원문 | 구분자로 이어진 한 자리 | 파서가 공백까지 이어 붙임, 규칙 `typed-with-separator` | 없음 |
| `codespace ports`는 그룹인가 leaf인가 | help(JSON FIELDS 절 + 하위 command) | 둘 다 — 실행 가능한 그룹 | leaf만 분류하되 flag·json 분모에 포함 | 없음 |
| `gh api`·`extension exec`·`copilot`·`codespace ssh`의 부작용은 무엇인가 | help 원문 | 입력이 정한다(메서드·임의 확장·원격 명령) | `sideEffect: 'arbitrary'`를 확인된 값으로 두고 `unknown`과 구분 | 없음 |
| 실행기 기동에 검사가 얼마나 드는가 | 통합 시험 실측 | 인벤토리 추출 27~28초(229 `--help`) | 기동 시 기다린 뒤 구독; 헬스체크 20s×15 안이라 compose 변경 없음 | 사내 호스트의 실제 시간 |
| `variable list`·`gist view`·`auth token`의 결과는 얼마나 민감한가 | help 원문 | 값·비밀 gist 내용·토큰을 그대로 찍는다 | sensitive 4 · secret 2로 분류(이름만 보면 「list」다) | 없음 |

**배터리 (커밋 직전, 격리 서비스 postgres 55434 · redis 56380 · es 59201, 순차, 단계마다 종료 코드를 파일로 기록).** `typecheck` 0 · `lint` 0 · `lint:deps` 0(패키지 16, 위반 0) · `test` 단위 146파일 2,553 통과·1 skipped(기존) · `test:regression` 8파일 477 통과(DEV-664의 실제 `docker compose` 대조 포함) · `test:integration` 109파일 1,710 중 **1 실패** → `merge-number-schema.test.ts` T06a가 「내려간 마이그레이션 목록」을 단언하는 시험이라 029가 생기자 즉시 깨졌다(그 시험의 의도대로 — 주석이 CR-085·086에서 같은 일이 있었다고 적는다). 단계 수를 5로, 목록에 `029`를 더한 뒤 그 파일 23/23 재실행 · `build` 0 · `test:a11y` 19파일 411 통과 · `test:contrast` 232쌍 0 실패 · web `build` 0(`/ops/gh-registry` 포함) · `test:e2e` 191 중 190 통과, **1 실패 = `flow-003.spec.ts:176`**(뒤로가기 뒤 `searchbox` 5초 안에 없음) — 이 변경과 무관한 기존 간헐 결함 `DEV-377`이며 같은 spec 단독 3회는 14/14·14/14·14/14 통과. 제품 코드는 손대지 않았다. PR의 필수 check가 이 자리에서 떨어지면 `DEV-662`의 선례대로 **시험의 대기 조건만** 넓히고 그 사실을 적는다.

**독립 검토 두 판.** (가) 분류 의미·인가·실행 범위·출력 안전·스냅숏 신뢰 — leaf 196개 전부의 `--help`를 받아 표의 인용 201건을 대조(200 일치, 1건 오인용 `alias set` → 원문으로 정정)하고 45개 행을 손으로 대조했다. 실행이 넓어질 경로는 층별로 찾지 못했고 인가·출력 안전에 결함이 없다고 적었다. 지적을 전부 코드로 확인해 반영했다: **major 1** — 드리프트를 확인하고도 DB 기록이 실패하면 `stale`이 서지 않아 실행이 계속되는 경로(029 미적용 DB에 새 이미지를 먼저 올린 경우가 대표) → 판정과 기록을 분리해 기록 실패가 판정을 버리지 않게 하고(`recordError`) 시험 추가(`DEV-679`); 분류 이견 8건 중 7건 수용 — `api`·`codespace ssh`를 사다리대로 R3, `pr update-branch`(`--rebase`=force push)·`agent-task create`(비용 발생)를 R2, `config get`(`oauth_token` 출력 가능)·`gist list`(비밀 gist 열거)를 sensitive, `codespace logs`의 SSH 근거를 「help 밖」으로 표기; flag 규칙 공백 — `--cleanup-tag` 승인 필요, `--clone`·`--push`·`--checkout`·`repo create --source` 터미널 전용, `--recover` 파일 입력, `release download --archive` 열거(파일 출력 아님), `skill list/update --dir` 입력, `api --field/--raw-field` 파일·stdin 채널, `secret set --body/--env-file`에 **비밀 값 표지**(`secretInput`), `cache delete --all`·`codespace delete --all/--days`·`alias delete --all`을 command별 승인 대상으로(이름 규칙으로는 `run list --all`이 무해해 불가); `prepare`가 정의의 `execution`도 보게 해 실행기와 대칭. 검토 가의 5~7절(출력 안전·스냅숏 신뢰·문서 수치)은 검토자 세션 한도로 **도착하지 않았다** — 1절 요약에 「문서의 민감도 수치가 manifest와 다르다」가 있어 6.85장의 수치를 재집계로 고쳤다. (나) 검증 독립성·결정성·드리프트 경계·DB·기동·시간 경계·회귀·CI — blocker 0, major 2, minor 9, note 7. **major 2** — 주기 검사의 `spawnSync` 229회가 이벤트 루프를 20~30초 막아 실행 중 gh의 하트비트·취소·stdout 소비·헬스가 멈추고 큰 출력이 `timed_out`될 수 있다 → 인벤토리 추출·드리프트 검사의 **비동기 판**(`spawn`, 동시 4)을 만들어 실행기가 그것을 쓰게 하고, 동기 판과 같은 해시를 내며 도는 동안 tick이 사는 것을 시험(`DEV-680`). minor 전부 반영: 구조 실패가 일시 오류에 가려지던 분기 순서, 진행 중 tick 건너뛰기, `stop()`이 재시도 대기(최대 65초)를 abort로 끊고 구독 종료 뒤에 멈춤, `localeCompare` → 코드 단위 정렬(`th` 로케일에서 순서가 달라져 영구 drift가 되던 것, `DEV-681`), 해시만 다른 드리프트의 detail, **`execution` 파생 재계산**(`policy_blocked` ↔ `not_implemented` 뒤바꿈을 `execution_derivation_mismatch`로, 변이 시험 추가), 활성화 CHECK를 네 차원(command·interaction·flag·positional) 전부로, CLI 불리언 플래그가 뒤 토큰을 삼키던 것, 회귀 문자열을 공백 허용 정규식으로; note 중 헬스 서버를 기동 검사 **앞**에 열고(`registry.status: unchecked`), `manifestHashObserved`를 재계산 해시로, SRS 실측 상수 시험에 「지우지 말 것」 주석, TRUNCATE가 행 트리거 밖이라는 사실을 DDL 주석에. CR-087의 시간 경계(집합 고정)에서는 결함을 찾지 못했다고 적었다. 검토자의 모든 주장은 코드와 재현으로 확인했고, 수용하지 않은 것은 없다.

**병합 뒤 main CI.** PR #184(head `2176636`, PR CI run `34787734188` — `verify` 3m35s·`integration` 5m36s success)를 squash 병합한 `a996540`의 run `34788054624` — `verify`(3m22s)·`integration`(5m06s) 두 잡 success (2026-09-14 실측; 주석은 actions의 Node 20 폐기 경고뿐). 병합된 main을 새 워크트리에서 재검증: `pnpm install` 뒤 `typecheck` 0 · `test:regression` 8파일 477 — 첫 실행은 격리 서비스 환경 변수 없이 돌려 DB를 쓰는 두 파일(`range-vs-git`·`releases-vs-git`)이 `ECONNREFUSED 127.0.0.1:5432`로 실패(19건 skip)했고, 환경을 주어 그 두 파일만 재실행해 19/19 → 합계 477/477 · `gh:validate-capabilities --diagnostic` = manifest `r0.2` hash `c381880e…`·인벤토리 `83128580…`·규칙 `rules-2026-09-14.2`·검증기 `validator-2026-09-14.1`·보고서 `88d25a5d…`, `GATE-GH-01`·`01b` PASS·`01d` FAIL(bindability 1/196·resource_type 1/196·io_port 0/0), 상태 `incomplete`(오류 0·미분류 0) — 커밋 전과 같은 판정. 한 번의 성공이 결함 부재를 증명하지는 않는다.

**사내에서만 확인할 수 있는 것 (NOT RUN).** 대상 GHES에서 command별 실제 지원(`hostSupport` → `verified`, FR-GH-011 AC-4·AC-5, `DEV-674`); 실제 실행기 컨테이너의 기동 검사 시간과 헬스 전이; 사내 CA·프록시 아래에서 `gh --help` 순회가 문제없이 도는지(도구는 네트워크를 쓰지 않는다 — 확인만); 운영 DB에서 029 적용과 `prs_app` 권한; A-006을 사내 `operator`·`security_officer` 계정으로 열어 보는 것.
...
**병합 뒤 main CI.** PR #183을 squash 병합한 `ce5a4b0`의 run `34771011490` — `verify`·`integration` 두 잡 success (2026-09-14 실측). 앞선 두 실패는 이 커밋에서 재발하지 않았다. 한 번의 성공이 원인을 증명하는 것은 아니며, 증명은 위의 결정적 시험과 수정 전 재현이다.
...
**무엇을 고쳤는가.** `CR-084`가 연 자동 표기 경로는 **성공 경로에서만** `AC-1`(원래 제목 보존)을 지키고 있었다. 이 판은 기능을 새로 만들지 않고 실패·재시작·설정 변경 상황에서도 그것이 성립하게 했다.

**먼저 재현하고 그다음 고쳤다.** 결정자의 분석이 지목한 다섯 경로를 **현재 코드에 대한 시험**으로 세워 일곱 건 전부 실패하는 것을 본 뒤 시작했다. 가설이 코드에서 반증된 자리는 없었다 — 다섯 전부 실재했다.

| 재현 | 무엇이 일어났는가 | 관측한 값 |
| --- | --- | --- |
| A-1 | 첫 PATCH가 5xx로 실패한 뒤 사람이 제목을 고쳤는데 재시도가 **옛 제목**을 다시 보냈다 | 보낸 값 `[M-1900-1] 원래 제목` (기대 `[M-1900-1] 사람이 고친 제목`) |
| A-2 | 그 사이 **다른 M 접두**가 생겼는데 재시도가 덮었다 | 결과 `updated` (기대 `mismatch`) — `ADR-022` 결정 5가 무너지는 자리 |
| A-3 | 그 사이 **정본이 바뀌었는데** 재시도가 나갔다 | 결과 `updated` (기대 `superseded`) |
| B | 본문이 바뀐 채 저장된 것을 확인하고도 **다음 회차가 `done`으로 덮었다** | 결과 `already_done` |
| C | 앞 회차가 줄을 쥔 동안 뒤에 선 회차가 **예산을 넘겨도 반환하지 않았다** | 시험 시간 초과(5초) — 앞 회차가 끝나야 반환 |
| D | 느린 응답 하나가 **회차 예산을 통째로 넘겼다** | 시험 시간 초과(5초) |
| E | 실패해서 다시 보내는 PATCH 사이에 **간격이 없었다** | 쓰기 간격 대기 0회 (PATCH 3회) |

**고친 뒤 같은 시험이 통과한다.** 재현 로그는 세션 기록에 남겼고, 수정 후 결과는 아래 배터리에 포함된다.

**외부에서 실행한 것과 사내에서만 확인할 수 있는 것.**

| 항목 | 외부 세션 | 사내 |
| --- | --- | --- |
| 재시도가 판단 전체를 다시 지나는가 (제목·정본·정책) | 실행 — **실제 로컬 HTTP 서버**가 요청 순서를 기록 (`GET,PATCH,GET,PATCH,GET,PATCH`) | - |
| 변경 요청 사이 간격 | 실행 — **목이 기록한 수신 시각**으로 실측, 실제로 기다림 | - |
| 시간 예산과 취소가 HTTP 왕복까지 닿는가 | 실행 — 실시간 측정 | - |
| 실행자 배제 | 실행 — 같은 DB에 **실제 자식 프로세스**를 띄워 락을 겨룸 | - |
| 새 상태(`body_changed`·`unknown`)의 정본 접근 | 실행 — **실제 PostgreSQL** | - |
| 마이그레이션 027 up/down/up과 데이터 보존 | 실행 — 격리 DB `prs_mig027`에서 실제 SQL 파일로 | 다음 반입 |
| 감사 결과 코드 분기 | 실행 — 대역이 기록한 `INSERT` 인자 | - |
| **실제 GHE PR 제목 변경** | **NOT RUN** — 외부에 GHE가 없다 | 다음 반입 |
| **실제 표기 전용 App의 쓰기 권한** | **NOT RUN** — 조회 권한만으로도 200이 오므로 써 보기 전까지 확인할 수 없다 | 다음 반입 |
| 마이그레이션 027 운영 적용 | NOT RUN | 다음 반입 |
| 사전 점검 CLI를 실제 운영 데이터로 실행 | NOT RUN | 다음 반입 |

**public 저장소의 PR 제목도 건드리지 않았다.** 쓰기 흐름은 전부 `127.0.0.1`의 목 서버를 향했다.

**검증 배터리 (전부 실행했고 이 수치가 정본이다).**

```
타입 · 린트 · 의존 방향   통과 (위반 0건)
단위      2,331 통과 · 1 skip (126 파일) — 기준선 2,314에서 17건 증가
회귀      463 통과 (8 파일) — 기준선 451에서 12건 증가
통합      1,649 통과 (102 파일) — 실제 PostgreSQL · Redis · Elasticsearch, 기준선 1,637에서 12건 증가
접근성    379 통과 (17 파일) · 대비 232쌍 실패 0 · e2e 181 통과
빌드      통과 (전체 · `@prs/web`)
변이 시험 18종 전부 kill
문서 검사기  변경 전(28d3ccf) 대비 신규 issue 0건 (오류 4·경고 2가 양쪽에 같다)
```

배터리는 격리된 시험 전용 백킹 서비스에서 돌렸다(`prs-wp075safety` 프로젝트, PostgreSQL 55433 · Redis 56379 · Elasticsearch 59200). 개발용 `prs-postgres` 등과 프로젝트·볼륨·포트를 모두 나눠 다른 세션의 데이터를 건드리지 않았다. **회귀 463 중 한 건은 배터리를 돌린 뒤에 더했다** — 변이 M18이 겨눈 자리를 덮는 시험이 없어 추가했고, 회귀만 다시 돌려 463 통과를 확인했다. 소스는 배터리 실행 시점과 같다.

**실패했다가 통과한 실행을 숨기지 않는다.** 첫 배터리에서 통합이 실패했고 e2e도 1건 실패했다. 원인을 둘로 갈랐다.

| 실패 | 원인 | 확인 방법 |
| --- | --- | --- |
| 통합 36개 파일 (`ECONNREFUSED 127.0.0.1:9200`) | **내가 환경 변수 이름을 틀렸다** — ES 접속은 `ELASTICSEARCH_NODE`인데 `ELASTICSEARCH_URL`을 줬다. 시험은 기본값 9200을 보는데 서비스는 59200에 있었다 | 이름을 고쳐 다시 돌리자 102개 파일 전부 통과 |
| 통합 시험 1건 (`merge-number-schema`) | **이 판의 마이그레이션 027 때문이며 시험이 의도대로 동작한 것이다** | 내려갈 목록을 `['027','026','025']`로 갱신 |
| e2e `flow-003` 1건 | 같은 환경 문제로 본다 — 이후 두 번의 실행에서 181건 전부 통과했다. **원인을 특정하지 못했다는 사실을 그대로 적는다**: 이 시험이 백킹 서비스를 쓰지 않으므로 ES 부재가 직접 원인이라고 단정할 수 없다 | 재실행 두 번 모두 통과 |

**마이그레이션 027 실증** (격리 DB, 실제 SQL 파일).

| 단계 | 결과 |
| --- | --- |
| up (001~027 전체) | 적용 — `적용: 001 … 027` |
| 데이터 투입 | `body_changed`·`unknown`·`done` 세 행과 근거 열 |
| down | 새 상태가 `failed`로 접히고 근거 세 열이 사라짐. 025 제약이 돌아와 `body_changed` 쓰기를 거부(`violates check constraint`) |
| up 다시 | 열이 복원되고 **`merge_number`가 그대로**(M1·M2·M3). 새 상태를 다시 받음 |

**down이 잃는 것을 적는다.** 되돌리면 「본문이 바뀌었다」와 「결과를 모른다」의 구분이 사라지고 둘 다 `failed`가 된다. 그 구분은 돌아오지 않는다 — 그래서 이 회수는 기능을 끄는 수단이 아니며, 끄는 것은 전역 스위치다. **이미 GHE에 붙은 접두는 어느 쪽으로도 되돌아가지 않는다.**

**변이 시험 16종 — 전부 kill.** 핵심 수정을 하나씩 되돌려 시험이 실제로 죽는지 확인했다.

겨눈 것: 재시도의 제목 재조회 · 재시도 앞 정본 확인 · 본문 불일치 상태 · 줄 서기의 예산 경주 · HTTP 중단 신호 · 쓰기 차례 대기 · 실행자 락 · 결과 불명 분류 · 대상 질의의 `body_changed` 제외 · 조회 성공의 차단 해제 · 보낸 시각 기록 시점 · 종료 신호 전달 · 예산 만료의 시각 판정 · 한도 유예의 게이트 적용 · 감사 결과 코드 분기 · 사전 점검의 읽기 전용 트랜잭션.
...
**다만 「결제만 풀면 된다」는 아니다 (2026-09-13 추가 실측).** 이 저장소에는 **self-hosted 러너가 이미 등록돼 있고 online이다** — `DESKTOP-HMAIKJ2`, 라벨 `self-hosted,Linux,X64`, `busy=false`. 그런데 `.github/workflows/ci.yml`의 두 잡이 모두 `runs-on: ubuntu-latest`라 **그 러너를 부르지 않는다.** `ubuntu-latest`는 GitHub이 호스팅하는 러너를 지목하는 라벨이고, self-hosted는 `self-hosted` 라벨로 불러야 잡을 받는다. 즉 러너가 대기 중인데 워크플로가 과금 대상 러너를 요구했고 그것이 막힌 것이다.

**왜 `design-system`은 멀쩡했는가도 이것으로 설명된다.** 이전 기록이 「계정 결제 차단은 pr-search에만 해당한다」고 적었는데, 계정 차단이 저장소별로 다를 수는 없다. 실측하니 **`design-system`은 public이고 `pr-search`는 private이다** — public 저장소의 호스팅 러너는 무료이고 private은 과금 대상이라, 한도가 막히면 private만 멈춘다.

**선택지는 둘이며 결정자가 「지금은 바꾸지 않는다」를 골랐다.** (1) 결제·한도를 푼다. (2) `runs-on`을 `[self-hosted, Linux, X64]`로 바꾼다 — 저장소가 private이라 공개 저장소 fork PR의 임의 코드 실행 위험이 없고, 러너 계정이 `docker` 그룹에 있어 `services:` 컨테이너도 뜬다. 바꿀 때 **포트 충돌을 먼저 본다**: `integration` 잡의 서비스 컨테이너가 `5432`·`9200`·`6379`를 호스트에 바인딩하므로 개발용 `prs-postgres` 등이 떠 있으면 그 잡이 실패한다.
...
| 검토 | 무엇이 나왔는가 | 처분 |
| --- | --- | --- |
| 외부 부작용·자격 | blocker·major 없음. note 넷 | `DEV-643`~`DEV-646`으로 전부 닫았다 |
| 시간·재시도·락 | blocker·major 없음. minor 둘·note 셋 | `DEV-648`을 닫고 `DEV-647`·`DEV-649`를 근거와 함께 열어 뒀다 |

**두 번째 검토가 확인해 준 것.** 쓰기 요청 호출이 코드 전체에 **하나뿐이고** 그 앞에 정본 재확인·제목 조회·차례 대기·보낸 시각 기록이 반드시 놓인다는 것, 줄 등록이 `await` 없이 동기적이라 두 회차가 동시에 통과할 수 없다는 것, 한 행의 반복이 실패 5회와 재시작 3회의 이중 상한으로 **최대 여덟 번**이고 PATCH는 최대 다섯 건이라는 것, 락 커넥션이 죽으면 정본 질의가 던져 쓰기가 시작되지 않는다는 것을 코드 줄로 확인했다. 「초록인 채 아무것도 지키지 않는 시험」도 찾지 못했다 — 예산 시험이 가짜 시계가 아니라 실제 타이머로 「언제 반환하는가」를 재고, 간격 시험이 목의 실제 수신 시각으로 하한을 재기 때문이다.

**열어 둔 것 둘의 근거.** `DEV-647`(지속 실패 시 파티션 head-of-line)은 유예 횟수 상한이 「정상 동작을 실패로 세지 않는다」는 `DEV-627`의 판단을 뒤집는 일이라 별도 설계가 필요하고, 그동안 일일 스윕이 backstop한다. `DEV-649`(토큰 발급이 중단 신호를 안 받음)는 조회 경로와 공유하는 클래스의 계약을 넓혀야 하므로 이 판의 범위가 아니며, 영향이 비변경 요청의 지연에 그친다. **둘 다 「고칠 수 있는데 안 한 것」이 아니라 「고치려면 다른 계약을 건드려야 하는 것」이다.**

**note 넷이 모두 「문서와 코드가 어긋난 자리」이거나 「계약이 회차에만 참인 자리」였다.** 사전 점검 주석이 없는 `--probe`를 안내했고(`DEV-643`), 감사 crash 창 서술이 한쪽만 적었으며(`DEV-644`), 재개한 행이 대기열 뒤로 밀렸고(`DEV-645`), 해제 표시가 실행자 락 밖에 있었다(`DEV-646`). 넷 중 둘은 **코드가 문서보다 안전한 쪽**이었지만 그대로 둘 수 없었다 — `CR-082`가 겪었듯 문서가 존재하지 않는 경로를 안내하면 그것을 따르는 사람이 막힌다.

**`0.1.0-pilot.6` 후보 번들 — 만들었고 발행하지 않았다.** 병합된 `main`(`7e4fd63`)에서 `--release` 없이 만들었다. 운반 아카이브는 `pr-search-0.1.0-pilot.6-offline.tar.gz`(1,137,184,744바이트), 번들 디렉터리는 2.6GB이며, `release-manifest.json`의 `migration_level`이 `027`이고 upstream commit이 `7e4fd63f30f488dc5c16bedc99c80e472aa9edf3`다.

**이미지에 파일이 있다는 것으로 기능 검증을 대신하지 않았다.** 아카이브를 풀어 체크섬 여섯을 전부 대조하고(`OK`), 시크릿 혼입을 다시 확인한 뒤(0건), `images/pr-search-app.tar`에서 **적재한 이미지로** 다음을 실행했다.

| 검증 | 어떻게 | 결과 |
| --- | --- | --- |
| 마이그레이션 027이 번들 이미지로 적용되는가 | `prs/db:0.1.0-pilot.6`으로 격리 DB에 적용 | `적용: 001 … 027`. 근거 열 셋이 실제로 생기고 상태 제약이 `body_changed`·`unknown`을 받는다 |
| **표기 OFF에서 아무것도 쓰지 않는가** | `prs/pipeline-worker:0.1.0-pilot.6`을 `annotate` 역할로 기동 | 정상 기동(`listening on 3003`)하고 「표기가 꺼져 있다」를 남긴다. **목 GHE가 받은 요청 0건** |
| **표기 ON에서 실제로 제목을 바꾸는가** | 같은 이미지에 전용 자격과 로컬 목 주소를 주고 스윕을 기다림 | 토큰 발급 → `GET` → `PATCH` 순서로 나갔고 본문은 `{"title":"[M-1900-7] Fix device initialization race"}` **하나뿐**이다 |
| 정본에 근거가 남는가 | 격리 DB 조회 | `done` / `patched` / 지문 `d8e20832c78f1721` / 시도 식별자 존재 |
| **다른 M 접두를 덮지 않는가** | 이미 `[M-1900-7]`이 붙은 제목에 `M-1900-8`을 시도 | `mismatch` / `foreign_prefix`로 남고 **`PATCH`가 나가지 않았다** (`ADR-022` 결정 5의 런타임 실증) |
| 감사가 남는가 | 파티션 생성 뒤 세 번째 대상으로 재시도 | `system:annotate \| pull_request.annotate \| pilot6org/smp1900#4244 \| annotated` |

**감사가 처음에는 실패했고 그 사실을 적는다.** 첫 표기에서 `no partition of relation "audit_record" found for row`가 났다 — 내 검증 환경이 파티션을 만들지 않았기 때문이며(`prsctl`이 파티션 수명 잡을 따로 세운다, `DEV-556`), 코드 결함이 아니다. **그 상황에서도 표기 결과가 뒤집히지 않고 `updated`로 기록됐다** — `FR-AUTH-004` AC-6의 「감사 실패가 주 동작의 결과를 바꾸지 않는다」가 우연히 실제 런타임에서 실증된 셈이다. 파티션을 만든 뒤 감사가 정상으로 남았다.

**발행하지 않았다.** CI가 계정 결제 문제로 코드를 실행하지 못한 상태이므로 `--release`를 쓰지 않았다 — 후보 번들까지만 완성하고 immutable 릴리스는 만들지 않는다. **구현 완료 · 후보 검증 완료 · 발행 보류**를 구분해 적는다. 후보는 `/tmp/pr-search-bundle-pilot6/`에 있으며 **임시 경로라 재부팅에 사라진다** — 발행하려면 그때의 `main`에서 다시 만든다(리뷰 수정으로 코드가 움직이면 이전 후보를 재사용하지 않는다는 규율과 같다).

**앞선 판이 심어 둔 시험이 이 판의 마이그레이션을 잡았다.** `merge-number-schema.test.ts`의 왕복 시험은 025의 회수를 재는데, `CR-084`가 그것을 「단계 수가 아니라 **내려간 목록**으로 단언」하도록 고치며 주석에 「다음에 027이 생기면 여기서 즉시 깨진다」고 적어 두었다. 이 판이 027을 더하자 정확히 그 자리에서 깨졌다 — **결함이 아니라 시험이 설계대로 동작한 것이며**, 내려갈 목록을 `['027','026','025']`로 갱신했다. 단계 수로 단언했다면 027만 내려간 상태에서 「025를 잰다고 믿는」 시험이 초록으로 남았을 것이다.

**변이 도구 자체가 검증 대상이었다.** 첫 실행에서 넷이 「살아남았다」고 보고됐는데, 그중 하나는 `-t` 패턴이 시험 이름과 어긋나 **대상 시험이 한 건도 돌지 않은 것**이었다(10건 전부 skip). 나머지 셋은 변이가 겨눈 자리를 덮는 시험이 아예 없었다. 도구의 미실행 판정을 고치고(skip도 미실행으로 셈) **시험 다섯을 새로 더한 뒤** 16종 전부 kill이 됐다. `WP-074`가 같은 자리에서 배운 것을 이 판이 다시 밟았다 — 변이가 초록이면 먼저 「그 시험이 돌기는 했는가」를 묻는다.

### 6.81 WP-075 PR 제목 M 넘버 표기 (2026-09-12, CR-084)

**무엇을 만들었는가.** 채번이 끝난 PR의 제목에 `[M-1900-1] ` 접두를 붙인다. 원래 제목은 한 글자도 바뀌지 않는다. 이 제품이 **사람의 지시 없이 GHE를 고치는 최초의 경로**이며(`ADR-022`), 전역 스위치 `MNUMBER_ANNOTATE_ENABLED`의 기본값이 **꺼짐**이라 이 변경을 받는 것만으로는 아무 제목도 바뀌지 않는다.

**외부에서 실행한 것과 사내에서만 확인할 수 있는 것을 가른다.**

| 항목 | 외부 세션 | 사내 |
| --- | --- | --- |
| 제목 판정(접두 없음·같은 접두·다른 M 접두·일반 접두·코드 불가) | 실행 — 순수 함수 단위 시험 | - |
| `PATCH` 메서드·경로·헤더·본문 | 실행 — **실제 로컬 HTTP 서버**로 요청을 관측 | - |
| 실패 분류(403 권한 · 403 한도 · 404 · 429 · 5xx · 401 · 422 · timeout) | 실행 — 목이 각 응답을 지시대로 냄 | - |
| 정본 접근(에폭 조인·상태 전이·해제 표시·차단 쿨다운) | 실행 — **실제 PostgreSQL** | - |
| 감사 기록 `pull_request.annotate` | 실행 — 실제 `audit_record` 행 | - |
| 자격 분리(패키지 의존·배포 환경 키 집합) | 실행 — 회귀 | - |
| **실제 GHE PR 제목 변경** | **NOT RUN** — 외부에 GHE가 없다 | 다음 반입 |
| **실제 표기 전용 App 등록과 권한** | **NOT RUN** | 다음 반입 |
| 마이그레이션 026 운영 적용 | NOT RUN | 다음 반입 |
...
```text
타입 · 린트 · 의존 방향   통과 (패키지 14개, 위반 0건)
단위      2,314 통과 · 1 skip (125 파일) — 기준선 2,233에서 81건 증가
회귀      451 통과 (8 파일) — 기준선 427에서 24건 증가
통합      1,637 통과 (101 파일) — 실제 PostgreSQL · Redis · Elasticsearch, 기준선 1,609에서 28건 증가
접근성    379 통과 (17 파일) · 대비 232쌍 실패 0 · e2e 181 통과
빌드      통과 (`@prs/web`)
변이 시험 27종 전부 kill
문서 검사기  main(8237ff5) 대비 신규 issue 0건 (오류 4·경고 2가 양쪽에 같다)
CI        verify · integration 둘 다 통과
```

**변이 시험 — 안전 규칙마다 하나씩 깨고 시험이 죽는지 봤다.** 「시험 파일이 존재한다」가 아니라 「그 결함을 실제로 잡는다」를 확인한 것이다.

| 변이 | 무엇을 깼는가 | 잡은 계층 |
| --- | --- | --- |
| M01 | 표기 설정이 조회 App의 `GHE_APP_ID`를 읽는다 | 회귀 |
| M02 | 최신 제목을 읽지 않고 저장된 값으로 판정한다 | 단위 |
| M03 | 다른 M 접두를 덮어쓴다 | 단위 |
| M04 | 같은 접두인데 다시 쓴다 | 단위 |
| M05 | 이전 에폭 이벤트를 통과시킨다 | 단위 |
| M06 | 질의가 에폭을 대조하지 않는다 | 통합 |
| M07 | 해제된 저장소도 대상으로 고른다 | 통합 |
| M08 | 요청 본문에 제목 외의 필드를 싣는다 | 단위 |
| M09 | 권한 오류를 재시도 가능으로 분류한다 | 단위 |
| M10 | 감사 실패가 표기 성공을 뒤집는다 | 단위 |
| M11 | 전역 스위치 기본값을 켜짐으로 바꾼다 | 회귀 |
| M12 | 코드를 못 정할 때 값을 지어낸다 | 단위 |
| M13 | 쓰기 직전 정본 재확인을 없앤다 | 통합 |
| M14 | 변경 요청 간격을 없앤다 | 단위 |
| M15 | 회차 직렬화를 없앤다 | 단위 |
| M16 | 토큰 발급 오류를 정규화하지 않는다 | 단위 |
| M17 | 성공 판정을 완전 일치로 되돌린다 | 단위 |
| M18 | 재채번 중인 공간을 목록에서 빼지 않는다 | 통합 |
| M19 | 스윕의 공평한 순서를 없앤다 | 통합 |
| M20 | 차단 해제를 쓰기 성공에만 둔다 | 단위 |
| M21 | 백오프를 상수로 바꾼다 | 단위 |
| M22 | 접두 탐지를 칸과 탭으로만 좁힌다 | 단위 |
| M23 | 이벤트 회차의 시간 예산을 없앤다 | 단위 |
| M24 | 대기 시간을 예산에서 뺀다 | 단위 |
| M25 | 감사를 정본 표시 뒤로 되돌린다 | 단위 |
| M26 | 성공 판정을 접두 존재로만 되돌린다 | 단위 |
| M27 | 토큰 발급 인증 실패를 재시도 가능으로 둔다 | 단위 |

**리뷰 세 판 — 이 판의 중심.** 자동 리뷰 한 판과 독립 검토 두 판(보안·아키텍처 / 정확성·적대적)을 돌렸다. **blocker는 없었고 실결함 여덟이 나왔다.** 그중 넷은 외부 쓰기의 안전에 직접 걸렸다.

| 판 | 무엇이 나왔는가 |
| --- | --- |
| 자동 | P1 둘(`DEV-621` 에폭 울타리가 목록 시점에만 · 제목 경합 `DEV-618`), P2 셋(`DEV-619` 토큰 오류 · `DEV-620` 회차 겹침 · `DEV-624` API 계약) |
| 보안·아키텍처 | blocker·major 없음. `DEV-626`(차단 해제 범위). 자격 분리가 코드·의존 그래프·두 배포 프로파일에서 모두 성립함을 확인했다 |
| 정확성·적대적 | blocker 없음. major `DEV-625`(스윕 굶주림). **시험 자체의 결함 셋** — 아무것도 증명하지 못하는 시험 둘과 살아남는 변이 하나 |
| 재검토(수정이 만든 결함을 겨냥) | blocker·major 없음. 공평성 정렬이 굶주림을 실제로 해소함을 **실측으로 확인**했다. minor 넷(`DEV-630` 성공 판정이 지나치게 느슨 · `DEV-631` 토큰 인증 분류 · `DEV-629` 스케일아웃 제약 · 백오프 상한 제거의 잠복 함정) |

**시험이 검증 대상이라는 것을 다시 겪었다.** 정확성 검토가 지목한 셋은 코드가 아니라 시험이었다 — 지표를 라벨의 **존재**로만 보아 오계상을 못 보는 시험, 부정 단언이 문자열 한 형태에만 걸려 열 이름이 바뀌면 조용히 통과하는 시험, 그리고 **백오프를 상수로 바꿔도 전부 통과하는** 상태. 셋을 메웠고 각각에 변이를 걸었다(M19~M22).

**구현이 드러낸 것 셋.**

1. **`403`의 두 원인을 공식 문서가 가르지 않는다** (`DEV-616`). 상태 코드만 보고 전부 권한으로 단정하면 한도 소진 한 번이 그 저장소의 표기를 영구히 멈춘다. 문서가 보장하는 대기 신호만으로 판정했다.
2. **감사 어휘의 정본과 코드가 어긋나 있었다** (`DEV-617`). SRS는 `pull_request.annotate`를 미활성으로 적는데 코드의 미활성 목록은 비어 있었다. 세 자리를 함께 대조하는 회귀를 세웠다.
3. **`025`의 마이그레이션 왕복 시험이 「마지막 마이그레이션」을 가정하고 있었다.** `026`이 생기자 `migrateDown(pool, 1)`이 `026`을 내리고는 `025`를 잰다고 믿었다. 단계 수가 아니라 **내려간 목록**으로 단언하도록 고쳤다 — 다음에 `027`이 생기면 그 자리에서 즉시 깨진다.

**시험이 두 번 자기 결함을 드러냈다.** 자격 분리 회귀의 첫 형태는 시크릿을 **정의하는** 파일까지 「참조하는 다른 manifest」로 세어 스스로 실패했고, 통합 픽스처의 첫 형태는 `acme/smp1900`을 써서 **단독 실행에서는 통과하고 전량 실행에서만** `(owner, name)` 유일 제약에 걸렸다. 둘 다 전량 실행이 아니었으면 병합 뒤에 드러났을 것이다.

**수정이 수정을 만들었다 (`DEV-627` → `DEV-628`).** 변경 요청 간격 1초를 넣자 한 회차가 200초까지 늘어났는데, 이벤트 버스의 회수 시한이 정확히 `JOB-SEQ-005`가 적은 30초였다. 그 문서의 숫자가 무엇을 재는 값인지 확인하지 않고 간격만 더한 것이 원인이다. 이벤트 경로에 예산을 두고 남은 것은 `defer`로 이어받게 했다. **그 수정도 한 번 더 틀렸다** — 예산을 락을 잡은 뒤부터 재어, 줄에서 기다리는 시간이 예산 밖에 있었다(`DEV-628`). 독립 검토가 그 자리를 지목했고 줄을 서기 전부터 재도록 고쳤다. 같은 라운드에서 같은 종류의 실수를 두 번 한 셈이며, 원인도 같다: **시간을 재기로 했으면 무엇부터 재는지까지 정해야 한다.**

**막을 수 없는 것 하나 (`DEV-618`).** 제목을 읽은 뒤 쓰기까지의 사이에 사용자가 제목을 고치면 그 편집이 덮인다. 조건부 쓰기로 막으려 했으나 공식 문서가 **「비안전 메서드의 조건부 요청은 특정 엔드포인트에서 따로 명시하지 않는 한 지원하지 않는다」**고 적고 PR 갱신 엔드포인트에는 그 명시가 없다. **없는 완화를 지어내지 않는다.** 창을 좁히고 피해 반경을 적었다 — 쓰기는 제목 한 필드이고 한 PR에 평생 한 번뿐이므로(그 뒤 `done`) 노출은 PR당 한 번의 수백 밀리초이며, 덮인 제목을 사용자가 다시 고치면 다음 회차는 접두를 보고 아무것도 하지 않는다.

**마지막 수정이 가장 미묘했다 (`DEV-630`).** `DEV-623`을 고치며 성공 판정을 「접두가 있는가」로 바꿨는데, 재검토가 **그것이 지나치게 느슨하다**고 지적했다 — 공식 문서가 제목 길이 상한을 밝히지 않으므로 서버가 본문을 조용히 자르는 경로가 있을 수 있고, 접두만 확인하면 **원래 제목이 잘린 것을 성공으로 기록한다.** 「원래 제목의 나머지 부분은 바꾸지 않는다」(`AC-1`)가 바로 그 자리에서 무너진다. 뒤 공백만 걷어 내고 나머지는 글자 그대로 대조하도록 좁혔다 — 완전 일치와 접두 확인 사이가 옳은 자리였다.

**남은 것.** 사내가 표기 전용 App을 등록하고 `MNUMBER_ANNOTATE_ENABLED=true`로 켠 뒤 한 저장소에서 먼저 확인한다 (런북 7.B). 실제 GHE가 제목 갱신을 받아들이는지, 설치 권한이 `Pull requests: write` 하나로 충분한지는 그때 확인된다.
...
| 제한 | 근거 문서 | 현재 상태 | 후속 |
| --- | --- | --- | --- |
| **사내 `0.1.0-pilot.3`의 `prs-releases` 색인이 비어 있다** | DEV-572 / `Dockerfile` | **upstream 해소, 배포본 잔존** — `pipeline-worker` 이미지에 `git`이 없던 원인은 public origin에서 수정됐다. 이미 반입된 pilot.3 이미지는 바뀌지 않는다 | 수정된 이미지로 새 번들을 만들고 사내 업그레이드한 뒤 미러 동기화와 릴리스 색인을 재확인한다 |
...
| ~~`prs_retention` 롤을 마이그레이션이 만들지 않는다~~ | `DEV-553` → `DEV-556` | **해소 (2026-09-08)** — 마이그레이션이 아니라 `install`·`upgrade`·`restore`가 만든다. 접속 주체와 같은 자리이며 그것이 이 저장소의 선례다(`DEV-510`). 복구 뒤 수동 재생성이 필요 없다 | 없음 |
