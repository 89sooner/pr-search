...
> 상태: review | 버전: v6.1 | 갱신일: 2026-08-30
...
| WP-040 | A-002·A-003 운영 콘솔 | REL-005 | todo | - | - | - | **CR-055 계약 선행 (2026-08-30).** 착수 전 감사가 화면이 누르라고 정한 동작 넷의 도달 경로가 없다는 것을 찾아 계약을 먼저 닫았다 — 등록 검토 요청의 처리 결과(DEV-428), 조정 스캔의 수동 실행(DEV-429), `FR-ADMIN-002` AC-1 다섯 중 둘의 러너 부재(DEV-430), 브랜치 추가 시 채번(DEV-431), 인덱스 상태 읽기 경로(DEV-432). SRS `v2.15`, `API-ADM-009` 신설, 마이그레이션 `020` 예정 |
...
| DEV-414 | 2026-08-29 | **병합된 PR 열에 미해결 리뷰 스레드 24건이 남아 있다.** `PR #32`(1)·`#49`(1)·`#65`(5)·`#66`(3)·`#67`(3)·`#68`(1)·`#69`(2)·`#70`(2)·`#72`(1)·`#75`(5)이다. **처음 센 수는 22였고 그것도 잘려 있었다**(`DEV-425` — `last:25`가 `#32`·`#49`를 뺐다). design-system 저장소에도 9건이 있다. **원장과 인계 기록이 "미해결 리뷰 0건"이라 적어 온 근거는 후속 PR에서 고쳤다는 사실이지, GitHub 스레드 상태를 센 것이 아니었다.** 지적 자체는 대부분 이미 정정된 것으로 보이나 **실측 없이 그렇게 단정하지 않는다** | 원장 6장 / CR-052, CR-053 | 문서 오류 | 없음 (별도 판단) | open — 스레드마다 실측해 이미 정정된 것은 답변·resolve하고, 아직 고쳐지지 않은 지적이 있으면 `DEV`로 등록한다. **`WP-039` 범위 밖이라 이번에 닫지 않는다.** 근본 원인은 "고쳤다"와 "스레드가 닫혔다"를 같은 것으로 세어 온 규율의 공백이며, 앞으로는 **머지 후 GraphQL로 `isResolved`를 직접 센다** |
