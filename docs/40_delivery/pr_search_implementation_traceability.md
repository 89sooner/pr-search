...
> 상태: review | 버전: v6.51 | 갱신일: 2026-09-08
...
`WP-073`이 파생 토큰 문서 §10에 `workbench.css`를 제품 배치의 정본으로 들였는데, 같은
