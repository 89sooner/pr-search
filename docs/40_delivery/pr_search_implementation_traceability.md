...
| WP-059 | capability 드리프트와 정책 관리 (A-005, A-006) | REL-011 (CR-088 부분) | **in_progress** | 에이전트 | #184 · a996540 | 6.85장 — A-006 a11y 8·e2e 2, registry 통합 6, 실행기 registry-check 통합 5 | 들어온 것: A-006 읽기 전용(`/ops/gh-registry`, `operator`·`security_officer`), 드리프트 → `registry_stale` 거절. 남은 것: A-005 정책 편집·정책 변경 감사·`admin_action_required` 흐름·스냅숏 활성화 |
...
| FR-GH-001 | WP-045, WP-060, WP-077, WP-078 | `packages/gh-cli/src/{inventory,manifest,capabilities,validate,drift}.ts`, `packages/gh-cli/src/classification/{commands,rules,classify,dimensions}.ts`, `apps/search-api/src/gh/{routes,registry}.ts`(API-GH-001·013·014), `apps/web/components/GhRegistryView.tsx`(A-006) | gh-cli 단위(help-parse·manifest·classify·commands·validate 변이 10), 드리프트 통합(동기·비동기), registry 통합, a11y QA-GH-01·32, e2e | partial — AC-1~AC-9 구현(분류 100%·근거·차원별 커버리지·미분류 0, `DEV-657` 종결), AC-10 core/extension 분리 보고. AC-11은 결과 종류·민감도만(bindability·port·자원 타입은 정의 1건, `DEV-675`), AC-12 미구현 |
...
| NFR-009 | WP-045, WP-060, WP-078 | `packages/gh-cli/manifest/gh-2.97.0.json`(r0.2), `packages/gh-cli/src/{validate.ts,classification/dimensions.ts}`, `scripts/gh-capabilities.mjs` | `validate.test.ts`(커밋된 manifest — 본표 차원 100%·변이 9), `integration/drift.test.ts`(실제 바이너리 = GATE-GH-02), registry 통합 | partial — `GATE-GH-01`(command·alias·positional·flag·inherited·short·반복·interaction·입출력·`--json`) 100%, `GATE-GH-01b` 분리 보고, `GATE-GH-02` 드리프트 검출 통과. `GATE-GH-01d`(bindability·port·자원 타입) 미달(`DEV-675`). 실측 기준 열의 `--json` command 41 vs 40은 `DEV-676` |
