...
#### 머지 후 리뷰 열넷 — 다섯 라운드 (PR #65~#70)

**정정이 새 리뷰를 불렀고 그것이 다섯 번 반복됐다.** 열넷 중 열셋이 실결함이었다.

| 라운드 | 무엇 | 판정 |
| --- | --- | --- |
| #67 (셋) | **감사 적재 실패가 조회를 막는다**(P1) · 감사 질의가 절반만 담긴다 · `WP-040`의 DoD가 자기 범위와 모순된다 | 전부 실결함 |
| #68 (하나) | `paged: true`만으로는 2쪽과 5쪽이 구분되지 않는다 | 실결함 — 커서 원문을 남겼다 |
| #69 (둘) | **커서 지문이 접근 범위를 평문으로 담는다** · 감사 회귀가 첫 페이지만 잰다 | 실결함 — **내가 "지문은 해시"라고 적은 것이 사실이 아니었고 그것이 커서를 감사에 남긴 근거였다** |
| #70 (둘) | 무열쇠 해시는 되계산으로 뚫린다 · 구분자로 이으면 서로 다른 질의가 충돌한다 | 실결함 — HMAC과 `JSON.stringify`로 고쳤다 |
