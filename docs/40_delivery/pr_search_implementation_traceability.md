...
| WP-045 | gh capability 레지스트리와 parity 검증기 | REL-007 (CR-086 · CR-088 부분) | **in_progress** | 에이전트 | #181 · c5c8aea / CR-088 PR | 6.83장·6.85장 — 인벤토리·manifest r0.2·분류 표·규칙·검증기(단위 110, 변이 10)·드리프트(통합 4, 비동기 판 포함)·029 | 들어온 것: 인벤토리·manifest(`r0.2`)·`API-GH-001`, leaf 196·flag 1,034·positional 164·`--json` 707 분류(`NFR-009` 본표 100%), `gh:validate-capabilities`·`gh:diff-capabilities`·`gh:inventory`, `gh_capability_snapshot`, CI 게이트(단위 시험 + integration 드리프트 시험). 남은 것: `GATE-GH-01d`(`DEV-675`)·호스트 지원 판정(`DEV-674`)·parity 최종(WP-060) |
...
| WP-059 | capability 드리프트와 정책 관리 (A-005, A-006) | REL-011 (CR-088 부분) | **in_progress** | 에이전트 | CR-088 PR | 6.85장 — A-006 a11y 8·e2e 2, registry 통합 6, 실행기 registry-check 통합 5 | 들어온 것: A-006 읽기 전용(`/ops/gh-registry`, `operator`·`security_officer`), 드리프트 → `registry_stale` 거절. 남은 것: A-005 정책 편집·정책 변경 감사·`admin_action_required` 흐름·스냅숏 활성화 |
