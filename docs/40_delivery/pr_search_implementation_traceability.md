...
> 상태: review | 버전: v5.7 | 갱신일: 2026-08-29
...
| WP-038 | W-006 통계 대시보드 | REL-005 | done | 에이전트 | (PR 대기) | DoD 통과 (6.49장) | **DEV-380 선행 해소.** Conductor dataviz 토큰을 CR-036으로 신설·0.2.0 배포한 뒤 착수했다. 패널 6개 독립 조회·실패 격리, epoch_stale·approximate·truncated를 사실대로, 그룹·분포 드릴다운은 서버 질의 그대로, 시계열 버킷은 클라이언트가 기간만(DEV-396). Conductor에 Tabs가 없어 WAI-ARIA 탭을 조합했다(DEV-397). `/stats`→`/analytics` 경로 정정 |
...
| DEV-397 | 2026-08-29 | **Conductor React에 Tabs 프리미티브가 없다.** W-001 집계 탭(`W-001-AGG`, `search.aggregate`)이 `[결과 | 집계]` 탭을 요구하는데 `@conductor-by-89soone/react@0.2.0` export에 Tabs가 없다 | FR-STAT-006 / WP-038, C 없음 / ADR-006 | 범위 공백 | resolved — ADR-006("없는 프리미티브는 semantic 토큰만으로 구현하고 DEV로 기여를 제안한다")대로 Conductor `Button`을 조합해 WAI-ARIA 탭(`role=tablist/tab/tabpanel`, roving tabindex, 화살표·Home·End)을 만들었다(`components/Tabs.tsx`). 외부 탭 라이브러리를 더하지 않았다. **Conductor에 `Tabs` 기여 후보로 남긴다** |
...
| `pnpm run test:a11y` | 271 통과 — `a11y/analytics.test.tsx` 9건(axe 0, 표 대체·unknown·순위 없음·탭 키보드) |
...
| `pnpm test:e2e flow-005` | **7 통과** — 패널 격리·approximate·epoch_stale·그룹 드릴다운·team 빈 그룹·PR 모집단 고지·W-001 집계 탭 |
