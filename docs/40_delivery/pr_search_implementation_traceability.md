...
| FR-AUTH-001 | WP-012, WP-015 | `packages/authz/src/{oidc,pkce,id-token,jwks,session,session-store,roles,config}.ts`, `apps/search-api/src/auth/*`, `apps/web/app/auth/{login,callback,logout}/route.ts`, `apps/web/app/api/[...path]/route.ts`, `apps/web/lib/{oidc-state,proxy}.ts`, **CR-091**: `apps/search-api/src/auth/{registration,role-command}.ts`·`src/role-cli.ts`(실효 역할·관리자 지정), `apps/web/lib/server/{effective-roles,config,page-guard}.ts`·`apps/web/instrumentation.ts`, `deploy/single-host/prsctl`(`role`·`auth_token_conflict`) | `packages/authz/src/{id-token,oidc,session,session-store,roles}.test.ts`, `apps/search-api/integration/authz/enforcement.test.ts`, `apps/web/lib/{oidc-state,proxy}.test.ts`, `apps/web/e2e/shell.spec.ts`, **CR-091**: `packages/authz/src/config.test.ts`·`apps/search-api/src/auth/registration.test.ts`·`integration/authz/{assigned-roles,role-command}.test.ts`·`apps/web/lib/server/effective-roles.test.ts`·`apps/web/app/auth/callback/route.test.ts`·`regression/cr091-auth-feedback.test.ts` | done (AC-1~AC-5. WP-015가 브라우저 왕복을 채웠다 — 인가 리다이렉트·PKCE·`state`/`nonce` 검증·세션 발급·원래 경로 복귀·로그아웃. 왕복 상태는 짧은 수명 HttpOnly 쿠키로 나른다(DEV-071). **실제 IdP 왕복은 자격 증명이 없어 NOT RUN** — e2e는 OIDC 미구성 배포의 503과 라우트 계약까지 건다) **CR-091: AC-2의 평문 HTTP 파일럿 면제(`DEV-694`)와 AC-10 실효 역할·관리자 지정(`DEV-695`) — 검증 6.89장, 사내 확인 NOT RUN** |
