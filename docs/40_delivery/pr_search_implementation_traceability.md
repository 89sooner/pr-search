...
| DEV-704 | 2026-09-17 | 사내 collaborator permission 응답 `read`·`write`/`writer`가 `READABLE_PERMISSIONS`에 없어 read 이상 사용자의 저장소가 접근 범위에서 제외됐다 | FR-AUTH-002 / WP-086 | GHE 버전 호환 결함 | CR-098 | resolved — 세 이름을 read 이상 호환 집합에 추가하고 `none`·빈 값·미지 값 기본 거부를 검사한다 |
...
| DEV-715 | 2026-09-17 | **채번 회차가 트랜잭션 밖에서 읽은 근거로 판정한 `unresolved`를 그대로 저장해, 조회 사이에 들어온 확정(`direct_confirmed`)을 덮었다.** `mnumber.ts`는 근거를 풀에서 읽고 GHE를 조회한 뒤 트랜잭션에서 저장하는데, 확정 보존 검사(`resolveEvidence`)는 그 오래된 읽기만 봤고 `upsertEvidence`의 SQL은 상태를 무조건 덮었다. 사내 반입에서 운영자가 SQL로 넣은 근거가 seq≥2에서 되돌아갔다(pilot.11 Upstream Feedback 관찰 4) | FR-SEQ-008 AC-3·AC-10 / WP-088 | 경합·근거 보존 | CR-100 | resolved — 트랜잭션 안에서 `FOR UPDATE`로 다시 읽어 `evidence_version`이 다르면 회차를 버리고(`retry evidence_moved`) 다음 회차가 새 근거로 판정한다. `upsertEvidence`는 `pr_confirmed`·`direct_confirmed` 위에 `unresolved`를 쓰지 않으며 걸리면 `EvidenceDowngradeError`다. 변이 시험으로 두 방어선이 각각 시험을 죽이는 것을 확인했다 |
| DEV-716 | 2026-09-17 | **문서 검사기가 SRS와 작업 패키지를 draft로 읽었다.** 검사기는 H1 아래 10줄에서 상태 줄을 찾는데 CR-094~099의 승인 메모와 FR-SRC 절이 상태 줄 위에 쌓여 SRS는 53행, 작업 패키지는 38행으로 밀렸다. `--strict`가 8c567b7 기준 ERROR 4·WARN 2에서 HEAD 기준 ERROR 6·WARN 4가 됐고, 이전 세션들은 이를 「기존 오류」로 적었다 | 문서 거버넌스 / WP-088 | 문서 구조 | CR-100 | resolved — 두 문서의 상태 줄을 제목 바로 아래로 되돌렸다. 검사기 결과는 6.94장. 남은 ERROR 4(`FR-CSS-005`·`D-002`·자리표시어 13+8)와 WARN 2(`risks.md` 경로)는 이전부터 있던 것이며 이 CR의 범위가 아니다 |
| DEV-717 | 2026-09-17 | **확인서로 지나간 항목에 뒤늦게 PR이 나타나도 자동으로 발견되지 않는다.** 회차는 checkpoint 너머만 보고 확정된 근거는 다시 묻지 않으므로, 유예 안에 PR 정보가 오지 않은 항목이 확인서로 `direct_confirmed`가 된 뒤 PR이 확인되면 그 PR은 이 에폭에서 번호를 받지 않는다(AC-3, 번호는 옮기지 않는다). 설계 3절의 「확정 direct에 후발 PR 출현 → conflict」는 이 경로에서 감지되지 않는다 | FR-SEQ-008 AC-3·AC-15 / WP-088 | 설계 한계 | CR-100 | open — 유예(기본 24시간, `committed_at` 기준)가 확률을 줄이고, 발견되면 에폭 재채번으로 바로잡는다. 후발 PR 발견 스윕은 별도 CR 후보다 |
| DEV-718 | 2026-09-17 | **투영이 병합된 PR을 `closed`로 색인했다.** GitHub REST·웹훅의 `pull_request.state`는 `open`·`closed`뿐이고 병합은 `merged`·`merged_at`이 따로 말하는데, `documents.ts`가 원시 값을 그대로 써서 `state = merged` 문서가 하나도 없었다. 그런데 API 계약의 예시·`is:merged`/`state:merged` 질의(`term state=merged`)·화면 배지(`ResultTable`·`PrDetailView`)·M 번호 조회(`merge-number-view` `not_merged`)·409 이웃 조회·늦은 PR 스냅숏의 채번 재개 관문(`snapshot.ts`, AC-11)·스냅숏 근거(`mnumber-evidence.ts`)는 전부 `merged`를 전제했다. 단위 시험은 투영의 `state`를 단언하지 않았고 통합·e2e 픽스처는 `state:'merged'`를 손으로 심어 두 계층이 각자 통과했다. pilot.12에서 Status=Merged·My merged PRs가 0건으로 드러났다(결정자 보고) | FR-SRCH-005 AC-1 · ENT-CORE-002 · FR-SEQ-008 AC-11 / WP-089 | 투영·계약 불일치 | CR-101 | resolved — `derivePullRequestState`(`@prs/domain`) 한 곳에서 병합 신호가 있으면 `merged`로 파생하고 투영이 쓴다. 032가 기존 스냅숏을 같은 규칙으로 바로잡으며(down은 `closed`로 결정적 복귀) 업그레이드 뒤 `prs-pull-requests` 재색인이 필요하다. 회귀 계약 시험이 투영 출력과 질의 번역을 맞댄다 |
...
### 6.94 M 번호 운영자 확인서와 확정 근거 보존 (2026-09-17, CR-100 / WP-088)

기준 `dcaad12`, 브랜치 `fix/cr100-mnumber-attestation`. `0.1.0-pilot.11` 사내 반입 피드백(`agent-context/upstream-feedback.md`, M-번호 채번 차단)을 처리했다. 요청 넷 중 둘은 운영자 확인서로, 하나는 결함 수정으로 반영했고 하나는 기각했다.

1. **운영자 확인서 (FR-SEQ-008 AC-15, ENT-SEQ-008)**: 마이그레이션 031이 `mnumber_attestation`을 만들고 `mnumber_evidence.source_kind`에 `operator_attestation`을 허용한다. 채번 회차(`apps/pipeline-worker/src/mnumber.ts`)는 활성 확인서가 있으면 `negative_evidence_unavailable`·`unsupported_merge_profile` 판정을 유예(`committed_at` 기준, 기본 24시간)가 지난 항목에 한해 번호 없이 지나가는 `direct_confirmed`로 바꾸고(`mnumber-attestation.ts`, 순수), 유예가 남은 항목에서는 멈추되 러너(`sequence-work-runner.ts`)가 work를 유예 종료 시각으로 미룬다(`attestation_grace_pending`). `prsctl mnumber attest|revoke|list`(`mnumber-attest-command.ts`·`mnumber-attest-cli.ts`, `worker-sequence` 이미지로 실행)가 확인서·감사 기록(`mnumber_attestation.create`·`revoke`)·채번 회차 요청을 한 트랜잭션에 남긴다. 지표 `mnumber_attested_total{repository}`. 사내 요청 1(프로파일 밖 건너뛰기)과 2(NULL-PR 자동 확정)는 이것으로 답한다 — 무조건 스킵은 AC-9·AC-1을, 시간만으로의 확정은 AC-10을 깨므로 운영자의 결정으로 만든다.
2. **확정 근거 보존 (DEV-715)**: 회차는 트랜잭션 안에서 `FOR UPDATE`로 근거를 다시 읽어(`lockEvidence`) 조회 사이에 `evidence_version`이 바뀌었으면 회차를 버린다(`retry evidence_moved`, 러너는 즉시 재시도). `upsertEvidence`의 SQL은 `pr_confirmed`·`direct_confirmed` 위에 `unresolved`를 쓰지 않으며 걸리면 `EvidenceDowngradeError`다. 사내가 손으로 넣은 `direct_confirmed` 행은 그대로 유효하다.
3. **기각 — 요청 3(batch 안의 unresolved를 건너뛰며 진행)**: AC-2(빈틈 없는 조밀 서수)·AC-3(앞이 미확정이면 멈추고 번호는 옮기지 않는다)에 어긋난다. 확인서가 있으면 흩어진 blocker(관찰 3)도 한 회차에 지나가므로 필요도 없다.
4. **문서 검사기 회귀 (DEV-716)**: SRS·작업 패키지의 상태 줄을 제목 바로 아래로 되돌렸다.

**검증.**

- 단위(`pnpm exec vitest run apps/pipeline-worker/src/mnumber-attestation.test.ts` 외): `mnumber-attestation.test.ts` 6/6 — 덮는 사유는 둘뿐, 확정 판정 불변, `through_seq` 포함 경계, 유예 기준 시각 `committed_at`, 유예 0, 저장 근거의 모양. 기존 `mnumber-plan`·`mnumber-config` 28/28.
- 통합(격리 DB `prs_test_cr100` @ `prs-cr091-postgres`, `vitest.integration.config.ts`): `apps/pipeline-worker/integration/sequence/mnumber-attestation.test.ts` 13/13(독립 검토 뒤 2건 추가: 2-parent 머지 커밋을 확인서가 지나감, `--through-seq` 경고) + `packages/db/integration/mnumber-attestation-schema.test.ts` 1/1(확인서 행이 있는 031 down·up) — 유예 0 확인서가 R·D·C를 지나 A=1·B=2·E=3이고 늦게 알려진 C는 번호 없음(AC-3), 유예 대기와 `retryAt`, 러너의 유예 재시도(`available_at` = 유예 종료, `attestation_grace_pending`), `through_seq` 경계, 철회 뒤 미적용과 근거 보존, 에폭 종속·활성 유일(23505), 경합(첫 회차 `retry evidence_moved` 뒤 다음 회차 진행), SQL 방어선(`EvidenceDowngradeError`), `attest`(확인서·감사·`reconcile` work·러너 연동으로 A=1·B=2·E=3), 거절 넷(중복·다른 에폭·비대상 브랜치·행위자 없음), `revoke`·재철회 거절·`list`. 기존 `mnumber.test.ts` 19/19.
- 변이: `mnumber.ts`의 잠금 재검증을 무효화하면 경합 시험이 `EvidenceDowngradeError`로 죽고, `upsertEvidence`의 SQL 방어선을 지우면 방어선 시험이 「거절되지 않았다」로 죽는다. 복원 뒤 둘 다 통과.
- typecheck 0 · eslint 0 · lint:deps 0. 회귀·전체 통합·build·문서 검사기: 아래 「후속 실측」.
...
### 6.95 병합된 PR의 state 파생 보정 (2026-09-17, CR-101 / WP-089)

기준 CR-100 브랜치 위 `fix/cr101-merged-state`. 결정자가 pilot.12 사내 화면에서 Status=Merged와 My merged PRs가 빈 결과라고 보고했다. 원인 추적(독립 에이전트)과 소스 대조로 단일 원인을 확정했다 — 질의가 아니라 문서다(DEV-718).

1. **파생 규칙 한 곳**: `packages/domain/src/pull-request-state.ts`의 `derivePullRequestState`가 `merged === true` 또는 `merged_at`이 있으면 `merged`, 아니면 원시 값 그대로다. `apps/pipeline-worker/src/documents.ts`의 `buildPullRequestDocument`가 이것을 쓴다. 질의(`DERIVED_STATE`)·매핑·UI는 바꾸지 않았다 — 이미 계약대로였다.
2. **기존 스냅숏 정정**: 마이그레이션 032가 `pull_request_snapshot.document`의 `state`를 `merged_at`이 있으면 `merged`로 바꾼다(`document_version` 불변). down은 `closed`로 되돌린다 — 병합된 PR의 GitHub 원시 state는 언제나 `closed`라 결정적이다. 재색인은 이 표를 그대로 색인하므로 업그레이드 뒤 `prs-pull-requests` 재색인이 필요하다(RUNBOOK 업그레이드 절·문제 해결 행).
3. **함께 고쳐지는 것**: 「Merged」 배지·PR 상세, M 번호 조회의 `not_merged` 오판, 409 이웃 조회의 상세, 늦은 PR 스냅숏의 채번 재개 관문(AC-11), GHE 자격 없는 배포의 스냅숏 근거 — 모두 실제 문서가 `merged`를 갖게 되어 설계대로 동작한다.
4. **문서**: API 계약 v0.36의 「state는 GitHub이 준 값 그대로」 문장을 파생 규칙으로 정정, 데이터 모델 v0.27 ENT-CORE-002.

**검증.**

- 단위: `packages/domain/src/pull-request-state.test.ts` 3/3, `apps/pipeline-worker/src/documents.test.ts` 41/41(신규 2 — closed+merged → `merged`, 병합 시각만 있어도 `merged`; open·closed 원시 유지).
- 통합(격리 DB `prs_test_cr101`): `packages/db/integration/snapshot-merged-state.test.ts` 1/1 — 032 up이 병합 시각 있는 문서만 `merged`로 바꾸고 `document_version`을 건드리지 않으며 down이 `closed`로 되돌린다. 마이그레이션 파수꾼 4파일(031·032 반영) 통과.
- 회귀: `regression/cr101-merged-state.test.ts` 3/3 — 실제 투영 출력(`merged`)과 실제 질의 번역(`is:merged`·`state:merged` → `term state=merged`)이 같은 값이고 `is:closed`는 병합 PR을 찾지 않는다; 투영·032·M 번호 관문이 같은 어휘를 쓴다.
