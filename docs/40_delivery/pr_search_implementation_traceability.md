...
**수정이 수정을 만들었다 (`DEV-627`).** 변경 요청 간격 1초를 넣자 한 회차가 200초까지 늘어났는데, 이벤트 버스의 회수 시한이 정확히 `JOB-SEQ-005`가 적은 30초였다. 그 문서의 숫자가 무엇을 재는 값인지 확인하지 않고 간격만 더한 것이 원인이다. 이벤트 경로에 예산을 두고 남은 것은 `defer`로 이어받게 했다.
