...
| 대비 | 80쌍 통과 |
| e2e | **114 통과** — 오래 "간헐"로 남아 있던 뒤로가기 실패 둘을 이 세션에서 조사했다. 실측 재현율이 **6회 중 4회**였고 원인은 `force-dynamic` 위의 연속 `router.replace`가 RSC 왕복을 겹치는 것이었다(DEV-376, 고쳤다). 남은 `flow-003.spec.ts:176`은 병렬 부하에서 서버 렌더가 5초를 넘는 별개 문제이며 **6회 중 1회로 줄었다**(DEV-377, WP-016 소관) |
| `pnpm build` | 통과 |
| 문서 검증기 `--strict` | `main` 대비 **증감 0** (WARN 1 · ERROR 2) |
