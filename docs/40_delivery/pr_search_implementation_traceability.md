...
> 상태: review | 버전: v6.91 | 갱신일: 2026-09-17
...
| WP-088 | M 번호 운영자 확인서와 확정 근거 보존 | M 채번 운영 (CR-100) | done | 에이전트 | 브랜치 `fix/cr100-mnumber-attestation` (PR·병합은 병합 기록에서) | 단위 6/6·통합 11/11(격리 DB)·변이 2/2 죽음·기존 M 통합 19/19·typecheck 0·eslint 0·회귀·전체 통합·build — 6.94장 | 실 GHE 데이터 NOT RUN; 후발 PR 자동 발견 없음(DEV-717); 검사기 잔여 오류 4(기존) |
...
- 통합(격리 DB `prs_test_cr100` @ `prs-cr091-postgres`, `vitest.integration.config.ts`): `apps/pipeline-worker/integration/sequence/mnumber-attestation.test.ts` 11/11 — 유예 0 확인서가 R·D·C를 지나 A=1·B=2·E=3이고 늦게 알려진 C는 번호 없음(AC-3), 유예 대기와 `retryAt`, 러너의 유예 재시도(`available_at` = 유예 종료, `attestation_grace_pending`), `through_seq` 경계, 철회 뒤 미적용과 근거 보존, 에폭 종속·활성 유일(23505), 경합(첫 회차 `retry evidence_moved` 뒤 다음 회차 진행), SQL 방어선(`EvidenceDowngradeError`), `attest`(확인서·감사·`reconcile` work·러너 연동으로 A=1·B=2·E=3), 거절 넷(중복·다른 에폭·비대상 브랜치·행위자 없음), `revoke`·재철회 거절·`list`. 기존 `mnumber.test.ts` 19/19.
