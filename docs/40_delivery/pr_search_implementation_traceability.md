...
> 상태: review | 버전: v6.29 | 갱신일: 2026-09-02
...
| 단계 | 결과 |
| --- | --- |
| 발행 전제 검사 | 같은 버전 재발행 → 종료 코드 1, 이미지 빌드 0회, "릴리스 … 이미 있다". 알 수 없는 옵션 → 종료 코드 1. 같은 태그를 예약한 초안을 API로 만들어 두고 실행 → 종료 코드 1, 이미지 빌드 0회(첫 검사가 초안까지 잡았고 초안 전용 검사는 그 뒤의 안전망이다) |
