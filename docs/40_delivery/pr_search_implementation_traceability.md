...
| WP-045 | gh capability 레지스트리와 parity 검증기 | REL-007 (CR-086 · CR-088 부분) | **in_progress** | 에이전트 | #181 · c5c8aea / CR-088 PR | 6.83장·6.85장 — 인벤토리·manifest r0.2·분류 표·규칙·검증기(단위 108, 변이 9)·드리프트(통합 3)·029 | 들어온 것: 인벤토리·manifest(`r0.2`)·`API-GH-001`, leaf 196·flag 1,034·positional 164·`--json` 707 분류(`NFR-009` 본표 100%), `gh:validate-capabilities`·`gh:diff-capabilities`·`gh:inventory`, `gh_capability_snapshot`, CI 게이트(단위 시험 + integration 드리프트 시험). 남은 것: `GATE-GH-01d`(`DEV-675`)·호스트 지원 판정(`DEV-674`)·parity 최종(WP-060) |
...
| WP-078 | REL-007 R1a — capability 분류·검증·드리프트·스냅숏·A-006 읽기 전용 | REL-007 (**CR-088**) | **done** | 에이전트 | CR-088 PR | 6.85장 — gh-cli 단위 108(변이 9) · 드리프트 통합 3 · db 통합 26(029 왕복·권한·불변) · 실행기 통합 16(registry-check 5) · routes·registry 통합 20 · web lib 810 · a11y 8 · e2e 2 · 회귀 REL-007 14 · CLI validate/diff | leaf 196·flag 1,034·positional 164·`--json` 707 분류(`NFR-009` 본표 100%), 검증기·드리프트·029·`JOB-GH-003`·`API-GH-013`/`014`·A-006. 실행 허용 `pr.list` 하나 그대로. `GATE-GH-01d` 미달(`DEV-675`)·호스트 확인 `NOT RUN`(`DEV-674`) |
...
| FR-GH-001 | WP-045, WP-060, WP-077, WP-078 | `packages/gh-cli/src/{inventory,manifest,capabilities,validate,drift}.ts`, `packages/gh-cli/src/classification/{commands,rules,classify,dimensions}.ts`, `apps/search-api/src/gh/{routes,registry}.ts`(API-GH-001·013·014), `apps/web/components/GhRegistryView.tsx`(A-006) | gh-cli 단위(help-parse·manifest·classify·commands·validate 변이 9), 드리프트 통합, registry 통합, a11y QA-GH-01·32, e2e | partial — AC-1~AC-9 구현(분류 100%·근거·차원별 커버리지·미분류 0, `DEV-657` 종결), AC-10 core/extension 분리 보고. AC-11은 결과 종류·민감도만(bindability·port·자원 타입은 정의 1건, `DEV-675`), AC-12 미구현 |
...
| DEV-672 | 2026-09-14 | **`gh_capability_snapshot`의 계획 DDL과 다르게 만들었다.** (1) `CHECK (unclassified_count = 0)`를 `CHECK (activated_at IS NULL OR unclassified_count = 0)`으로 — 미분류가 남은 manifest도 **진단용으로 기록**할 수 있어야 A-006이 「무엇이 미완인가」를 보인다. 막는 것은 활성화뿐이다. (2) `inventory_hash`·차원별 카운트·`coverage` JSONB·`first_seen_at`을 더하고 `activated_at`을 NULL 허용으로. (3) 검증 회차의 기록은 스냅숏에 덮어쓰지 않고 신설 `gh_capability_verification`(ENT-GH-012, append-only — UPDATE·DELETE는 트리거가 거부)에 더한다. (4) 마이그레이션 번호는 계획의 009가 아니라 029다 | ENT-GH-006 · ENT-GH-012 · WP-045 | 기술 제약 | CR-088 | resolved — 데이터 모델 3.5장 정정 |
| DEV-671 | 2026-09-14 | **`JOB-GH-003`(드리프트 점검)의 자리와 실패 처분을 확정했다.** 잡은 `pipeline-worker`가 아니라 **`gh-executor`**에 있다 — gh 바이너리와 DB를 둘 다 가진 프로세스가 그것뿐이다. 기동 시 검사를 **기다린 뒤** 구독을 세우므로 켜진 실행기의 첫 실행은 검사 뒤에 온다(20~30초, command마다 `--help`). 드리프트·구조 실패의 처분은 프로세스 종료가 아니라 **실행마다 `registry_stale` 거절**이다 — 종료하면 헬스가 사라져 「왜」를 잃는다. 일시 오류(시간 초과)는 3회 재시도 뒤 기록만 남기고 `stale`을 바꾸지 않는다 — 시간 초과 하나로 실행을 멈추지 않는다. 주기는 `GH_EXECUTOR_REGISTRY_CHECK_MS`(기본 1일, 최소 1분) | JOB-GH-003 · FR-GH-011 AC-3 · WP-045 | 기술 제약 | CR-088 | resolved — 비동기 9.4·인프라 12장 정정 |
...
**분모와 수치 — 검증기 보고서(`pnpm gh:validate-capabilities --diagnostic`, manifest `r0.2` hash `13623d63…`) 실측.**
...
| GATE-GH-01d | result_kind · sensitivity | 196/196 · 196/196 | 결과 종류 8종, 민감도 4종(secret 2: `auth token`·`secret set`) |
...
support 분포: supported 144 · preview 16 · policy_blocked 15 · requires_extension 9 · terminal_only 8 · requires_local_workspace 4 · unsupported_by_host 0 · admin_only 0. 실행 차원: allowed 1 · not_implemented 180 · policy_blocked 15. 위험도: R0 80 · R1 67 · R2 38 · R3 11. 부작용: read 78 · write 68 · destructive 22 · local 24 · arbitrary 4. flag 컨트롤: typed 881 · generic 328 · web 등가 38 · 승인 필요 35 · 터미널 전용 7 · 정책 차단 6(flag 분모는 leaf 기준 1,295 = 고유 1,015 + inherited 280). 보고서 상태 `incomplete`(오류 0 · 미분류 0 · `GATE-GH-01d` 미달). 같은 입력의 보고서 해시가 같다(JSON 왕복 포함).

**검증기가 실제 결함을 잡는가 — 변이 9종과 실제 실수 하나.**
...
**드리프트 (GATE-GH-02, `integration/drift.test.ts`, 실제 고정 gh 2.97.0).** `checkDrift` → `match`, 관측 버전 2.97.0, 바이너리 SHA-256 `141507c3…`, 인벤토리 해시 기대 = 관측, diff 0/0/0(추출 27초). 변이: manifest에서 `pr list --state`를 지우면 `changed ['pr list']`, `pr checkout`을 지우면 `added ['pr checkout']`, 없는 command를 더하면 `removed ['pr frobnicate']`; gh 버전이 다르면 인벤토리를 뽑지 않고 `version_mismatch`, 바이너리가 없으면 `error`. 3/3. CLI `pnpm gh:diff-capabilities` 종료 0.
...
**실행기 (JOB-GH-003, `registry-check.test.ts` 5/5 + `executor.test.ts` 11/11).** 실제 바이너리 기동 검사 → `incomplete`·stale=false·시도 1·스냅숏 1행(`manifest_hash`=적재 해시, unclassified 0, executable 1, leaf 196, flag 1,034, `activated_at` NULL)·검증 1행(`gh-executor`/`startup`, 관측 gh 2.97.0, 인벤토리 해시 일치, 환경에 바이너리 경로); 두 번째 회차는 같은 스냅숏에 기록만 더함. 주입: drift → `stale`·detail `added 1 · removed 0 · changed 1`·기록 status `drift`+diff·오류 로그 → 다음 match에서 해제; `binary_mismatch` → `failed`·stale; 오류 2회 뒤 match → 시도 3·`incomplete`; 오류 지속 → 시도 4·`error`·stale 유지·기록 error 문구; 주기 30ms로 돌린 검사기가 `periodic` 행을 남기고 `stop`이 회차를 기다림. 러너: `registry.isStale()`이 참이면 대기 실행이 `registry_stale`로 거절된다(기존 재검증 시험에 추가).
