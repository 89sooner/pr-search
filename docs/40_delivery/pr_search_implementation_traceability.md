...
**변이.** 셋을 심고 각각을 잡는 시험을 확인했다: 중복 제출 가드 제거 → a11y QA-GH-14가 POST 2건으로 실패, `parsePrefill` 여분 키 통과 → `lib/gh.test` 실패, `canExecute` 위반 무시 → `lib/gh.test` 실패. 이전 세션의 실행기 통합 실패 1건(ESC 상수)은 도구 결함이었고(`DEV-660`), 실제 ESC를 넣자 gh의 caret 표기가 드러났다(`DEV-658`).
