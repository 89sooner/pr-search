...
> 상태: review | 버전: v1.3 | 갱신일: 2026-08-25
...
| DEV-161 | 2026-08-25 | **이웃의 단위가 SRS와 API 계약에서 다르다.** FR-REL-001의 요구사항 문장은 "시퀀스 값이 인접한 **PR**"이고 AC-2도 PR 번호를 요구하는데, API-REL-001 응답 예시에는 `kind: "commit"`·`pr_number: null`(직접 푸시)이 섞여 있다. PR만 실으면 목록의 서수가 `1339 → 1341`처럼 건너뛴 채 보이고 사용자는 그것을 **누락으로 읽는다** — "서수는 `git log --first-parent`로 검증 가능해야 한다"(ADR-007)는 불변식이 화면에서 깨진다 | WP-027 / FR-REL-001 AC-2, API-REL-001, C-019 | 문서 간 모순 | **CR-031** | **open** — 사용자 결정으로 **커밋 포함**. 하위 문서는 SRS 범위를 넓힐 수 없으므로 **`srs_final.md`를 직접 고쳤다(v2.4)**: 요구사항 문장을 "인접한 항목(PR 머지 커밋과 직접 푸시 커밋)"으로, AC-2를 유형·SHA 중심으로 정정했다. 직접 푸시 행의 제목·작성자는 `null`이고 **소속 PR 값으로 채우지 않는다**(DEV-090) — 보강은 WP-067 |
| DEV-162 | 2026-08-25 | **선행·후행을 진입 조회로 만들면 WP-017이 세운 규율이 깨진다.** WP-017은 "진입 시 요청은 PR 문서 하나뿐"을 a11y·e2e 다섯 곳에 걸었고(`expect(calls).toHaveLength(1)`), WP-024는 C-020이 진입 조회를 하다 **CI에서 걸려** 확장 조회로 되돌린 전력이 있다(6.24장). 그런데 와이어프레임은 선행·후행을 진입 레이아웃에 그리고, IA 원칙 4는 지연 조회 대상으로 관계·동시 변경만 적는다 | WP-027, WP-017, WP-024 / QA-W002-17, IA 원칙 4 | 문서 간 모순 | **CR-031** | **open** — C-020 선례대로 **열 때 1회 조회**로 통일했다. IA 원칙 4를 "자기 문서 밖을 부르는 섹션"으로 넓히고 QA-W002-17의 대상에 포함 릴리스·선행·후행을 더했다 — 문서가 실측보다 좁았던 자리다 |
| DEV-163 | 2026-08-25 | **W-003의 앵커가 없다.** WP-027 범위는 "W-003 시퀀스 위치 섹션 연결"인데 API-REL-001은 `pr_number`만 받는다 — **직접 푸시 커밋은 PR이 없어** 자기 위치를 물을 수 없다. 프론트 아키텍처의 `/commit/...` 행에는 API-REL-001이 아예 없어 문서끼리도 어긋난다 | WP-027, WP-018 / API-REL-001, W-003-SEQPOS | 범위 공백 | **CR-031** | **open** — 앵커를 `pr_number` 또는 `commit_sha` 둘 중 하나로 정하고(둘 다면 400) 프론트 아키텍처·추적 매트릭스에 W-003을 실었다. 두 화면이 **같은 API를 다른 앵커로** 부른다 |
| DEV-164 | 2026-08-25 | **미머지와 미채번을 API가 가르지 않는다.** 계약은 409 `NO_SEQUENCE` 하나뿐인데, C-014는 `unassigned`(미머지)와 `not_computed`(미채번)를 **절대 같이 그리지 말라**고 못 박는다(CR-019, DEV-077) — 전자는 사실 주장이고 후자는 "아직 모른다"다. 머지됐으나 채번 전인 PR은 어느 쪽으로도 답할 수 없다 | WP-027 / FR-REL-001 예외 처리, C-014, API-REL-001 | 빈칸 | **CR-031** | **open** — 코드는 `NO_SEQUENCE` 하나로 두고 **사유를 가른다**(`detail.reason: not_merged` / `not_sequenced`) — API-SEQ-002의 `tag_not_found`/`release_not_indexed`와 같은 관용구이며 오류 코드 표면을 넓히지 않는다. 화면은 **미머지면 요청 자체를 하지 않는다**: PR 문서의 `state`로 아는 사실을 409로 되묻지 않는다 |
| DEV-165 | 2026-08-25 | **QA-W002-16(에폭 경고)의 소유자가 없다.** WP-017이 DEV-087로 "도달 불가라 만들지 않는다 — WP-021이 시퀀스를 채울 때 감지 경로와 함께 정한다"며 이월했는데, WP-021·022가 서서 W-002가 실제 서수를 그리는 지금도 어느 WP의 DoD에도 없다. **이월의 조건이 충족됐는데 이월된 사실만 남았다** | WP-027, WP-017, WP-021 / QA-W002-16, FR-SEQ-005 AC-4, DEV-087 | 범위 공백 | **CR-031** | **open** — WP-027이 갖는다. 감지는 W-004(QA-W004-21)와 같은 규칙이다: PR 문서의 `seq_epoch`와 선행·후행 응답의 현재 에폭을 비교하고 **자동 재조회하지 않는다** |
| DEV-166 | 2026-08-25 | **이웃 항목에 `indexed`·`url`이 없다.** 범위 결과는 DEV-130대로 정본에는 있고 색인에 없는 항목을 서수·SHA만으로 그리고 "색인 대기"를 밝히는데, 같은 두 소스(PostgreSQL 정본 + Elasticsearch 표시값)를 쓰는 이웃 목록에는 그 자리가 없다 — 색인이 늦은 이웃이 조용히 빠지거나 제목 없는 행이 된다. 직접 푸시 커밋은 보강 전까지 **언제나** 그 상태다 | WP-027 / API-REL-001, C-019, DEV-130 | 빈칸 | **CR-031** | **open** — 항목에 `indexed`·`url`을 싣고 C-019에 "서버 순서를 그대로 믿는다"·"색인 대기 표기" 규칙을 명시했다 (`RangeResultTable`과 같은 규칙) |
| DEV-167 | 2026-08-25 | **"범위로 확장"이 넘길 앵커가 정의되어 있지 않다.** W-002·W-003 와이어프레임 모두 "범위 조사 확장 링크"라고만 적고 `from`·`to`가 무엇인지 없다. 구간은 두 앵커를 요구하는데 화면에는 기준 개체 하나뿐이다 | WP-027 / W-002-NEIGHBORS, W-003-SEQPOS, FLOW-003 | 빈칸 | **CR-031** | **open** — **지금 보고 있는 이웃 창**으로 정했다: 목록 첫 항목의 서수가 시작(제외), 마지막 항목의 서수가 끝(포함). 화면이 본 것과 W-004가 조회할 것이 같아지고, 서수를 앵커로 넘기므로 새 앵커 유형을 만들지 않는다 (WP-026 미배포 링크와 같은 규칙) |
...
**다음은 WP-027 선행·후행 조회와 상세 화면 통합이다.** REL-003의 남은 것은 WP-027·WP-028과 WP-067·WP-068이며, WP-027이 붙으면 SCN-003의 앞쪽(의심 PR 주변 확인)이 W-002에서 성립한다. WP-067·WP-068은 여전히 독립이다.
