...
> 상태: review | 버전: v6.10 | 갱신일: 2026-08-31
...
| WP-041 | 안전 구간 표식 | REL-006 | done | 에이전트 | PR #103 | DoD 11항 전부 통과 · 변이 16종 전부 킬 (6.56장) | **CR-057 계약 선행 (2026-08-31).** 착수 전 감사가 `API-SEQ-004`에 상세 절이 아예 없다는 것을 찾았고, 그 공백 하나가 다섯을 함께 비워 두고 있었다 (DEV-458~463). `OD-008`도 그 CR에서 닫혔다 — 저장소의 마지막 오픈 결정이다 |
...
| `pnpm run test:integration` | **1429 통과** | 1399 |
| `pnpm run test:regression` | **298 통과** | 288 |
| `pnpm run test:a11y` | **357 통과** (axe 0) | 346 |
| `pnpm run test:contrast` | 232/232 | 232/232 |
| `pnpm run test:e2e` | **154 통과** | 147 |
| `pnpm build` | 통과 | — |

targeted: `pnpm run test:integration safe-marker` **30 통과** · `pnpm run test:e2e safe-marker` **7 통과**.
