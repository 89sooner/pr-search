...
CR-008은 문서만 강화했다. GitHub Operations Plane(REL-007~011) 구현 순서는 바뀌지 않는다 — Search/Data Plane을 end-to-end로 닫은 뒤다. WP-006은 Data Plane의 GitHub REST 클라이언트이며 gh CLI를 실행하지 않는다 (ADR-013).
