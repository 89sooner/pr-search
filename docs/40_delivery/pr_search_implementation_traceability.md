...
> 상태: review | 버전: v6.4 | 갱신일: 2026-08-30
...
**검증 (로컬 전 계층).** `typecheck` · `lint` · `lint:deps` 통과. 단위 1865(+9) · 통합 1399(+4) · 회귀 288(+1). 화면을 건드리지 않아 `a11y` · `contrast` · `e2e` · `build`는 돌리지 않았고, 그것을 통과로 세지 않는다. 적대적 변이 일곱을 걸어 **일곱 모두 킬**했다 — 취소 신호 무시(M1), 중단 뒤 후속 단계 실행(M2), 스윕이 신호를 넘기지 않음(M3), 취소를 연속 미완주로 계수(M4), 되돌리기 앞 확인 제거(M5), 경합에서 식별자 없는 충돌 반환(M6), 라우트가 `job_id`를 다시 조건부로 실음(M7). **CI는 판정에 쓰지 못했다** — 계정 결제 문제로 잡이 시작조차 하지 않는다(단계 0개).
