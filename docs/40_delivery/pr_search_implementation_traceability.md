...
현재 이 저장소는 **문서 단계**다. 구현 착수 전 필요한 것:

1. `srs_final.md`를 사용자 승인으로 `baseline` 전환 (Gate 1)
2. `validate_srs_prd_env.py --strict` 통과 확인
3. OD-003, OD-006, OD-007 결정 (REL-001 착수 조건)
4. GitHub App 발급, 웹훅 엔드포인트 등록, OIDC 클라이언트 등록
5. WP-001부터 순서대로 착수. WP-001·WP-002·WP-003·WP-005·WP-006은 병렬 가능
