...
> 상태: review | 버전: v6.16 | 갱신일: 2026-09-01
...
| DEV-502 | 2026-09-01 | **조정 스캔 취소 시험이 경합에 취약하다.** `apps/pipeline-worker/integration/reconcile/manual-run.test.ts`의 '저장소 여럿 중 첫 번째에서 취소하면 나머지를 돌지 않는다'가 CI에서 `expected 4 to be less than 4`로 한 번 죽었다. 그 시험은 저장소 넷을 만들고 **첫 진입에서 취소한 뒤 진입이 넷 미만**임을 단언하는데, `runReconcileSweep`이 `listRepositories({status:'active'})`로 **활성 저장소 전체**를 먼저 가져오고 취소 확인은 저장소마다 `isJobRunning` **DB 조회**로 이루어진다 — 취소 반영과 다음 진입 사이가 좁으면 넷이 다 들어간다. **이 세션은 그 코드를 건드리지 않았다**(`git diff main...HEAD`가 그 경로에 대해 비어 있다) | WP-040 / FR-ADMIN-002, DEV-443 | 시험 결함 (경합) | 별도 판단 필요 | open — **flake로 단정하지 않는다.** 실측: 로컬 전량 2회 통과·단독 5회 통과, CI 1회 실패 뒤 재실행 통과. 재현율이 낮아 원인을 특정하지 못했다. **`Promise` 타이밍을 조정해 '고쳤다'고 적지 않는다** — 그것은 재현율을 낮출 뿐 성질을 바꾸지 않는다. 성질을 실제로 바꾸려면 취소 확인이 **저장소 목록을 가져오기 전과 각 진입 직전**에 모두 걸려야 하고, 그 판단은 `FR-ADMIN-002`를 소유한 WP의 것이다. `DEV-447`(30초 강제 종료 미증명)과 같은 자리에 있다 |
