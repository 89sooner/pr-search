...
> 상태: review | 버전: v6.76 | 갱신일: 2026-09-14
...
| WP-047 | 격리 gh 실행기와 실행 수명주기 | REL-007 (CR-086 · CR-090 부분) | **in_progress** | 에이전트 | #181 · c5c8aea / feature/rel007-registry-approval | 6.83장 — 실행기 통합 10/10(실제 gh + HTTPS 목), spawn 단위, smoke 6절 · 6.87장 — 운영 정책 흐름 통합(claim 경합·정책 확인 불가 포함) | 들어온 것: `gh-executor`(spawn·재검증·argv 대조·취소·상한·하트비트·고아 회수·잔여 스윕·헬스)·`prs:gh:executions`·028, 큐의 운영 정책 판정과 정책 공유 잠금 claim·claim 가드 030(`CR-090`). 남은 것: 잠금·승인 전이·출력 스트리밍(`DEV-651`) |
| WP-048 | W-010 GitHub Command Center 수직 슬라이스 | REL-007 (CR-086 · CR-090 부분) | **in_progress** | 에이전트 | #181 · c5c8aea / feature/rel007-registry-approval | 6.83장 — lib/gh 41·a11y 24·콜백 라우트 8·e2e 8 · 6.87장 — W-010 실행 판정 표시(a11y QA-GH-49·e2e) | 들어온 것: W-010 최소·W-021 최소(`WP-077`), W-010 실행 판정 표시(`CR-090`). 남은 것: 생성형 폼 일반화·Recipe·A-007 (A-006은 `WP-078`~`WP-080`이 들였다) |
...
| WP-059 | capability 드리프트와 정책 관리 (A-005, A-006) | REL-011 (CR-088 · CR-090 부분) | **in_progress** | 에이전트 | #184 · a996540 / feature/rel007-registry-approval | 6.85장 — A-006 a11y 8·e2e 2, registry 통합 6, 실행기 registry-check 통합 5 · 6.87장 — 운영 승인·차단 (WP-080) | 들어온 것: A-006 읽기(`/ops/gh-registry`, `operator`·`security_officer`), 드리프트 → `registry_stale` 거절(`CR-088`), 운영 승인·철회와 스냅숏 활성화·A-005 최소(`/ops/gh-policy`, `pr.list` 차단·재개)·정책 변경 감사(같은 트랜잭션)·`admin_action_required` 흐름(`CR-090`, `DEV-685` 종결). 남은 것: A-005 나머지(허용 목록 편집·위험도 재정의·승인 지정·엔드포인트·확장 허용 목록과 그 실행)·A-006의 GHES 버전·parity 최종(WP-060) |
