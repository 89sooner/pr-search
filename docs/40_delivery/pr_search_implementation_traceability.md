...
| FR-GH-009 | WP-048, WP-054, WP-059, WP-080 | `packages/gh-cli/src/registry-policy.ts`(`decideExecution`의 운영자 차단 단계), `packages/db/migrations/030_gh_operations_policy.up.sql`(`block`·`resume`, claim 가드의 차단 확인), `apps/search-api/src/gh/{policy,routes,executions}.ts`, `apps/web/components/GhPolicyView.tsx`(A-005 최소) | gh-cli 단위, DB 통합(차단·재개·가드), 흐름 통합(차단 → 거절·대기 요청 닫힘 → 재개), HTTP 통합(403 `GH_POLICY_BLOCKED`), a11y·e2e QA-GH-48 | partial — AC-8(운영자의 capability 차단·재개, 대상 `pr.list`)만 구현(CR-090). AC-1~7(위험도·확인·승인)은 R0 밖 |
