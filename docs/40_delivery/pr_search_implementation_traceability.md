...
| WP-020 | 커밋 그래프 접근 계층 | REL-003 | done | 에이전트 | (이 PR) | DoD 5항 전부 통과 (6.20장). 단위 36건 + 통합 24건, **실제 git 픽스처로 `git rev-list --first-parent --reverse`와 직접 대조** | CR-023이 메운 빈칸(저장소 컨텍스트·미러 루트·git 자격 증명·워커 역할)과 **실측으로 드러난 ADR-005의 대가**(DEV-111). OD-001 결정 전이라 두 경로 모두 구현했다. 커밋 메타데이터 채우기는 뺐다(DEV-112) |
