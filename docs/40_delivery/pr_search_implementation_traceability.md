...
> 상태: review | 버전: v6.7 | 갱신일: 2026-08-31
...
| DEV-453 | 2026-08-30 | **`author_team_ids`를 채우는 후속이 지목되지 않았다.** `WP-037`이 그 일을 제외로 두면서 "GHE에서 작성자 팀을 읽는 별도 작업"이라고만 적고 **어느 WP가 소유하는지 말하지 않았다.** 그 사이 `group_by=team`과 `author_team:` 드릴다운은 언제나 빈 결과를 낸다 — 매핑에는 필드가 있고 투영은 쓰지 않는다. 승인된 집계가 구현 가능한 상태가 되려면 그 자리가 누구 것인지 정해져야 한다 | FR-STAT-006 / WP-037 | 문서 오류 | CR-056 | resolved (2026-08-30) — `WP-046`(위임 GitHub 신원과 Operations App)의 후속으로 명시했다. 그 WP가 작성자 팀을 읽을 자격 경로를 세우며, 그전까지 빈 결과인 것은 **누락이 아니라 미구현이고 그 사실을 화면이 그대로 말한다.** **PR #75 리뷰 P1** |
...
**처음 적은 수가 다시 낡아 있었다.** 원장은 24건이라 적었는데 실측은 **35건**이었다.

| 원인 | 차이 |
| --- | --- |
| 계수 명령이 좁은 창(`last: 30`)으로 되돌아가 `#32`·`#49`를 뺐다 (DEV-446) | +2 |
| `PR #91`에 병합 후 리뷰 둘이 새로 도착했다 (DEV-443·444) | +2 |
| design-system 9건이 처음부터 별도로 세어져 있었다 | +9 |

**판정은 셋으로 갈렸다.**

| 판정 | 건수 | 무엇 |
| --- | --- | --- |
| 이미 정정됨 | 19 | `#32`·`#49`·`#65`(5)·`#66`(3)·`#67`(3)·`#68`·`#69`(2)·`#70`(2)·`#72`, design-system `#1` |
| 이 세션이 고침 | 2 | `#91`의 P1·P2 (`DEV-443`·`444`, PR #94) |
| 실결함 | 14 | pr-search `#75`(5) → `CR-056` (PR #95), design-system `#3`·`#6`·`#8`(6) → design-system `PR #12` |
...
**"이미 정정됨"을 실측 없이 단정하지 않았다.** 열아홉 건 모두 현재 `HEAD`의 코드나 문서를 열어 어느 자리가 어떻게 달라졌는지 확인한 뒤 그 근거를 답변에 적었다 — 여러 건은 지적한 리뷰 번호가 코드 주석에 그대로 남아 있어 확인이 빨랐다.
