...
| WP-045 | gh capability 레지스트리와 parity 검증기 | REL-007 (CR-086 · CR-088 부분) | **in_progress** | 에이전트 | #181 · c5c8aea / #184 · a996540 | 6.83장·6.85장 — 인벤토리·manifest r0.2·분류 표·규칙·검증기(단위 110, 변이 10)·드리프트(통합 4, 비동기 판 포함)·029 | 들어온 것: 인벤토리·manifest(`r0.2`)·`API-GH-001`, leaf 196·flag 1,034·positional 164·`--json` 707 분류(`NFR-009` 본표 100%), `gh:validate-capabilities`·`gh:diff-capabilities`·`gh:inventory`, `gh_capability_snapshot`, CI 게이트(단위 시험 + integration 드리프트 시험). 남은 것: `GATE-GH-01d`(`DEV-675`)·호스트 지원 판정(`DEV-674`)·parity 최종(WP-060) |
...
| WP-059 | capability 드리프트와 정책 관리 (A-005, A-006) | REL-011 (CR-088 부분) | **in_progress** | 에이전트 | #184 · a996540 | 6.85장 — A-006 a11y 8·e2e 2, registry 통합 6, 실행기 registry-check 통합 5 | 들어온 것: A-006 읽기 전용(`/ops/gh-registry`, `operator`·`security_officer`), 드리프트 → `registry_stale` 거절. 남은 것: A-005 정책 편집·정책 변경 감사·`admin_action_required` 흐름·스냅숏 활성화(활성화 조건에 결과 계약 차원 반영 — `DEV-685`) |
...
| FR-AUTH-004 | WP-002, WP-039, WP-041, WP-044 | `packages/domain/src/audit.ts` (정본 어휘), `apps/search-api/src/audit/{recorder,routes,cursor}.ts`, `apps/search-api/src/export/routes.ts`, `apps/pipeline-worker/src/retention.ts`, `packages/db/src/{partitions,pool,config}.ts`, `packages/db/src/repositories/audit.ts`, `packages/db/migrations/{004,005,018}_*.sql`, `apps/web/{lib/audit.ts,components/Audit*.tsx,app/ops/audit/page.tsx,lib/nav.ts}` | `integration/audit/{audit-records,failure-isolation,action-coverage}.test.ts`, `apps/search-api/integration/export/export.test.ts`, `pipeline-worker/integration/retention/*.test.ts`, `packages/db/integration/audit-grants.test.ts`, `apps/web/{lib/audit.test.ts,a11y/audit.test.tsx,e2e/audit.spec.ts}`, `regression/runtime-reachability.test.ts` | done — AC-1~8. `safe_marker.set`과 `export.create`까지 소유 WP가 활성화해 미활성 액션은 0건이다 (CR-054·057·072) |
