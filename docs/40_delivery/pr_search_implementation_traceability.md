...
> 상태: review | 버전: v6.56 | 갱신일: 2026-09-09
...
읽기 토큰, 위의 SHA-256. 그리고 이 번들도 `compose.yml`을 덮으므로 CA 마운트 여섯 자리와
...
같다. 다만 이 번들도 `compose.yml`을 덮으므로 CA 마운트 여섯 자리는 런북 5장 표대로 다시
얹는다 — `prs_retention` 롤은 이제 그 표에서 빠졌다(`DEV-556`).
