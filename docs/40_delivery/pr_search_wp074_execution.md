...
> 상태: review | 버전: v0.2 | 갱신일: 2026-09-26

## 1. 후속 세션의 입력과 실행 권한

이 실행서는 사용자가 **별도 구현 지시를 했을 때** 사용하는 인계 문서다. 작성 세션은 설계만 수행했다. 모델·에이전트 이름과 무관하게 아래 계약과 증거를 따른다. WP-074의 현재 상태는 `pr_search_work_packages.md` 상태 표에서 읽는다. PR·CI·병합·후보 번들도 실제 수행 결과가 생기기 전 완료로 기록하지 않는다.
