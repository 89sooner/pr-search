...
> 상태: review | 버전: v2.68 | 갱신일: 2026-09-25
...
- 상태: done — main `df3d8ef`(PR #237 squash 병합, 2026-09-25). 검증·병합 판정은 원장 6.112장이다.

## WP-103 prs-commits 재색인의 기대 집합과 메타데이터 복원 (CR-119)

- 요구사항: `FR-ING-008` AC-10(신설). 잡 JOB-ING-006(커밋 재구축 순서·전환 전 검증), 계약 API-ADM-004(`progress.target_uuid`), 런북 RUNBOOK 7.H. 선행: WP-035(재색인 울타리·이중 쓰기), WP-067(커밋 메타데이터 보강), WP-098(시퀀스 replay), WP-101(관계 정본·replay), WP-102(체인 커밋 규칙).
...
- 요구사항: `FR-SRCH-002` AC-7(신설), `FR-SRCH-003` AC-5(신설), `OD-016`(resolved, 사용자 결정). 잡 JOB-REL-008(읽기 규칙), JOB-SEQ-001·재채번·복구(재투영 의도), JOB-ING-006(재색인 replay·전환 전 검증), 데이터 ENT-CORE-009(원시 관측과 유효 연결 — **스키마 변경 없음**), 계약 API-SRCH-003(`source_commits_excluded`)·PSI v1 D-22, 런북 RB-29·RUNBOOK 7.G. 선행: WP-101(관계 정본·전용 투영기), WP-021·WP-022·WP-028(채번·재채번·복구), WP-017(PR 상세 C-018), WP-035(재색인).
- 범위: (1) `packages/db` — 유효 연결 술어 `EFFECTIVE_LINK_SQL`(관계 읽기 전부와 미확정 계수), `readCommitLinkSets`(유효·체인 제외 번호를 한 문장으로), `requeueChainChangedCommitLinks`(`source` 근거가 있는 커밋만 세대 올림), `listCommitsWithMergeEvidence`, `mergeSequenceRepo.findCurrentChainLanders`·`listChangedCommitsBetweenEpochs`. 체인은 `repository.sequence_branches`의 현재 에폭이다. (2) `apps/pipeline-worker` — `ProjectionSource.chainShas`(비선택)와 `snapshot.ts`의 `chainShasOf`(실시간 `project.ts`·백필 `backfill.ts`가 같은 함수), `buildCommitDocuments`와 재색인 `rebuildProjectedCommits`의 체인 SHA 건너뛰기, `sequence.ts` 세 경로(채번·강제 푸시 재채번·복구 재채번)가 같은 트랜잭션에서 남기는 관계 재투영 의도, `link-repair.ts`(체인 규칙으로 빠지는 번호를 관측 확정과 무관하게 제거 대상으로, 덮인 체인 커밋 역할을 병합 근거까지 보고 되돌림 — `--pr` 실행은 역할 대조를 하지 않음)·`link-repair-command.ts`(출력). (3) `packages/es` — `restoreChainCommitRole`(`source_commit`일 때만 바꾸는 단방향 쓰기, shadow 404는 실패가 아님). (4) `apps/search-api` — `getPullRequestDetail`이 이미 읽는 커밋 문서의 `pull_request_numbers`로 원본 커밋을 거르고 `source_commits_excluded`와 뺀 뒤의 `source_commits_total`을 싣는다. (5) `apps/web` — `excludedCommitsLabel`·`ExcludedCommitsNote`, C-018 `CommitList`와 Repository Workspace·legacy 작업 공간 상세 창의 안내. (6) PIPE handoff — OpenAPI(`source_commits_excluded` 필수 키, `source_commits`·`source_commits_total`의 뜻), captured 예시, CONTRACT_DIFF D-22, manifest 재생성. (7) 문서 연쇄와 RUNBOOK 7.G, `agent-context/upstream-feedback.md`에 사내 pilot.18 보고 세 항목 원문과 첫 항목의 상류 반영 주석.
- 제외: 같은 반입에서 함께 보고된 prs-commits 재색인 verify 과대 계산과 prs-links 재색인 shadow 부분 갱신 실패(별도 CR). **상호작용 한 줄**: 이 WP로 `rebuildProjectedCommits`가 체인 커밋을 건너뛰어도 그 문서 ID는 재구축 기대 건수에서도 함께 빠지므로 verify 과대 계산을 키우거나 줄이지 않는다. M 번호 채번(`mnumber.ts`)·제목 표기·태그. merge commit 방식 병합의 두 번째 부모 쪽 커밋 판정(미러 도달성 질의) — squash-only 사내 프로파일에서는 생기지 않으며 SRS 예외 칸에 한계로 적었다. 원본 목록에서 빠진 체인 밖 커밋의 `role`(DEV-752 잔여). 독립 검토가 찾은 둘 — 추적 브랜치 목록 변경 시의 자동 재투영(DEV-756, 운영 조치는 `links apply` 한 번)과 역할 판정 규칙의 일원화(DEV-757, 커밋 보강·재구축은 체인 행의 PR 대응만 보고 복구는 병합 근거도 본다). 운영 서버 변경, 사내 배포·적용, Release 발행.
- 완료 기준: 단위 — 문서 생성기의 체인 건너뛰기(2건), PR 상세의 제외·모름 보존·절삭(3건), 웹 제외 수·안내 문구(4건). 통합(격리 PG·ES·Redis, 실제 git 이력) — `apps/pipeline-worker/integration/sequence/chain-links.test.ts` 9건(실제 채번·실제 투영 경로의 `git merge dev` 모양에서 머지 커밋에 올린 PR만 남고 역할이 덮이지 않음, 원시 관측 보존, 미확정 PR이 체인 규칙으로 빠지면 `partial`을 만들지 않음, fast-forward 보존절, 늦은 채번의 재계산, 강제 푸시 재채번 뒤 체인에서 빠진 커밋의 원본 커밋 복귀, 복구 재채번 뒤 재계산, 관측이 미확정이어도 체인 규칙 번호를 지우고 역할을 되돌리는 복구·반복 멱등, `--pr` 실행의 역할 대조 생략), `jobs/link-reindex.test.ts` +1건(재색인이 체인 커밋을 올린 PR만으로 복원하고 역할을 덮지 않으며 전환 전 검증을 통과). **변이 17종이 모두 시험을 죽인다** — 술어 무력화·보존절·미확정 계수·투영 건너뛰기·세 채번 경로(정상·강제 푸시·복구) 각각의 재투영 의도·계획/실행의 체인 분류·역할 방향·병합 근거·재색인 건너뛰기·replay 술어·PR 상세 필터·연결 PR 조회·총계·단위 건너뛰기. typecheck·lint·lint:deps·단위·통합·회귀·build 통과, 문서 검증기 기준선과 오류·경고 목록 동일. 독립 리뷰가 N:M 보존, 체인 판정의 에폭·추적 브랜치 경계, 재색인 전환 검증, 복구의 삭제 근거, PSI 계약 호환을 본다.
- 상태: done — main `65acf83`(PR #230 squash 병합, 2026-09-24). 검증·병합 판정은 원장 6.108장이다.
...
- 상태: done — main `f9cda82`(PR #228 squash 병합, 2026-09-23). 검증·병합 판정은 원장 6.107장이다.

## WP-100 확정된 M 번호의 원격 lightweight 태그 (CR-115)

- 요구사항: `FR-SEQ-012` AC-1~AC-11(신설), `OD-015`(open). 설계 ADR-026(+ADR-022 Amendment), 잡 JOB-SEQ-007, 데이터 ENT-SEQ-001(태그 결과 열)·ENT-SEQ-006(`tag` kind)·ENT-CORE-001(`tag_enabled`·`tag_blocked_*`, 마이그레이션 035), 계약 API-ADM-002(`mnumber_tag_reconcile`)·API-ADM-001(`tag_enabled`), 위협 THR-061, 경보·런북 RB-28, RUNBOOK 7.F. 선행: WP-074(M 번호 정본·durable work), WP-075(표기의 durable 쓰기 규율·pacing), WP-098(문서 단위 투영·재색인 replay), WP-099(이 브랜치가 CR-114 브랜치 위에 stacked).
- 범위: (1) `packages/github-tag`(신규) — 설정(`GHE_TAG_*`·`MNUMBER_TAG_ENABLED` 기본 꺼짐·스윕·쿨다운·쓰기 간격 하한 1초), 순수 판정(`resolveTagTarget`·`decideTagAction`·`mergeNumberOfTagName`), 클라이언트(`readTagRef`·`createTagRef`·`listTagRefs` — 이동·삭제 메서드 없음, 오류 분류 `TagApiError`), `node:http` 목 GHE(`testing/mock-tag-ghe.ts`). (2) `packages/github/src/pacing.ts`로 `PassDeadline`·`WriteGate` 이동(표기는 재수출). (3) 마이그레이션 035와 `@prs/db`(`findTagTarget`·`listTagSweepTargets`·`listNumberedRowsPage`·`markTagState`·`isTagTargetCurrent`·`countTagStates`·`findNumberedRowBySha`, `blockTagging`·`clearTagBlock`·`tag_enabled`, kind `tag`, 잡 유형 `mnumber_tag_reconcile`). (4) `apps/pipeline-worker` — `mnumber.ts`(채번 트랜잭션이 PR마다 `tag` 의도, `materialize`가 PR·커밋 문서 둘 다), `mnumber-tag.ts`(JOB-SEQ-007 work 실행 — 정본 재확인 → GET → 판정 → 간격 → 쓰기 직전 재확인 → POST 하나 → 정본 기록 → 실제 생성만 감사; 러너; 일 1회 잔여 스윕), `mnumber-tag-reconcile.ts`(대조·잡 러너), `mnumber-tag-command.ts`·`mnumber-tag-cli.ts`(`prsctl mnumber tags reconcile|status`), `sequence-work-runner.ts`(`tag`는 집지 않음), `commit-enrich.ts`(`merge_commit` 문서 생성 뒤 `materialize` 재요청), `reindex.ts`(`prs-commits` 재색인 뒤 재투영 의도), `metrics.ts`(`mnumber_tag_total{result}`·`mnumber_tag_conflict_total`), `index.ts`(`tag` 역할 — 자격은 `GHE_TAG_*`에서만). (5) `packages/es` — 커밋 매핑 M 세 필드, `applyMergeNumberToCommitDocument`(역할·저장소·브랜치·SHA 가드 + 에폭 스크립트), `clearMergeNumbersBelowEpoch` 커밋 인덱스. (6) `packages/domain` — 감사 `merge_number.tag`·`TAG_PRINCIPAL`. (7) `apps/search-api` — `mnumber_tag_reconcile` 등재·검증(`dry_run` 거절), 저장소 설정 `tag_enabled`. (8) `apps/web/lib/ops-jobs.ts` — A-003 실행 폼 항목. (9) deploy — compose `x-tag-env`·`worker-annotate`의 `annotate,tag`, `.env.example` 태그 절, `prsctl mnumber tags`, k8s `pipeline-worker-annotate.yaml`(`annotate,tag`·`prs-tag-secrets`)·`tag-secret.example.yaml`. (10) 시험 설정 — vitest 세 설정·`tsconfig.tests.json`의 `@prs/github-tag` 별칭(접두 겹침 주의). (11) 문서 연쇄 전부와 RUNBOOK 7.F, `agent-context/upstream-feedback.md` 상류 반영 주석.
- 제외: 운영 서버 변경, 이 제품의 릴리스·태그 발행, 태그 이동·삭제·정정 자동화, GHE ruleset 생성(운영 절차로 권고만), annotated 태그, 다중 시퀀스 브랜치 저장소의 태그(OD-015), 커밋 hit DTO·web 배지의 M 노출(DEV-742), `API-SEQ-007` 변경, 에폭 상향 뒤 기존 태그의 일괄 정정, 새 화면.
- 완료 기준: 단위 — `packages/github-tag/src/{config,decision,client}.test.ts`(스위치 기본 꺼짐·오타 거절·자격 전용 변수·비면 사유·간격 하한; 이름 왕복·코드·SHA 소문자·판정 4종; 전송 `{ref, sha}`·토큰 헤더·이동·삭제 부재·조회·목록 페이지·오류 분류 8종). 통합(격리 PG·ES·Redis + `node:http` 가짜 GHE) — `apps/pipeline-worker/integration/sequence/mnumber-tag.test.ts`(26건: 035 열 3·생성·멱등·코드 불가·다른 SHA·annotated·충돌 비재시도·에폭 상향·403 차단·429·시한 초과 재읽기·미확정 복구·SHA 없음·옛 에폭·다중 브랜치·저장소 해제·러너 전이 2·`sequence` 러너 비집기·스윕 backfill·차단 쿨다운·대조 dry-run·대조 재생성·잡 러너·다중 브랜치 거절), `mnumber-tag-git.test.ts`(6건: `rev-parse`·`show`·`log a..b`·`diff a..b`·`describe --tags`·`checkout`·`log --decorate`·요청은 조회·생성뿐), `mnumber.test.ts`(채번 트랜잭션의 `tag` work 4건·`ready` 유지), `projection.test.ts`(commits-only 재색인 뒤 재투영 의도), `packages/es/integration/merge-number-commit.test.ts`(8건: 역할·브랜치 가드·문서 없음·구 에폭·pending·에폭 상향 정리), `apps/search-api/integration/search/mnum-commit.test.ts`(`kind:commit mnum:`), 파수꾼 6파일의 035. 회귀 — 잡↔러너 쌍, `resolveJobTarget` 네 갈래, 역할 `tag`의 manifest·인프라 표, 태그 클라이언트의 이동·삭제 부재, 감사 어휘. typecheck·lint·lint:deps·단위·통합·회귀·build·a11y·contrast·e2e 통과, 문서 검증기 기준선과 오류·경고 목록 동일. 변이 — 생성 push·역할 가드·쓰기 직전 재확인을 각각 지워 시험이 죽는 것을 본다. 독립 리뷰가 쓰기 반경(이동·삭제 부재)·자격 분리·쓰기 직전 재확인·감사 어휘·에폭 규율·대조가 두 번째 생성 구현을 갖지 않음을 본다.
- 상태: done — main `83d30fb`(PR #226 squash 병합, 2026-09-22). 원장 6.106장.
...
- 상태: done — main `9782ba9`(PR #225 squash 병합, 2026-09-22). 원장 6.105장.

## WP-098 머지 시퀀스의 Elasticsearch 투영 수렴 (CR-113)

- 요구사항: `FR-SEQ-001` AC-7·AC-8, `FR-ING-008` AC-8, `FR-ADMIN-003` AC-6. 설계 ADR-004 Amendment(CR-113), 잡 JOB-SEQ-006, 데이터 ENT-SEQ-006(`project` kind·`progress`, 마이그레이션 034), 계약 API-ADM-002(`sequence_reproject`). 선행: WP-021·WP-022·WP-028(채번·재채번·복구), WP-035(재색인 울타리·이중 쓰기), WP-074(`sequence_work` generation/lease/CAS).
- 범위: (1) `packages/es/src/sequence-projection.ts` — 문서 ID 기반 guarded 투영기(`mget` 사전 읽기 + 스크립트 가드 bulk update, 문서별 `updated`·`noop`·`document_missing`·`guard_rejected`·`stale_epoch`·`transient`, active/shadow 결과 분리, dry-run `would_update`)와 `applyEpochBump`의 판정 재료(`version_conflicts`·`failures`·`timed_out`·`noops`); 옛 `applySequenceToDocuments` 제거. (2) `apps/pipeline-worker/src/sequence-projection.ts` — 정본 해석(PR은 `pull_request_snapshot`의 `merge_commit_sha`·`base_branch` 대조, 커밋은 `commit_snapshot`·PR 참조, `mapping_conflict`·`awaiting_creation`·`no_target`·`other_space` 구분, 같은 SHA 다른 공간의 `reclaim` 규칙)과 공간 sweep(`tail`·`full`, 페이지 커서·generation·완료 요약을 `progress`에 저장, 예산마다 lease 반납)·문서 단위 work. (3) 훅 — 채번·재채번·복구 트랜잭션의 `project` 의도, 늦은 PR 스냅숏 트랜잭션의 문서 단위 의도, 커밋 보강 직후 인라인 투영과 실패분 위임. (4) durable 러너의 `project` kind(`MNUMBER_ENABLED=false`에서도 집는다, `document_missing`은 상한 뒤 `parked`이며 완료로 닫지 않음). (5) 재색인 — PR·커밋 재구축 직후 replay(`job.progress.sequence_replay`), `verifyBeforeCutover`의 시퀀스 검사(문서별 원시 필드 + 대표 범위 정렬, 한 번의 인라인 재투영 뒤 재확인), 전환 울타리 안 에폭 대조, 전환 직후 full sweep. (6) `repairSequence` `consistent` → full sweep 예약 + 러너의 `db`/`projection` 분리 보고. (7) 잡 `sequence_reproject`(API-ADM-002, `expected_epoch` 필수, `aliases` 선택, `dry_run` 거절, `allowed_actions: [cancel]`), JOB-SEQ-006 러너(예약 → 완료 대기 → `completed`/`failed(projection_partial|projection_incomplete)`), `prsctl sequence reproject|status`(dry-run 무변경, 감사 `job.run`), 운영 콘솔 실행 폼의 예상 에폭 입력. (8) 문서·RUNBOOK(RB-27)·시험.
- 제외: 운영 서버 직접 변경, 공용 인덱스 삭제·재생성, 기존 번호 재채번, Release/태그 발행, 보호 규칙 우회, 날짜 정렬 대체·클라이언트 재정렬·M 문자열 정렬. 체인에서 빠진 문서의 옛 서수(orphan) 정리와 PR 재투영이 커밋 문서의 `base_branch`를 되돌리는 기존 동작은 DEV로만 등재한다(DEV-734·DEV-735). 운영 콘솔 재채번 요청의 본문 키 불일치(`confirm`/`confirmation`)는 별건 등재(DEV-736). 새 감사 액션은 만들지 않는다(`job.run` 재사용).
- 완료 기준: 현 코드에서 실패하던 시험(시퀀스 먼저/PR 문서 나중, 늦은 `merge_commit_sha`, 두 순서 동일 결과, 직접 푸시 커밋 문서, ES 실패 뒤 새 push 없음, consistent인데 ES만 누락, PR·커밋 재색인 복원 — `projection.test.ts` 8건)이 통과하고 각 수렴 경로를 끄는 변이에 최소 하나가 죽는다. 재투영 잡·CLI·API·consistent 보고 시험(`reproject-runner.test.ts` 8건, `admin/jobs.test.ts` 7건), ES 투영기 시험(20건), Search API 정렬·`seq:`·cursor·이전 PIT 시험(6건), RepositoryWorkspace e2e(2건, UI 순서만) 통과. 정렬 fixture는 PR 번호·merged_at·서수 순서가 서로 다르고, 판정 기준은 같은 repo/base/현재 epoch의 서수 순서다. ES 원시 필드를 직접 대조한다. typecheck·lint·lint:deps·단위·통합(격리 PG·ES·Redis)·회귀·build·a11y·e2e 통과, 마이그레이션 034 왕복(파수꾼 4파일). 독립 리뷰가 잘못된 브랜치/에폭 쓰기·정본 변경·재색인 경쟁과 shadow 완료 판정·누락 작업의 성공 종료·공유 시험 인프라 오염을 본다.
- 상태: done — main `a6ea00d`(PR #223 squash 병합, 2026-09-22). 구현·검증·독립 리뷰·병합 기록은 원장 6.104장이다. 사내 배포 SHA는 NOT VERIFIED, 내부망 적용은 NOT RUN이다.
...
- 완료 기준: typecheck·lint(기준선 실패 1건은 병합 전 ESLint 설정으로 해소 — 오류 0)·lint:deps·단위·통합(127.0.0.1의 실제 mTLS·PostgreSQL·Redis·Elasticsearch)·회귀·build 통과. 공통 수용 시험 PSI-A~G 중 pr-search 책임 항목을 시험 이름으로 연결. 계약 시험이 OpenAPI·`INTEGRATION_OPERATIONS`·operation map의 일치, 예시의 스키마 적합, 적합성 벡터와 구현의 일치를 대조. 핵심 검사를 지우는 변이를 시험이 잡음. 기존 통합·회귀가 기준선 그대로 통과(공개 경로 불변). TEST_RESULTS가 실행한 명령과 NOT_RUN을 구분.
- 상태: done — main `e7b4cb4`(PR #220 squash 병합, 2026-09-21). 구현·검증·병합 기록은 원장 6.103장이다. 제외 목록의 commit·push·PR은 구현 보고 뒤 사용자 지시로 진행했다. 사내 CA·운영 HAProxy·실제 GHE를 거친 검증은 NOT_RUN이다.
...
- 상태: **done**. 독립 코드 리뷰(high) 반영 완료. 로컬 검증 전부 통과(typecheck·lint·lint:deps·단위 2930/2930·통합 1857+8/1865·회귀 506/506·a11y 436/436·build). PR #213 스쿼시 병합 `95674b8`, main CI(run `35292695927`) verify·integration success. 발견 편차 `DEV-726`·`DEV-727`은 이 CR의 정정 사항과 무관한 별개 관찰로 후속 CR 후보에 등재. 사내 실데이터 확인 NOT RUN. 진행 정본: 구현 추적 원장 6.100장.
...
- 상태: **done**. 백엔드 4개 API(`/search`·`/exports`·집계·W-004 범위 조사)와 웹 필터 폼 구현 완료, 독립 코드 리뷰(high) 2회 전부 반영. 로컬 검증 전부 통과(typecheck·lint·lint:deps·단위 2919/2920·통합 1847/1847·회귀 506/506·build). PR #211 스쿼시 병합 `03da17c`, main CI(run `35256612621`) verify·integration success. 미반영 발견 둘은 DEV-724·DEV-725로 변경 대장에 등재(이 CR의 정정 사항과 무관한 별개 결함, 후속 CR 후보). 사내 실데이터 확인 NOT RUN. 진행 정본: 구현 추적 원장 6.99장.

## WP-091 검색 결과 무한 스크롤 (CR-104)

- 요구사항: FR-SRCH-008(커서 페이지네이션, 트리거 방식 중립). 선행: WP-090(같은 결과 표 영역).
- 범위: `RepositoryWorkspace.tsx`의 "More changes" 버튼을 `IntersectionObserver` 기반 스크롤 자동 로드로 교체. 커서 거절 시 자동 재시도 금지(CR-043) 유지, 결과 카운트 `aria-live` 전환, 기존 `Spinner`/`CursorPager` 패턴 재사용. 모바일 대응은 사용자 결정으로 제외.
- 제외: 맨 아래로 스크롤된 채 데이터만 초기화되는 경우의 재발화(알려진 경미한 한계, 스크롤 제스처로 자기 교정).
- 완료 기준: 타입·lint·lint:deps·빌드 통과. 네트워크 호출 횟수 실측으로 폭주 없음, 강제 커서 거절로 자동 재시도 없음, 키보드 전용 스크롤 확인.
- 상태: done — 구현·자체 검증(위 항목 전부 실측 확인) 완료. 진행 정본: 구현 추적 원장 6.97장.
...
- 범위: operator Legacy search 진입 보존, Base branch/Label Select, Radix 달력, PR#/M number/Title 열, `pr_number DESC`, canonical 상태 필터, 배지 tone 보강.
- 완료 기준: query·sort 단위 검사, 달력 a11y, 실제 Chromium 필터/열/GHE 링크/M number/정렬 확인, 전체 타입·lint·빌드·CI.
- 상태: done — query/sort/parser82/82·전체 a11y436/436·아키텍처 포함 집중148/148·대비18쌍·타입·lint·빌드·언어/의존성 gate·실제 Chromium 필터/표/Source 회귀·PR/main 전체 CI·bundle smoke 통과, `0.1.0-pilot.12` 발행. 사내 실제 GHE 데이터 검증은 NOT RUN.

## WP-086 사내 권한명·operator 바로가기 보정 (CR-098)
...
- 상태: done — 권한22/22·reader/operator DOM/접근성33/33·FLOW-002 반복20/20·전체 PR/main CI·bundle smoke 통과, `0.1.0-pilot.11` 발행. 사내 재반입 NOT RUN.
...
- 요구사항: FR-SRCH-003·005·006·007·008, NFR-007. 선행: WP-082.
...
| WP-099 | 검색창 M 번호 문자열 해석과 `mnum:` 단일 값 | 기능 누락 보완 (CR-114) | WP-014, WP-074, WP-092 | done — main `9782ba9`(PR #225), 원장 6.105장 |
...
| WP-091 | 검색 결과 무한 스크롤 | UI 품질 (CR-104) | WP-090 | **done** — `IntersectionObserver` 구현, 네트워크 호출 실측(폭주 없음)·강제 커서 거절로 자동 재시도 없음 확인·키보드 전용 스크롤 확인·타입/lint/lint:deps/빌드 통과(원장 6.97장); 맨 아래 스크롤 상태에서 데이터만 초기화되는 경우의 재발화는 알려진 경미한 한계(제외), 모바일 제외(사용자 결정) |
...
> **`CR-092` 정정 (2026-09-15, 사내 `0.1.0-pilot.7` 반입 피드백).** 접근 범위 출처가 **쓰이지 않는 조직·팀 조회**까지 늘 불러, App에 조직 `Members` 권한이 없으면 저장소가 몇 개든 범위 전체가 503이었다(`DEV-698`). 이제 조직·팀은 `org_team`으로 표현되는 범위(저장소 500개 초과)에서만 읽고, 실패는 단계·상태 코드·필요 권한으로 운영 로그에 남긴다. `/me`의 `org_count`·`team_count`는 `explicit` 모드에서 `null`이다. 로그인 때 팀 동기화를 시작하자는 사내 요청은 원인이 아니고 `FR-AUTH-003`이 요구하지 않아 받지 않았다. 검증 기록은 원장 6.91장이다.

> **`CR-091` 정정 (2026-09-15, 사내 `0.1.0-pilot.6` 반입 피드백).** 이 WP가 세운 역할 합집합(`CR-015`·`DEV-049`)이 **요청 경로에서 성립하지 않았다** — 로그인 콜백이 관리자 지정 자리에 빈 배열을 넘겼고, 세션을 읽는 두 서비스가 세션의 역할만 봤으며, 관리자 지정을 쓰는 운영 경로가 없었다(`DEV-695`). 이제 `search-api`가 세션을 읽을 때마다 `app_user.roles[]`를 더하고 화면 관문이 `/me`에서 받으며, `prsctl role`이 지정·회수와 감사를 한 트랜잭션으로 한다(`FR-AUTH-001` AC-10). 쿠키 계약에 평문 HTTP 파일럿 면제가 더해졌다(`DEV-694`, AC-2). 검증 기록은 원장 6.89장이다. 새 WP를 만들지 않았다 — 이 WP의 계약을 코드가 이행하지 못한 자리를 고친 것이다.
...
> **`CR-092` 후속 (2026-09-15, 사내 `0.1.0-pilot.7` 반입 피드백).** 셸에 로그아웃이 없었다(`DEV-700`) — C-001의 사용자 메뉴에 「로그아웃」 폼 제출과 공개 완료 화면(`/auth/signed-out`)을 더했다. 로그인·Operations 콜백이 요청 출처로 복귀 주소를 만들어 역방향 프록시 뒤에서 `localhost:3000`으로 보냈다(`DEV-699`) — 경로만 담은 `Location`으로 바꿨다. 원장 6.91장.
...
> **`CR-092` 정정 (2026-09-15).** `prsctl smoke`의 헬스체크가 본문과 헤더를 한 파이프로 읽어, `docker compose exec`가 둘을 뒤바꿔 나르면 정상 `search-api`를 `→ HTTP/1.1`로 실패시켰다(`DEV-697`, 간헐). 상태 코드만 읽는 `http_status`로 바꿨다. 런북 2.C에 수집용 App 권한 표와 시퀀스 대상 브랜치 변경 절차를 더했다. 원장 6.91장.
...
`WP-081`·`WP-082`는 `WP-073`과 같은 UI 품질 트랙이며 위 REL 합계에 넣지 않는다. 기존 기능 릴리스의 완료 판정을 재개방하지 않는다.
...
>
> **`CR-091` 후속 (2026-09-15).** `0.1.0-pilot.6` 반입에서 이 경로로 로그인한 뒤 막힌 세 자리를 고쳤다 — TLS 없는 파일럿의 로그인(`ALLOW_INSECURE_COOKIES`, 접두 없는 쿠키 이름), 세션 인증 뒤 `operator`를 얻는 경로(`prsctl role`), 토큰이 남은 `.env`로 인증을 켠 업그레이드의 사전 감지. 원장 6.89장.
>
...
- 완료: PR #196 squash `a8796de`; PR CI `35061974889`와 main CI `35062529329` success. `0.1.0-pilot.8` 번들 런타임·시크릿 검사와 자산 SHA-256 대조 뒤 발행. 사내 GHE 확인은 `NOT RUN`이며 기존 릴리스 게이트는 재판정하지 않는다.

## WP-082 기본 저장소 작업 공간과 operator 전용 기존 UI (CR-094)

- 요구사항: FR-SRCH-003·005·006·007·008, FR-ING-009, NFR-007. 선행: WP-081.
- 구현: `template.html` 및 설계 분석 문서를 바탕으로 Conductor 기반 기본 저장소 작업 공간(`RepositoryWorkspace`) 재구현, 기존 컴포넌트는 operator 전용(`/search?legacy=1`)으로 보존, developer 등록 요청 절차 폐지 및 진입 즉시 PR 목록 표시, 필터 유지 및 URL 상태 보존, deep link `WorkspaceEntityPage`, `prsctl smoke` 워커 로그 재시도, RUNBOOK 컨테이너 외부망 차단 오프라인 빌드 절차 보완.
- 완료 기준: 의존성·타입·lint·빌드·단위·접근성·대비 통과, PR CI 및 main CI 확인 뒤 `0.1.0-pilot.9` 릴리스 번들 발행.
