...
> 상태: review | 버전: v2.11 | 갱신일: 2026-08-31
...
- 구현 범위:
  - 작성자의 소속 팀을 GHE에서 읽어 `author_team_ids`에 싣는다. 조회 단위는 사용자이며 저장소가 아니다
  - 소속은 자주 바뀌지 않으므로 캐시하되 **문서에 박제된 값이 낡는 것을 전제한다** — 재색인과 백필이 현재 소속으로 다시 쓴다
  - 소속을 읽지 못한 작성자는 **필드를 비운다.** 빈 배열과 미조회를 같은 값으로 두지 않는다
- 제외:
  - `allowed_team_ids` (WP-068에서 완료)
...
- 완료 기준(DoD):
  ...
  - [ ] 소속 변경이 재색인·백필로 반영된다
- 검증 방법: `pnpm run test:integration analytics`, `pnpm run test` (투영)
- 기록: 원장 WP-069 상태, FR-STAT-006 매핑, DEV-453 종결
