...
> 상태: review | 버전: v1.1 | 갱신일: 2026-08-26
...
| WP-032 | 패싯·커서 페이지네이션·전문 검색 | REL-004 | WP-013, WP-016, **WP-035** | todo (WP-035 대기) |
...
- 완료 기준(DoD):
  ...
  - [ ] 질의 변경 후 이전 커서가 `CURSOR_QUERY_MISMATCH`를 낸다 (AC-3)
  - [ ] **훼손·만료 커서가 `CURSOR_INVALID`를 내고 화면 문구가 mismatch와 다르다** (예외 처리, QA-W001-15)
  - [ ] **커서 봉투를 손으로 고친 값이 거절된다** — 서명 검증이 실제로 걸린다
  - [ ] **권한 회수로 `access_scope_version`이 바뀌면 진행 중 커서가 거절된다**
  - [ ] 오프셋 파라미터가 API에 존재하지 않는다 (AC-4, ADR-010)
  - [ ] **어떤 정렬 키에서도 페이지 사이에 항목이 중복·누락되지 않는다** — `relevance`는 새 문서 색인 뒤에도, `updated_at`은 페이지 밖 문서의 값을 옮긴 뒤에도 그대로다 (QA-W001-29, DEV-286)
  - [ ] **`sort=relevance&order=asc`가 실제로 오름차순이다** (FR-SRCH-007 AC-1, DEV-275)
  - [ ] **패싯 검증이 bucket 단위다** (QA-W001-16): 각 bucket에 그 필터만 더한 독립 조회의 건수가 bucket count와 같다. `sum(buckets) == total`로 검증하지 않는다
  - [ ] 패싯 실패가 목록을 막지 않는다 (예외 처리) — **`budget_omitted`와 `failed`가 다른 상태로 나간다**
  - [ ] **패싯 집계가 강제 필터를 지난다** — 접근 범위 밖 저장소의 팀·라벨·작성자가 bucket·count 어디에도 없다 (THR-003)
  - [ ] **이어 보기가 패싯을 다시 계산하지 않는다** (QA-W001-27)
  - [ ] 제목 일치가 본문 일치보다 상위다 (FR-SRCH-011 AC-2)
  - [ ] **강조 응답에 raw HTML이 없다** — 마크업을 담은 제목도 평문 조각과 일치 구간으로 나간다 (QA-W001-26, THR-018)
  - [ ] **`source_commit` 메시지가 전문 검색 점수 절에 들어가지 않는다** (DEV-283)
  - [ ] 한글·영문 혼용 질의에서 두 언어 토큰이 매칭된다 (AC-4). **`nori` 없이 `standard` + `edge_ngram`으로 성립함을 증명한다** (OD-005, CR-040)
  - [ ] **코드 포인트 한 글자(`𠮷`·이모지)가 `QUERY_TOO_SHORT`로 거절된다** (QA-W001-28)
  - [ ] **QA-W004-23 ~ QA-W004-29가 통과한다** — 특히 "일치가 첫 페이지 밖이어도 요약과 목록이 어긋나지 않는다"(DEV-270)와 "한 chunk의 일치가 `size`보다 많아도 나머지가 사라지지 않는다"(DEV-287)
  - [ ] **W-004 패싯이 페이지가 아니라 구간 전체를 센다** (FR-SEQ-002 AC-8)
  - [ ] **에폭이 바뀐 뒤 이전 구간 커서가 거절된다** (FR-SEQ-002 AC-7)
  - [ ] **새 매핑 버전이 WP-035의 재색인·별칭 전환으로 적용된다** — 이 WP가 `putMapping`으로 분석기를 바꾸려 시도하지 않는다
- 검증 방법: `pnpm test:integration search/facets`, `pnpm test:integration sequence/range`, `pnpm test:e2e search-paging`
- 기록: 원장 WP-032 상태, FR-SRCH-008·009·011·FR-SEQ-002 매핑
...
- 완료 기준(DoD):
  ...
  - [ ] **코드 갈래와 배포 단위 표가 서로를 덮는다** — 표에만 있는 단위와 코드에만 있는 역할을 각각 잡는다 (DEV-310)
  - [ ] **대상 버전은 아직 쓰이지 않은 다음 번호다** — 실패로 남은 shadow가 재시도를 막지 않는다 (DEV-309)
  - [ ] **울타리를 두 인덱스 쓰기가 끝날 때까지 쥔다** — 진행 중인 논리 쓰기가 있으면 전환하지 않는다 (DEV-308)
