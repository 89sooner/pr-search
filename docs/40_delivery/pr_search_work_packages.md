...
- 제외:
  ...
  - 시퀀스 채번 (WP-021)
- 완료 기준(DoD):
  - [~] QA-A003-05, QA-A003-06이 통과한다 — **06은 통합 시험이 있으나 로컬 NOT RUN**, **05는 구조로만 세웠다**(우선순위·워커 풀 분리). 실제 지연 영향은 운영 규모 측정이 필요하다
  - [ ] 백필 실행 중 실시간 수집 지연 p95가 10초를 유지한다 (FR-ING-006 AC-3) — **NOT RUN**, 부하 harness 부재 (DEV-058과 같은 형태)
  - [~] 중단 후 재개 시 마지막 커서부터 이어진다 (AC-4) — 통합 시험 있음, **로컬 NOT RUN** (Docker 부재)
  - [~] 백필 문서가 더 새로운 실시간 문서를 덮어쓰지 않는다 (AC-5) — 통합 시험 있음, **로컬 NOT RUN**
  - [~] 동시 실행 4개 요청 시 3개만 실행된다 (AC-6) — 통합 시험 있음, **로컬 NOT RUN**
  - [~] API 한도 소진 시 잡이 대기하고 회복 시각에 자동 재개된다 (예외 처리) — 통합 시험 있음, **로컬 NOT RUN**

  `[~]`는 **시험은 썼으나 이 환경에서 실행하지 못했다**는 뜻이다. Docker가 없어 `pnpm test:integration`이 돌지 않으므로 **CI가 처음 판정한다.** 통과로 적지 않는다.
