...
- 완료 기준(DoD):
  - [ ] QA-W003-01, QA-W003-02, QA-W003-04 ~ QA-W003-08이 통과한다
  - [ ] **QA-W003-03은 절반만** — "PR이 없을 때 사유를 표시한다"는 여기서, "**직접 푸시**로 표시한다"는 `role: 'direct_push'`가 도달하는 WP-021 뒤 (CR-021, DEV-093 / DEV-061)
  - [ ] 상태 매트릭스 W-003의 전 상태가 렌더링된다 (`no_pr`의 `direct_push` 갈래는 도달 불가 — 매핑은 있고 시험은 합성 입력으로 건다)
