...
- 완료 기준: 단위 — 문서 생성기의 체인 건너뛰기(2건), PR 상세의 제외·모름 보존·절삭(3건), 웹 제외 수·안내 문구(4건). 통합(격리 PG·ES·Redis, 실제 git 이력) — `apps/pipeline-worker/integration/sequence/chain-links.test.ts` 8건(실제 채번·실제 투영 경로의 `git merge dev` 모양에서 머지 커밋에 올린 PR만 남고 역할이 덮이지 않음, 원시 관측 보존, 미확정 PR이 체인 규칙으로 빠지면 `partial`을 만들지 않음, fast-forward 보존절, 늦은 채번의 재계산, 강제 푸시 재채번 뒤 체인에서 빠진 커밋의 원본 커밋 복귀, 관측이 미확정이어도 체인 규칙 번호를 지우고 역할을 되돌리는 복구·반복 멱등, `--pr` 실행의 역할 대조 생략), `jobs/link-reindex.test.ts` +1건(재색인이 체인 커밋을 올린 PR만으로 복원하고 역할을 덮지 않으며 전환 전 검증을 통과). **변이 16종이 모두 시험을 죽인다** — 술어 무력화·보존절·미확정 계수·투영 건너뛰기·세 채번 경로 중 둘의 재투영 의도·계획/실행의 체인 분류·역할 방향·병합 근거·재색인 건너뛰기·replay 술어·PR 상세 필터·연결 PR 조회·총계·단위 건너뛰기. typecheck·lint·lint:deps·단위·통합·회귀·build 통과, 문서 검증기 기준선과 오류·경고 목록 동일. 독립 리뷰가 N:M 보존, 체인 판정의 에폭·추적 브랜치 경계, 재색인 전환 검증, 복구의 삭제 근거, PSI 계약 호환을 본다.
