...
- 구현 범위:
  - **백엔드 — 계약이 연 자리를 실재시킨다** (CR-055):
    ...
    - `JOB-ING-005` 수동 실행 — `API-ADM-002`의 `type: reconcile`과 **`reconcile` 역할의 러너**. 기존 `runReconcileSweep`를 부른다
...
- 완료 기준(DoD):
  ...
  - [ ] **취소가 완료로 덮이지 않는다** — 러너가 종료를 기록할 때 이미 `cancelled`인 잡을 `completed`로 만들지 않는다
