...
> 상태: review | 버전: v1.5 | 갱신일: 2026-08-27
...
- 완료 기준(DoD):
  - [ ] QA-W008-01 ~ QA-W008-12가 통과한다
  - [ ] `team` 공개 검색이 **지정된 대상 팀**에게만 보이고 다른 팀에는 `NOT_FOUND`다 (FR-SRCH-010 AC-2)
  - [ ] 같은 `slug`을 가진 다른 조직의 팀으로 공유가 새지 않는다 (AC-1, DEV-331 계열)
  - [ ] 공유받은 구성원이 편집·삭제를 할 수 없고, 화면이 액션을 그리지 않을 뿐 아니라 **서버가 거절한다** (AC-2)
  - [ ] 실행자 권한이 적용되고 저장자 권한이 승계되지 않는다 (AC-3, THR-012). 저장자만 볼 수 있는 저장소를 담은 질의를 공유받은 사람이 실행해도 그 저장소 결과가 나오지 않는다
  - [ ] 저장자의 접근 범위가 행에도 응답에도 없다 (AC-3)
  - [ ] 100건 초과 시 409다 (AC-4)
  - [ ] **99건 상태에서 동시 저장 둘이면 하나만 성공하고 최종 개수가 100이다** (AC-4)
  - [ ] 목록이 커서로 이어지고 누락·중복이 없으며, 팀 구성이 바뀐 뒤의 커서는 `CURSOR_QUERY_MISMATCH`다 (AC-5)
  - [ ] 훼손·만료·모르는 스키마 커서는 `CURSOR_INVALID`이고, **남의 커서는 `CURSOR_QUERY_MISMATCH`다** — 사용자 ID가 지문에 있어 서명은 유효하고 지문이 다르다 (AC-5, API-SRCH-005 커서 절)
  - [ ] 문법 오류가 있는 질의는 저장·수정에서 거절되고, 이미 저장된 무효 질의는 목록에서 사유와 함께 표시되며 직접 실행하면 `SAVED_SEARCH_QUERY_INVALID`다 (AC-6)
  - [ ] 무효·권한 없는 실행에서 `last_run_at`이 갱신되지 않는다 (AC-6)
  - [ ] 저장자가 대상 팀에서 이탈해도 항목이 남고, 삭제·`private` 전환은 되며 그 팀을 대상으로 남기는 수정은 거절된다 (AC-7)
  - [ ] 마이그레이션 015가 up/down/up을 견디고, 불변식이 어긋난 행을 거절한다
  - [ ] `/share-targets`가 정적 경로로 먼저 등록되어 `{id}`로 해석되지 않는다
  - [ ] 운영 `buildServerDeps`가 의존을 넘기고 `buildServer`가 경로를 세우는 것을 회귀가 건다 (DEV-177 계열)
  - [ ] axe 위반 0건
