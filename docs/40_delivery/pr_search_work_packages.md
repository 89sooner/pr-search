...
| WP-029 | 관계 간선 인덱스와 참조 추출 | REL-004 | WP-008, WP-003, **WP-067** | in_progress |
...
- 완료 기준(DoD):
  ...
  - [ ] 미해결 참조가 대상 색인 시 **같은 `link_id`로** 해결 상태로 갱신된다 (AC-3, DEV-217)
  - [ ] 코드 블록·인용 안의 표현이 추출되지 않는다 (AC-4)
  - [ ] 중복 제거된 고유 참조 100건 상한이 적용되고 **재실행 시 같은 100건**이 선택된다 (AC-5)
  - [ ] 추출 실패가 색인을 막지 않고 `links_pending: true`가 되며 **기존 간선을 지우지 않는다** (예외 처리, DEV-220)
  - [ ] 같은 문서를 두 번 처리해도 간선이 중복되지 않는다 (ADR-009)
  - [ ] **본문에서 사라진 참조의 간선이 제거된다** (DEV-220)
  - [ ] **이벤트 순서가 역전돼도 최종 결과가 현재 정본과 일치한다** (DEV-215)
  - [ ] **직접 푸시 커밋의 참조가 실제 운영 방아쇠 사슬로 간선이 된다** — 이벤트를 손으로 만들어 넣지 않는다 (DEV-215)
  - [ ] **`commit-enrich`가 자기 이벤트를 되받지 않는다** (DEV-216)
  - [ ] **축약 SHA가 모호하면 해결하지 않는다** (DEV-217)
  - [ ] **`link_summary`의 WP-030 leaf가 보존된다** (DEV-222)
  - [ ] **접근 통제 material 없이 간선이 만들어지지 않는다** (THR-035)
  - [ ] **PostgreSQL 정본만으로 `prs-links`를 다시 만들 수 있다** — JOB-REL-006 (ADR-004, DEV-221)
  - [ ] **`link` 역할이 선언 → 기동 → 핸들러 → 종료 → manifest → README 적용 순서로 이어진다** (DEV-214 선례)
