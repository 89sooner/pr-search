...
> 상태: review | 버전: v2.27 | 갱신일: 2026-09-12
...
| WP-075 | PR 제목 M 넘버 표기 | REL-003 (**CR-077 · CR-084**) | WP-074, WP-012 | **done** — 검증 6.81장. 전역 스위치 `MNUMBER_ANNOTATE_ENABLED`의 기본값이 **꺼짐**이라 이 변경을 받는 것만으로 제목이 바뀌지 않는다. **사내 실제 GHE 표기는 `NOT RUN`** — 외부에 GHE가 없어 쓰기 흐름 전부를 로컬 HTTP 목으로 검증했다 |
