...
> 상태: review | 버전: v2.6 | 갱신일: 2026-08-29
...
- 구현 범위:
  ...
  - `JOB-AUD-001` 보존 만료 파티션 드롭 — **`raw_event`(3년)와 `audit_record`(1년) 둘 다**. 관리 롤 연결 seam을 연다 (DEV-411)
...
- 완료 기준(DoD):
  ...
  - [ ] `JOB-AUD-001`이 두 테이블의 만료 파티션을 드롭하고, 재실행이 멱등하며, 드롭마다 `retention.purge`를 남긴다
