...
> 상태: review | 버전: v0.9 | 갱신일: 2026-08-26
...
- 구현 범위:
  - **W-001 커서**: PIT + `search_after`. 봉투는 스키마 버전·PIT 식별자·정렬 키 값·질의 지문·만료를 싣고 **HMAC-SHA256으로 무결성 보호**한다. 전용 서명 키를 쓰고 새 범용 crypto 추상을 만들지 않는다 (ADR-010 Amendment)
  ...
  - **`runRange`의 필터-후-절삭 정정** (DEV-270): 정본 구간을 상한 있는 chunk로 읽어 판정하고 `size`가 차거나 구간 끝까지 스캔한다. 일치가 적다는 이유로 순회가 중간에 끝나지 않는다
...
- 완료 기준(DoD):
  ...
  - [ ] **`relevance` 정렬에서 페이지 사이에 항목이 중복·누락되지 않는다** — 페이지 1과 2 사이에 새 문서를 색인해도 PIT이 뷰를 고정한다
  ...
  - [ ] **QA-W004-23 ~ QA-W004-27이 통과한다** — 특히 "일치가 첫 페이지 밖이어도 요약과 목록이 어긋나지 않는다"(DEV-270)
