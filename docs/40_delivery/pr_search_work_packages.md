...
> 상태: review | 버전: v2.26 | 갱신일: 2026-09-11
...
- 완료 기준(DoD):
  - [ ] 채번된 PR의 제목이 `[M-1900-1] <원래 제목>`이 되고 나머지 문자열이 그대로 보존된다
  - [ ] 같은 접두가 이미 있으면 GHE를 호출하지 않는다 (멱등)
  - [ ] 다른 M 넘버 접두가 있으면 덮어쓰지 않고 불일치로 기록된다
  - [ ] **표기 실패가 채번을 되돌리지 않는다** — `merge_number`는 남고 `annotate_state`만 실패로 표시된다
  - [ ] 감사 기록 `pull_request.annotate`가 남고, 감사 쓰기 실패가 표기 결과를 바꾸지 않는다
  - [ ] **조회용 Data App 자격 증명으로는 이 쓰기가 불가능하다** — 자격 증명 분리를 시험이 단언한다
  - [ ] 해제된 저장소에서 GHE 호출이 발생하지 않는다
  - [ ] 403·404가 재시도를 유발하지 않고 사유가 기록된다
  - [ ] `pending` 상태의 PR에는 쓰지 않는다
