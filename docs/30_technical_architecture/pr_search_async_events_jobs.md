...
| `JOB-GH-007` | 구현 (위 표) |
| `JOB-GH-003` capability 드리프트 점검 | **구현 (CR-088, `DEV-671`)** — `apps/gh-executor/src/registry-check.ts`. 자리는 실행기다(gh 바이너리와 DB를 둘 다 가진 프로세스). **기동 시 한 번 기다린 뒤** 구독을 세우고(20~30초, command마다 `--help`), 그 뒤 `GH_EXECUTOR_REGISTRY_CHECK_MS`(기본 1일) 주기로 돈다. 동시 1(체인), 일시 오류 재시도 3회(5·15·45초). 검사 = `validateManifest`(적재한 manifest 재계산 대조·커버리지) + `checkDrift`(실제 바이너리 해시·버전·인벤토리 해시·command/flag/JSON 필드 diff). 결과는 `gh_capability_snapshot`(해시마다 한 행)·`gh_capability_verification`(회차마다 한 행, append-only)에 남는다. 드리프트·구조 실패면 `stale` 플래그가 서고 러너의 재검증이 실행을 `registry_stale`로 거절한다(FR-GH-011 AC-3의 `execution_disabled`) — 프로세스는 종료하지 않고 헬스 `registry.stale`·지표 `gh_registry_stale`이 사실을 말한다. 일시 오류는 `stale`을 바꾸지 않는다 |
