...
**별칭 단위 PostgreSQL advisory lock으로 조정한다.** `@prs/db`의 세션 범위·트랜잭션 범위 락이 이미 있다(CR-037이 세션 범위를 신설했다). 논리 쓰기는 **시작 시점에 구체 대상을 확정**하고, 활성화와 전환은 그 확정 구간과 겹치지 않는다. 성능을 이유로 울타리를 빼려면 **먼저 정확성 증명이 있어야 한다.**
