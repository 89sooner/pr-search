...
> 상태: review | 버전: v0.21 | 갱신일: 2026-09-26
...
CR-117 / FR-SRCH-002 AC-7: 새 잡·새 이벤트·새 역할은 없다. JOB-REL-008(커밋 PR 연결 투영)이 정본을 읽을 때 **유효 연결 술어**를 지난다 — 추적 브랜치의 현재 체인에 이미 오른 커밋에서는 그 커밋을 올린 PR의 연결만 센다. 체인 소속이 바뀌는 세 자리가 그 투영의 의도를 **같은 트랜잭션에** 남긴다: JOB-SEQ-001 채번(`prepareAndAssignSequence` — 새로 오른 커밋), 강제 푸시 재채번(`reassignSequence` — 옛 에폭과 새 에폭의 `(sha, pr)` 대칭차), 복구 재채번(`repairSequence` — 같다). 세 자리 모두 `source` 근거가 있는 커밋만 세대를 올리므로 평시 채번에는 쓰기가 없다. M 번호 채번(JOB-SEQ-004)은 건드리지 않는다. 재색인(JOB-ING-006)의 PR 연결 replay와 전환 전 검증도 같은 술어를 쓰며, 재구축(`rebuildProjectedCommits`)은 체인 커밋에 원본 커밋 문서를 만들지 않는다 — 그 문서는 `rebuildCommits`가 체인에서 정한 역할로 만든다.
...
| JOB-SEQ-007 | M 번호 lightweight 태그 (CR-115) | durable `sequence_work` `tag` — 채번 트랜잭션이 PR마다 남긴다(기능 스위치와 무관; 꺼진 배포에서는 `ready`로 쌓인다) / 일 1회 잔여 스윕(`MNUMBER_TAG_SWEEP_MS`, 회차당 `MNUMBER_TAG_SWEEP_LIMIT` — `tag_state`가 NULL·`failed`·`unknown`인 현재 에폭 행) / **수동 대조 (API-ADM-002 `type: mnumber_tag_reconcile` · `prsctl mnumber tags reconcile`)** — `startTagReconcileRunner`가 잡을 claim해 정본 ↔ 원격 `M-<코드>-*`를 대조하고 `missing`을 같은 `tag` work로 재요청한다(`conflict`였던 행은 다시 열고 권한 차단은 푼다), `conflict`·`unexpected`는 보고만 | **`tag` 역할** — 원격 태그 쓰기 자격(`GHE_TAG_*`, `contents:write`)을 가진 유일한 역할이며 `annotate`와 같은 컨테이너에 뜬다(ADR-026). `sequence` 역할의 러너는 `tag` kind를 집지 않는다. 실행자 중복은 durable work의 lease가 막는다 | 시도 5회 지수 백오프. **재시도는 요청이 아니라 판단 전체를 다시 지난다** — 정본 재확인(현재 에폭·번호·SHA·저장소 정책·차단·추적 브랜치) → `GET ref` → 순수 판정 → 쓰기 간격(프로세스당 게이트, 하한 1초) → **쓰기 직전 정본 재확인** → `POST` 하나. 변경 요청의 403·404는 저장소 차단·`failed(permission_blocked)`·work `parked`(쿨다운 뒤 스윕이 재요청, 대조는 즉시 푼다), 429·대기 신호 있는 403은 게이트 pause 뒤 `retry`, 응답 없음은 `unknown` 뒤 다음 시도의 GET이 확정, 422는 ref를 다시 읽어 뜻을 정한다. `conflict`·`disabled`·`code_unavailable`은 터미널(`done`) — 스윕이 다시 두드리지 않고 대조만 다시 판정한다 | work 예산 50초 — lease 60초(heartbeat 10초)보다 짧아야 다른 워커가 회수하기 전에 손을 뗀다. 스윕에는 시간 상한이 없고 종료 신호가 회차를 끊는다 | 없음 — 정본 이벤트를 새로 내지 않는다 | FR-SEQ-012 |
...
| 정확한 키(`pr:N`·`commit:<40자>`)인데 대상 문서가 서비스 색인에 없다 | `resolved: false` 유지. 역방향 조회는 대상 T의 문서를 실시간 존재 확인한 뒤에만 붙인다 — 재구축은 T를 정본 스냅숏에서 읽으므로 문서 없는 커밋(생성 근거 없음, 손으로 전환한 인덱스의 누락)도 T로 온다. 파생·전환 전 검증의 판정(`findReferenceTargets`)과 같은 기준이다 (`CR-123`, DEV-775) |
...
**replay가 비추는 값은 유효 연결이다 (CR-117 / FR-SRCH-002 AC-7).** 열거는 원시 행으로 하되(체인 규칙으로 연결이 전부 빠진 커밋에도 `[]`를 비추려고) 값은 유효 연결 술어를 지난 번호다. 전환 전 검증도 같은 값과 대조하므로, 합집합 시절에 남은 「`git merge dev`로 받아 온 PR 번호」는 잉여로 잡혀 새 인덱스로 넘어가지 않는다.
...
#### 커밋 재구축의 순서와 기대 집합 (CR-119 / FR-ING-008 AC-10)

체인 밖 커밋(PR의 원본 커밋)의 문서는 **PR 투영만 만든다** — 보강 경로는 접근 범위를 확신할 수 없어 문서를 만들지 않는다(DEV-213). 그래서 그 커밋의 메타데이터는 문서가 생긴 뒤에만 반영된다. 평시에는 보강 방아쇠(`EVT-ING-003`)가 투영 뒤에 와서 순서가 저절로 맞지만 재구축은 그 순서를 스스로 지켜야 한다. 전에는 한 번의 스냅숏 스캔이 메타데이터를 PR 유래 문서보다 **먼저** 보내 새 인덱스에서 값이 404로 사라졌고(사내 pilot.18 재색인에서 드러난 숨은 누락), 같은 스캔이 쓰기 결과와 무관하게 모든 스냅숏 행을 기대 문서로 세어 생성 근거 없는 행이 있으면 전환이 영영 막혔다.

| 패스 | 무엇을 | 문서를 만드나 |
| --- | --- | --- |
| 체인 | 스냅숏 행 가운데 `merge_sequence`에 행이 있는 커밋 — 역할·범위와 메타데이터를 한 번에 | 만든다(`createWith`) |
| PR 유래 | PR 스냅숏의 원본 커밋과 그 PR의 유효한 `source` 연결(현재 체인 커밋 제외, CR-117), 병합된 PR의 머지 커밋 | 만든다(투영과 같은 빌더) |
| 원본 커밋 메타데이터 | 체인 밖 스냅숏 행의 메타데이터 | 만들지 않는다 — 문서가 없으면 `document_missing` |
| 관계 replay | PR 연결 (CR-116) | 만들지 않는다 |

**PR 유래 패스의 근거에 보존된 연결이 들어간다.** 관측이 불완전하면(조회 실패, 250건 절삭, 읽는 동안 head/base 변경) CR-116은 목록에서 빠진 커밋의 `source` 연결을 지우지 않는다. PR 스냅숏은 최신 목록으로 덮이므로 그 커밋은 목록에 없지만, 정본은 그 커밋이 아직 그 PR에 속한다고 말하고 서비스 인덱스에는 그 문서가 있다. 재구축이 그 문서를 만들지 않으면 관계 replay가 「문서를 만들지 못했다」로 재색인을 실패시키고, 250건을 넘는 PR은 완전성을 영영 증명할 수 없어 그 실패가 `refetch`로도 풀리지 않는다(DEV-763). 접근 범위는 그 PR의 저장소에서 온다.

**판정은 대상 인덱스의 결과로 한다.** 재구축도 `withReindexWrite`를 지나 서비스와 대상에 함께 쓰는데, 서비스 쪽 결과는 새 인덱스에 대해 아무것도 말하지 않는다. `upsertCommitMetadata`가 두 결과를 따로 돌려주고(`created`·`updated`·`already_equal`·`document_missing`, 대상 쓰기 실패는 `failed`), 재구축은 대상 결과만 본다. 대상이 없거나 실패했으면 재구축을 멈춘다 — 잡은 이미 이중 쓰기 중이 아니거나 실패로 기록됐다.
...
| JOB-SEQ-007 M 번호 태그 (잔여 스윕, CR-115) | 일 1회 (`MNUMBER_TAG_SWEEP_MS`) | 기동 30초 뒤 첫 회차, 이후 간격마다 | 주 전달은 채번 트랜잭션의 durable `tag` work다. 스윕은 현재 에폭 번호 행 중 `tag_state`가 NULL·`failed`·`unknown`인 것을 회차당 `MNUMBER_TAG_SWEEP_LIMIT`(기본 500)건씩 work로 되살린다 — **이것이 CR-115 이전 채번분의 backfill이자** 유실·죽은 프로세스의 복구다. `done`·`conflict`·`disabled`는 다시 보지 않는다: `conflict`를 다시 두드리면 손대지 않기로 한 태그에 요청만 반복하고(대조가 원격을 보고 다시 판정한다), `disabled`는 운영자가 저장소를 다시 켠 뒤 대조가 연다. 권한 차단 저장소는 쿨다운(`MNUMBER_TAG_BLOCK_COOLDOWN_MS`)을 지난 뒤에만 대상이 된다. 회차 상한과 쓰기 간격(1초) 때문에 과거 채번분이 많은 저장소는 스윕만으로는 며칠이 걸린다 — 켠 직후에는 `prsctl mnumber tags reconcile`이 `missing` 전부를 한 번에 되살린다 (RUNBOOK 7.F) |
...
| `mnumber_annotate_total{result}` | PR 제목 표기 회차 결과 (JOB-SEQ-005 / WP-075). 이 표에 빠져 있던 것을 CR-115가 메웠다(DEV-743) | `failed`·`unknown`이 30분 동안 늘고 같은 창에서 `annotated`·`already_done`이 늘지 않으면 경보 | FR-SEQ-009 AC-3 |
| `mnumber_annotate_mismatch_total` | 다른 M 넘버 접두를 발견해 덮지 않은 수 (JOB-SEQ-005). 라벨 필터 없이 경보에 건다 | 1건이라도 오르면 경고 — 사람이 판단한다 | FR-SEQ-009 AC-2 |
| `mnumber_tag_total{result}` | M 번호 태그 work 결과 (JOB-SEQ-007, CR-115). `result`는 고정 enum — `created`·`already_done`·`observed_after_unknown`·`conflict`·`code_unavailable`·`disabled`·`multiple_sequence_branches`·`permission_blocked`·`superseded`·`obsolete`·`unprocessable`·`outcome_unknown`·`rate_limited`·`deferred`·`failed`. 저장소·태그 이름은 라벨에 넣지 않는다 | `failed`·`outcome_unknown`·`permission_blocked`가 30분 동안 늘고 같은 창에서 `created`·`already_done`이 늘지 않으면 경보 (RB-28) | FR-SEQ-012 AC-5 |
| `mnumber_tag_conflict_total` | 같은 이름의 태그가 다른 SHA(또는 annotated 태그)를 가리켜 손대지 않은 수 (JOB-SEQ-007). `mnumber_tag_total{result="conflict"}`와 같은 사실이지만 라벨 필터 없이 경보에 건다 | 1건이라도 오르면 경보 P2 (RB-28) — 원격 태그가 정본과 어긋난 것이며 사람이 본다 | FR-SEQ-012 AC-3 |
| `sequence_work_total{kind="tag",outcome}` | durable 태그 work의 처리 결과 (`created`·`already_done`·`observed_after_unknown`·`conflict`·`obsolete`·`superseded`·`deferred`·`rate_limited`·`rate_limited_skip`·`permission_blocked`·`unprocessable`·`outcome_unknown`·`failed`·`parked`·`exception`·`lease_lost`·`not_this_role`) | 미수렴 결과(`retry`·`parked`·`exception`)가 30분간 늘고 종결 결과가 늘지 않으면 경보 (RB-28) | FR-SEQ-012 AC-2 |
...
| 항목 | 상태 |
| --- | --- |
| `JOB-GH-001` | 구현 — `apps/gh-executor/src/runner.ts`. 큐에서 꺼낼 때 **재검증**(manifest 해시·gh 버전 → capability 열림 → 저장소 활성·이름 불변 → 연결 살아 있음·호스트 일치·만료 전·행위자 일치 → 봉인 해제 → 같은 빌더로 argv 재조립해 저장값과 대조) → `claim` → spawn → 결과 기록. 러너는 던지지 않고 이벤트는 결과와 무관하게 `ack`된다 — 재전달하면 같은 행을 다시 본다. **CR-090:** 재검증 앞뒤로 실행 판정(`decideExecution` — 정책은 DB, 레지스트리는 실행기 인메모리 검사, 수락 revision)을 두 번 한다. 먼저 잠금 없이 판정해 막힐 요청이면 토큰을 꺼내지 않고, 재검증 뒤에는 **정책 공유 잠금(`gh:policy:<scope>`)을 쥔 한 트랜잭션**에서 정책을 다시 읽고 같은 판정을 한 뒤 claim한다 — 차단·철회(배타 잠금)가 먼저 커밋됐으면 여기서 보이고, 아직이면 이 트랜잭션이 끝날 때까지 기다린다. 운영 정책이 이유(`admin_action_required`·`policy_blocked`·`policy_changed`)면 `policy_blocked` 종료 상태로 닫고(`error` = 사유) 자동으로 다시 실행하지 않는다. 레지스트리가 이유(`registry_stale`·`registry_evidence_expired`·`registry_unchecked`)면 기존처럼 `failed`다. 정책을 읽지 못하거나 claim 트랜잭션이 정책 공유 잠금을 `lock_timeout`(10초) 안에 얻지 못하면(55P03 — 다른 정책 변경이 배타 잠금을 오래 쥠) `policy_unavailable`로 보고 닫지 않고 `queued`로 남기며, 이때도 러너는 던지지 않는다 — 던지면 버스가 이벤트를 재시도로 돌려 같은 파티션의 뒤 이벤트까지 막는다. 잔여 스윕이 복구 뒤 다시 판정한다. 지표 `result="policy_unavailable"`은 두 원인을 합치고 로그의 `reason`(`policy_lock_timeout`)이 가른다. claim UPDATE는 DB 가드 트리거(PRS11)가 한 번 더 보고, 거절되면 `policy_changed`로 닫는다. gh는 이 트랜잭션 밖에서 띄운다 |
