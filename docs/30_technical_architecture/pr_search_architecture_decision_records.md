...
### Amendment — CR-116 PR↔커밋 관계의 정본은 PostgreSQL이다 (2026-09-23)

**문제.** 이 결정의 불변 조건은 「Elasticsearch의 모든 엔티티 문서는 PostgreSQL 데이터만으로 재구성 가능해야 한다」였다. 커밋 문서의 `pull_request_numbers`는 그 조건이 **성립하지 않던 자리**였다. 그 배열은 업서트 스크립트의 합집합(`params.union`)으로만 자랐고(CR-011, DEV-019), PostgreSQL 어디에도 「이 커밋을 소유하는 PR 전부」를 말하는 행이 없었다. `pull_request_snapshot.document`는 **최신 스냅숏 하나**라 갱신이 지워야 할 옛 `source_commit_shas`를 그 순간 덮어쓴다. 그래서 두 가지가 동시에 깨졌다 — 합집합에는 한 번 더해진 번호를 **빼는 경로가 없었고**(PR이 rebase되어 원본 커밋 목록에서 빠진 커밋에도 그 번호가 영영 남았다), 정본이 없으니 **재구축으로 고칠 수도 없었다**. 사내 `0.1.0-pilot.17` 실측: PR 279개, 잘못 연결된 커밋 3,481건(1차)과 660건(웹훅 재발생분), 열린 PR #2355 하나가 dev 체인 커밋 110개에 붙어 있었다. DEV-019가 적은 근거 「집합 소속은 단조 증가하고 순서에 무관하다」가 사실이 아니었다 — rebase는 소속을 줄인다.

**결정.** PR↔커밋 소속은 파생이 아니라 정본이다. 다음을 요구한다.

1. **관계의 정본은 PostgreSQL에 있다.** `pull_request_commit_link`(ENT-CORE-009)가 PR별 소유를 담고 근거(`source`·`merge`)를 키에 두어 원본 목록에서 빠져도 실제 병합 근거는 남는다. 커밋 문서의 `pull_request_numbers`는 그 정본의 파생이며, 색인에만 있는 관계는 더 이상 없다.
2. **파생은 합집합이 아니라 대입이다.** 소유자는 커밋별 전용 투영기(`packages/es/src/commit-links.ts`의 `applyCommitLinks`)이고 입력은 정본이 계산한 **전체 집합**이다. 한 PR의 투영이 전체 배열을 쓰면 같은 커밋의 다른 PR 연결이 사라지므로(N:M), 쓰는 단위는 PR이 아니라 커밋이다. `UpsertRequest.union`은 이 필드를 타입과 실행 시점 둘 다로 막는다.
3. **삭제에는 완전성 근거가 필요하다.** 「목록에 없다」가 「속하지 않는다」를 뜻하려면 그 목록이 원격의 전부여야 한다. `pull_request_link_observation.commits_complete`(ENT-CORE-010)가 그 판정이며 **삭제 권한** 그 자체다 — 거짓이면 그 관측은 관계를 더할 수는 있어도 뺄 수 없다. 근거 없는 기존 스냅숏에서 seed한 관측은 전부 `unverified`이고, 소급해서 권한을 주지 않는다.
4. **관계에는 관계 전용 세대가 있다.** 커밋의 `document_version`은 웹훅 수신 시각이라 **다른 PR의 이벤트 시각이 이 PR의 관계를 막거나 통과시킨다.** `commit_link_state`(ENT-CORE-011)의 세대가 그 자리를 대신하고, 색인에는 `pr_links_generation`·`pr_links_state`로 실린다. 같은 세대에 다른 집합이 색인에 있으면 덮어쓰지 않고 충돌로 기록한다 — 구버전 합집합 writer가 아직 도는지를 그것으로 안다.
5. **수렴과 재구축은 CR-113과 같은 규율을 따른다.** 관계 채택·세대 올림·투영 의도는 스냅숏과 **같은 트랜잭션**이고 머지 게이트 **앞**이라 열린 PR도 처리한다. 재색인은 정본에서 관계를 복원하며(대상 인덱스를 직접 읽고 쓴다 — 서비스 인덱스를 별칭으로 거치면 그쪽 충돌이 새 인덱스의 쓰기를 막는다, DEV-750), 전환 전 검증은 누락뿐 아니라 **잉여** PR 번호도 양방향으로 대조한다.
6. **빈 배열은 사실이다.** 검증한 범위에서 연결이 0개이면 `[]`를 대입하고 필드를 지우지 않는다 — 필드의 부재(아직 모름)와 다른 사실이며, `exists` 질의가 둘을 같게 보아서는 안 된다.

**범위.** 여기서 말하는 관계는 커밋 문서가 싣는 **PR↔커밋 소속**(`pull_request_numbers`)이다. ADR-009의 follow-up(CR-039)이 「간선은 재파생 가능하므로 PostgreSQL 간선 표를 두지 않는다」고 정한 `prs-links`의 관계 간선은 이 개정의 대상이 아니며, 그 축의 재구축 근거는 여전히 `pull_request_snapshot`·`commit_snapshot`이다. 이 개정은 그 간선이 아니라, 스냅숏만으로는 복원할 수 없던 소속 자체를 정본으로 옮긴다.

**한계.** `role` 필드의 소유권 이전은 이 개정의 범위 밖이다(DEV-752, open) — 원본 목록에서 빠진 커밋의 `role: source_commit`은 그대로 남는다. 롤링 업데이트 중 남은 **구버전 워커는 여전히 합집합을 쓰고**, 새 세대 가드로 막을 수 없다. 지표 `commit_link_conflict_total`이 그 조기 경보이며, 복구를 실행하기 전에 모든 워커가 새 빌드여야 한다. old-binary 롤백은 오염을 재발시킨다. 마이그레이션 036의 down은 PostgreSQL 표만 지우고 색인의 `pr_links_generation`은 남긴다.

정본: FR-SRCH-002 AC-6, FR-ING-004 AC-6, FR-ING-008 AC-9, ENT-CORE-009·ENT-CORE-010·ENT-CORE-011(마이그레이션 036), JOB-REL-008, RB-29, WP-101.
...
**정렬 키가 문서 자신의 필드라는 것은 그 값이 불변이라는 뜻이 아니다.** CR-099가 `pr_number`를 더한 현재 정렬 키 아홉 중, 조사 중 값이 바뀌지 않는 것은 `created_at`과 `pr_number`다.
...
- 구현 판 (CR-090 / WP-080, 2026-09-14): 위험도 모델 앞에 **운영 정책 층**을 두었다 — 실행 허용 = 기능 켜짐 ∧ 현재 적재 정의의 운영 승인 ∧ 코드·manifest 실행 목록 ∧ 운영자 차단 없음 ∧ 레지스트리 판정 ∧ 사용자 권한. 판정식은 순수 함수 `decideExecution` 하나이고 API 수락과 실행기 claim이 같은 식을 부르며, claim은 정책 공유 잠금을 쥔 트랜잭션에서 수락 revision을 대조한다. 정책 쓰기는 `SECURITY DEFINER` 함수 하나(revision·이력·감사가 한 트랜잭션)이고 DB 가드 트리거가 이전 앱 경로의 실행권 확정을 거절한다. 운영 승인은 이 ADR의 결정(옵션 3)을 바꾸지 않으며 실행 목록을 넓히지 않는다.
