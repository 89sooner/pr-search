...
**3. 점수 정렬에는 Point In Time이 필요하다.** 이 결정은 `search_after`가 "결과 크기와 무관하게 일정한 성능"을 준다고 적었고 그것은 참이지만, **살아 있는 인덱스 위에서는 일관성을 주지 않는다.** 새 문서가 색인되면 BM25의 term statistics가 바뀌어 같은 문서의 `_score`가 페이지 사이에서 달라지고, 그러면 항목이 중복되거나 누락된다. `relevance` 정렬은 첫 페이지에서 PIT을 열어 색인 뷰를 고정한다(keep-alive 5분, 페이지마다 갱신, 마지막 페이지에서 close). 만료된 PIT은 `cursor_invalid`이며 화면은 첫 페이지로 되돌아간다. 다른 정렬 키는 값이 불변이거나 문서 자신의 필드라 이 문제가 없다.
