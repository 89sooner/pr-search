...
- `outcome`은 `created` 또는 `unchanged`다. 뒤엣것은 **멱등 재시도**이며 아래 「멱등과 동시성」이 정의한다.
...
| 3 | 본문 형식 (`repository` 형식, `base_branch` 비어 있지 않음, `merge_seq` 1 이상 정수, `seq_epoch` 1 이상 정수, `note` 500자 이하) | 400 `INVALID_PARAMETER` |
...
- 현재 표식이 이미 그 넷과 같으면 **아무것도 쓰지 않는다.** 새 이력 행을 만들지 않고, 기존 행을 `outcome: "unchanged"`로 그대로 반환하며, **감사 기록도 남기지 않는다.** `note`만 다른 재요청도 같다 — 멱등 키에 `note`가 없는 것은 메모가 "어느 지점까지 검증됐는가"라는 사실을 바꾸지 않기 때문이다.
- 현재 표식이 다르면 **이력과 감사를 만든다.** 직전 표식의 `superseded_at`을 기록하고 새 행이 current가 된다.
...
- 오류: `UNAUTHENTICATED` (401), `FORBIDDEN_ROLE` (403), `INVALID_PARAMETER` (400), `NOT_FOUND` (404), `SEQUENCE_EPOCH_STALE` (409, `detail.reason: "epoch_stale"`·`current_seq_epoch`·`requested_seq_epoch`), `SEQUENCE_NOT_FOUND` (400, `detail.merge_seq`·`seq_epoch`)
