...
> 상태: review | 버전: v0.18 | 갱신일: 2026-08-30
...
- 오류: **`JOB_CONFLICT`** (409) — 같은 `(type, target)`에 활성 잡이 이미 있다. 부분 유니크 인덱스가 DB에서 강제하므로 경합에서도 둘이 뜨지 않는다. **`detail.job_id`에 실행 중 잡의 식별자를 싣는다** (AC-4, PR #89 리뷰 P2): 대상 문자열만 돌려주면 화면이 "이미 돌고 있다"까지만 말하고 운영자가 그 잡을 찾아갈 곳이 없다. 경합에서 진 요청도 이긴 행을 다시 읽어 채운다
