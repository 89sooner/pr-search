...
| API-SRC-002 | `/history` | ref, path, page(1~1000) | 고정 revision, 경로 커밋50개(행별 `pull_request_numbers`: 배열=확정/`null`=미확정, CR-107 — 배열의 뜻은 **현재 유효한 연결**이다, CR-116), `pull_requests_unavailable`(조회 실패·미배선 시), next_page |
...
**GitHub Operations Plane (CR-005 신규).** 아래 API는 `search-api`가 아니라 Operations 경로에 속하며, 인증은 세션 인증에 더해 사용자 위임 GitHub 신원(FR-GH-008)을 요구한다. 운영 화면의 조회·변경(`API-GH-008`·`API-GH-013`·`API-GH-014`)은 위임 신원을 요구하지 않고 역할로 닫는다 (CR-088·CR-090).

| API ID | Method | Path | 목적 | 인증/권한 | 관련 FR |
| --- | --- | --- | --- | --- | --- |
| API-GH-001 | GET | `/gh/capabilities` | capability manifest 조회·검색. 분류 상태와 호스트 지원 여부 포함. CR-089: `commands[]`에 `composability`, 실행 정의에 `result_adapter`(구현 adapter)와 `result_contract` 요약(`kind`·`sensitivity`·`composability`·`bindable`·`resource_kind`) — 실행 정의의 `result`는 없어졌다. CR-090: 실행 정의마다 `execution_gate`(`allowed`·`reason`·`detail`·`revision`) — 실행기 claim과 같은 판정이며, 정책을 읽지 못해도 목록은 답하고 판정만 `policy_unavailable`이다 | 인증 | FR-GH-001, FR-GH-011, FR-GH-013 |
| API-GH-002 | POST | `/gh/executions` | 명령 실행 요청. `Idempotency-Key` 헤더 필수(8~128자 `[A-Za-z0-9_-]`, 본문 `idempotency_key`도 받는다). **`POST /gh/executions/preview`**(CR-086)는 실행과 같은 준비 단계(중복 키·발행 제외)를 지나되 큐에 넣지 않고 argv·환경 키·유효 컨텍스트·`executable`·`blockers`를 낸다. CR-090: 미리보기에 `gate`가 붙고 판정이 허용이 아니면 그 사유가 `blockers` 맨 앞이다. 실행 수락은 판정이 허용일 때만 한다 — 아니면 `GH_ADMIN_ACTION_REQUIRED`(409)·`GH_POLICY_BLOCKED`(403)·`GH_REGISTRY_STALE`(409)·`GH_POLICY_UNAVAILABLE`(503)이고 `detail`에 `reason`·`detail`·`policy_revision`이 있다. 수락한 실행 기록은 판정의 `policy_revision`을 적는다 | 인증 + 위임 신원 + 위험도 정책 + 운영 정책(CR-090) | FR-GH-002, FR-GH-003, FR-GH-009, FR-GH-011 |
...
| API-GH-008 | GET · POST | `/gh/policies` · `/gh/policies/changes` | 실행 정책 조회·변경 (capability 허용/차단, 위험도 재정의, 승인 필요, 엔드포인트·확장 목록). **CR-090 구현 범위: 현재 적재 정의의 운영 승인·철회와 실행이 열린 capability의 차단·재개** — 위험도 재정의·승인 필요 지정·엔드포인트·확장 목록은 아직 없다. 계획의 `PUT /gh/policies`(정책 전체 교체)는 두지 않고 행위 단위 변경만 받는다(아래 운영 정책 계약) | 조회 `operator` 또는 `security_officer` · 변경 `operator` | FR-GH-009, FR-GH-010, FR-GH-011, FR-GH-013 |
...
**운영 정책 계약 추가 (CR-090 / WP-080).** `API-GH-008`은 `apps/search-api/src/gh/policy.ts`에 있고 라우트는 `routes.ts`가 등록한다. **배포 범위는 서버 설정 `GHE_BASE_URL`의 호스트이며 요청이 지정하지 않는다.** `GET /gh/policies`(역할 `operator`·`security_officer`, 검사를 돌리지 않는다)는 `scope`·`now`(DB 시각), `served`(적재 정의: gh 버전·고정 버전·manifest 판·해시·인벤토리 해시·보고서 판·보고서 해시·검증기 판·스냅숏 ID와 최초 승인 시각), `policy`(`revision`·`updated_at`·`updated_by`·`approval`(스냅숏·기록·보고서 해시·정의·승인 시각·승인자·`matches_served`) 또는 `null`·`blocked_capabilities`), `approval_preview`(`eligible`·`reasons[]`(`code`·`detail`)·`snapshot_id`·`verification_id`·`report_hash`·`report_version`·`evidence`(기록 번호·시각·출처·계기·상태·실행기 ID·호스트명·`registry_check_ms`)·`evidence_max_age_ms`·`evidence_expires_at`·`gates[]`·`required_gates`·`opens`·`not_opened`·`scope_note`), `capabilities[]`(실행이 열린 capability마다 `blocked`·`gate`), 최근 `revisions[]` 20건(행위·대상 capability·정의·근거·행위자·사유·상관 ID·시각 — 중복 방지 키와 요청 지문은 내지 않는다), `host_verification`(`not_verified`)을 낸다. `POST /gh/policies/changes`(역할 `operator`만 — `security_officer`만 가진 사용자는 403 `FORBIDDEN_ROLE`)는 **`application/json` 본문만** 받고 `Idempotency-Key`(본문 `idempotency_key`도 받는다)가 필수다. 본문은 `{ action: approve | revoke | block | resume, expected_revision, reason(1~500자, 제어 문자 없음), capability_id(block·resume — 코드와 manifest가 실행을 연 capability만), snapshot_id·verification_id·report_hash(approve — 미리보기의 값 그대로) }`다. 서버는 한 트랜잭션·배타 잠금(`gh:policy:<scope>`) 안에서 같은 키의 기존 revision → 기대 revision → (approve) 미리보기 묶음과 자격을 다시 판정한 뒤 DB 함수 `gh_operations_policy_apply`를 부르고, 함수가 같은 사실을 잠금 아래 DB에서 다시 확인해 상태·revision 이력·감사를 함께 쓴다. 성공은 200 `{ outcome: applied | replayed, revision, policy, capabilities }`이며, 같은 키·같은 내용의 재요청은 `replayed`로 아무것도 더 쓰지 않는다. 거절은 400 `INVALID_PARAMETER`(`detail.field`), 403 `FORBIDDEN_ROLE`, 409 `GH_POLICY_CONFLICT`(`detail.reason`: `revision_changed`와 `current_revision` · `evidence_changed`와 현재 근거 · `evidence_not_latest` · `already_approved` · `not_approved` · `already_blocked` · `not_blocked`), 409 `GH_REGISTRY_APPROVAL_INELIGIBLE`(`detail.reasons[]`), 409 `GH_DUPLICATE_REQUEST`(`idempotency_key_reused`), 503 `GH_POLICY_UNAVAILABLE`(호스트 미구성, 또는 다른 정책 변경이 배타 잠금을 `lock_timeout` 10초 넘게 쥐어 `detail.reason = policy_lock_timeout` — 트랜잭션이 롤백돼 적용된 것이 없으므로 새 키로 다시 제출해도 두 번 적용되지 않는다)다. **그 밖의 DB 오류는 처리하지 않은 오류(500)다** — 변경 경로에서 정책 표를 읽지 못한 경우(조회 `GET /gh/policies`에서는 503)와, 적용이 커밋된 뒤 응답에 실을 상태를 다시 읽다 실패한 경우가 여기에 든다. 호출자는 같은 키·같은 본문으로 다시 보내 결과를 확인한다 — 이미 적용됐으면 `replayed`이고, 아니면 그때의 판정을 받는다. web은 503이 아닌 5xx에서 키를 유지해 그렇게 한다. **적용 감사는 DB 함수가 변경과 같은 트랜잭션에서 한 행 남기고(FR-AUTH-004 AC-6 예외), 거절은 결과 코드(`forbidden`·`invalid`·`conflict`·`ineligible`·`key_reused`)로 best-effort 한 행, `replayed`는 0행이다.** 거절 감사의 액션은 본문 `action`에서 정하므로 `action`이 없거나 네 값이 아니면 남기지 않는다(정본 어휘에 없는 액션을 만들지 않는다). 세션 없음(401)과 503은 결정의 거절이 아니므로 남기지 않는다. 기능이 꺼지면 두 경로 모두 등록되지 않아 404다.
...
### API-SEQ-004 안전 구간 표식 (CR-057, DEV-458)

- 목적: 시퀀스 공간마다 "여기까지는 검증됐다"는 지점 하나를 조직이 공유한다.
- 관련 요구사항: FR-SEQ-006
- 신설 사유: 이 API는 3장 카탈로그의 한 행으로만 존재했고 요청·응답·오류가 정의된 적이 없다 (DEV-458). 그 공백이 에폭 불일치 쓰기의 오류 어휘(DEV-459), `sequence_not_found`의 오류 모델 등재(DEV-460), 읽기와 쓰기의 권한 구분(DEV-461), 멱등 키의 뜻(DEV-463)을 함께 비워 두고 있었다.

**읽기와 쓰기의 자격이 다르다** (DEV-461). `FR-SEQ-006` AC-3이 403으로 정한 것은 **쓰기 요청**이다. 조회는 인증된 사용자가 자기 접근 범위 안에서 하며, 그 이상의 제한은 승인된 적이 없다 — 표식은 조직이 공유하는 판정이고, 볼 수 없으면 공유가 성립하지 않는다.

요청 (조회): `GET /api/v1/safe-markers?repository=acme/payments&base_branch=main`

**`seq_epoch`를 받지 않는다.** `API-SEQ-001`·`003`이 그것을 받는 것은 **구간 인용이 딛고 선 세대**를 고정하기 위해서인데, 이 조회가 묻는 것은 "지금 이 공간의 현재 표식이 무엇인가" 하나다. 에폭을 받으면 "그 세대의 표식을 달라"는 뜻이 되고, 그것은 이력 조회이며 `FR-SEQ-006`이 승인한 것이 아니다.
...
```json
{
  ...
  "marker": {
    "merge_seq": 1342,
    "seq_epoch": 3,
    "note": "결제 회귀 스위트 통과 — 2026-08-14 릴리스 검증 완료",
    "created_by": "kim",
...
응답 200 (표식 없음): `marker`가 `null`이다. **404가 아니다** — 공간은 실재하고 그 공간에 아직 표식이 없다는 것이 사실이며, 404는 "그런 공간이 없다"를 뜻한다.

- 봉투의 `seq_epoch`는 **그 공간의 현재 에폭**이고 `marker.seq_epoch`는 **표식이 저장될 때의 에폭**이다. 이름이 같은 두 값이 다른 것을 말하므로 계층이 다르다.
- `marker.epoch_stale`은 두 값의 비교 결과다 (`FR-SEQ-005` AC-4). **행을 고쳐 계산하지 않는다** — 재채번은 표식에 아무것도 쓰지 않고 (백엔드 아키텍처 4.3, CR-026 DEV-126), 조회가 현재 에폭과 대조해 그 자리에서 판정한다.
- **낡은 표식을 감추지 않는다.** 무효라는 사실 자체가 사용자가 알아야 할 답이며, 숨기면 "표식이 없다"로 읽힌다. 반대로 **현재 에폭의 같은 숫자로 자동 재해석하지도 않는다** — 그 서수는 다른 커밋을 가리킬 수 있고, 옮기는 것은 언제나 사용자의 명시적 행위다 (ADR-007).
- 접근 범위 밖 저장소와 미등록 저장소, 채번된 적이 없는 브랜치는 **모두 404 `NOT_FOUND`이며 메시지가 같다** (ADR-008, THR-006). 다르면 비공개 저장소의 존재가 샌다.

요청 (등록): `PUT /api/v1/safe-markers`
...
```json
{
  ...
  "seq_epoch": 4,
  "note": "결제 회귀 스위트 통과",
  "expected_marker_seq": 1280
}
```

- `seq_epoch`는 **필수다.** 생략을 허용하면 서버가 현재 에폭을 대신 채우게 되고, 사용자가 본 세대와 저장되는 세대가 갈릴 수 있다 — `API-SRCH-005`가 저장된 검색에서 같은 이유로 이 값을 본문에서 받는다.
- **`created_by`와 `created_at`은 서버가 만든다.** 클라이언트가 등록자나 등록 시각을 지정할 수 없다 — 그 둘을 받으면 감사 기록이 주장하는 사실을 요청자가 쓰게 된다.
- `note`는 선택이며 500자 이하다 (`AC-2`). 생략하면 `null`이다.
- **`expected_marker_seq`는 필수이며 요청자가 본 현재 표식의 서수다.** 표식이 없는 상태를 봤으면 `null`을 명시한다 — **키를 빼는 것과 `null`을 보내는 것은 다르다**: 앞엣것은 "확인하지 않았다"이고 뒤엣것은 "없는 것을 봤다"이다. 값이 현재와 다르면 서버가 `SAFE_MARKER_CONFLICT`(409)로 거절한다.

**왜 이 필드가 필요한가** (CR-057, DEV-464). 이 필드가 없으면 **응답을 잃은 요청의 재시도가 그 사이의 갱신을 조용히 되돌린다.** `1342`를 등록한 응답이 유실되고, 다른 매니저가 `1400`으로 올린 뒤, 원 요청이 재시도되는 경우다 — 현재 표식만 보고 판정하면 "다르다"이므로 `1342`가 다시 current가 되고 **조직이 공유하는 안전 경계가 아무 오류 없이 뒤로 물러난다.** 표식을 뒤로 옮기는 것 자체는 정당한 동작이므로(AC-1은 단조 증가를 요구하지 않는다) 값만으로는 사고와 의도를 가를 수 없고, **요청자가 무엇을 보고 눌렀는가**가 그 둘을 가르는 유일한 재료다.

**요청 멱등 키를 저장하는 대안은 택하지 않았다.** 그 방법은 새 표나 열을 요구하고(마이그레이션), 무엇보다 **다른 문제를 푼다** — 재시도의 중복은 막지만 두 사람이 같은 표식을 동시에 겨루는 잃어버린 갱신은 그대로 남는다. 위 시나리오에서 유실이 없었더라도, 두 매니저가 각자 본 화면을 근거로 누르면 나중 요청이 먼저 것을 말없이 덮는다.
...
```json
{
  ...
  "marker": {
    ...
    "note": "결제 회귀 스위트 통과",
    "created_by": "kim",
    "created_at": "2026-08-31T02:40:11Z",
    "epoch_stale": false
  },
  "replaced_merge_seq": 1280,
  "correlation_id": "0f0a1b2c-3d4e-5f60-7182-93a4b5c6d7e8"
}
```

- `outcome`은 `created` 또는 `unchanged`다. 뒤엣것은 **멱등 재시도**이며 아래 「멱등과 동시성」이 정의한다. `note`만 바뀐 요청은 `created`다 — 그것은 재시도가 아니라 변경이기 때문이다.
- `replaced_merge_seq`는 이 등록이 대체한 직전 표식의 서수이며, 첫 표식이거나 `unchanged`면 `null`이다. **이전 표식 전체를 싣지 않는다** — 그것은 이력 조회이고 `FR-SEQ-006`이 요구하는 것은 이력 **보존**이지 이력 조회 API가 아니다.

**서버가 강제하는 것과 그 순서.** 화면의 `canWrite`는 보안 경계가 아니다 — 직접 보낸 `PUT`도 같은 검사를 지난다.
...
**5가 6보다 먼저다.** 에폭이 다르면 그 세대의 `merge_seq` 실재 여부는 물을 필요가 없고, 물으면 **낡은 에폭의 서수를 현재 에폭에서 찾게 되어** 있지도 않은 값을 `sequence_not_found`라 답한다. 두 오류가 사용자에게 뜻하는 것이 다르다 — 앞엣것은 "다시 조회하라"이고 뒤엣것은 "그 서수는 없다"이다.

**7이 8보다 먼저다.** 순서를 뒤집으면 **정직한 재시도가 거절된다** — 응답을 잃은 요청이 같은 본문으로 다시 오면 그 `expected_marker_seq`는 이미 낡았고(자기 자신이 그것을 바꿨다), 8을 먼저 보면 409가 된다. 7이 먼저면 "이미 그 상태다"로 성공하고, 그 사이 **다른 값**이 끼어든 경우에만 8이 걸린다. 두 검사가 함께 있어야 재시도의 멱등과 잃어버린 갱신의 차단이 동시에 성립한다.

**7·8은 원자적이어야 한다.** 읽고 비교한 뒤 쓰기까지가 하나의 임계 구역이며, 그 사이에 다른 `PUT`이 끼어들면 두 검사가 본 값과 실제로 대체되는 값이 갈린다.

#### 멱등과 동시성 (CR-057, DEV-463)

백엔드 아키텍처 6.2가 이 액션의 멱등 키를 `(repo, branch, seq, epoch)`로 적는다. **그 값이 뜻하는 것을 여기서 정한다.**

**그 키가 판정하는 것은 "같은 요청인가"이지 "아무것도 바뀌지 않았는가"가 아니다.** 둘을 같은 것으로 보면 메모 수정이 사라진다 — 아래 두 규칙이 그 경계다.

- 현재 표식이 요청과 **완전히 같으면**(`merge_seq`·`seq_epoch`·`note` 셋 다) **아무것도 쓰지 않는다.** 새 이력 행을 만들지 않고, 기존 행을 `outcome: "unchanged"`로 그대로 반환하며, **감사 기록도 남기지 않는다.**
- **`note`만 다르면 그것은 변경이다** (CR-057, DEV-465). 이력과 감사를 만든다. `FR-SEQ-006` AC-2가 메모를 표식의 구성 요소로 두고 AC-5가 **"등록·변경"**을 감사 대상으로 요구하므로, 메모 수정을 멱등 재시도로 접으면 **승인된 감사 대상 하나가 기록되지 않고 사용자가 잘못 적은 근거를 고칠 방법도 없다.** 6.2 표의 멱등 키에 `note`가 없는 것은 그것이 **같은 요청을 알아보는 재료**이기 때문이지 메모가 사소해서가 아니다.
- 서수나 에폭이 다르면 물론 변경이다. 직전 표식의 `superseded_at`을 기록하고 새 행이 current가 된다.
- **`safe_marker_current_uk`만으로는 이 의미가 성립하지 않는다.** 그 partial unique index가 강제하는 것은 "현재 표식은 최대 하나"뿐이며, 같은 값의 재등록도 이력을 하나 더 만드는 것을 막지 않는다. 멱등 판정은 대체 직전에 현재 행을 읽어서 한다.
- **현재 행과의 비교만으로는 재시도의 멱등이 시간을 건너 성립하지 않는다** (DEV-464). 그래서 `expected_marker_seq`가 함께 온다 — 위 「왜 이 필드가 필요한가」와 검사 순서 7·8이 그 규칙이다.
- **같은 시퀀스 공간의 동시 `PUT`은 직렬화된다.** 대체는 하나의 트랜잭션이며 다음 셋을 관측 가능한 사실로 보장한다: (1) 어느 시점에도 current가 둘이 되지 않는다, (2) 어느 시점에도 current가 **영구히 0개로 남지 않는다** — 실패한 `PUT`은 직전 current를 그대로 둔다, (3) 첫 등록이 동시에 둘 오더라도 유일 제약 충돌로 **둘 다 실패하지 않는다.** 직렬화 수단은 `(repository_id, base_branch)` 단위 잠금이며 유일 제약을 그 대신 쓰지 않는다 — 제약은 잘못을 거절할 뿐 순서를 만들지 않는다.

#### 감사 (FR-SEQ-006 AC-5)

성공한 등록은 `safe_marker.set`을 남긴다. `target`은 `{시퀀스 공간}@{시퀀스 값}`이고 `query`는 `null`이다 (`FR-AUTH-004` AC-1 정본 표). **`note`를 감사에 싣지 않는다** — 그 칸은 "행위가 적용한 조건"을 담고 메모는 조건이 아니다.

**거절도 남기되 성공으로 남기지 않는다.** 저장된 검색이 상한 거절을 기록하는 것과 같은 이유로(그 사실이 "왜 저장되지 않았는가"의 답이다) **3~6과 8의 거절**을 `result_code`에 사유를 담아 기록한다. 401·403은 기록하지 않는다 — 세션도 자격도 없는 요청이라 남길 주체가 없거나(401), 그 기록이 역할 없는 사용자의 반복 요청으로 감사 평면을 채운다(403). **7은 거절이 아니라 성공이며, 아무것도 바뀌지 않았으므로 기록하지 않는다** — 남기면 재시도 횟수가 등록 횟수로 보인다.

`result_code`의 어휘는 다음 여섯이다. **감사 로그를 읽는 사람이 이 목록 없이는 사유를 복원할 수 없으므로 계약이 소유한다.**

| `result_code` | 무엇 | `target` |
| --- | --- | --- |
| `created` | 등록·대체 성공 | `{시퀀스 공간}@{시퀀스 값}` |
| `invalid:{필드}` | 검사 3 — 본문 형식 | `null` (무엇을 가리켰는지 확정되지 않았다) |
| `not_found` | 검사 4 — 저장소·범위·공간 | `null` |
| `epoch_stale` | 검사 5 | `{시퀀스 공간}@{요청 서수}` |
| `sequence_not_found` | 검사 6 | `{시퀀스 공간}@{요청 서수}` |
| `marker_conflict` | 검사 8 | `{시퀀스 공간}@{요청 서수}` |

`target`이 `null`인 두 자리는 **공간이나 서수를 아직 확정하지 못한 거절**이다. 지어내지 않는다 — "없다"와 "빈 값으로 기록됐다"가 같은 모양이 되면 감사가 사실을 말하지 못한다 (FR-AUTH-004 AC-2).

- 오류: `UNAUTHENTICATED` (401), `FORBIDDEN_ROLE` (403), `INVALID_PARAMETER` (400), `NOT_FOUND` (404), `SEQUENCE_EPOCH_STALE` (409, `detail.reason: "epoch_stale"`·`current_seq_epoch`·`requested_seq_epoch`), `SEQUENCE_NOT_FOUND` (400, `detail.merge_seq`·`seq_epoch`), `SAFE_MARKER_CONFLICT` (409, `detail.current_marker_seq`·`expected_marker_seq` — 화면은 그 값을 보이고 사용자가 다시 결정한다. **자동 재시도하지 않는다**)
...
- 저장소 필드를 펼치고 **이번 조작이 만든 것을 곁들인다**: `documents_marked`, `backfill_job_id`, `fulfilled_request_ids`, `sequence_job_ids`
...
**`reconcile`의 대상은 클라이언트가 고르지 않는다.** `FR-ING-011` AC-2가 스캔 대상을 "등록된 각 저장소"로 정했으므로 이 잡의 대상은 언제나 등록 저장소 전체이며, 서버가 `all`을 쓴다. 저장소 식별자는 언제나 `/`를 포함하므로 충돌하지 않는다.
...
- 오류: **`JOB_CONFLICT`** (409) — 같은 `(type, target)`에 활성 잡이 이미 있다. 부분 유니크 인덱스가 DB에서 강제하므로 경합에서도 둘이 뜨지 않는다. **`detail.job_id`에 실행 중 잡의 식별자를 싣는다** (AC-4, PR #89 리뷰 P2): 대상 문자열만 돌려주면 화면이 "이미 돌고 있다"까지만 말하고 운영자가 그 잡을 찾아갈 곳이 없다. 경합에서 진 요청도 이긴 행을 다시 읽어 채운다. **이긴 행이 그 사이 끝났으면 충돌이 아니다** (PR #91 리뷰 P2, DEV-444): 부분 유니크 인덱스는 활성 상태에만 걸리므로 그 행이 종료되는 순간 제약도 사라진다 — 서버는 식별자 없는 409를 내지 않고 **큐잉을 다시 시도한다.** `detail.job_id`가 빠진 `JOB_CONFLICT`는 이 계약에 없다
- 오류: `NOT_FOUND` (404) — 등록되지 않은 저장소. **잡을 만들어 두지 않는다**: 만들면 워커가 잡을 때마다 실패하고 운영자는 원인이 미등록임을 알 수 없다
- 오류: `INVALID_PARAMETER` (400) — 알 수 없는 `type`

**`PATCH`는 잡 유형이 좁힌 목록을 강제한다** (AC-7, PR #89 리뷰 P2). `allowed_actions`는 화면을 위한 안내이자 **변이 경로의 판정 그 자체**다 — 응답에서만 빼고 `PATCH`가 일반 전이표로 받으면 직접 호출하는 클라이언트가 그것을 지나간다. `reconcile`을 `paused`로 옮기면 러너에 멈출 지점이 없어 전량 스윕은 완주하고 행만 `paused`가 되며, `resume`이 같은 스캔을 다시 큐에 넣는다. 지원하지 않는 동작은 400 `INVALID_PARAMETER`이고 `detail.allowed_actions`에 가능한 것을 담는다.
...
### API-ADM-005 감사 기록 조회

- 목적: 보안 담당자가 누가 무엇을 조회·변경했는지 조사한다.
- 관련 요구사항: FR-AUTH-004, NFR-006
- 관련 화면: A-004

요청: `GET /api/v1/admin/audit-records`
...
권한: **`security_officer` 전용.** 그 외 역할은 403이다 (FR-AUTH-004 AC-5, NFR-006).
...
- **접근 범위 필터를 걸지 않는다.** 다른 조회 경로와 다른 이 판단은 `ADR-008`의 예외가 아니라 **대상이 다르기 때문**이다 — 감사 기록은 저장소의 내용이 아니라 **행위의 기록**이고, `security_officer`는 그 평면 전체를 조사하는 역할이다. 조사자의 저장소 접근 범위로 감사를 자르면 **자기가 볼 수 없는 저장소에서 일어난 일을 조사할 수 없게 되며**, 그것이 이 역할의 존재 이유를 없앤다. 반대 방향의 완화도 하지 않는다: `operator`나 일반 관리 토큰에 이 경로를 열지 않는다 (CR-054)
- **`target`과 `query`는 `null`을 그대로 낸다** (AC-2). 의미상 대상이 없는 액션(`audit.view`의 일부 호출, `retention.purge`의 질의)이 있고, `N/A`나 빈 문자열을 채우면 "없다"와 "빈 값으로 기록됐다"가 같은 모양이 된다
- **응답 본문은 어디에도 담기지 않는다** (보안 문서 7.1). `query`는 적용된 조건 전부를 담되 그 조회가 무엇을 돌려주었는지는 담지 않는다 — 감사 로그가 데이터 유출 경로가 되면 안 된다
...
- 감사: `repository_registration_request.dismiss`. **`target`은 요청 ID, `query`는 운영자가 남긴 처리 메모다** — `audit_record`가 가진 자유 칸은 `target`과 `query` 둘뿐이고 `FR-AUTH-004` AC-1의 정본 표도 이 액션의 메모를 `query` 칸에 배정한다. `detail` 같은 이름을 쓰면 저장할 수 없는 필드를 가리키게 되고 구현은 사유를 그냥 빠뜨린다 (PR #88 리뷰 P1).
...
| `SEQUENCE_NOT_FOUND` | 400 | 지정한 서수가 그 `(시퀀스 공간, 에폭)`에 실재하지 않음 (CR-057, DEV-460). `FR-SEQ-006` 예외 처리가 지목한 사유 코드이며 `detail`이 `merge_seq`·`seq_epoch`를 담는다 | 구간에 있는 서수 지정 |
...
| `SEQUENCE_EPOCH_STALE` | 409 | **쓰기 요청이 딛고 선 시퀀스 에폭이 현재와 다름** (CR-057, DEV-459). `detail`이 `reason: "epoch_stale"`·`current_seq_epoch`·`requested_seq_epoch`를 담는다. 조회 경로가 200에 `epoch_stale: true`를 실어 답하는 것과 **짝이되 같지 않다** — 읽기는 무효를 알리고 끝나지만 쓰기는 그 세대에 값을 남기는 일이라 실행하면 안 된다. 현재 에폭으로 조용히 옮겨 저장하지 않는다 (ADR-007) | 현재 에폭으로 다시 조회한 뒤 재요청 |
...
| `SAFE_MARKER_CONFLICT` | 409 | **요청자가 본 안전 구간 표식이 더 이상 현재가 아님** (CR-057, DEV-464). `detail`이 `current_marker_seq`·`expected_marker_seq`를 담는다. 표식을 뒤로 옮기는 것 자체는 정당하므로 값만으로는 사고와 의도를 가를 수 없고, **요청자가 무엇을 보고 눌렀는가**가 그 둘을 가른다 | 현재 표식을 확인한 뒤 다시 결정 (자동 재시도 금지) |
...
| `GH_DUPLICATE_REQUEST` | 409 | 동일 중복 방지 키의 재요청. CR-090: 운영 정책 변경에서 같은 키를 **다른 내용**에 다시 쓰면 이 코드이며 `detail.reason = idempotency_key_reused`(그 키가 만든 `revision` 포함) — 같은 내용의 재요청은 200 `replayed`다 | 기존 실행 확인. 정책 변경은 새로 읽고 새 키로 제출 |
...
| `GH_POLICY_UNAVAILABLE` | 503 | 운영 정책 상태를 읽지 못해 새 실행을 허용하지 않는다 (CR-090, FR-GH-011 AC-9). 다른 API는 계속 응답한다. 운영 정책 변경(`API-GH-008`)에서는 호스트 미구성, 또는 다른 변경이 정책 잠금을 오래 쥔 경우다(`detail.reason = policy_lock_timeout` — 적용·감사 없음) | 잠시 후 재시도. 계속되면 마이그레이션 030 적용 여부 확인. 잠금 대기 초과면 상태를 다시 확인하고 제출한다 |
