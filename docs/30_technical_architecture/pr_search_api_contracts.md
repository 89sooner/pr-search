...
> 상태: review | 버전: v0.21 | 갱신일: 2026-08-31
...
**거절도 남기되 성공으로 남기지 않는다.** 저장된 검색이 상한 거절을 기록하는 것과 같은 이유로(그 사실이 "왜 저장되지 않았는가"의 답이다) 3~6의 거절은 `result_code`에 그 사유를 담아 기록한다. 401·403은 기록하지 않는다 — 세션도 자격도 없는 요청이라 남길 주체가 없거나(401), 그 기록이 역할 없는 사용자의 반복 요청으로 감사 평면을 채운다(403). **어느 경우에도 거절이 `result_code`를 성공으로 갖지 않는다.**
