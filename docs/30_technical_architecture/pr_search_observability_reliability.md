...
| 감사 기록 실패 | 감사 저장 실패 5분 지속 | P2 | RB-18 |
...
| RB-15 | 원본 저장 용량 부족 | ① 파티션별 크기 확인 ② 보존 기간 정책 확인(OD-003) ③ 승인 시 오래된 파티션 드롭 ④ 볼륨 확장 |
...
| RB-18 | 감사 기록 실패 | ① PostgreSQL 쓰기 상태 확인 ② 파티션 존재 확인(월 경계) ③ 감사 누락 구간을 보안 담당자에게 보고 |
...
| ES 디스크 | 약 250GB (복제본 포함) | 노드별 디스크 사용률 | 75% 도달 |
| PostgreSQL `raw_event` | 3년 4TB | 파티션별 크기 | 볼륨 75% 도달 |
...
분기 1회 용량 리뷰에서 위 지표를 검토하고, 트리거에 걸린 항목은 CR로 설계 수치를 조정한다 (OD-007 실측 반영 포함).
