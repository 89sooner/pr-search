...
CR-116 / FR-SRCH-002 AC-6: 커밋 관계 투영(JOB-REL-008)은 지표 둘을 더한다 — `commit_link_projection_total{outcome}`(회차별 판정: `updated`·`noop`·`stale`·`conflict`·`document_missing`과 러너 판정 `vanished`·`absent_ok`·`error`·`lease_lost`)와 `commit_link_conflict_total`(같은 세대에 **다른** 관계 집합이 색인에 있어 덮지 않은 수). 두 번째 지표가 오르는 것은 데이터 문제가 아니라 **배포 문제**의 신호다 — 롤링 업데이트 중 남은 구버전 워커가 아직 합집합으로 쓰고 있다는 뜻이며, 새 세대 가드로는 막을 수 없다. 경보 「커밋 관계 투영 충돌·미수렴」(P2)과 런북 RB-29를 더했다. `commit_link_state`에 밀린 투영(`ready`·`retry`·`parked`)은 지표로 내지 않는다 — `prsctl links status`의 「밀린 투영」이 state별 잔량을 답한다.
...
| 커밋 관계 투영 충돌·미수렴 (CR-116) | `commit_link_conflict_total` 증가 — **구버전 합집합 writer가 아직 돌고 있다는 신호**이며 데이터가 아니라 배포를 먼저 본다, 또는 `commit_link_projection_total{outcome=~"conflict\|document_missing\|error\|stale"}`가 10분 동안 증가하고 같은 창에서 `outcome=~"updated\|noop"`이 늘지 않음(관계 투영이 밀린 채 수렴하지 않는다 — `commit_link_state`의 `parked` 잔량은 `prsctl links status`가 답한다) | P2 | RB-29 |
...
| RB-29 | 커밋 관계 투영 충돌·미수렴 (CR-116, CR-117) | ① **먼저 배포를 본다.** `commit_link_conflict_total`이 오르고 있으면 같은 세대에 다른 관계 집합이 색인에 있었다는 뜻이고, 그것을 쓸 수 있는 것은 **구버전 합집합 writer**뿐이다(`prsctl lineage`로 워커 이미지 판을 본다). **모든 워커가 새 빌드가 되기 전에는 복구를 실행하지 않는다** — 지우는 족족 다시 더해진다. old-binary 롤백도 같은 이유로 오염을 재발시킨다. CR-117 반입 뒤에도 같다 — 구버전 워커는 체인 규칙(FR-SRCH-002 AC-7) 없이 관계를 비추고 dev 체인 커밋의 `role`을 다시 `source_commit`으로 덮는다 ② `prsctl links status --repository <owner/name>`으로 관계 규모(커밋 수·간선 수)와 **밀린 투영**을 state별(`ready`·`retry`·`parked`)로 본다 — `parked`는 재시도 상한을 넘었거나 `generation_conflict`로 바로 보류된 것이다. 어느 쪽인지는 정본 표 `commit_link_state.last_reason`에서 본다(status 출력에는 없다) — `generation_conflict`면 ①이 아직 풀리지 않았다 ③ `prsctl links plan --repository … [--pr <번호>]`로 정본과 색인을 맞대어 무엇이 바뀔지만 센다 — **아무 데도 쓰지 않는다**(PostgreSQL·Elasticsearch·작업 큐 모두). 출력의 「바뀔 커밋(더할 간선 · 지울 간선)」이 정리 규모이고, 「근거 없어 보류」와 「완전성 근거 없는 PR」이 완전성 근거가 없어 지울 수 없는 몫이다. CR-117부터 「그중 dev 체인 커밋에서 빠질 간선: N (커밋 M)」은 체인 규칙으로 빠지는 몫이며, 근거가 완전성이 아니라 PostgreSQL의 `merge_sequence`라 관측 확정과 무관하게 지운다(보류에 들지 않는다). 「역할이 source_commit으로 덮인 체인 커밋: N」은 ⑥이 되돌릴 수이고, `--pr`로 좁힌 실행은 역할이 커밋 단위라 이 대조를 하지 않는다 ④ 원인 분기: 「근거 없어 보류」가 남으면 ⑤로, `document_missing`이면 커밋 문서 생성 경로(RB-12·RB-14)를 먼저 본다 — 문서가 없는 것은 투영기가 만들 수 없다, `generation_rewound` 경고는 마이그레이션 036을 되돌렸다 다시 적용해 정본 세대가 1부터 시작한 흔적이다(정본 세대는 그대로인데 색인이 더 높다 — 사람이 판단할 일이며 자동 조치가 없다) ⑤ `prsctl links refetch --repository … [--pr <번호>] [--limit <수>]`로 **완전성 근거가 없는 PR만** GHE에서 다시 읽어 삭제 권한을 확정한다 — 이 명령에만 GHE 자격이 필요하고, 없으면 이것만 거절된다. 끝나면 ③을 다시 돌려 「근거 없어 보류」가 줄었는지 본다. 체인 규칙 몫에는 `refetch`가 필요 없다 ⑥ `prsctl links apply --repository … [--pr <번호>]`로 바뀔 커밋에 **투영 의도만** 만든다 — 색인은 관계 투영 러너(JOB-REL-008)가 쓴다. 예외 하나(CR-117): `--pr` 없이 돌리면 덮인 체인 커밋의 `role`을 `apply`가 색인에 직접 되돌리고 출력에 「체인 커밋 역할 되돌림: N건」을 찍는다 — `source_commit`일 때만 체인이 정한 역할(병합 근거나 PR 대응이 있으면 `merge_commit`, 없으면 `direct_push`)로 바꾸는 단방향 쓰기이고 반복 실행은 멱등이다. 관계 러너는 `role`을 비추지 않으므로 갈라질 두 번째 경로가 생기지 않는다. 계획은 파일로 넘기지 않고 실행 시점에 다시 계산하며, 잡 `pr_link_repair`와 감사 `job.run`이 누가 무엇을 정리했는지 남긴다(`--actor` 필수) ⑦ ②를 다시 돌려 밀린 투영이 0으로 가는지 확인하고, 커밋 하나를 골라 상세의 연결 PR을 본다 ⑧ **이 절차가 바꾸는 것은 `pull_request_numbers`와 그 세대, 그리고 PR 투영이 덮은 체인 커밋의 `role`(CR-117)뿐이다** — `merge_seq`·`merge_number`·`seq_epoch`·head·원격 M 태그를 건드리지 않고, 인덱스를 만들거나 전환하지도 않는다(재색인은 RB-19). 원본 목록에서 빠진 **체인 밖** 커밋의 `role`은 범위 밖이라 그대로 남는다(DEV-752 잔여) |

런북 커버리지 요구(NFR-008)는 상위 실패 모드 8종이다. 위 22종이 이를 초과 충족한다.
