...
> 상태: review | 버전: v0.10 | 갱신일: 2026-09-23
...
| RB-29 | 커밋 관계 투영 충돌·미수렴 (CR-116) | ① **먼저 배포를 본다.** `commit_link_conflict_total`이 오르고 있으면 같은 세대에 다른 관계 집합이 색인에 있었다는 뜻이고, 그것을 쓸 수 있는 것은 **구버전 합집합 writer**뿐이다(`prsctl lineage`로 워커 이미지 판을 본다). **모든 워커가 새 빌드가 되기 전에는 복구를 실행하지 않는다** — 지우는 족족 다시 더해진다. old-binary 롤백도 같은 이유로 오염을 재발시킨다 ② `prsctl links status --repository <owner/name>`으로 관계 규모(커밋 수·간선 수)와 **밀린 투영**을 state별(`ready`·`retry`·`parked`)로 본다 — `parked`는 재시도 상한을 넘었거나 `generation_conflict`로 바로 보류된 것이다. 어느 쪽인지는 정본 표 `commit_link_state.last_reason`에서 본다(status 출력에는 없다) — `generation_conflict`면 ①이 아직 풀리지 않았다 ③ `prsctl links plan --repository … [--pr <번호>]`로 정본과 색인을 맞대어 무엇이 바뀔지만 센다 — **아무 데도 쓰지 않는다**(PostgreSQL·Elasticsearch·작업 큐 모두). 출력의 「바뀔 커밋(더할 간선 · 지울 간선)」이 정리 규모이고, 「근거 없어 보류」와 「완전성 근거 없는 PR」이 완전성 근거가 없어 지울 수 없는 몫이다 ④ 원인 분기: 「근거 없어 보류」가 남으면 ⑤로, `document_missing`이면 커밋 문서 생성 경로(RB-12·RB-14)를 먼저 본다 — 문서가 없는 것은 투영기가 만들 수 없다, `generation_rewound` 경고는 마이그레이션 036을 되돌렸다 다시 적용해 정본 세대가 1부터 시작한 흔적이다(정본 세대는 그대로인데 색인이 더 높다 — 사람이 판단할 일이며 자동 조치가 없다) ⑤ `prsctl links refetch --repository … [--pr <번호>] [--limit <수>]`로 **완전성 근거가 없는 PR만** GHE에서 다시 읽어 삭제 권한을 확정한다 — 이 명령에만 GHE 자격이 필요하고, 없으면 이것만 거절된다. 끝나면 ③을 다시 돌려 「근거 없어 보류」가 줄었는지 본다 ⑥ `prsctl links apply --repository … [--pr <번호>]`로 바뀔 커밋에 **투영 의도만** 만든다 — 색인은 관계 투영 러너(JOB-REL-008)가 쓴다. 계획은 파일로 넘기지 않고 실행 시점에 다시 계산하며, 잡 `pr_link_repair`와 감사 `job.run`이 누가 무엇을 정리했는지 남긴다(`--actor` 필수) ⑦ ②를 다시 돌려 밀린 투영이 0으로 가는지 확인하고, 커밋 하나를 골라 상세의 연결 PR을 본다 ⑧ **이 절차가 바꾸는 것은 `pull_request_numbers`와 그 세대뿐이다** — `merge_seq`·`merge_number`·`seq_epoch`·head·원격 M 태그를 건드리지 않고, 인덱스를 만들거나 전환하지도 않는다(재색인은 RB-19). 원본 목록에서 빠진 커밋의 `role`은 이번 범위 밖이라 그대로 남는다(DEV-752) |
