...
> 상태: review | 버전: v0.3 | 갱신일: 2026-08-29
...
| RB-18 | 감사 기록 실패 | ① PostgreSQL 쓰기 상태 확인 ② **파티션 존재 확인(월 경계)** — `audit_record`는 파티션 테이블이라 해당 월 파티션이 없으면 모든 INSERT가 거부된다. `JOB-ING-006`의 파티션 생성이 도는지 본다 ③ 감사 누락 구간을 보안 담당자에게 보고 ④ **주 동작은 계속 처리되고 있다** (FR-AUTH-004 AC-6) — 이 경보는 조회가 막혔다는 신호가 아니라 **기록이 비고 있다**는 신호다 |
