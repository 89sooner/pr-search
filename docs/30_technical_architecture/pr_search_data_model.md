...
| ENT-GH-012 | GhCapabilityVerification | 레지스트리 검사 한 회차의 기록 (CR-088, JOB-GH-003) — **append-only**, 갱신·삭제는 DB 트리거가 거부한다. 누가(실행기·CI·CLI)·언제·어떤 바이너리(버전·SHA-256)·어떤 규칙(검증기·규칙 버전)으로 무엇(manifest 해시·인벤토리 해시)을 확인했고 결과(`passed`/`incomplete`/`drift`/`failed`/`error`)와 diff가 무엇이었나 | `verification_id`, `snapshot_id`(FK), `checked_at`, `checked_by`, `trigger`(startup/periodic/manual), `scope`(CR-090: 배포 범위 — 실행기 기록은 필수), `environment`(실행기 ID·호스트명·바이너리 경로·CR-090 `registry_check_ms`), `gh_version_expected/observed`, `binary_sha256_expected/observed`, `manifest_hash_expected/observed`, `inventory_hash_expected/observed`, `validator_version`, `rules_version`, `status`, `drift`(added/removed/changed), `report`(검증기 보고서 원문), `report_hash`, `error` | PostgreSQL (029) | gh-registry | FR-GH-001 AC-5, FR-GH-011 AC-2·AC-3, NFR-009 |
| ENT-GH-013 | GhOperationsPolicy | 배포 범위 하나의 현재 운영 정책 (CR-090) — 운영 승인한 정의와 그 근거, 운영자가 막은 capability, revision. 범위마다 한 행이고 쓰기는 `gh_operations_policy_apply` 함수만 한다. 행이 없으면 revision 0·승인 없음·차단 없음이다 | `scope`(PK, `GHE_BASE_URL`의 호스트), `revision`, `approved_snapshot_id`, `approved_verification_id`, `approved_report_hash`, `approved_manifest_version`, `approved_manifest_hash`, `approved_gh_version`, `approved_at`, `approved_by`, `blocked_capabilities[]`, `updated_at`, `updated_by` | PostgreSQL (030) | gh-policy | FR-GH-011 AC-6~AC-9, FR-GH-009 AC-8 |
| ENT-GH-014 | GhOperationsPolicyRevision | 운영 정책 변경 한 건의 불변 이력 (CR-090) — **append-only**. 이전 revision과의 관계, 행위, 대상, 근거, 행위자·사유·상관 ID, 중복 방지 키와 요청 지문 | `revision_id`, `scope`, `revision`, `previous_revision`, `action`(approve/revoke/block/resume), `capability_id`, `snapshot_id`, `verification_id`, `report_hash`, `manifest_version`, `manifest_hash`, `gh_version`, `actor`, `reason`, `correlation_id`, `idempotency_key`, `request_fingerprint`, `created_at` | PostgreSQL (030) | gh-policy | FR-GH-011 AC-8, FR-AUTH-004 |
...
| 권한 | `prs_app`: 두 표 SELECT·INSERT만(UPDATE·DELETE 없음). `prs_admin`: ALL | 실행기는 기록만 한다. 활성화(`activated_at`)는 운영자가 `prs_admin`으로 — 이 판은 어느 행도 활성화하지 않았다. **CR-090: 활성화 시각은 운영 승인 함수가 최초 승인 때 한 번 채운다(아래 030)** |
...
**030이 만든 것 (CR-090 / WP-080).** additive이며 001~029를 건드리지 않고 과거 행을 다시 쓰지 않는다.

| 대상 | 030 | 이유 |
| --- | --- | --- |
| `gh_capability_verification.scope` | 열 추가(TEXT, NULL 허용) + CHECK `checked_by <> 'gh-executor' OR scope 1~260자` **NOT VALID** + 인덱스 `(scope, checked_at DESC, verification_id DESC) WHERE checked_by = 'gh-executor'` + `BEFORE INSERT` 트리거가 정책 잠금(공유)을 잡는다 | 운영 승인의 근거는 「이 배포 범위의 가장 최근 실행기 기록」이다. 과거 행은 검사하지 않고(NOT VALID) 범위가 없는 과거 기록은 근거가 되지 못한다. 공유 잠금 때문에 승인이 최신 기록을 읽는 동안 새 기록이 커밋되지 않는다 |
| `gh_operations_policy` (`ENT-GH-013`) | **신설** — 배포 범위마다 한 행(PK `scope`): `revision`, 승인 정의와 근거(스냅숏·기록(FK)·보고서 해시·manifest 판·manifest 해시·gh 버전·승인 시각·승인자 — 전부 있거나 전부 없음 CHECK), `blocked_capabilities TEXT[]`(64개 이하), `updated_at`·`updated_by` | 「지금 무엇이 승인·차단돼 있나」의 정본. 코드의 실행 목록을 넓히는 열은 없다 |
| `gh_operations_policy_revision` (`ENT-GH-014`) | **신설** — 변경마다 한 행: `revision`·`previous_revision`(= revision − 1 CHECK)·`action`·`capability_id`(block·resume만)·정의와 근거(approve는 승인한 것, revoke는 철회한 것)·`actor`·`reason`(1~500자)·`correlation_id`·`idempotency_key`·`request_fingerprint`(sha256 hex)·`created_at`. UNIQUE `(scope, revision)`·`(scope, actor, idempotency_key)`. **append-only**(UPDATE·DELETE를 트리거가 거부) | 이전 revision과의 관계와 누가·언제·왜·무엇을 근거로 바꿨는지를 남긴다. 같은 키의 재요청을 지문으로 가른다 |
| `gh_operations_policy_apply(…)` | **신설 함수** — `SECURITY DEFINER`, 소유자 `prs_admin`, `SET search_path = pg_catalog, public, pg_temp`, `PUBLIC` 실행 회수, `prs_app`에 EXECUTE. 인자 모양 확인 → `pg_advisory_xact_lock(hashtext('gh:policy:' \|\| scope))` → 같은 행위자·키의 기존 revision(같은 지문이면 `replayed`, 다르면 PRS02) → 기대 revision 대조(PRS01) → 행위별 DB 사실 재확인(approve: 스냅숏 미분류 0 · 기록이 실행기·같은 범위 · **범위의 가장 최근 실행기 기록**(아니면 PRS01) · 보고서 해시 = 미리보기 · `passed` · 관측 = 기대 · 신선도 한도(인자는 60,000~691,200,000ms로 제한) · 이미 승인됨 PRS04 / revoke·block·resume의 전제 PRS04 / 부적격 PRS03 / 모양 PRS05) → 상태 갱신 → 이력 한 행 → `audit_record` 한 행 → (approve) 스냅숏 `activated_at` 최초 1회 | `prs_app`에 정책 표 쓰기를 주지 않고, search-api에 `prs_admin` 자격을 배포하지 않으면서 운영자의 결정을 받는 유일한 길이다. 이 경계가 막는 것은 **애플리케이션 롤의 우회**이며 소유자·superuser의 직접 변경은 경계 밖이다. 보고서 해석과 재현 해시는 호출자(search-api)가 같은 잠금 안에서 먼저 한다 |
| `gh_execution.policy_revision` + 가드 트리거 | 열 추가(BIGINT, NULL 허용) + `BEFORE INSERT OR UPDATE` 트리거: 새 행은 `policy_revision` 필수이고 `queued`로만 들어온다(PRS10 — `running`·종료 상태로 바로 넣어 아래 대조를 건너뛰지 못한다), 그 값은 바뀌지 않는다(PRS10), `queued`→`running` 전이는 정책 잠금(공유) 아래 현재 revision·승인 정의(manifest 판·해시·gh 버전)·차단 목록과 맞을 때만 된다(PRS11) | 수락 판정의 revision을 실행 기록에 남기고 claim 시점에 과거 승인을 승계하지 않는다. **이전 앱 버전으로 되돌려도 남는다** — 옛 search-api의 INSERT와 옛 실행기의 claim이 거절되어 실행이 조용히 다시 열리지 않는다(새 migrate 이미지와 옛 코드로 실측, 원장 6.87장). 트리거는 월 파티션마다 복제된다. 백업 복원(`prsctl restore` — 빈 DB에 `pg_restore`)은 종료 상태·revision 없는 과거 행을 그대로 복원한다: 트리거는 덤프의 post-data 절이고, 전체 복원에서 데이터 뒤에 만들어지는 것을 복원 스모크로 관측했다(원장 6.87장) |
| 권한 | `prs_app`: 두 정책 표 SELECT만 + 시퀀스 USAGE·SELECT + 함수 EXECUTE. `prs_admin`: ALL | 쓰기는 함수로만 간다. 통합 시험이 `SET LOCAL ROLE prs_app`으로 직접 쓰기·스냅숏 활성화·실행 기록 revision 바꾸기를 실제로 시도해 거절을 본다 |
| 되돌리기(`030.down`) | 가드·함수·두 표·`scope` 열과 잠금 트리거를 지운다. 스냅숏 `activated_at`과 `audit_record` 행은 남는다 | **내리기 전에 `GH_OPERATIONS_ENABLED=false`로 실행을 끈다** — 가드가 사라지면 이전 앱의 실행이 다시 열린다(런북 7.C). 다시 올려도 승인은 자동으로 되살아나지 않는다 |
...
| `gh_operations_policy` | 영구 — 배포 범위마다 한 행 (CR-090) | 삭제하지 않음 (애플리케이션 롤에 쓰기 권한 없음) | 현재 승인·차단 상태다. 복원하면 복원 시점의 결정으로 돌아가므로 복원 뒤 A-006에서 승인 상태를 다시 확인한다 |
| `gh_operations_policy_revision` | 영구 (CR-090) | 삭제하지 않음 (append-only 트리거) | 운영 결정의 이력이다. 같은 행위가 `audit_record`에도 남는다(보존 1년). 사람의 결정이라 연 수십 행 규모다 |
