...
| `gh_execution.policy_revision` + 가드 트리거 | 열 추가(BIGINT, NULL 허용) + `BEFORE INSERT OR UPDATE` 트리거: 새 행은 `policy_revision` 필수(PRS10), 그 값은 바뀌지 않는다(PRS10), `queued`→`running` 전이는 정책 잠금(공유) 아래 현재 revision·승인 정의(manifest 판·해시·gh 버전)·차단 목록과 맞을 때만 된다(PRS11) | 수락 판정의 revision을 실행 기록에 남기고 claim 시점에 과거 승인을 승계하지 않는다. **이전 앱 버전으로 되돌려도 남는다** — 옛 search-api의 INSERT와 옛 실행기의 claim이 거절되어 실행이 조용히 다시 열리지 않는다(새 migrate 이미지와 옛 코드로 실측, 원장 6.87장) |
