...
전용 클러스터인 이유는 ADR-008의 필수 접근 범위 필터와 `dynamic: strict` 매핑, 그리고 분석기 플러그인(OD-005 `nori`)이 모두 클러스터 수준 설정에 걸리는데 공용 클러스터에서는 그 설정을 PR Search 단독으로 통제할 수 없기 때문이다. 부하 격리도 인덱스 수준이 아니라 클러스터 수준에서 얻는다.
