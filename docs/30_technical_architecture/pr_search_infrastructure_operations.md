...
> 상태: review | 버전: v0.9 | 갱신일: 2026-09-01
...
**복구는 `pg_dump` 산출물 하나에서 시작한다** — PostgreSQL 복원 → **마이그레이션 재적용** → 접속 주체 프로비저닝 → **파생 색인 삭제** → ES 매핑 적용 → 전량 재색인 예약. 이 경로가 실제로 도는지는 `WP-070`이 destructive restore로 검증했다.
