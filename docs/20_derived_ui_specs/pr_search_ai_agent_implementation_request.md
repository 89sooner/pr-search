...
> 상태: review | 버전: v0.9 | 갱신일: 2026-09-26
...
UI 기준: ADR-006(CR-096 개정)에 따라 Radix 기반 제품 UI 계층(`apps/web/components/ui/`)과 시맨틱 토큰(`apps/web/app/ui.css`의 `--ui-*`)을 쓴다.
...
- `apps/web/components/ui/`의 Radix 기반 제품 프리미티브를 쓴다. 이 계층과 ADR-006의 아이콘 자산 예외 이외의 UI 라이브러리를 추가하지 않는다.
- 제품 코드에 리터럴 색상값(`#rrggbb`)을 두지 않는다.
- 제품 UI 계층에 없는 프리미티브(차트, 그래프 캔버스)는 `--ui-*` 시맨틱 토큰만 사용해 구현한다.
...
- ADR-006(CR-096 개정)의 Radix 기반 UI 계층과 아이콘 자산 예외 이외의 UI 라이브러리 추가, 리터럴 색상값 사용
- 오프셋 페이지네이션 파라미터 추가
...
**OD-001(미러 허용)·OD-002(권한 소스)·OD-004(릴리스 앵커)는 2026-08-22 CR-024로, OD-005(nori)는 2026-08-26 CR-040으로 모두 해소됐다.** 넷 다 두 경로 모두 구현하거나 대체 경로가 있어 착수를 막지는 않았지만, **결정 전까지 두 경로를 유지하는 비용은 실제로 들었다.** 현재 열린 결정과 기한은 `../10_requirements/srs_final.md`의 OD 표에서 읽는다.
