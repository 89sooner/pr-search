...
## W-001 선택 요약과 보기 상태 (CR-067 / WP-073)

FR-SRCH-006~009·FR-SEQ-005·NFR-007에 따른 C-013 내부 상태다. 미선택은 목록만, 선택은 현재 응답의 식별자·제목·작성자·시각·시퀀스 공간/에폭·변경 규모·관계 요약을 보인다. 누락 값은 미확인으로 표기한다. 질의/정렬/에폭 변경·명시적 갱신·이어 보기는 선택을 폐기하고 새 목록에서 다시 고른다. 조회 실패·권한·에폭 무효 상태에서는 이전 미리보기를 유지하지 않는다. 클립보드 실패는 대체 복사 안내를 표시하고, 그 결과가 늦게 도착해도 다른 선택의 알림을 덮지 않는다. 필터 펼침/접힘은 결과 집합과 독립인 로컬 보기 상태다.
