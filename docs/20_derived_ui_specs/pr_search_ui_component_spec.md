...
- **시퀀스 인용 상태도 항목마다 다르다** (CR-051): `current`·`epoch_stale`·`unbound`·`unavailable` 넷이며 각각 문자 레이블을 갖는다. `epoch_stale`은 저장된 값과 현재 값을 함께 적고, `unbound`는 실행을 막으며, **`unavailable`은 아무 수치도 적지 않는다** — 볼 수 없는 저장소의 에폭을 적는 순간 그것이 유출이다 (THR-043)
