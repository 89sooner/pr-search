...
**이 표는 Conductor 컴포넌트가 쓰는 토큰을 적는다.** 제품이 직접 만드는 전환의 목록이 아니며, 아래 마지막 규칙이 그것을 금지한다. 제품이 쓸 수 있는 모션은 Conductor가 이미 컴포넌트 안에 넣어 둔 것뿐이다.

**제품 스타일시트가 생겼어도 이 규칙은 그대로다** (`DEV-555`). `CR-064`가 이 절을 좁힐 때는 제품이 스타일시트를 하나도 두지 않는다는 당시의 사실을 근거로 삼았는데, `CR-067`이 §14의 `workbench.css`를 들이면서 그 근거가 사라졌다. **근거는 사라졌지만 결론은 남는다** — 그 파일은 배치만 정하고 전환·애니메이션 선언을 두지 않으며(실측), §14가 "semantic/component 변수의 값을 재선언하지 않는다"로 그것을 못 박는다. 모션이 필요해지면 Conductor에 넣고 토큰으로 받는다. 회귀가 제품 CSS 전부를 훑어 그 선언이 없는 것을 매 실행 확인한다.
...
## 13. GitHub Operations 토큰 매핑 (CR-005 신규)
