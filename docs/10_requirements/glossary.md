...
> 상태: review | 버전: v0.19 | 갱신일: 2026-09-24

CR-117 / FR-SRCH-002 AC-7, FR-SRCH-003 AC-5, OD-016: **원본 커밋**은 **그 PR이 대상 브랜치에 새로 가져오는 커밋**이다. GitHub의 PR 커밋 목록(`GET /pulls/{n}/commits`)은 그 판정의 **재료**이지 정의가 아니다 — 피처 브랜치가 `git merge dev`로 받아 온 dev 체인 커밋은 그 목록에 있어도 원본 커밋이 아니며, 그 커밋을 dev에 올린 PR의 머지 커밋이다. **유효 연결**은 관계 정본의 원시 관측 가운데 이 정의를 통과한 것이고, 커밋 문서의 `pull_request_numbers`와 PR 상세의 `source_commits`가 둘 다 여기서 나온다. **체인 소속**은 저장소가 추적하는 시퀀스 브랜치의 현재 에폭 first-parent 체인에 그 커밋의 행이 있다는 뜻이다. 금지 동의어: "GitHub 커밋 목록"을 원본 커밋과 같은 뜻으로 쓰지 않는다(수가 다를 수 있다), "오염된 정본"(원시 관측은 GitHub이 말한 그대로이고 틀린 것은 읽는 규칙이었다).

CR-116 / FR-SRCH-002 AC-6, FR-ING-004 AC-6: **관계 정본**(`pull_request_commit_link`)은 어떤 PR이 어떤 커밋을 소유하는지를 근거(`source`·`merge`)와 함께 담는 PostgreSQL 행이며, 커밋 검색 문서의 `pull_request_numbers`는 그 정본의 파생이다 — 색인의 배열이 정본이던 자리를 대신한다. **관측 완전성**(`commits_complete`)은 한 PR의 커밋 목록을 읽은 그 회차가 원격의 전부를 보았는가에 대한 판정이고 **삭제 권한** 그 자체다 — 참이 아니면 그 관측은 관계를 더할 수는 있어도 뺄 수 없다. **관계 세대**(`pr_links_generation`)는 관계에만 쓰는 순서 기준이다. 커밋의 `document_version`은 웹훅 수신 시각이라 다른 PR의 이벤트가 이 PR의 관계를 막거나 통과시키므로 그 자리를 쓰지 않으며, 시퀀스 에폭과도 무관하다. 금지 동의어: "PR 번호 합집합"·"누적 필드"(더하기만 하던 옛 방식이며 CR-116이 걷어 냈다), "관계 간선"(`prs-links`의 `link`는 다른 것이다), "보강 완료"(`enrichment_pending`은 구성 요소를 가르지 못한다), "문서 버전"·"시퀀스 에폭"(관계 세대와 다른 축이다).

CR-115 / FR-SEQ-012: **M 번호 lightweight 태그**는 확정된 M 번호의 표기 문자열을 이름으로 원격 GHE 저장소의 머지 커밋에 만드는 ref(`refs/tags/M-<코드>-<번호>`)다 — tagger·메시지·서명이 없는 lightweight이며 annotated 태그가 아니다. 시스템은 이 태그를 **만들기만** 하고 옮기거나 지우지 않는다. 같은 이름의 태그가 다른 커밋이나 annotated 태그를 가리키면 **태그 충돌**(`conflict`)이며 보고만 한다. **태그 대조**(reconcile)는 시퀀스 공간 하나의 정본 번호 행과 원격 `M-<코드>-*` 목록을 맞춰 보는 운영 작업이다 — 누락(`missing`)은 다시 만들고, 충돌·원격에만 있는 것(`unexpected`)은 보고만 한다. 금지 동의어: "M 태그 푸시"(로컬 미러가 아니라 원격 REST로 만든다), "태그 갱신·동기화"(옮기지 않는다), "릴리스 태그"(이 제품의 릴리스와 무관하다), "annotated 태그".
...
| 원본 커밋 | `source_commit` | 그 PR이 대상 브랜치에 새로 가져오는 커밋 — 머지 이전 PR head 브랜치에 있으나 저장소가 추적하는 브랜치의 현재 체인에는 아직 오르지 않은 커밋. Squash 후 target 브랜치에는 존재하지 않는다. 피처 브랜치가 `git merge dev`로 받아 온 dev 체인 커밋은 GitHub의 PR 커밋 목록에 있어도 원본 커밋이 아니며 그 커밋을 dev에 올린 PR의 머지 커밋이다 (CR-117, OD-016) | 개별 커밋, 작업 커밋 | FR-SRCH-003, FR-SRCH-002 AC-7 |
...
| M 번호 lightweight 태그 | `refs/tags/M-<코드>-<번호>` (`merge_sequence.tag_state`) | 확정된 M 번호의 표기 문자열을 이름으로 원격 GHE 저장소의 머지 커밋(정본 행의 `commit_sha`)에 만드는 lightweight 태그. 채번 트랜잭션이 남긴 durable `tag` work가 만들며 실제로 만든 경우에만 감사(`merge_number.tag`)를 남긴다. 같은 SHA의 태그가 이미 있으면 성공이고, 다른 SHA·annotated 태그면 **충돌**로 보고만 한다 — 이동·삭제 경로가 없다. 에폭이 올라도 기존 태그는 그대로다 (CR-115) | M 태그 푸시, 태그 동기화, 릴리스 태그, annotated 태그 | FR-SEQ-012, ENT-SEQ-001, JOB-SEQ-007 |
| 태그 대조 | `mnumber_tag_reconcile` (`prsctl mnumber tags reconcile`) | 시퀀스 공간 하나의 현재 에폭 번호 행 전부와 원격 `refs/tags/M-<코드>-*` 목록을 맞춰 보는 운영 작업. `missing`은 `tag` work를 다시 요청해 재생성하고(앞선 회차가 `conflict`로 남긴 행도 원격에 없으면 다시 연다), `ok`는 정본에 `done`을 적으며, `conflict`(다른 SHA·annotated)와 `unexpected`(원격에만 있음)는 보고만 한다. 운영자의 실행은 권한 차단을 푸는 명시적 재개이기도 하다. dry-run은 원격을 읽기만 한다 (CR-115) | 태그 동기화, 태그 복구, 강제 정렬 | FR-SEQ-012 AC-10, API-ADM-002 |
...
| 관계 정본 | `pull_request_commit_link` | 어떤 PR이 어떤 커밋을 소유하는지를 담는 PostgreSQL 행. 근거(`source` 원본 커밋 / `merge` 머지 커밋)가 키에 있어 원본 목록에서 빠져도 실제 병합 근거는 남는다. 커밋 검색 문서의 `pull_request_numbers`는 이 정본에서 계산한 전체 집합을 커밋별 전용 투영기가 대입한 **파생**이며, 색인에만 존재하는 관계는 없다(ADR-004 Amendment). **`prs-links`의 관계 간선(`link`)과 다른 것이다** — 이쪽은 소속이고 저쪽은 유형·근거·신뢰도를 가진 간선이다 (CR-116). `source` 행은 GitHub 목록을 그대로 담은 **원시 관측**이며, 읽는 자리는 체인 규칙(`FR-SRCH-002` AC-7)을 통과한 유효 연결만 쓴다 (CR-117) | PR 번호 합집합, 누적 필드, 관계 간선, 링크 테이블 | FR-SRCH-002 AC-6·AC-7, ENT-CORE-009 |
| 관측 완전성 | `commits_complete` | 한 PR의 커밋 목록을 읽은 그 회차가 **원격의 전부**를 보았는가에 대한 판정. 네 조건이 모두 성립할 때만 참이다 — 조회 성공, 우리 상한 미절삭, 원격이 말한 커밋 수와 읽은 수의 일치, 읽기 전후 head·base 동일. **이 값이 곧 삭제 권한이다** — 거짓이면 그 관측은 관계를 더할 수는 있어도 뺄 수 없다. 조회 실패로 얻은 빈 목록은 빈 목록이 아니라 모름이다 (CR-116) | 보강 완료, `enrichment_pending`, 커밋 목록 없음 | FR-ING-004 AC-6, ENT-CORE-010 |
| 관계 세대 | `pr_links_generation` | 관계에만 쓰는 순서 기준. 관계가 바뀐 커밋만 오르고, 색인의 세대가 더 높으면 쓰기를 거절하며(`stale`), 같은 세대에 다른 집합이면 덮지 않고 충돌로 기록한다. 커밋의 `document_version`은 웹훅 수신 시각이라 **다른 PR의 이벤트가 이 PR의 관계를 막거나 통과시키므로** 그 자리를 쓰지 않으며, 시퀀스 에폭과도 다른 축이다(관계의 정체성은 base 브랜치에도 에폭에도 속하지 않는다). 마지막 연결을 지운 뒤에도 세대와 빈 배열이 남는다 (CR-116) | 문서 버전, 시퀀스 에폭, 관계 버전 | FR-SRCH-002 AC-6, ENT-CORE-011 |
