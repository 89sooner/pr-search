...
**검증.** 타입·린트·의존 방향 통과. 단위·회귀·통합·접근성·e2e 수치는 6.78장이 정본이다. 변이 시험을 세 곳에 돌렸다 — GHE OAuth 모듈 5종, 정본 등록 4종, 배포 정의 4종이 전부 kill됐다. 문서 검사기는 `main` 대비 신규 issue 0건이다.
