...
- [x] 기술 아키텍처: 데이터 모델 v0.32 → v0.33(머리 주석, 엔티티 카탈로그 ENT-REL-003, 3.1장 `pull_request_stack`·`reindex_link_pending`, 4.3장 스택 간선의 정본), 비동기 v0.19 → v0.20(머리 주석, 3.4장 스택의 정본·역방향 재평가, 3.5장 이중 쓰기 대상 15개·재구축 정본 표·「간선 재구축의 미처리와 기대 간선」 절·전환 전 검증 세 항목·실패 처분 두 행·보관의 대기열 정리), API 계약 v0.45 → v0.46(머리 주석 — 경로·응답 변경 없음, `prsctl links import-stacks`), 관측성 v0.12 → v0.13(재색인 로그 세 줄, `link_relations_total`이 해제 간선의 재쓰기를 세지 않음).
- [x] 전달: 작업 패키지 v2.66 → v2.67(WP-104 절·상태 표), 원장 v6.111 → v6.112(머리 절, 3장 WP-104, 4장 FR-ING-008 AC-11·FR-REL-006 AC-6, 5장 DEV-766~DEV-773, 6.112장). `deploy/single-host/RUNBOOK.md` 7.I(스택 가져오기와 prs-links 재색인 확인)와 8장 세 행. `agent-context/upstream-feedback.md` 셋째 항목에 상류 반영 주석.
- [x] 코드·시험: 원장 6.112장.
- 문서 검증기: 원장 6.112장.
