...
**`safe_marker.set`의 활성 전환은 `WP-041` 구현 PR이 한다.** `FR-AUTH-004` AC-1 정본 표의 그 행은 지금 **미활성**이며, 그 상태가 사실인 시점은 기능이 실제로 서는 순간이다. 근거는 이 CR이고, 구현 PR이 그 행 하나를 옮기면서 SRS 버전을 함께 올린다 — 계약이 먼저 열려 있어야 baseline 문서를 고칠 수 있다는 규율은 그 시점에도 지켜진다.
