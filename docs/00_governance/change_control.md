...
9. **구현** — `deploy/single-host/prsctl`(프로비저닝·복구·스모크), `build-bundle.sh`(계보 검사), `compose.yml`(reindex 서비스·관리 토큰), `.env.example`, `RUNBOOK.md`, 통합 헬퍼 넷과 시험 여섯, `packages/db/migrations/022_*`, `audit-grants.test.ts`.
