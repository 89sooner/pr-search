...
- 유형: correction(설계 결함 — 간선의 부분 갱신이 재색인의 새 인덱스에서 대상 간선을 찾지 못해 재색인이 실패했고, 같은 경로에서 해제된 스택 이력이 색인에만 있어 재색인이 그것을 조용히 잃었다). `FR-ING-008` AC-11과 `FR-REL-006` AC-6 신설, `OD-017`을 열고 같은 날 사용자 결정으로 닫는다. **기존 FR의 요구사항 문장과 AC 문장은 바꾸지 않는다. 안정 ID 재번호화 0건.** 상태: **open** — 브랜치 `feature/cr121-links-reindex`, worktree `/home/roqkf/pr-search-wt/links-reindex`, 기준 main `ee72de7`(CR-119·CR-120 병합 기록 뒤).
