...
- 유형: correction/reliability(ADR-004 결함 — 관계 정본이 PostgreSQL에 없고 색인의 합집합에만 있었다). ADR-004 Amendment, `FR-SRCH-002` AC-6 신설, `FR-ING-004` AC-6 신설, `FR-ING-008` AC-9 신설. **기존 FR의 요구사항 문장과 AC-1~AC-5는 바꾸지 않는다. 안정 ID 재번호화 0건.** 상태: **implementing** — PR #228(브랜치 `feature/cr116-pr-links`, worktree `/home/roqkf/pr-search-wt/cr116-pr-links`, base `main` `83d30fb`). 병합은 사용자 지시 뒤. 사내 배포 SHA는 NOT VERIFIED.
...
- 상태: **implementing** — PR #228(브랜치 `feature/cr116-pr-links`). 병합은 사용자 지시 뒤.
