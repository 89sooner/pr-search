...
| CR-104 | 2026-09-17 | scope | 사용자 결정 2026-09-17 — W-001-PAGER의 버튼식 더 보기(WP-016)를 스크롤 자동 로드로 전환 | **"More changes" 버튼을 `IntersectionObserver` 기반 무한 스크롤로 대체.** 커서 거절 시 자동 재시도 금지(CR-043)는 그대로 유지, 트리거 방식만 바뀌어 FR-SRCH-008 문장 변경 없음. 모바일은 범위 밖(사용자 결정) | WP-091 · FR-SRCH-008 | wireframe_spec(W-001-PAGER) · WP 문서 · 원장 | 코드 커밋 완료, push 대기(CR-103과 함께 진행) |
...
**병합.** 코드는 main에 직접 커밋 예정(CR-103과 같은 방식, 사용자 지시). 원장 3장·6.97장에 실제 커밋 해시를 기록한다.
