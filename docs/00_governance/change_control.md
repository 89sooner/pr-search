...
- 유형: correction/reliability(ADR-004 결함). 정렬 UI 변경이 아니다. PostgreSQL에 확정된 `merge_seq`가 Elasticsearch 문서에 비치지 않거나 재색인 뒤 사라지는 결함을, **기존 번호를 바꾸지 않고** 정본에서 복원하고 이후 자동으로 수렴하게 고친다. 상태: **implementing**.
...
- 상태: **implementing** — 병합 뒤 「병합 판정」을 적고 닫는다. 내부망 적용은 NOT RUN, 사내 배포 SHA는 NOT VERIFIED.
