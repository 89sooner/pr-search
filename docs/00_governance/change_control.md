...
- 유형: scope(기능 누락 보완 — `FR-SEQ-012` 신설, `OD-015` 신설, ADR-026 신설·ADR-022 Amendment). **읽기 전용 원칙의 세 번째 예외이자 시스템이 스스로 하는 두 번째 GHE 쓰기다.** 상태: **implementing** — 브랜치 `feature/cr115-mnumber-tags`(worktree `/home/roqkf/pr-search-wt/cr115-mnumber-tags`), CR-114 브랜치 `feature/cr114-mnumber-search` 위에 stacked(기준 `7f980be`). 병합은 사용자 지시 뒤에 하며 CR-114가 먼저다. 사내 배포 SHA는 NOT VERIFIED.
...
- 상태: **implementing** — PR #226(base `feature/cr114-mnumber-search`, CI success). 병합은 사용자 지시 뒤, CR-114(PR #225) 병합 뒤 이 PR의 base가 `main`으로 옮겨졌는지 확인하고 나서.
