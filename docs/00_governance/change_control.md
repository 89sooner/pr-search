...
사용자가 Perforce GUI에 대응하는 파일 조사 흐름의 구현을 승인했다. FR-SRC-001~004 / WP-085로 추적한다. 실제 GHE Git 트리와 경로별 커밋 이력을 조회하고, PR/커밋/두 리비전의 파일 비교 및 커밋 슬라이더·라인 변경 이력을 제공한다. NFR-005의 소스 미저장 원칙에 **인가된 요청에서의 일시적 소스 열람** 예외를 명시한다. 소스·diff 본문을 PG/ES/Redis/미러·로그에 저장하지 않는다. 기존 세션과 저장소 접근 범위를 모든 source 요청에서 먼저 검증하고, 없는 저장소/범위 밖 저장소는 동일404다. 화면은 Radix·영문·기존 테마·SaaS 10곳 참고 기준을 따른다. 상태: 로컬 구현·검증 완료(2026-09-17). SRS·PRD·용어/흐름·추적 → IA·와이어프레임·flow/state·컴포넌트·QA → FE/BE/API·보안·운영·ADR → WP·원장 cascade 완료. 집중94/94·브라우저·접근성·빌드 통과. 기존 문서 validator 오류 둘로 종료 gate 보류, 실 GHE NOT RUN, 미발행.

## CR-096 — 전체 화면 Radix·영문 UI·사용자 테마 (2026-09-16)

사용자 승인: CR-095의 일반 검색 전환을 운영·분석·상세·저장된 검색 등 전체 화면으로 확장한다. Conductor 런타임 의존성을 제거하고 Radix 기반 제품 프리미티브로 교체한다. 제품 제공 문구·접근성 라벨·오류 안내는 영어로 통일하며 저장소의 PR 제목·본문·사용자 데이터는 번역하지 않는다. 다크/라이트 전환 버튼과 저장된 선호도를 제공한다. `template.html` 통합 검색 배치는 유지하며, 동일한 SaaS 10곳의 정보 위계·표·폼·내비게이션 패턴을 전 화면에 적용한다. NFR-007·FR-SRCH-003·FR-ADMIN-001 관련 표현 변경이며 기존 권한·서버 동작은 보존한다. WP-084. 상태: 구현·검증 완료, 기존 문서 gate 오류로 종료 판정 보류. SRS·PRD·추적 → UI 토큰/컴포넌트/QA → 프런트엔드/ADR → WP·원장 cascade 완료. 빌드·접근성 433/433·17개 브라우저 상태 확인, 미발행.

## CR-095 — 일반 검색 UI Radix 전환 (2026-09-16)

사용자 승인: 일반 사용자 화면에서 Conductor 사용을 중단하고 `@radix-ui`로 재구현한다. 통합 검색의 배치는 `artifacts/template.html`의 헤더 → 300px 저장소 탐색 + 탭/필터/결과를 따른다. operator 화면과 기존 컴포넌트는 보존한다. 공식 SaaS 사이트 10곳 조사 결과를 표현 설계에 반영한다. FR-SRCH-003·005~008, NFR-007의 표현 계층 변경이며 새 API·DB·수집 기능은 추가하지 않는다. WP-083으로 추적한다. 상태: 구현·로컬 검증 완료, 문서 gate pending(기존 FR-CSS-005·D-002 오류). SRS 정정 → IA·와이어프레임·컴포넌트·토큰·QA → 프런트엔드·ADR-006 amendment → WP·원장 cascade 완료. 미발행.
...
이전 지시의 pilot.8 미발행은 **2026-09-16 재개 시 사용자의 새 릴리스 발행 지시로 대체**됐다. 발행 여부·태그·자산 digest는 실제 실행 후 원장에 기록한다. 사내 확인 및 기존 미통과 릴리스 게이트를 통과로 바꾸지 않는다.
