...
- 유형: scope(기능 누락 보완 — `FR-SEQ-012` 신설, `OD-015` 신설, ADR-026 신설·ADR-022 Amendment). **읽기 전용 원칙의 세 번째 예외이자 시스템이 스스로 하는 두 번째 GHE 쓰기다.** 상태: **implementing** — PR #226(브랜치 `feature/cr115-mnumber-tags`, worktree `/home/roqkf/pr-search-wt/cr115-mnumber-tags`). CR-114 브랜치 위에 stacked로 시작했고(기준 `7f980be`), CR-114가 PR #225로 main `9782ba9`에 squash 병합된 뒤(2026-09-22 22:59 KST) 세 커밋을 `rebase --onto origin/main`으로 main 위에 다시 올려 PR base를 `main`으로 옮겼다. 병합은 사용자 지시 뒤. 사내 배포 SHA는 NOT VERIFIED.
...
- 유형: scope(기능 누락 보완 — `FR-SRCH-001` AC-7 신설, `FR-SRCH-005` AC-9 보완, `OD-014` 신설). 상태: **implementing** — 브랜치 `feature/cr114-mnumber-search`(worktree `/home/roqkf/pr-search-wt/cr114-mnumber-search`), 기준 main `13074f8`(착수 시 `origin/main`과 같음). 병합은 사용자 지시 뒤에 한다. 사내 배포 SHA는 NOT VERIFIED.
...
- 상태: **implementing** — PR #226(base `main` — CR-114가 `9782ba9`로 병합된 뒤 `rebase --onto`로 다시 올리고 base를 옮겼다; 코드 트리는 검증한 head와 같고 차이는 main이 그 사이 받은 상류 피드백 파일의 네 번째 항목뿐이다). 병합은 사용자 지시 뒤.
