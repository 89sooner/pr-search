...
- 유형: correction(요구사항 공백 — 용어집의 「원본 커밋」 정의가 두 문장으로 갈려 있었고, 구현은 GitHub의 PR 커밋 목록 전체를 원본 커밋으로 읽었다). `FR-SRCH-002` AC-7 신설, `FR-SRCH-003` AC-5 신설, 용어집 「원본 커밋」 정의 보완, `OD-016` 신설과 동시 결정(resolved). **기존 FR의 요구사항 문장과 AC 문장은 바꾸지 않는다. 안정 ID 재번호화 0건.** 상태: **open** — 구현 중. 기준 main `5c52aa2`, worktree `/home/roqkf/pr-search-wt/cr117-source-commits`(브랜치 `feature/cr117-source-commits`).
