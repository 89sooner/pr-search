...
| CR-030 | 2026-08-24 | correction | WP-026 착수 전 W-005 계약 감사에서 확인된 DEV-155~159 | **W-005 릴리스 화면의 계약 다섯 곳이 비어 있거나 실측과 어긋난다.** (1) W-005-LIST(릴리스 타임라인)를 채울 **API가 카탈로그에 없다** — API-SEQ-003은 두 릴리스의 "비교"이지 목록이 아니고, `release` 표를 읽는 사용자용 경로가 없다(DEV-152와 같은 유형). `GET /releases`(API-REL-005)를 신설한다: 세션 인증 + `resolveSpace` 접근 통제, "직전 릴리스 대비 PR 수"는 시퀀스 차이가 아니라 `merge_sequence`의 실제 PR 행 수(QA-W005-06), 미수집 저장소는 200 + `releases: []` + `reason: "release_not_indexed"` + 저장소 개요 경로(DEV-146의 자세)이며 미배포 블록(head 서수·대기 PR 수)을 함께 싣는다(DEV-155). (2) API-SEQ-003 응답 예시의 `reverted_pull_request_count`는 관계 파생(WP-030) 전에 데이터가 없다 — DEV-133·150이 API-SEQ-001에서 이미 정한 "키 자체를 뺀다"를 API-SEQ-003에도 연장하고 화면은 `준비 중(WP-030)`을 그린다(DEV-156). (3) API-SEQ-003의 오류·판정 목록이 불완전하다: FR-SEQ-002 재사용에 따라오는 `RANGE_TOO_LARGE`(400), 저장 서수 trio를 믿지 않고 현재 에폭에서 커밋 SHA로 재해석하는 에폭 규율(DEV-149), `to=unreleased`의 뜻(끝 서수 = 브랜치 head, `from`은 필수)이 계약에 없다(DEV-157). (4) 딥링크 `select=`의 의미가 "비교 대상 2건"이라는 말뿐이다 — 콤마 구분 최대 2개의 태그 이름으로 정하고, 현재 타임라인에 없는 태그(다른 브랜치 포함)는 복원 불가 경고 + 비교 차단으로 처리하며, 직전 릴리스가 없는 첫 릴리스의 상세는 비교 호출 없이 "첫 릴리스" 안내와 비활성 구간 링크로 그린다(DEV-158). (5) 와이어프레임의 "시각 내림차순" 정렬은 git 검증 불변식과 어긋날 수 있다 — 태그는 과거 커밋에도 붙으므로 시각순과 서수순이 갈리고, "직전 릴리스"가 시각 기준이면 구간이 겹친다. **서수 내림차순 고정**, "직전"은 서수 선행 릴리스로 정정한다(DEV-159) | DEV-155, DEV-156, DEV-157, DEV-158, DEV-159, FR-SEQ-004, FR-REL-002, QA-W005-01~06, W-005, C-032, API-REL-005, API-SEQ-003 | API 계약, 와이어프레임, 화면 흐름, 프론트 아키텍처, 작업 패키지, 원장 | closed (2026-09-10) |
...
### CR-030 반영 내역 (2026-09-10)

1. `pr_search_api_contracts.md` — **API-REL-005 `GET /releases` 신설** (표 행·상세 절·stable 등재): 서수 내림차순 타임라인, 구간별 실제 PR 행 수, 미배포 블록, 미수집 200+사유+`/repositories` (DEV-155·159). API-SEQ-003 절 재작성: FR-SEQ-002 재사용 명시, reverted 키 생략(DEV-156), 태그 무분류·에폭 재해석·`RANGE_TOO_LARGE`·`to=unreleased` 의미(DEV-157), 404의 미수집/태그 부재 구분
2. `pr_search_wireframe_spec.md` — W-005-LIST 정렬 서수 내림차순 정정(DEV-159), 진입 경로에 `select=`와 `/releases` 공간 선택, 구현 메모에 첫 릴리스·낯선 select 처리·비교의 W-004 이동(DEV-158)
3. `pr_search_frontend_architecture.md` — `/releases`(공간 선택)·`/releases/[owner]/[repo]` 행 정정(렌더링·API), 라우트 트리에 `releases/page.tsx` 추가, `range/`→`ranges/` 표기 잔재 정정(CR-029 후속)
4. `pr_search_work_packages.md` — WP-026 구현 범위에 API-REL-005·에폭 재해석·이중 방어 반영, 제외 명시(되돌림 값 WP-030, 커서·패싯 WP-032, W-009 화면), DoD 상태 목록 명시
5. 원장 — DEV-155~159 등록·해소, WP-026 검증 기록(6.26장), FR-SEQ-004 done, v1.3
6. 검증 — WP-026 시험 전 계층 통과(원장 6.26장): 단위 15·통합 22(실 PG+Redis)·a11y 18(axe 0)·e2e 4·contrast 80쌍, 변이 12종 전부 킬. SRS·PRD는 건드리지 않았다 (FR-SEQ-004·FR-REL-002가 이미 부여한 범위의 계약 정정·보완)
