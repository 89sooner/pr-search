...
- [x] `packages/es`, `packages/db`, `apps/search-api` — 구현은 WP-013 커밋
...
**DEV-052의 `team` 필터는 이 WP에서 결과를 내지 못한다.** 이름→ID 해석은 서지만 문서의 `allowed_team_ids`·`author_team_ids`가 비어 있다 (WP-012가 남긴 제한). 투영이 그 값을 채우는 것은 별도 CR이며, 그때 이 필터가 저절로 동작한다. **조용히 0건을 내지 않고 응답에 그 사실을 남긴다.**
