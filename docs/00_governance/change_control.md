...
- 유형: SRS 개정(`FR-SRCH-005`/`006`/`008` AC 추가·개정) + 구현. 상태: **open — SRS 개정 착수**.
...
- 진행: (병합 결과는 완료 시 이 항목에 채운다. SRS 개정·구현·독립 리뷰 2회 반영 완료. `pnpm typecheck`·`lint`·`lint:deps`·`test`(2919/2920, GHE 자격 증명 없음 1건 skip 기존과 동일)·`test:integration`(1847/1847, 워크트리별 격리 DB `prs_test_cr106tests`)·`test:regression`(506/506)·`pnpm build` 전부 통과. `/search` 화면 실제 브라우저 확인(새 필드 렌더링·접근성 이름·비활성 상태·입력 확인, 세션 부재로 실제 검색 결과 왕복은 NOT RUN). 아직 커밋·push·PR·main 병합 전.)
