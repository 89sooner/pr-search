...
**병합 판정.** 코드는 main에 직접 커밋했다(`d0e74c7`, 문서 3종은 다른 세션의 미커밋 CR-102 작업과 같은 파일이라 별도로 병합). PR은 아직 만들지 않았다. push·PR 진행은 사용자 확인 후 진행한다(원격에 영향을 주는 작업이라 별도 승인 필요). 사내 실제 GHE 데이터로의 확인은 `NOT RUN`.
