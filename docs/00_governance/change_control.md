...
5. **구현은 `WP-071`이 한다** — `build-bundle.sh`(운반 아카이브), `prsctl`(`load`의 검사 위치), `deploy/single-host/RUNBOOK.md`(외부망/사내망 경계로 재구성), 회귀 시험. 이 CR은 계약을 세우고 편차를 `open`으로 둔다.
