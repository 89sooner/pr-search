...
- 범위: (a) `packages/es` — `isReferenceTargetIndexed`(대상 문서 한 건의 실시간 존재 확인). (b) `apps/pipeline-worker` — `resolveReferencesTo`가 미해결인 정확한 키 후보가 있을 때만 대상 문서를 한 번 확인하고, 없으면 붙이지 않는다. 접두 키 경로(매번 유일성 재계산)는 그대로다. (c) `deploy/single-host/RUNBOOK.md` — 7.I 4번(prs-links 재색인)을 7.H 5번(prs-commits 재색인)이 `completed`된 뒤에 하도록 고치고, 이미 먼저 전환했다면 7.H 뒤에 한 번 더 돌리게 한다. 순서를 한곳에 모은 7.J를 더하고 3장·7장 표·8장을 맞춘다. (d) 비동기 문서의 JOB-REL-005 해결 규칙에 한 행. (e) `packages/es/src/architecture.test.ts` — ADR-008 우회 조회 검사가 `exists`도 잡는다(독립 리뷰 지적).
- 결정과 대가: 재구축 호출부가 아니라 함수에서 고친다 — 평시 경로(문서 없이 보강 신호가 온 커밋)도 같은 규칙을 탄다. 존재 확인은 검색이 아니라 문서 ID·routing의 실시간 조회라 refresh 지연이 없고, 비용은 해결 갱신 한 번당 최대 한 번이다. **코드만으로는 순서 문제가 풀리지 않는다** — 고친 코드에서 prs-links를 먼저 재색인하면 검증은 통과하지만 손으로 전환한 인덱스에 빠진 커밋을 가리키는 참조가 미해결로 전환되고, 재색인은 이벤트를 내지 않아 prs-commits가 복원된 뒤에도 다시 해결되지 않는다. 그래서 절차의 순서를 함께 고친다. 대가: 업그레이드 전 서비스 인덱스에서 해결돼 있던 참조 가운데 대상 커밋의 문서가 CR-119 재구축으로 사라지는 것(생성 근거 없는 커밋)은 prs-links 재색인 뒤 미해결이 된다 — 대상 문서가 없으므로 AC-3에 맞는 상태이며, 간선과 근거 문장은 남는다.
- 제외: 참조 추출 규칙, 접두 키 판정, 스택·되돌림 파생, CR-119의 문서 생성 정책, DEV-773, 단일 호스트 compose의 `GHE_BASE_URL` 배선(DEV-776, 후속), Release 발행과 사내 적용.
- 설계·세부 정본: FR-REL-003 AC-3, JOB-REL-005(비동기 3장 「해결 규칙」), RUNBOOK 7.I·7.J, 원장 5장 DEV-775, 6.113장. 연쇄 기록은 5장 「CR-123 cascade」에 적는다.
...
- 결정과 대가: CLI 진입점이 아니라 명령에서 고친다 — 인자로 부르는 모든 호출자가 같은 경로를 탄다. 우선순위는 인자 > 주입이다. 다른 운영 CLI는 이미 인자에서 읽는다(`sequence-reproject-command.ts`의 `optionText(args, 'actor')`).
- 제외: `prsctl`과 RUNBOOK의 명령 문장(이미 `--actor`를 넘긴다), Release 발행과 사내 적용.
- 설계·세부 정본: 원장 5장 DEV-774, 6.113장. 연쇄 기록은 5장 「CR-122 cascade」에 적는다.

## CR-121 — prs-links 재색인의 부분 갱신 경합과 해제된 스택 이력: 필요한 간선이 올바른 내용·권한으로 모두 있는 새 인덱스만 전환한다 (2026-09-25)

- 유형: correction(설계 결함 — 간선의 부분 갱신이 재색인의 새 인덱스에서 대상 간선을 찾지 못해 재색인이 실패했고, 같은 경로에서 해제된 스택 이력이 색인에만 있어 재색인이 그것을 조용히 잃었다). `FR-ING-008` AC-11과 `FR-REL-006` AC-6 신설, `OD-017`을 열고 같은 날 사용자 결정으로 닫는다. **기존 FR의 요구사항 문장과 AC 문장은 바꾸지 않는다. 안정 ID 재번호화 0건.** 상태: **closed** — main `df3d8ef`(PR #237 squash 병합, 2026-09-25). worktree `/home/roqkf/pr-search-wt/links-reindex`(브랜치 `feature/cr121-links-reindex`), 기준 main `ee72de7`(CR-119·CR-120 병합 기록 뒤).
- 요청: 사용자 지시(2026-09-24, 18차 착수 지시의 작업 B) — `agent-context/upstream-feedback.md` 「prs-links 재색인 shadow_write_failed」(사내 `0.1.0-pilot.18` 운영 보고, 2026-09-23). prs-links v1 재색인이 shadow 쓰기의 `document_missing_exception`으로 실패했고 v1이 계속 서비스 중이다. 지시는 오류 메시지만 없애지 말고, 처리 순서·지연·재시작과 관계없이 필요한 간선이 올바른 내용과 권한으로 새 인덱스에 있을 때만 전환하게 하라는 것이다. 부분 payload에 `scripted_upsert`·`doc_as_upsert`를 붙여 근거·권한 필드 없는 문서를 만드는 해결은 지시가 기각했다. 사용자 결정(2026-09-25, `OD-017`): 해제된 `stacks_on` 이력을 PostgreSQL에 저장하고, 스택 간선은 그 상태의 전체 쓰기로 바꾸며, 배포 전부터 서비스 인덱스에만 있는 스택 간선은 일회성 가져오기 명령으로 옮긴다.
- 확인된 결함 경로(격리 인프라에서 재현, 수정 전 코드에서 두 사례가 실패): (1) 재구축은 source를 PR 번호·SHA 순서로 다시 파생하는데(JOB-REL-006 경로), source 하나를 처리하면 그 source를 **대상**으로 삼는 다른 source의 참조 간선을 해결 상태로 부분 갱신한다(`resolveReferencesTo` → `resolveReferenceLinks`). 후보는 서비스 별칭에서 찾고 같은 갱신을 shadow에도 보내는데, 그 간선의 소유 source가 재구축에서 아직 처리되지 않았으면 shadow에 문서가 없어 `document_missing_exception`이 나고 울타리가 잡을 `failed`로 만든다 — 대상 PR을 먼저, 소유 PR을 나중에 처리하는 사례로 사내 보고와 같은 실패를 재현했다. (2) **숨은 결함**: 해제된 스택 간선(`FR-REL-006` AC-3, DEV-238)은 Elasticsearch에만 있었다. 성립 조건이 「하위 PR의 base = 다른 **열린** PR의 head」라서 상위 PR이 병합되거나 분기가 바뀌면 현재 스냅숏에서 그 관계가 보이지 않는다. 재구축은 해제 간선을 만들지 못했고, 잡은 `completed`로 끝나 새 인덱스에서 그 간선이 조용히 사라졌다(ADR-004 공백 — 재현했다). 스택 해제 표시(`setLinkDetached`)도 (1)과 같은 부분 갱신이라 같은 실패 경로였다. (3) 결과 판정의 구멍 셋: 서비스 bulk 응답에 항목이 없어도 성공으로 셌고, msearch 항목 오류를 「대상 없음」으로 읽어 간선을 미해결로 굳혔으며, `runReferenceRebuild`가 파생이 불완전한(`complete=false`) source를 만나도 커서를 넘겨 그 source를 끝난 것으로 다루었다. (4) 전환 전 검증에 prs-links의 기대가 없어(`expectedDocuments=null`) 간선의 존재도 내용도 대조하지 않았다.
- 보고서와 다른 점: 보고서는 upsert 스크립트가 문서 존재를 가정한다고 했지만, 실패한 연산은 upsert가 아니라 **기존 간선의 부분 갱신**(해결 상태·해제 표식)이다. 문서가 없는 것은 그 간선의 소유 source가 재구축에서 아직 처리되지 않았기 때문이다. 요청 둘(「`document_missing_exception`을 무시」, 「`scripted_upsert: true`로 없으면 생성」)은 받지 않았다 — 앞의 것은 새 인덱스에 간선이 빠진 채 전환하게 하고, 뒤의 것은 근거·접근 범위 필드가 없는 간선 문서를 만든다.
- 범위: (a) `packages/es` — `sendLinkBulk`의 항목 판정을 고친다: 서비스 항목이 없거나 연산 종류가 다르면 실패다. shadow의 부분 갱신이 404 `document_missing_exception`이고 응답의 `_index`가 그 shadow, `_id`가 요청한 간선일 때만 **미처리**로 넘기며 소유 source를 싣는다. 그 밖의 shadow 오류(전체 쓰기의 모든 오류·인덱스 없음·429·5xx·매핑·스크립트 오류·항목 누락)는 기존대로 실패다. 미처리를 받을 기록자가 없으면 실패로 올리고, 소유 source 문서 ID의 저장소가 routing과 다르면 보내지 않는다. msearch 항목 오류는 던진다. 부분 갱신 원시체 `setLinkDetached`·`setLinkResolved`를 없앤다(이중 쓰기 대상 17 → 15, `setLinkResolved`는 운영 호출부가 없었다). (b) `packages/db` — 마이그레이션 037: `pull_request_stack`(ENT-REL-003, 스택 관계의 정본 — 성립한 적 있는 관계만 행이 되고 지우지 않는다)과 `reindex_link_pending`(재색인 잡의 미처리 source 작업 대기열, 세대 표식). 울타리는 실행 중인 prs-links 재색인의 **바로 그** 대상 인덱스에서 난 미처리만 그 잡의 대기열에 남기고, 아니면 실패로 올린다. (c) `apps/pipeline-worker` — 스택 파생은 정본 행을 먼저 맞춘 뒤 그 하위 PR의 행 전부를 전체 쓰기로 낸다(해제 포함). 역방향 재평가는 서비스 색인 대신 정본 행에서 하위 PR을 찾는다. 재구축 포트(`createLinkRebuildPort`)를 운영 배선과 시험이 함께 쓰고, 불완전한 source를 미처리로 남긴다. 재색인은 검증 전에 미처리의 소유 source를 최신 정본에서 다시 파생해 회수하고(읽은 세대만 지운다, 상한 5라운드), 전환 전 검증은 재구축과 같은 계획 함수로 기대 간선을 계산해 대상과 `link_id`마다 대조한다(누락·잉여·필드 불일치·고아·남은 미처리·계획 불가·가져오지 않은 스택). 전환 울타리가 미처리를 다시 보고, 있으면 울타리를 놓은 뒤 회수하고 다시 검증한다(상한 3). `prsctl links import-stacks [--dry-run]`이 배포 전 서비스 스택 간선을 정본으로 한 번 옮긴다. (d) 시험 — 재현 시험이 운영 배선과 같은 포트로 재색인을 돌리고, 관계 시험의 정리에 스택 정본을 더한다.
- 결정과 대가: 회수는 소유 source를 최신 정본에서 **다시 파생**한다 — 옛 서비스 간선을 옮기지도, 부분 갱신 값을 나중에 적용하지도 않는다. 미처리는 메모리가 아니라 잡에 묶인 표에 남아 잡이 멈췄다 재개돼도 이어진다. 이 표는 「재색인 상태 표를 늘리지 않는다」(DEV-299)의 예외다 — 상태가 아니라 크기가 정해지지 않는 작업 대기열이라 잡 행의 `progress`에 담지 않는다(DEV-772). 검증은 **고치지 않는다** — 읽기만 하고, 고치는 것은 검증 전의 회수다. 그래서 검증 중에 들어온 변경과의 경주는 전환을 막는 쪽으로 실패할 수 있고, 그때는 다시 실행한다. 스택의 정본을 PostgreSQL로 옮긴 것은 ADR-004를 지키기 위해서다 — 해제 이력은 현재 스냅숏에서 다시 계산할 수 없다. 서비스 인덱스에서 PostgreSQL로 가는 경로는 일회성 가져오기 하나뿐이고(파생 행을 덮지 않는다), 가져오지 않은 스택 간선이 서비스 인덱스에 남아 있으면 재색인은 전환하지 않는다. 검증 비용이 는다 — 전환 전에 모든 source의 기대 간선을 참조 대상 조회까지 다시 계획하므로 재구축의 파생 계획만큼 시간이 더 든다(사내 규모는 NOT MEASURED).
- 제외: prs-commits 재색인(CR-119에서 끝났다), 운영 서버 변경·사내 배포·사내 데이터 변경(가져오기 명령의 사내 실행 포함), Release 발행, 시퀀스·M 번호·에폭·head의 의미 변경, 참조 추출·해석 규칙과 스택 성립 조건(`FR-REL-006` AC-1~AC-5)의 변경, 검증 중 경주를 흡수하는 재확인(재실행으로 다룬다).
- 설계·세부 정본: `FR-ING-008` AC-11, `FR-REL-006` AC-6, `OD-017`, ENT-REL-003(데이터 모델 3.1장), JOB-REL-004·JOB-REL-006·JOB-ING-006(비동기 3.4·3.5장), RUNBOOK 7.I, WP-104, DEV-766~DEV-773. 연쇄 기록은 5장 「CR-121 cascade」에 적는다.
...
- 요청: 사용자 지시(2026-09-24, 18차 착수 지시의 P0) — main `3d831a9`의 CI run 36003771848이 failure였다. integration 잡은 success였고 verify 잡의 `test:e2e`에서 209건 중 1건(`apps/web/e2e/flow-003.spec.ts:176` 「뒤로가기로 커밋 상세를 거쳐 검색으로 돌아온다」)이 213행 `toHaveURL(/\/search/)`에서 `9 × unexpected value ".../commit/acme/payments/...?from_q=..."`로 실패했다. 지시는 문서 커밋이라는 이유로 flaky라고 단정하지 말고, 같은 코드에서 단독·반복 실행으로 제품의 뒤로가기 결함과 시험의 대기·경합을 가르라는 것이다. 녹색이 나올 때까지 재실행하거나 skip·기대값 약화·근거 없는 시한 연장을 하지 않는다.
- 분류(실측, 원장 6.109): **시험의 대기 문제이며 제품의 이동 로직 결함이 아니다.** PR 상세 링크(`linked-pr-link`)는 진짜 `<a href>`라 PR 상세는 새 문서로 열리고, Playwright의 Chromium은 back-forward cache를 끈 채로 뜨므로 첫 `goBack()`은 커밋 상세를 새 문서로 다시 불러온다. 그 문서의 SSR HTML에 이미 `commit-detail`이 있어 시험의 URL 판정과 `toBeVisible()`이 수화 전에 풀리고, 그 순간 부른 두 번째 `goBack()`(같은 문서 안의 popstate)을 App Router는 청취자를 수화 뒤 효과에서 붙이므로 놓친다. 이어지는 수화의 `history.replaceState`가 `/search?q=` 항목을 커밋 URL로 덮거나(CI와 같은 213행 실패), URL은 맞지만 화면이 커밋 상세에 머문다(마지막 판정의 시한 초과).
- 실측: 같은 코드에서 단독 40회 실패 0, 작업자 4 반복 40회 실패 3, 파일 전체 140회 실패 2, CI 모사 전량(작업자 2) 3회 모두 209건 통과. 계측 322회에서 자동 이동 `pushState`는 늘 1회였고 두 번째 뒤로가기 뒤 push는 0회였다 — 과거 가설(push 두 번, 검색 화면의 튕김)은 아니다. 실패 19건은 모두 두 번째 이동이 Next의 popstate 청취자 등록보다 앞이었고, 통과 261건은 모두 뒤였다.
- 범위: `apps/web/e2e/flow-003.spec.ts` 한 파일. 두 번째 `goBack()` 전에 클라이언트에서만 생기는 신호 `data-screen-state="ready"`를 기다린다(같은 파일의 다른 시험이 이미 같은 판정을 쓴다). 도착 뒤 세션 히스토리 항목(Navigation API)을 판정해 push 중복과 항목 덮어쓰기가 실패 메시지에서 갈리게 한다. DEV-689가 근거 없이 늘린 15초 시한은 기본값으로 되돌린다.
- 결정과 대가: 기대값을 약화하지 않는다 — URL·검색어 판정은 그대로이고 `ready`는 `toBeVisible()`보다 강한 조건이다. 원인이 시험 코드에 있으므로 CI 재실행으로 녹색을 얻는 방식은 쓰지 않았다. 실사용자도 새 문서로 돌아온 직후 수화가 끝나기 전에 뒤로가기를 한 번 더 누르면 같은 일을 겪을 수 있으나 App Router의 특성이다. `linked-pr-link`를 클라이언트 `<Link>`로 바꾸면 그 창이 사라지지만, 가운데 클릭·새 탭을 위한 「진짜 링크」 설계를 바꾸는 제품 변경이라 이 CR에서 하지 않는다(DEV-758 잔여).
- 제외: 제품 코드, DEV-708이 넣은 자동 이동 가드(원인 진단은 틀렸으나 가드 자체는 무해하다 — 되돌리지 않는다), CI 설정.
- 설계·세부 정본: 원장 5장 DEV-758과 6.109장. 연쇄 기록은 5장 「CR-118 cascade」에 적는다.

## CR-117 — `git merge dev`로 받아 온 dev 체인 커밋의 PR 연결: 원본 커밋은 「그 PR이 새로 가져온 커밋」이다 (2026-09-24)

- 유형: correction(요구사항 공백 — 용어집의 「원본 커밋」 정의가 두 문장으로 갈려 있었고, 구현은 GitHub의 PR 커밋 목록 전체를 원본 커밋으로 읽었다). `FR-SRCH-002` AC-7 신설, `FR-SRCH-003` AC-5 신설, 용어집 「원본 커밋」 정의 보완, `OD-016` 신설과 동시 결정(resolved). **기존 FR의 요구사항 문장과 AC 문장은 바꾸지 않는다. 안정 ID 재번호화 0건.** 상태: **closed** — main `65acf83`(PR #230 squash 병합, 사용자 지시 2026-09-24). 구현·검증·독립 리뷰는 원장 6.108, 연쇄와 병합 판정은 5장 「CR-117 cascade」. 사내 배포 SHA는 NOT VERIFIED이고 내부망 적용은 NOT RUN이다. 기준 main `ea59bb6`(착수는 `5c52aa2`에서 했고, 그 뒤 main에 올라온 사용자의 `agent-context/upstream-feedback.md` 정리 커밋 위로 rebase했다 — 코드 diff는 rebase 전후 같다), worktree `/home/roqkf/pr-search-wt/cr117-source-commits`(브랜치 `feature/cr117-source-commits`).
- 요청: 사용자 지시(2026-09-24) — `agent-context/upstream-feedback.md` 「`git merge dev`로 인해 `source_commit_shas`가 dev 체인 커밋으로 오염된다」(사내 `0.1.0-pilot.18` 운영 보고, 2026-09-23). 커밋 `ebc781d`(merge_seq 1183, PR #1671의 squash 머지)의 commit history에 PR #983과 #1855가 함께 보인다. 보고서 수치로는 #983의 GitHub 커밋 목록 207개 중 202개, #1855의 85개 중 46개가 dev 체인 커밋이다(이 저장소에서 재현한 값이 아니다).
- 사용자 결정(2026-09-24, `OD-016`): **원본 커밋은 그 PR이 새로 가져온 커밋이다.** 판정 기준은 「저장소가 추적하는 브랜치의 현재 체인에 이미 오른 커밋은, 그 커밋을 체인에 올린 PR에만 속한다」이다. 사내 GHE 정책(브랜치는 dev와 피처 브랜치뿐이고 main과 승격 PR이 없다, 병합은 squash)을 전제로 확인받았다.
- 확인된 결함 경로(현 코드): (1) 보강이 `GET /pulls/{n}/commits` 결과를 그대로 `source_commit_shas`로 싣고(`enrich.ts`), 관계 채택이 그 전부를 `source` 근거로 받는다(`snapshot.ts` → `adoptLinkObservation`). 그 커밋이 이미 대상 브랜치 체인에 있는지 보는 판정이 어디에도 없다. GitHub은 PR의 기준점(base) 쪽에 없는 커밋을 PR 커밋으로 돌려주므로, 기준점이 옛 시점에 머문 PR의 피처 브랜치가 `git merge dev`를 하면 그 사이 dev에 들어간 다른 PR의 머지 커밋이 목록에 섞인다. CR-116 보고의 #2355와 같은 모양이며, 그 보고의 영향 표에 있던 #983(250개)이 이번 사례다. CR-116의 완전성 조건 넷은 그 목록이 원격의 전부라는 것만 증명하므로 이 연결을 **정상으로 확정**한다. (2) PR 투영(`buildCommitDocuments`)과 재색인(`rebuildProjectedCommits`)이 목록의 모든 SHA에 `role: source_commit` 문서를 쓴다. 조건부 업서트는 버전(웹훅 수신 시각)이 더 크면 `role`·`base_branch`를 대입하므로, `git merge dev` 뒤의 PR 이벤트가 dev 체인 머지 커밋의 역할을 `source_commit`으로 덮는다. 커밋 보강은 체인 밖 경로(`EVT-ING-003`)에서 `role`을 싣지 않으므로 되돌리지 못한다. (3) PR 상세(`getPullRequestDetail`)가 PR 문서의 원시 목록을 그대로 `source_commits`로 내므로 PR 화면도 같은 오염을 보인다.
- 지킬 조건: 원시 관측은 바꾸지 않는다 — `pull_request_commit_link`의 `source` 행과 관측의 완전성·삭제 권한은 여전히 GitHub 목록 그대로이며 CR-116의 규율이 그 위에서 유지된다. 새 규칙은 **연결을 읽는 자리**에만 선다. 이 CR은 `merge_seq`·`merge_number`·`seq_epoch`·head·원격 M 태그·제목 표기를 읽거나 바꾸지 않고, M 번호 채번 경로(`mnumber.ts`)를 건드리지 않는다. 머지 시퀀스 채번·재채번·복구 트랜잭션에는 관계 재투영 의도 한 문장만 더한다.
- 범위: (a) `packages/db` — 유효 연결 술어 하나를 관계 읽기 전부(러너 투영, 재색인 열거, 번호 조회 단건·배치, 전체 열거, 미확정 계수)가 함께 쓴다. 현재 에폭 체인 소속 조회와, 체인이 바뀐 커밋의 관계 세대 올림을 더한다. (b) `apps/pipeline-worker` — 투영 네 경로와 재색인이 체인 위 SHA에 원본 커밋 문서를 만들지 않는다. 채번·강제 푸시 재채번·복구 재채번이 같은 트랜잭션에서 영향 커밋을 관계 투영 큐에 다시 넣는다. `prsctl links plan|apply`가 체인 규칙으로 빠지는 번호를 관측 확정 여부와 무관하게 제거 대상으로 세고, 덮인 역할을 되돌린다. (c) `packages/es` — 체인 커밋의 역할을 `source_commit`일 때만 되돌리는 단방향 쓰기. (d) `apps/search-api` — PR 상세가 커밋의 연결 PR에 그 PR이 없는 항목을 빼고 `source_commits_excluded`를 싣는다. (e) `apps/web` — 커밋 목록에 제외 안내.
- 결정과 대가: 상류 요청 1(투영 전 필터)을 받아들이되 **쓰기 시점이 아니라 읽기 시점**에 둔다. 원시 관측을 지우면 강제 푸시로 체인에서 빠진 커밋의 연결을 되살릴 근거가 없고, 그때는 GHE를 다시 읽어야 한다. 요청 2(`compareCommits(base, head)`)는 채택하지 않았다 — 병합이 끝난 PR에서는 범위가 비거나 뜻이 달라지고, 원격 호출이 늘며, 사내 프로파일에서는 요청 1과 결과가 같다. 기각한 기준(「PR의 base 브랜치에 이미 있는 커밋만 뺀다」)과 그 이유는 `OD-016`에 적는다. 대가: 우리 PR 화면의 원본 커밋 수가 GitHub Commits 탭과 달라진다(#983 기준 207 → 5). 그 차이는 `source_commits_excluded`와 화면 안내로 드러낸다. 체인 판정은 first-parent 체인만 본다 — squash-only에서는 충분하지만, merge commit 방식 병합이 섞이면 두 번째 부모 쪽으로 대상 브랜치에 들어온 커밋은 걸러지지 않는다.
- 제외: 같은 반입에서 함께 보고된 prs-commits 재색인 verify 과대 계산과 prs-links 재색인 shadow 부분 갱신 실패 — 별도 CR로 연다. 운영 서버 변경, 사내 배포·적용, Release 발행. `role` 소유권의 나머지(원본 목록에서 빠진 체인 밖 커밋의 `source_commit`, DEV-752 잔여). 독립 검토가 찾은 둘은 편차로 남기고 이 CR에서 코드로 고치지 않는다: 추적 브랜치 목록을 바꿀 때 관계 재투영 의도가 남지 않는 것(DEV-756 — SRS 예외 칸과 RUNBOOK 7.G에 운영 조치를 적었다)과 역할 판정 규칙이 두 갈래인 것(DEV-757).
- 시험 원칙: 격리 인프라(`prs-cr117-*` 컨테이너, DB `prs_test_cr117`). 대표 시험(`git merge dev` 모양)을 현 코드에서 먼저 실패시킨 뒤 고친다. 술어·체인 SHA 건너뛰기·세대 올림·복구 분류를 각각 되돌려 시험이 죽는 것을 본다.
- 설계·세부 정본: `FR-SRCH-002` AC-7, `FR-SRCH-003` AC-5, `OD-016`, 용어집 「원본 커밋」, JOB-REL-008, RB-29, RUNBOOK 7.G, WP-102. 연쇄 기록은 5장 「CR-117 cascade」에 적는다.

## CR-116 — 커밋에 남는 과거 PR 번호: 관계 정본과 전용 투영기 (2026-09-23)

- 유형: correction/reliability(ADR-004 결함 — 관계 정본이 PostgreSQL에 없고 색인의 합집합에만 있었다). ADR-004 Amendment, `FR-SRCH-002` AC-6 신설, `FR-ING-004` AC-6 신설, `FR-ING-008` AC-9 신설. **기존 FR의 요구사항 문장과 AC-1~AC-5는 바꾸지 않는다. 안정 ID 재번호화 0건.** 상태: **closed** — main `f9cda82`(PR #228 squash 병합, 2026-09-23). 구현·검증·독립 리뷰는 원장 6.107, 연쇄와 병합 판정은 5장 「CR-116 cascade」. 기준 main `83d30fb`, worktree `/home/roqkf/pr-search-wt/cr116-pr-links`. 사내 배포 SHA는 NOT VERIFIED이고 내부망 적용은 NOT RUN이다.
- 요청: 사용자 지시(2026-09-23) — `agent-context/upstream-feedback.md` 「`pull_request_numbers` union semantics로 과거 PR 번호가 커밋에 영구히 남는다」(사내 `0.1.0-pilot.17` 운영 보고, 2026-09-22)의 요청 1~4 전부. 사내 실측: 영향 PR 279개, 잘못 연결된 커밋 3,481개(1차) + 660개(웹훅 재발생분), PR #2355 하나가 dev 체인 커밋 110개에 남았다.
- 확인된 결함 경로(현 코드): (1) `packages/es/src/upsert.ts`의 `params.union`은 `HashSet` 합집합이라 **한 번 더해진 PR 번호를 빼는 경로가 없다**. 대입으로 바꾸는 것만으로도 안 된다 — 커밋 하나가 여러 PR에 속하므로(데이터 모델 5장의 N:M) 한 PR의 투영이 전체 배열을 쓰면 다른 PR의 번호가 사라진다. (2) `pull_request_snapshot.document`는 **최신 스냅숏 하나**다. 갱신이 옛 `source_commit_shas`를 덮어쓰므로 지워야 할 옛 SHA가 그 순간 사라지고, 「old → new 차이」만으로는 이미 굳은 오염을 찾을 수 없다. (3) 그 문서에는 `enrichment_errors`가 없어 **커밋 조회 실패로 나온 `[]`와 진짜 빈 목록이 구분되지 않는다**. (4) `GET /pulls/{n}/commits`는 GitHub이 250건에서 자르고 그 응답에는 `rel="next"`가 없어 `getAllPaged`의 `truncated`가 **거짓**이 된다 — 400 커밋 PR이 「250건이 전부」로 읽힌다. (5) `reindex.ts`의 `rebuildProjectedCommits`는 `state` 확인 없이 `merge_commit_sha`를 머지 커밋으로 써, 열린 PR의 **시험 병합 SHA**에 `role: merge_commit` 문서를 만들어 왔다(실시간 투영은 `merged === true`로 막는다). 반대로 실시간 투영의 `pr.merged === true`는 **백필의 목록 끝점이 그 필드를 주지 않아** 병합된 PR의 머지 커밋 문서를 만들지 못했다 — 세 경로가 서로 다른 판정을 쓰고 있었다(DEV-747·DEV-753). (6) `containments.ts`는 `pull_request_numbers[0]`을 「그 커밋의 PR」로 읽는데, 합집합의 `HashSet` 순서는 비결정적이라 그 값은 아무 PR이었다.
- 지킬 조건: 관계 대입은 **관계 재발견이 아니다** — 이 CR은 `merge_seq`·`merge_number`·`seq_epoch`·`head`·권한 정보·원격 M 태그를 읽지도 바꾸지도 않는다. CR-113의 시퀀스 replay·에폭 검증과 CR-115의 커밋 M 투영은 후퇴하지 않으며, 재색인 전환 전 검증에 항목이 하나 늘어날 뿐이다. 삭제는 **완전성을 증명한 관측**에만 허용한다 — 불완전한 목록은 관계를 더할 수는 있어도 뺄 수 없다. 첫 PR만 보이거나 merged PR만 허용해 문제를 숨기지 않는다.
- 범위: (a) 마이그레이션 036 — 관계 정본 `pull_request_commit_link`(ENT-CORE-009, 근거 `source`·`merge`가 키에 있어 원본에서 빠져도 실제 병합 근거는 남는다), 관측 신뢰도 `pull_request_link_observation`(ENT-CORE-010, `commits_complete`가 **삭제 권한**이고 `verification_state`가 `verified`·`unverified`·`conflict`·`pending_refetch`), 커밋별 투영 의도 `commit_link_state`(ENT-CORE-011, 관계 전용 generation·lease·tombstone), `job.type`에 `pr_link_repair`. 기존 스냅숏에서 관계를 seed하되 관측은 전부 `unverified`다 — 근거 없는 목록에 삭제 권한을 소급해 주지 않는다. (b) `packages/domain` — `EnrichedPullRequest.commits_count`(원격이 말한 커밋 수), `IngestionEnriched.source_commits_complete`. (c) `apps/pipeline-worker` — 보강이 완전성 넷(조회 성공·우리 상한 미달·원격 커밋 수 일치·읽기 전후 head/base 동일)을 판정하고, `recordProjectionSnapshot`이 관계 채택·세대 올림을 **머지 게이트 앞에서** 스냅숏과 같은 트랜잭션에 수행하며(열린 PR도 처리), 관계 투영 러너(JOB-REL-008, `project` 역할, M 스위치와 무관)가 정본을 실행 시점에 읽어 대입하고, 재색인이 `replayCommitLinks`로 대상 인덱스에 직접 복원하며 전환 전 검증이 누락과 **잉여**를 양방향으로 대조한다. 복구 경로 `prsctl links plan|apply|refetch|status`. (d) `packages/es` — 전용 투영기 `applyCommitLinks`(세대 가드·충돌 판정·`[]` 대입), 매핑에 `pr_links_generation`·`pr_links_state`, `UpsertRequest.union`에서 `pull_request_numbers`를 타입·실행 시점 둘 다로 금지. (e) `apps/search-api` — `containments.ts`가 후보 중 `merge_seq` 최소를 고른다. (f) 목 GHE가 PR 상세에 `commits`를 싣고 API 자체 상한을 흉내 낸다.
- 결정과 대가: 상류 요청 1(「ES의 옛 목록을 읽어 먼저 지우고 기존 합집합을 유지한다」)은 **채택하지 않았다** — 삭제의 근거를 색인에서 읽는 것이 ADR-004 위반이고, 지우기와 합집합 사이에 늦은 이벤트가 끼면 곧바로 다시 오염된다. 요청 2의 stateful 대입을 택하되 「PR별 소유권 + 커밋별 전체 집계」로 구현했다. 요청 3(빈 배열)은 `[]`를 `_source`에 대입하고 필드를 지우지 않는 것으로 답했다. 요청 4(재색인 재투영)의 전제는 정정한다 — `rebuildCommits → rebuildProjectedCommits` 경로는 **이미 있었고** 문서와 역할을 복원해 왔다. 없던 것은 그 경로의 관계 복원이며 이번에 더한다. 대가: 표 셋과 러너 하나가 늘고, 관계 삭제가 완전한 관측을 기다리므로 GHE를 다시 읽지 못하는 동안 잘못된 번호가 남아 있을 수 있다(그 사실은 `blocked`로 보고된다). 롤링 업데이트 중 남은 구버전 워커는 여전히 합집합을 쓰므로 새 세대 가드로 막을 수 없고, `commit_link_conflict_total`이 그 조기 경보다.
- 제외: 운영 서버 변경, 공용 인덱스 삭제, 사내 배포·적용, 새 Release 발행, 보호 규칙 우회. `role` 필드의 소유권 이전(DEV-752, open — 원본 목록에서 빠진 커밋의 `role`은 그대로 남는다. 사내에서 오염된 커밋들은 보강이 준 `role: merge_commit`을 갖고 있어 화면에 드러나지 않는다). 커밋 hit DTO의 관계 상태 노출. **재색인이 GHE 전수조회에 의존하게 만드는 것**(재구축은 원격을 읽지 않는다). `refetch`는 **확정되지 않은 PR 전부**를 읽으므로 마이그레이션 직후에는 그 저장소의 거의 모든 PR이 대상이다 — `--pr`(계획이 알려 주는 `blockingPullRequests`)와 `--limit`으로 좁히는 것이 운영 절차다.
- 시험 원칙: 격리 인프라(`prs-cr116-*` 컨테이너 신규, DB `prs_test_cr116`). 대표 시험은 **현 코드에서 실패함을 먼저 실측했다** — 같은 시험 파일을 `git stash`로 되돌린 `83d30fb` 위에서 돌려 `[521, 2355]`가 남는 것을 보았고, 새 코드에서 `[521]`이 된다. 단위(합집합 금지 가드·문서 생성기), 통합(실 PG·ES — 목록 축소 시 해당 PR만 제거·다른 PR 보존·늦은 옛 이벤트 부활 금지·동시 add/remove·같은 버전 같은 집합/다른 집합·commits 실패와 리뷰 실패 구분·250 경계와 400 커밋·열린 PR의 시험 병합 SHA·`[]`와 필드 부재·PG 커밋 직후 종료·lease 만료 수렴·전체 대조·dry-run 무변경·apply 직전 가드·반복 멱등·재색인 replay·전환 전 양방향 검증), 회귀(잡↔러너 쌍). 변이: 버전 가드·완전성 게이트·세대 가드·머지 게이트·커밋 수 대조를 각각 지워 시험이 죽는 것을 본다. 독립 리뷰가 정상 N:M 삭제, 불완전 근거, 과거 add 부활, 동시성, bootstrap/reindex, 기존 시퀀스·태그 기능 회귀를 본다.
- 설계·세부 정본: `FR-SRCH-002` AC-6, `FR-ING-004` AC-6, `FR-ING-008` AC-9, ADR-004 Amendment, JOB-REL-008, ENT-CORE-009·ENT-CORE-010·ENT-CORE-011(마이그레이션 036), RB-29, RUNBOOK 7.G, WP-101. 연쇄 기록은 5장 「CR-116 cascade」에 적는다.

## CR-115 — 확정된 M 번호를 원격 저장소의 lightweight 태그로 굳힌다 (2026-09-22)

- 유형: scope(기능 누락 보완 — `FR-SEQ-012` 신설, `OD-015` 신설, ADR-026 신설·ADR-022 Amendment). **읽기 전용 원칙의 세 번째 예외이자 시스템이 스스로 하는 두 번째 GHE 쓰기다.** 상태: **closed** — main `83d30fb`(PR #226 squash 병합, 사용자 지시 2026-09-22). 구현·검증·독립 리뷰는 원장 6.106, 연쇄와 병합 판정은 5장 「CR-115 cascade」. CR-114 브랜치 위에 stacked로 시작했고(기준 `7f980be`), CR-114가 PR #225로 main `9782ba9`에 병합된 뒤 `rebase --onto`로 다시 올려 base를 `main`으로 옮겼다. 사내 배포 SHA는 NOT VERIFIED.
- 요청: 사용자 지시(2026-09-22) — `agent-context/upstream-feedback.md` 「수집 파이프라인에서 머지된 M-번호 단위로 커밋에 태그를 달아야 한다」(사내 `0.1.0-pilot.17` 확정 요건, 2026-09-22)의 요청 1~6 전부. M 번호를 `git rev-parse M-1900-1450`·`git log M-1900-2010..M-1900-2130`처럼 Git revision으로 쓰려면 원격 저장소에 그 이름의 ref가 있어야 한다.
- 확인된 현 코드: (1) 확정된 M 번호는 PostgreSQL 정본·PR 문서·PR 제목 접두에만 있고 Git ref로는 존재하지 않는다. (2) `prs-commits` 문서에는 `merge_seq`만 있고 M 값 세 필드가 없어 `kind:commit`의 `mnum:`이 성립하지 않으며, 재색인 뒤 채울 경로도 없다. (3) 표기(`@prs/github-annotate`)의 pacing 유틸리티가 그 패키지에 갇혀 있어 두 번째 쓰기 경로가 재사용할 수 없었다. (4) 목 GHE 서버는 참조 API(`git/refs`)를 모른다.
- 범위: (a) 새 패키지 `@prs/github-tag`(설정 `GHE_TAG_*`·전역 스위치 `MNUMBER_TAG_ENABLED` 기본 꺼짐, 순수 판정 `decideTagAction`·`resolveTagTarget`·`mergeNumberOfTagName`, 클라이언트 `readTagRef`·`createTagRef`·`listTagRefs` — **이동·삭제 메서드 없음**, 오류 분류 `TagApiError`, `node:http` 목 GHE). `@prs/github`로 옮긴 `PassDeadline`·`WriteGate`(표기는 재수출). (b) 마이그레이션 035 — `merge_sequence`에 `tag_state`·`tagged_at`·`tag_attempt_id`·`tag_result_reason`·`tag_found_sha`, `repository`에 `tag_enabled`(기본 true)·`tag_blocked_at`·`tag_blocked_reason`, `sequence_work.kind`에 `tag`, `job.type`에 `mnumber_tag_reconcile`. (c) `apps/pipeline-worker` — 채번 트랜잭션이 PR마다 `tag` work를 번호·checkpoint와 같은 트랜잭션에 남김(`mnumber.ts`); `tag` 역할(JOB-SEQ-007): durable 러너(`mnumber-tag.ts` — 정본 재확인 → GET → 판정 → 간격 → 쓰기 직전 재확인 → POST 하나 → 정본 기록 → 실제 생성만 감사 `merge_number.tag`/`system:tag`), 일 1회 잔여 스윕(backfill), 대조 잡 러너(`mnumber-tag-reconcile.ts` — `missing` 재요청·`ok`·`conflict` 보고·`unexpected` 보고, dry-run 무변경), CLI `mnumber-tag-cli.js`(`prsctl mnumber tags reconcile|status`); `sequence` 역할 러너는 `tag`를 집지 않음; `materialize`가 PR·커밋 문서 둘 다 씀; 커밋 보강이 `merge_commit` 문서를 만든 뒤 `materialize` 재요청; `prs-commits` 재색인 뒤 재투영 의도; 지표 `mnumber_tag_total{result}`·`mnumber_tag_conflict_total`. (d) `packages/es` — `prs-commits` 매핑에 M 세 필드, `applyMergeNumberToCommitDocument`(역할·저장소·브랜치·SHA 가드 + 에폭 스크립트), `clearMergeNumbersBelowEpoch`가 커밋 인덱스도. (e) `search-api` — API-ADM-002 `mnumber_tag_reconcile`(target `owner/repo@브랜치`, `dry_run` 거절), API-ADM-001 `PATCH`의 `tag_enabled`. (f) web — A-003 실행 폼에 `M-number tag reconcile (GHE tags)`. (g) deploy — compose `x-tag-env`·`worker-annotate`에 `annotate,tag`, `.env.example` 태그 절, `prsctl mnumber tags`, k8s `pipeline-worker-annotate`에 `annotate,tag`와 `prs-tag-secrets`. (h) 회귀 — 잡↔러너 쌍, 태그 클라이언트의 이동·삭제 부재.
- 결정과 대가: 태그 이름은 `FR-SEQ-008` AC-5의 표기 문자열 그 자체이고 lightweight(ref만)다 — annotated 태그는 tagger·서명·메시지가 붙어 「누가 만들었는가」를 이 제품이 주장하게 된다. **같은 이름의 태그가 다른 것을 가리키면 옮기지 않는다** — Git 태그는 이미 인용된 뒤 옮기면 그 인용이 조용히 다른 커밋을 가리키므로(ADR-007 규칙 5의 원격 확장) 코드에 이동·삭제 경로를 두지 않고 `conflict`로 보고만 한다. 자격은 태그 전용 App(`contents:write`)이며 표기 App과도 나눈다 — 권한 반경이 더 넓어 한 키의 유출이 두 반경을 함께 열지 않게 한다(ADR-026). 대가: GHE 쓰기 역할이 둘이 되고 App 등록·시크릿·ruleset 운영 절차가 하나 더 늘며, 태그 생성 속도는 쓰기 간격(1초) 상한을 받는다(과거 채번분 backfill은 회차당 상한으로 나눠 돈다). 시퀀스 브랜치가 둘 이상인 저장소는 태그 이름에 브랜치가 없어 만들지 않는다(`OD-015`, 권고안: 저장소별 태그 대상 브랜치 설정). 커밋 검색 결과·화면 배지에 커밋 문서의 M 값을 노출하는 것은 이번에 열지 않았다(DEV-742, 색인 값은 있다).
- 제외: 운영 서버 변경, 릴리스·태그 발행(이 제품의 릴리스 태그를 말한다 — 대상 저장소의 `M-*` 태그는 기능이다), 태그 이동·삭제·정정 자동화, GHE ruleset 생성(운영 절차로 권고만), annotated 태그, 다중 시퀀스 브랜치 저장소의 태그(`OD-015`), 커밋 hit DTO·web 배지의 M 노출(DEV-742), `API-SEQ-007` 변경, 에폭 상향 뒤 기존 태그의 일괄 정정.
- 시험 원칙: 격리 인프라(`prs-cr113-*` 컨테이너 재사용, DB `prs_test_cr115`, ES `cluster.name=prs-cr113-isolated`). 단위(설정 스위치·자격·판정·이름 왕복·클라이언트 전송·오류 분류·이동·삭제 부재), 통합(실 PG + `node:http` 가짜 GHE — 생성·멱등·충돌(다른 SHA·annotated)·에폭 상향 충돌·권한 차단·한도·시한 초과 재읽기·미확정 복구·SHA 없음·옛 에폭·다중 브랜치·저장소 해제·러너 상태 전이·`sequence` 러너 비집기·스윕 backfill·차단 쿨다운·대조 dry-run 무변경·대조 재생성·잡 러너·다중 브랜치 거절; 실 git 저장소에 가짜 GHE가 받은 ref를 적용해 `rev-parse`·`show`·`log a..b`·`diff a..b`·`describe --tags`·`checkout`·`log --decorate` 검증; 실 ES — 커밋 문서 가드·에폭·정리; 재색인 commits-only 뒤 재투영 의도; 파수꾼 6파일의 035), 회귀(잡↔러너 쌍, 역할 manifest, 인프라 표, 감사 어휘), 문서 검증기 기준선 대조. 변이: 태그 생성 push·`base_branch` 가드·역할 가드를 각각 지워 시험이 죽는 것을 본다. 독립 리뷰가 쓰기 반경(이동·삭제 부재)·자격 분리·쓰기 직전 재확인·감사 어휘·에폭 규율을 본다.
- 설계·세부 정본: `FR-SEQ-012` AC-1~AC-11, `OD-015`, ADR-026(+ADR-022 Amendment), JOB-SEQ-007, ENT-SEQ-001·ENT-SEQ-006·ENT-CORE-001(마이그레이션 035), API-ADM-001·API-ADM-002, THR-061, RB-28, RUNBOOK 7.F, WP-100. 연쇄 기록은 5장 「CR-115 cascade」에 적는다.

## CR-114 — 검색창 M 번호 문자열 해석과 `mnum:` 단일 값·`base:` 생략 (2026-09-22)

- 유형: scope(기능 누락 보완 — `FR-SRCH-001` AC-7 신설, `FR-SRCH-005` AC-9 보완, `OD-014` 신설). 상태: **closed** — main `9782ba9`(PR #225 squash 병합, 사용자 지시 2026-09-22). 구현·검증·독립 리뷰는 원장 6.105, 연쇄와 병합 판정은 5장 「CR-114 cascade」. 기준 main `13074f8`(착수 시 `origin/main`과 같음). 사내 배포 SHA는 NOT VERIFIED.
- 요청: 사용자 지시(2026-09-22) — `agent-context/upstream-feedback.md` 「검색창에서 M-번호 완전 검색이 안 된다」(사내 `0.1.0-pilot.17`, 2026-09-22)의 요청 1(판별기에 M 번호 패턴 추가)과 요청 2(`mnum:` 스칼라 지원·바인딩 완화)를 다룬다.
- 확인된 현 코드: (1) `detectIdentifier`가 GHE URL·`#N`·정수·hex만 알아 `M-1900-1450`이 `text`로 떨어지고 전문 검색의 토큰 `m`·`1900`·`1450`이 엉뚱한 제목에 걸린다. (2) `mnum:`은 범위 전용(`rangeOnlyKey`)이라 한 건을 찾는 데도 `mnum:1450..1450`이 필요하고 `repo:`+`base:` 둘을 요구한다. (3) `API-SEQ-007`은 저장소·브랜치·에폭 파라미터로만 닿아 검색창 `q`와 연결되지 않는다.
- 범위: (a) `@prs/query` — `merge_number` 해석(`@prs/domain`의 `parseMergeNumber` 재사용 — 정규식을 따로 적지 않는다; `[M-…]` 제목 접두와 소문자 `m-`를 정규화), `mnum:` 단일 값을 닫힌 범위로 옮기는 파서 분기(`SINGLE_VALUE_RANGE_KEYS`), `analyzeMergeNumberBinding`의 `repository_only`와 문구. (b) search-api — `/resolve`가 M 문자열을 **정본**에서 해석(`lookupMergeNumberCandidates`: 코드 → 활성 저장소 → 접근 범위 → 추적 브랜치마다 현재 에폭 `merge_sequence`, 한 REPEATABLE READ 스냅숏; 색인 문서는 표시 필드만 덧댄다), 후보에 M 세 키와 정본 시퀀스 값, 기능 꺼짐은 `reason_code: merge_number_disabled`(200)와 사유별 hint; `resolveMergeNumberRangeEpoch`가 `repo:`만 있을 때 유일한 추적 브랜치로 묶고 둘 이상이면 `branch_required`(400 `INVALID_PARAMETER`, `sequence_space_ambiguous`, `sequence_branches`). (c) `@prs/db` — `repositoryRepo.listActiveRepositoriesByCode`(이름의 유일한 숫자 run이 코드와 같은 활성 저장소). (d) web — `chooseRoute`와 두 workspace의 식별자 판정에 `merge_number`, 후보 타입에 M 키(렌더링 변경 없음). (e) PIPE handoff — OpenAPI additive(`IdentifierKind`에 `merge_number`, `reason_code`에 `merge_number_disabled`, PR 후보의 M 세 키), 합성 예시, CONTRACT_DIFF D-21, manifest 재생성. (f) SRS v2.41, PRD·용어집·매트릭스·화면 흐름·실행 지시서·백엔드·API 계약·WP-099·원장.
- 결정과 대가: 보고의 요청 2 중 「`repo:`·`base:`를 생략하면 접근 범위 내 전체 저장소에서 검색」은 **열지 않았다** — 서수는 시퀀스 공간 안에서만 뜻이 있고(ADR-007 규칙 1) 공간마다 유효 에폭이 달라 `merge_number_epoch` 게이트를 공간마다 걸어야 하며, 같은 코드의 공간이 여럿이면 같은 번호가 다른 PR을 가리킨 채 한 목록에 섞인다(DEV-359의 뿌리). 저장소를 건너 한 줄로 찾는 요구는 코드를 담은 M 문자열(AC-7)이 채운다. 제품 결정으로 `OD-014`에 권고안과 함께 열어 두었다. `base:` 생략은 「고를 것이 없을 때만」이다 — 브랜치가 둘 이상이면 서버가 고르지 않고 거절한다. M 문자열 해석은 색인의 `merge_number`를 읽지 않는다(CR-113의 교훈: 투영은 늦을 수 있고 표기 문자열은 색인에 없다). 단일 값은 `mnum:`에만 연다 — `seq:`·`pr_number:`·시각 키는 범위 전용 그대로다.
- 제외: 운영 서버 변경, 릴리스·태그 발행, `API-SEQ-007` 계약 변경, W-002·W-004의 M 표시 변경, 후보 카드(C-017)의 M 배지 렌더링, 새 마이그레이션·잡·감사 액션·운영 절차.
- 시험 원칙: 격리 인프라(`prs-cr113-*` 컨테이너 재사용, DB `prs_test_cr114`, ES `cluster.name=prs-cr113-isolated`). 단위(판별기 정규화 3종·거절 7종, 파서 단일 값=범위 동치·부정·따옴표·거절 4종·다른 키 범위 전용 유지, 지목 `repository_only`·`seq:` 불변, 경로 선택), 통합(실 PG·ES·Redis — 단일·복수 저장소·복수 브랜치·범위 밖 비누설·미발급·코드 불일치·옛 에폭·기능 꺼짐·색인 문서 없음·정본 우선·절삭·대괄호; 단일 값=범위 동일 결과·부정·0건·`base:` 생략·둘 이상 브랜치 거절과 목록·`base:` 명시·`repo:` 없음 거절·미등록 404), PIPE 계약 시험(단위·실 mTLS). CR-106의 시험 하나(「`base:` 없으면 400」)는 새 규칙에 맞게 고쳤고 「`repo:` 없으면 400」으로 대체했다.
- 설계·세부 정본: `FR-SRCH-001` AC-7, `FR-SRCH-005` AC-9(CR-114 보완), `OD-014`, API-SRCH-001(M 문자열 응답·`merge_number_disabled`), 「식별자 범위 지목 계약」 CR-114 보완, WP-099. 연쇄 기록은 5장 「CR-114 cascade」에 적는다.

## CR-113 — 머지 시퀀스의 Elasticsearch 투영 누락 복구 (durable 재투영·재색인 replay·수동 재투영) (2026-09-22)

- 유형: correction/reliability(ADR-004 결함). 정렬 UI 변경이 아니다. PostgreSQL에 확정된 `merge_seq`가 Elasticsearch 문서에 비치지 않거나 재색인 뒤 사라지는 결함을, **기존 번호를 바꾸지 않고** 정본에서 복원하고 이후 자동으로 수렴하게 고친다. 상태: **closed** — main `a6ea00d`(PR #223 squash 병합, 사용자 지시 2026-09-22). 구현·검증·독립 리뷰는 원장 6.104, 연쇄와 병합 판정은 5장 「CR-113 cascade」. 사내 배포 SHA는 NOT VERIFIED, 내부망 적용은 NOT RUN이다.
- 요청: 사용자 지시(2026-09-22). 사내 `0.1.0-pilot.17` 운영 보고(`agent-context/upstream-feedback.md` — 재색인 뒤 `merge_seq` 보유 PR 문서 0/4,214, 수동 `update_by_query` 뒤 1,457/4,214, DB `merge_sequence` 1,467행)를 계기로 한다. 조사 기준 main `2c3004f`. 착수 시 `origin/main`은 `2c3004f`와 같았고, 이 저장소에는 `pilot.17` 태그·Release가 없다(최신 `0.1.0-pilot.16` = `5f0d7e0`). `5f0d7e0..2c3004f` 사이에 이 CR이 다루는 파일(`packages/es/src/sequence.ts`, `apps/pipeline-worker/src/{sequence,reindex,snapshot,commit-enrich,sequence-work-runner,mnumber}.ts`, `packages/db/src/repositories/sequence-work.ts`)의 차이는 없다. **사내 배포 바이너리의 SHA는 여기서 확인할 수 없으므로 NOT VERIFIED로 남긴다** — 보고의 집계 숫자는 fixture의 유일한 정답으로 쓰지 않는다.
- 확인된 결함 경로(현 코드): (1) 채번·재채번·복구는 PostgreSQL 커밋 뒤 `applySequenceToDocuments`(`update_by_query`, **새로 채번된 구간만**)를 한 번 부르고 실패는 로그·지표로만 남긴다 — 재시도가 없다. (2) PR 문서가 늦게 오거나 `merge_commit_sha`가 늦게 채워지면 그 갱신은 0건 매칭으로 끝나고 다시 오지 않는다. (3) 직접 푸시 first-parent 커밋의 문서는 `sequence.assigned` 이벤트 **뒤에** 커밋 보강이 만들므로 구조적으로 서수를 받지 못한다(`commitCreateFields`에 서수가 없다). (4) 재색인은 스냅숏에서 문서를 만들며 스냅숏에 서수가 없고, `merge_seq`의 replay가 없다(M 번호만 `materialize`를 재예약한다, DEV-597의 「다음 채번이 다시 쓴다」는 전제가 성립하지 않는다). `verifyBeforeCutover`는 서수를 보지 않는다. (5) `sequence_assign` 수동 잡은 새 커밋이 없으면 ES에 아무것도 쓰지 않고, `sequence_reassign`의 `consistent`는 ES를 보지 않는다. (6) Search의 「M number」 정렬은 ES `merge_seq` 정렬이라 누락 문서가 목록 뒤로 밀린다.
- 범위: (a) `packages/es`에 문서 ID 기반 guarded 투영기(`mget` 사전 읽기 + 스크립트 가드 `update`, 문서별 결과 보존, active/shadow 결과 분리) — `applySequenceToDocuments`를 대체한다. (b) 애플리케이션 계층(`apps/pipeline-worker`)에 정본 해석 + 순수 재투영 서비스 — PR 대응은 `pull_request_snapshot`의 `merge_commit_sha`·`base_branch`와 현재 에폭 `merge_sequence`의 대조이며 `merge_sequence.pull_request_number`를 근거로 쓰지 않는다. (c) `sequence_work`에 `project` kind(공간 tail·공간 full sweep·문서 단위)와 `progress` 열 — 마이그레이션 034. 채번·재채번·복구 트랜잭션, 늦은 PR 스냅숏 트랜잭션, 커밋 문서 생성 뒤에 의도를 남기고 durable 러너가 새 push 없이 재시도한다. `MNUMBER_ENABLED=false`에서도 돈다. (d) 재색인: PR·커밋 재구축 각각에 현재 정본 replay를 인라인으로 잇고(`job.progress.sequence_replay`에 공간·페이지·에폭 기록), `verifyBeforeCutover`에 대상 문서 집합의 서수 누락·정본 불일치·대표 범위 정렬 검사를 더하며, 전환 울타리 안에서 에폭 이동을 다시 확인한다. 전환 뒤 durable full sweep을 남긴다. (e) 수동 재투영: 잡 유형 `sequence_reproject`(API-ADM-002 생성, `expected_epoch` 필수, `aliases` 선택) + `prsctl sequence reproject`(`--dry-run`은 PG·ES·작업 큐에 아무것도 쓰지 않는다). (f) `repairSequence`가 `consistent`여도 durable full sweep을 예약하고 러너가 DB 정합과 ES 복구를 **다른 결과**로 보고한다 — 에폭은 올리지 않는다. (g) SRS v2.40(FR-SEQ-001 AC-7·AC-8, FR-ING-008 AC-8, FR-ADMIN-003 AC-6), API 계약·데이터 모델·비동기 계약·백엔드·ADR-004 Amendment·관측성·RUNBOOK·WP-098·원장.
- 지킬 조건: 재투영은 재채번이 아니다 — PostgreSQL의 `merge_seq`·`merge_number`·`seq_epoch`·`head_sha`·`head_seq`를 바꾸지 않는다. ES 값·표시용 M 문자열은 입력이 아니다. 기존 `document_version`·PR `base_branch`·`merge_commit_sha`·권한 필드는 바꾸지 않는다. 다른 브랜치의 같은 SHA에 서수를 덮지 않는다(커밋 문서는 SHA당 하나라 **문서가 단 `base_branch`의 공간이 현재 에폭에 그 SHA를 갖고 있으면 그 공간의 값을 지킨다**). 구 에폭 작업은 새 에폭을 덮지 못한다(스크립트 가드 + 작업 에폭 대조). `updated=0`은 그 자체로 성공도 실패도 아니다 — `updated`·`noop`·`document_missing`·`guard_rejected`·`stale_epoch`·`transient`를 구분하고 미반영 작업을 `done`으로 닫지 않는다.
- 제외: 운영 서버 직접 변경, 공용 인덱스 삭제·재생성, 기존 번호 재채번, Release/태그 발행, 보호 규칙 우회, 날짜 정렬 대체·클라이언트 재정렬·M 문자열 정렬. 체인에서 빠진 문서가 단 옛 서수의 정리(orphan)와 다중 시퀀스 브랜치에서 PR 재투영이 커밋 문서의 `base_branch`를 되돌리는 기존 동작은 이번에 고치지 않고 DEV로만 등재한다. 운영 콘솔 재채번 요청의 본문 키 불일치(`confirm` vs 서버 `confirmation`, `OpsJobsView.tsx`/`routes.ts`)도 별건으로 등재만 한다.
- 시험 원칙: PostgreSQL·Elasticsearch·Redis를 모두 격리한다(`prs-cr113-*`, ES `cluster.name=prs-cr113-isolated`, DB `prs_test_cr113`). 고정 `prs-*` 별칭을 쓰는 시험은 그 ES에서만 돌린다. 먼저 현 코드에서 실패하는 시험(시퀀스 먼저/PR 문서 나중, 늦은 `merge_commit_sha`, PG 유지 재색인, consistent인데 ES만 누락, ES 실패 뒤 새 push 없음, 직접 푸시 커밋 문서)을 만든 뒤 구현한다. 정렬 fixture는 PR 번호·merged_at·merge_seq 순서가 일부러 다르다. 검증 기준은 같은 repo/base/current epoch에서 확정된 M 번호·시퀀스 순서의 일치이며 Merged at 정렬과의 동일성은 기준이 아니다. ES 원시 필드를 직접 대조한다.
- 설계·세부 정본: ADR-004 Amendment(CR-113), `FR-SEQ-001` AC-7·AC-8, `FR-ING-008` AC-8, `FR-ADMIN-003` AC-6, API-ADM-002(`sequence_reproject`), ENT-SEQ-006(`project` kind·`progress`), JOB-SEQ-006(durable 시퀀스 투영), JOB-ING-006 3.5장 replay·검증, WP-098. 연쇄 기록은 5장 「CR-113 cascade」에 적는다.
...
- 요청: 사용자 지시(2026-09-18). main HEAD `2a9fe51`, CR-106(`03da17c`)·CR-107(`95674b8`) 반영 확인, main CI green 확인 뒤 착수. 기존 검색 의미·URL 상태·범위 검색 semantics·Source History API 계약은 바꾸지 않는다. 목표는 기능 축소가 아니라 중복 UI 제거와 공간 효율 개선이다.
- 범위(`apps/web/components/RepositoryWorkspace.tsx`, `apps/web/components/source/SourceHistory.tsx`, 지원 파일):
  1. 좌측 사이드바의 「Browse repositories」 heading·검색창·전체 목록·행별 chevron·「More repositories」 버튼을 제거하고, `owner/name` 값을 보이는 compact `FieldSelect` 콤보박스(라벨 "Find Repository") 하나로 대체한다. 기존 저장소 pagination/loading/error는 유지하되, "More repositories"를 별도 노출 버튼이 아니라 드롭다운 안의 sentinel 옵션(선택 시 `repoNext`만 갱신하고 현재 선택값은 바꾸지 않는다)으로 접는다. 내부 검색은 Radix Select의 기존 typeahead로 충분하다고 판단해 별도 검색창·목록을 새로 만들지 않는다.
  2. 「Branch」 섹션 heading(아이콘+`h2`)을 제거하고 「Base branch」 `FieldSelect`만 남긴다.
  3. 독립된 「Find files and paths」 입력+「View path history」 폼 전체를 삭제한다. `Files & folders`(`SourceTree`) 트리와 트리 내부 「Filter root entries…」 필터는 그대로 둔다.
  4. 사이드바 flex 배치에서 제거된 두 영역(저장소 목록·path 폼)의 공간을 `Files & folders`로 되돌린다. 사이드바 하단 안내 문단(「Every change, with its context.」 + 아이콘)을 완전히 삭제한다(짧은 뷰포트에서 숨기는 방식이 아니라 요소 자체를 제거).
  5. `SourceHistory.tsx`의 Revision·Linked PRs 옆 별도 `CopyButton`(「Copy SHA」/「Copy PR #…」)을 제거하고, 표시 텍스트 자체(축약 SHA·`#123`)를 클릭 가능한 semantic `<button>`으로 만든다. border/background/padding 없이 텍스트처럼 스타일링하고 Enter/Space·focus-visible·`aria-label`·기존 `role="status"` 복사 완료 안내를 유지한다. SHA는 화면 표시가 축약형이어도 full SHA를 복사하고, PR 번호 복사 값과 `pull_request_numbers`/`pull_requests_unavailable` 의미(CR-107)는 그대로 둔다.
  6. 필터 패널의 Owner/Repository/Base branch 세 필드(사이드바 선택값의 읽기 전용 중복)를 제거한다. 검색 질의는 여전히 사이드바에서 고른 repository/base 값을 사용한다.
  7. 필터 조건 영역 전체를 하나의 collapsible section(기본 collapsed, 토글 하나로 개폐, "Filters" + 활성 개수)으로 만든다. Search/Reset 버튼은 collapsible 바깥, 기존 위치 그대로 항상 보이게 둔다. 접힘은 `draft`/URL 상태를 초기화하지 않는다.
  8. Status·Title·PR 번호·commit SHA(`q`)·Author·Label 네 기본 필터는 collapsible 안에서 그대로 유지한다(검증·질의 생성 로직 불변).
  9. PR number/M number/Merged date/Merge order 8개 입력을 하나의 Range filter 선택자(유형 드롭다운 + From/To)로 통합한다. 유형별 URL 파라미터(`pr_from/to`, `mnum_from/to`, `from/to`, 클라이언트 `shaFrom/shaTo` → `seq:`)는 그대로 두고, 로컬 `rangeType` state가 어떤 쌍을 보여줄지만 결정한다. 유형을 바꿔도 다른 유형의 값을 지우지 않으며, 여러 range가 동시에 활성화될 수 있는 기존 동작도 보존한다. 활성 range는 compact summary/active indicator로만 알린다.
  10. 「Combine additional filters…」 힌트, Merge order의 「Select a repository and base branch…」/「Two commit SHAs become…」 안내 블록과 그 아이콘을 제거한다. 비활성 사유는 M number 필드가 이미 쓰던 `title` 방식을 Merge order SHA 필드에도 적용해 control 자체로 표현한다.
  11. 최종 시각적 우선순위: 좌측은 Find Repository → Base branch → Files & folders(최대 공간), 우측은 Tabs → 간결한 헤더 → collapsed Filters → Results table(최대 공간).
  12. JSX와 접근성 트리 자체를 제거한다(CSS로 숨기지 않는다). 백엔드 질의 파라미터·검색 기능은 삭제하지 않는다.
- 설계 결정(구현 착수 전 확정):
  - `apps/web/components/LegacyRepositoryWorkspace.tsx`가 `repository-workspace.css`의 `.repo-repositories`/`.repo-sidebar-heading`/`.repo-sidebar-note`와 `SourceTree`/`SourceHistory` 컴포넌트를 동일하게 재사용한다(grep으로 확인). 이 CR은 그 클래스들의 CSS 규칙을 삭제하지 않는다 — `RepositoryWorkspace.tsx`가 해당 JSX를 더 이상 그리지 않을 뿐이며, flex 재배분은 형제 `flex:1 1 0` 요소가 하나(`.source-tree`)만 남아 자연히 일어난다. `LegacyRepositoryWorkspace.tsx`·`SearchView.tsx`(`PRS_LEGACY_SEARCH=1`/operator `?legacy=`로만 도달)는 이번 CR에서 손대지 않는다.
  - Repository 선택은 새 컴포넌트를 만들지 않고 기존 `FieldSelect`(Radix Select 래퍼)를 재사용한다. Radix `Select.Item`은 `onSelect` 가로채기 API가 없음을 타입 정의(`@radix-ui/react-select@2.3.7`)로 확인했으므로, pagination은 별도 로직 없이 sentinel 값(`__more__`) 선택을 가로채 `repoNext`만 갱신하는 방식으로 구현한다. `FieldSelect`의 옵션 타입에 `disabled?: boolean`을 추가한다(기존 호출부는 그대로 동작).
  - `⌘K`/`Ctrl+G`(`ReaderShell.tsx`)와 hash(`#omni-search-input`) 포커스 이동이 `[data-reader-search]`(= `q` 입력)를 `document.querySelector`로 찾는다. Radix `Collapsible.Content`는 기본적으로 닫히면 마운트 해제하므로 그대로 두면 필터가 접힌 상태에서 두 단축키가 조용히 무동작이 된다. `Collapsible.Content`에 `forceMount`를 주고 `[data-state='closed']`를 CSS `display:none`으로 숨기는 방식으로 요소를 항상 DOM에 남긴다(tab 순서·스크린리더에서는 여전히 제외됨). `element.focus()`가 `display:none` 요소에서 조용히 no-op이므로, `RepositoryWorkspace` 자체에 `q` input용 `ref`와 캡처 단계가 아닌 별도 `window` keydown 리스너(같은 키 집합)를 추가해 "닫혀 있었다면" `setFiltersOpen(true)`와 함께 포커스를 큐에 넣고, `filtersOpen` 값이 바뀌는 effect에서 DOM이 보이게 된 뒤 실제로 포커스·선택한다. 이미 열려 있을 때는 기존 `ReaderShell`의 동작이 그대로 동작하므로 손대지 않는다.
  - Copy UI는 기존 `CopyButton`을 수정하지 않고 형제 컴포넌트 `CopyText`를 `components/ui/index.tsx`에 신설한다(`CopyButton`의 다른 사용처 `ResultWorkbench.tsx`에 영향 없음). 클립보드 로직·상태 문구·`role="status" aria-live="polite"`는 `CopyButton`과 동일하게 유지한다.
  - `.repo-results-scroll`의 `max-height: calc(100dvh - 689px)`는 접힌 필터 기준으로 다시 측정해 갱신한다(689px는 펼쳐진 441px 필터 폼 기준 실측값이라는 것이 기존 주석에 명시돼 있다). `LegacyRepositoryWorkspace.tsx`는 이 클래스를 쓰지 않으므로 영향 없다.
- 제외: 백엔드 API·쿼리 파라미터·검색 semantics 변경, CR-106(`pr_number:`/`mnum:`/`seq:`)·CR-107(`pull_request_numbers`/`pull_requests_unavailable`) 계약 변경, `LegacyRepositoryWorkspace.tsx`·`SearchView.tsx` 변경, 새 SRS 요구사항 신설, CR-103 사이드바 고정/CR-104 무한 스크롤/CR-105 VANGUARD-1 정정 로직 변경.
- 신설 WP-096.
- 착수 규칙: 공유 checkout(`/home/roqkf/pr-search`)이 아니라 전용 worktree(`/home/roqkf/pr-search-wt/cr111-search-simplify`, 브랜치 `feature/cr111-search-simplify`, `origin/main` `2a9fe51` 기준)에서 진행한다.
- 발견 편차(DEV-728, 이 CR의 정정 사항과 무관한 선재 결함 — 등록만 하고 이번 범위에서 고치지 않는다): `apps/web/playwright.config.ts`의 `webServer.env`가 `PRS_LEGACY_SEARCH: '1'`을 고정해, e2e 스위트 전체가 `/search`에서 `SearchView`만 렌더링하고 `RepositoryWorkspace.tsx`(실제 기본 `/search` 화면)는 어떤 e2e 시험에도 도달하지 못한다. `a11y/`에도 `RepositoryWorkspace`를 직접 렌더링하는 기존 시험이 없었다(`lib/repository-search.test.ts`의 순수 함수 시험만 존재). 이번 CR은 `a11y/repository-workspace.test.tsx`를 신설해 컴포넌트 자체의 접근성·상호작용 회귀를 잡지만, 실제 브라우저 종단 e2e 공백은 `playwright.config.ts`의 공유 설정을 바꾸는 별도 CR 없이는 닫히지 않는다.
- 독립 코드 리뷰: 서로 독립된 에이전트로 2회 수행했다(1차 수동 정독+실제 재실행 검증, 2차 `code-review` 스킬 effort high + 1차와의 대조). 총 8건 발견, 대조 결과:
  - **수정 완료(정확성 버그, `RepositoryWorkspace.tsx`)**: (A) 사이드바 저장소/Base branch 콤보박스가 예전 `<input type="search">`(전역 단축키 가드에 걸림)를 Radix `Select.Trigger`(`role="combobox"`, 가드에 안 걸림)로 바꾸면서, Radix의 닫힌-트리거 typeahead 키 입력이 막히지 않고 새어나가 파일이 선택된 상태에서 `t`를 치면 TimeLapseModal이 열릴 수 있었다(Radix 소스로 직접 확인) — 전역 keydown 가드에 `[role="combobox"],[role="listbox"]` 추가, 회귀 시험 신설. (B) merge-order 요약("Merge order set")이 `seqRange`가 지금의 repository/base에 대해 유효한지 확인하지 않아, 실제 질의(`repository-search.ts`)는 이미 무시하는 stale range를 화면은 계속 걸려 있다고 표시할 수 있었다 — 같은 `repository@base` 일치 검사를 표시 로직에도 적용. (C) 저장소 목록 조회 실패 시 드롭다운의 "No repositories"(0개 단정)와 에러 배너("불러오지 못함, 다시 시도")가 동시에 떠서 서로 모순됐다 — 플레이스홀더 라벨이 에러 상태를 반영하도록 수정. (D) 아직 안 불러온 페이지에 있는 저장소로 딥링크하면 옵션 목록에 일치 항목이 없어 콤보박스가 빈칸으로 보였다 — 미매칭 현재 값을 위한 합성 옵션 추가.
  - **재현·수정 완료(시험 인프라, `apps/web/a11y/source-history.test.tsx:149`)**: (H) 신설 a11y 시험 하나가 간헐적으로 실패함을 실제 재실행(8회 중 2회 실패)으로 확인 — `SourceHistory.tsx`의 `commits`(passive effect로 채워짐)와 `pull_requests_unavailable` 캡션(진짜 비동기 setState) 사이에 "캡션은 떴는데 행은 아직 없는" 프레임이 실존해, 그 프레임을 잡는 동기 `getByRole` 단정이 가끔 실패했다. 이 CR이 건드리지 않은 pre-existing 데이터 계층 문제이며, 시험만 `await screen.findByRole(...)`로 정정.
  - **반증(실측으로 결론)**: (G) 필터 그리드가 `repository-workspace.css`의 `repeat(5, minmax(0,1fr))`(5칸) 정의만 보면 4개 필드로는 빈 칸이 남을 것으로 우려됐으나, 이 CR이 건드리지 않은 더 구체적인 기존 규칙 `reader-workspace.css:100`의 `.reader-ui .repo-filter-grid { grid-template-columns: 1fr 2fr 1.15fr 1fr; }`가 이겨서 실제로는 4개 필드에 정확히 맞는 비율로 렌더된다 — 실제 브라우저에서 4개 필드의 렌더 폭(160/320/184/160px)을 실측해 이 비율과 정확히 일치함을 확인했다. 코드 변경 불필요.
  - **미수정, 등재만(설계 공백, 저자 판단 필요)**: (E) range 유형 전환 시 반쪽짜리 값의 활성/차단 판정 불일치, (F) "Load more repositories…" 선택 시 드롭다운이 닫힘(기능 정상, UX만 저하) — `DEV-729`로 등재.
- 진행: 구현·독립 리뷰 2회·발견 결함 수정·재검증 완료(원장 6.102장), 사용자 요청으로 필터 영역 여백·range 한 줄 배치·배너 제거·사이드바-결과 하단 정렬 후속 조정 2회 추가 반영(원장 6.102장 후속 조정 절). typecheck/lint/lint:deps/build/유닛998·a11y(10회 연속 465, 후속 조정 뒤 재확인 포함) 전부 성공, 통합1857·회귀506·e2e204(1차 수정 전 코드 기준 1회, 이후 diff가 건드리지 않는 영역이라 재실행 생략). 사용자 지시(2026-09-19)로 worktree `feature/cr111-search-simplify`에 로컬 커밋했다(push·PR·main 병합은 아직 없음, 릴리즈 발행도 보류 지시받음) — 이 원장의 다른 CR들처럼 실제 병합 완료 후 별도로 done을 기록하므로 상태는 `implementing`으로 유지한다.
...
**후속(2026-09-19, 사용자 요청):** `deploy/single-host/compose.yml`의 `web:` `environment:`에 `REGRESSION_FIXTURE_ENABLED: ${REGRESSION_FIXTURE_ENABLED:-0}`가 빠져 있어, 위 런타임 플래그를 실제 단일-host 배포에서 켤 방법이 없었다(런타임 flag 자체는 CR-109가 이미 애플리케이션 코드에 구현해 뒀다). 기본값 0(꺼짐)으로 추가했다. `docker compose -f compose.yml config`로 필수 변수를 더미로 채워 문법·해석 결과를 확인했다(`REGRESSION_FIXTURE_ENABLED: "0"`로 정상 렌더). 새 코드·새 CR 없이 이 CR이 이미 규정한 계약을 배포 설정에 마저 연결하는 변경이다.
...
- 요청: 10차(CR-103·104·105)가 컨텍스트 한도로 다음 세션에 넘긴 항목 두 건 중 두 번째(Work B). 사용자 지시(2026-09-17/18)로 승인: Source History(경로별 커밋 이력) 각 행에 연결된 PR 번호를 표시·복사할 수 있게 한다. CR-106(Work A) main 반영 완료를 확인한 뒤 착수한다.
- 범위: `FR-SRC-002` AC-1을 "SHA·부모·메시지·작성자·시각 + 연결 PR 번호(행별 배열, 연결 조회 상태 구분)"로 개정한다. `API-SRC-002`(`/history`) 응답에 행별 `pull_request_numbers`(연결 확정 시 배열, 아직 확정 못했으면 `null`)와 응답 단위 `pull_requests_unavailable`(PR 연결 조회 자체가 실패·미배선이면 `true`)을 추가한다.
- 설계 결정(구현 착수 전 확정): 기존 `SourceComparison.pull_requests: SourcePullRequest[]`(제목까지 포함한 전체 PR 객체) 패턴을 그대로 재사용하지 않는다. 지시서가 "History 전용 항목 타입에 `pull_request_numbers` 배열과 연결 조회 상태를 추가"라고 명시했고, 화면도 PR 번호만 표시·복사하면 되므로 제목 조회는 불필요한 확장이다. `SourceCommit`(공용 타입)은 바꾸지 않고 `SourceHistoryCommit extends SourceCommit`으로 `SourceHistory.commits`만 좁힌다. 상태는 두 층이다 — 행 단위 `pull_request_numbers: number[] | null`(배열은 확정이며 빈 배열도 확정된 "연결 없음"이다, `null`은 아직 확정하지 못했다는 뜻이며 `prs-commits`에 그 SHA 문서가 없거나 해당 필드가 아직 없는 경우다), 응답 단위 `pull_requests_unavailable?: boolean`(true는 이번 요청에서 PR 연결 조회 자체를 시도하지 못했거나 실패했다는 뜻이며 ES 미배선 또는 질의 예외가 원인이다). 후자가 true여도 History 본문(커밋 목록)은 그대로 반환한다 — PR 연결 조회 실패가 유효한 History를 없애지 않는다. 조회 함수는 `resolve/detail.ts::loadSourceCommits`를 호출하지 않고 `source/service.ts`에 새로 만든다 — 그 함수의 `_source`는 `pull_request_numbers`를 가져오지 않고, 그 경로를 넓히면 PR 상세 표시(FR-SRCH-003, 별개 기능)까지 영향을 받는다. 새 함수는 한 페이지의 SHA 전체를 `terms` 질의 하나로 묶어 조회해 N+1을 금지하며, `prs-commits`를 `{repository} + {commit_sha in […]}`로 `applyMandatoryScopeFilter`(ADR-008)까지 통과시켜 조회하고 `_source`는 `commit_sha`·`pull_request_numbers`만 가져온다. ES 클라이언트는 `SourceRouteOptions`에 선택 필드로 추가한다 — `source.test.ts`의 기존 fixture가 `es` 없이 `registerSourceRoutes`를 호출하고, `deps.source`도 `parts.github && parts.auth`일 때만 배선되는 기존 조건부 배선 패턴과 맞춰야 하기 때문이다. `es`가 없거나 질의가 실패하면 `pull_requests_unavailable: true`로 응답하고 500으로 떨어지지 않는다. 새 매핑·마이그레이션은 추가하지 않는다 — `packages/es/src/mappings/commits.ts`의 `pull_request_numbers`(integer, WP-067부터 존재)를 그대로 읽는다. History는 GHE 실시간 조회이고 `prs-commits`는 투영된 것만 가지므로, 임의 브랜치 경로에서는 "미확정"(`null`)이 예외가 아니라 흔한 사례다 — AC 문구와 화면이 이를 고장으로 보이게 쓰지 않는다.
- 제외: `SourceComparison`/`TimeLapseModal`의 기존 관련 PR 조회(이미 승인된 별개 기능이며 손대지 않는다), `resolve/detail.ts`의 PR 상세 원본 커밋 표시(API-SRCH-002/003, 다른 기능), 새 수집 시스템과 GHE 무제한 병렬 호출, `Compare selected` 게이팅 로직(파일 + 정확히 리비전 2개) 변경.
- 신설 WP-093. 이미 완료된 WP-085(CR-097, 파일 Tree·경로 History·Diff/TimeLapse)는 이 CR로 미완료로 되돌리지 않으며, 이 WP가 그 산출물을 참조·확장한다.
- 착수 규칙: 공유 checkout(`/home/roqkf/pr-search`)이 아니라 전용 worktree(`/home/roqkf/pr-search-wt/cr107-history-pr-links`, 브랜치 `feature/cr107-history-pr-links`, `origin/main` `69a01eb`(CR-106 병합·문서 반영 완료 시점) 기준)에서 진행한다.
- 발견 편차(DEV-726, 미반영 — 후속 CR 후보): `resolve/detail.ts`의 단일 커밋 조회(FR-SRCH-002, `/commits/{repository}/{commit_sha}`)가 읽는 `pull_request_numbers`는 이 CR의 `loadPullRequestLinks`와 달리 정렬·중복 제거를 하지 않고 원본(union 스크립트 `packages/es/src/upsert.ts`의 `HashSet`→`ArrayList` 순서, 비결정적) 그대로 노출한다. 같은 저장 필드가 History에서는 정렬된 순서로, PR 상세 커밋 조회에서는 원본 순서로 보일 수 있어 두 화면의 표시 순서가 어긋날 수 있다 — `resolve/detail.ts`는 이 CR의 제외 범위이므로 손대지 않았다.
- 발견 편차(DEV-727, 미반영 — 후속 CR 후보): `loadPullRequestLinks`의 Elasticsearch 질의가 `routing`(저장소 ID 기준)을 넘기지 않는다. 이웃 질의(`containments.ts`)는 이미 넘기고 있고, 이 경로는 History 페이지마다 실행되는 scatter-gather라 최적화 여지가 있다 — 독립 코드 리뷰도 정확성 문제는 아니라고 확인했다. CR-107·WP-093·API-SRC-002 어디에도 이 요구가 없으므로 승인되지 않은 확장을 더하지 않는다.
- 독립 코드 리뷰(code-review 스킬, high) 수행. 정확성 결함 없음 — ADR-008 범위 필터, `null`/`[]` 의미, N+1 금지, 조회 실패의 502 미전파 네 가지를 독립적으로 재확인했다. 아키텍처/관례 발견 6건 중 3건 반영(`sourceHistory()`의 미확정 응답 모양을 `unavailable()` 헬퍼 하나로 통합해 두 곳에서 따로 만들던 것을 정리, PR 번호를 기존 `Badge` 컴포넌트로 표시하도록 교체(`RelationBadgeGroup`/`ResultTable.tsx`와 같은 패턴), `runtime.test.ts` 신규 시험명에 FR/AC 표기 추가), 2건 반려(`SourceRouteOptions.es` 필수화 — 이 CR이 선택 필드로 못박은 설계 결정이라 반려, `loadSourceCommits`와 공통 헬퍼 추출 — `resolve/detail.ts` 수정 금지 범위라 반려), 1건은 `DEV-727`로 유보.
- 진행: SRS 개정·구현·독립 코드 리뷰 반영 완료. `pnpm typecheck`·`lint`·`lint:deps` 전부 통과. 단위 `pnpm run test` 2930/2930(GHE 자격 증명 없음 1건 skip, 기존과 동일). 통합 `pnpm run test:integration` 1857/1857(전체) + 신규 `apps/search-api/integration/source/history-pull-requests.test.ts` 8/8. 회귀 `pnpm run test:regression` 506/506. `pnpm --filter @prs/web run build`·루트 `pnpm build` 통과. `pnpm run test:a11y` 436/436. `pnpm run test:e2e`는 이 CR과 무관한 기존 간헐 실패(`flow-003.spec.ts:176`, Source 참조 0건, 단독 재실행 통과, WP-016부터 기록된 표본잡음) 한 자리 제외 전부 통과. 변이 규율 2건 모두 대상 시험이 실제로 죽었다가 원복 후 재통과 확인. 사내 실제 GHE 데이터 왕복 `NOT RUN`(CR-106과 동일 사유). PR [#213](https://github.com/89sooner/pr-search/pull/213), 브랜치 `feature/cr107-history-pr-links`, PR CI(run `35289418836`) verify(4m21s)·integration(7m17s) success. 스쿼시 병합 `95674b8`(2026-09-18T00:48:00Z, 사용자가 직접 병합 — Claude Code auto mode의 [Merge Without Review] 가드로 이 세션의 직접 병합은 차단됨). 병합 커밋 main CI(run `35292695927`) verify(4m44s)·integration(7m34s) 모두 success(2026-09-18).

## CR-106 — 식별자 범위 검색: PR 번호·M 번호·SHA 구간 (2026-09-18)

- 유형: SRS 개정(`FR-SRCH-005`/`006`/`008` AC 추가·개정) + 구현. 상태: **closed — PR #211 병합, main·PR CI 전부 success**.
- 요청: 10차(CR-103·104·105)가 컨텍스트 한도로 다음 세션에 넘긴 항목 두 건 중 첫 번째(Work A). 사용자 지시(2026-09-17/18)로 승인: Search 필터에 PR 번호 범위(`pr_number:`), M 번호 범위(`mnum:`), SHA 두 개로 지정하는 머지 순서 구간(클라이언트 `seq:` 변환) 검색을 추가한다.
- 범위: `FR-SRCH-005` AC-1(키 목록에 `pr_number`·`mnum` 추가)·신규 AC-8(PR 번호 범위 — 정확히 하나의 positive `repo:` 필요, `base:`·에폭 불요. PR 번호는 저장소별이라 시퀀스 공간과 무관)·신규 AC-9(M 번호 범위 — `seq:`와 같은 공간 지목 규칙 AC-7 적용, `pending`/미채번/미대상 항목은 어떤 구간과도 매치 안 됨)·신규 AC-10(SHA 구간 — 새 질의 키가 아니라 두 SHA를 기존 식별자 해석(`FR-SRCH-004`)으로 `merge_seq`에 대응시켜 `seq:` 범위로 변환하는 클라이언트 동작, 양끝 포함). `FR-SRCH-006` AC-1(필터 목록에 대응 문구 반영). `FR-SRCH-008` AC-3(`mnum:` 질의의 유효 M 번호 에폭도 커서 지문 재료 — `seq:`가 `seq_epoch`에 대해 이미 정한 것과 같은 처리이되 별개 필드에 건다).
- 설계 결정(구현 착수 전 확정): `mnum:`의 세대 게이트는 ES 필드 `merge_number_epoch`에 건다 — `seq_epoch`가 아니다. 두 필드는 서로 다른 투영 작업이 다른 시점에 쓰는 별개 필드이며(`packages/es/src/mappings/pull-requests.ts` 주석), `seq_epoch`로 게이트하면 M 투영이 뒤처진 문서가 새 세대의 PR 투영과 함께 통과해 결과에 두 세대가 섞인다 — `buildQuery`가 `seq:`에서 이미 막아 둔 것과 같은 결함이다. 유효 에폭 값 자체는 `seq:`와 동일하게 `resolveSpace()`가 답하므로 공간 해석 호출은 하나만 공유하고 거는 ES 필드만 나눈다. `packages/query/src/sequence-binding.ts`의 공간 지목 판정(현재 `seq`만 봄)을 `mnum`도 같은 규칙으로 묶도록 일반화하고, `pr_number:`는 별개의 더 단순한 결합 규칙(저장소 하나만 필요, 에폭 불요)으로 다룬다 — 세 범위 키가 서로 다른 지목 규칙을 갖는다는 것이 이 CR의 핵심 설계 지점이다.
- 제외: CR-102(Regression 확장, 별도 트랙이며 이 CR은 손대지 않는다), 디자인 시스템 개편, REL-007 무관 확장, 운영 권한 변경, 사내 데이터 변경·재색인·업그레이드, 새 Release 실제 발행. Work B(Commit history PR 번호 병기)는 이 CR이 main 반영을 완결한 뒤 별도 CR로 착수한다 — 두 기능을 동시에 벌리지 않는다.
- 신설 WP-092. 이미 완료된 WP-085(CR-097, 파일 Tree·경로 History·Diff/TimeLapse)는 이 CR로 미완료로 되돌리지 않으며, 후속(Work B) WP가 이를 참조한다.
- 착수 규칙: 공유 checkout(`/home/roqkf/pr-search`)이 아니라 전용 worktree(`/home/roqkf/pr-search-wt/cr106-range-search`, 브랜치 `feature/cr106-range-search`, `origin/main` `a496367` 기준)에서 진행한다. 다른 세션의 변경(CR-102 등)은 건드리지 않는다.
- 발견 편차(DEV-723): `apps/search-api/src/search/cursor.ts`의 `computeFingerprint`가 배열을 합칠 때 실제로 커밋된 바이트는 공백(` `)이 아니라 NUL 바이트(`\x00`)였다 — `git cat-file blob`으로 원본 객체 자체에 있음을 확인했고, 모든 뷰어(Read·cat·grep)가 시각적으로 공백과 구분하지 못해 어느 리뷰에서도 드러나지 않았다. 영향은 없다(구분자는 해시 이전의 내부 연결 문자일 뿐이고, 어느 재료 값도 NUL을 담지 않으므로 지문의 유일성·안정성에 실질적 차이가 없다). 이 CR이 같은 함수(같은 `.join(...)` 호출)의 재료 배열을 확장하므로 함께 정정한다 — 이 정정만으로도 배포 시점의 진행 중 커서는 무효가 되는데, `mergeNumberEpoch` 추가가 어차피 같은 효과를 내므로 추가 비용은 없다. 이 CR과 무관한 별도 결함이므로 SRS·API 계약은 바꾸지 않는다.
- 발견 편차(DEV-724, 미반영 — 후속 CR 후보): `apps/search-api/src/export/routes.ts`의 비동기 내보내기 계획은 `seq:`가 있을 때만 `sequenceContext`를 저장해 재검증한다(`packages/db`의 `assertExportScope`). `mnum:`만 쓰는(`seq:` 없는) 비동기 내보내기는 유효 M 번호 에폭을 저장하지 않으므로, 큐 대기 중 강제 푸시가 에폭을 올려도 정지 사유(`export_epoch_changed`에 대응하는 M 번호 버전)로 잡히지 않고 낡은 필터로 조용히 실행될 수 있다(독립 검토 2차가 실측). 동기(즉시) 내보내기·미리보기는 영향 없다 — 매 요청마다 현재 에폭을 새로 읽기 때문이다.
- 발견 편차(DEV-725, 미반영 — 후속 CR 후보): `apps/search-api/src/saved-search/service.ts`의 실행 전 점검(`sequence_unbound`)이 `hasSequenceRangeFilter`만 본다. `mnum:`만 쓰는 저장된 검색이 기능 꺼짐이나 공간 미지정으로 실제 실행 시 거절될 상태여도 이 점검을 통과해 `last_run_at`이 갱신될 수 있다(독립 검토 2차가 실측) — 감사 기록이 낙관적으로 남는 문제이며 결과가 잘못 나가는 문제는 아니다.
- 독립 코드 리뷰(code-review 스킬, high) 2회 수행, 아래 발견을 전부 반영(DEV-724·725 제외, 위에 사유 기재):
  - 1차: `/exports`·집계 API가 `mnum:`/`pr_number:` 판정·`mergeNumberEpoch`를 `buildQuery`에 넘기지 않아 처리되지 않은 오류(503/500)로 떨어지던 결함 — 세 API 모두 `/search`와 같은 판정 함수(`checkMergeNumberFeatureFlag`·`toPrNumberRangeFailure`·`resolveMergeNumberRangeEpoch`)를 공유하도록 정정. `mnum:` 응답 문구가 `seq:`의 것을 그대로 물려받던 실수 정정(`MERGE_NUMBER_BINDING_MESSAGE` 신설). `pr_number:`/`mnum:`의 하한(1) 미검증 정정(`MIN_RANGE_VALUE`). 프런트엔드: SHA 두 칸을 비우고 재제출해도 이전 `seqRange`가 지워지지 않던 결함, `mnum:` 입력이 `base:` 미선택 상태를 안내하지 않던 결함 정정. `analyzeSequenceBinding`/`analyzeMergeNumberBinding`의 중복 로직을 `analyzeSpaceBinding` 공용 함수로 통합.
  - 2차(1차 반영 뒤 재검토): (a) `/search`·`/exports`·집계 셋의 `seq:` 낡은 에폭 판정과 `mnum:`/`pr_number:` 판정의 순서가 서로 달라 같은 입력에 다른 상태 코드(400 vs 409)를 내던 결함 — 낡은 에폭 판정을 항상 먼저 보도록 셋을 통일. (b) **이 CR이 건드리지 않은 네 번째 소비처** `apps/search-api/src/sequence/range.ts`(W-004 범위 조사 화면, `FR-SEQ-002` 계열)도 같은 전역 파서(`@prs/query`)를 쓰므로 `mnum:`/`pr_number:`가 그 화면에서도 파싱은 되지만 `mergeNumberEpoch`를 넘기지 않아 처리되지 않은 500이 될 수 있었다 — 이 화면은 이 CR이 승인받은 범위(`FR-SRCH-005`/`006`/`008`) 밖이므로 기능을 넓히지 않고 `mnum:`/`pr_number:`를 조회 전에 명시적으로 거절하도록 정정(승인되지 않은 확장 금지). (c) 저장소 전환·Base branch 변경 뒤에도 `mnum_from`/`mnum_to` draft 값이 남아 있다가 조용히 제출되던 결함 — 개별 전환 지점과 `commitFilters()` 마지막 방어선 둘 다 정정.
- 진행: SRS 개정·구현·독립 리뷰 2회 반영 완료. `pnpm typecheck`·`lint`·`lint:deps`·`test`(2919/2920, GHE 자격 증명 없음 1건 skip 기존과 동일)·`test:integration`(1847/1847, 워크트리별 격리 DB `prs_test_cr106tests`)·`test:regression`(506/506)·`pnpm build` 전부 통과. `/search` 화면 실제 브라우저 확인(새 필드 렌더링·접근성 이름·비활성 상태·입력 확인, 세션 부재로 실제 검색 결과 왕복은 NOT RUN). PR [#211](https://github.com/89sooner/pr-search/pull/211), 브랜치 `feature/cr106-range-search`, PR CI(run `35255901048`) verify·integration success. 스쿼시 병합 `03da17c`(2026-09-17T18:04:37Z). 병합 커밋 main CI(run `35256612621`) verify·integration success(2026-09-17). 원장 6.99장. 사내 실데이터 확인 NOT RUN.

## CR-105 — 하이픈으로 쪼개지는 식별자 전문 검색 과매칭 정정 (2026-09-17)

사용자가 실제 화면에서 발견해 보고한 버그(2026-09-17): Search 페이지 제목 검색에 `VANGUARD-1`처럼 공백 없는 식별자를 입력하면, `VANGUARD-1` 완전 일치가 아니라 숫자 `1`만 포함된 무관한 결과까지 전부 섞여 나왔다. `standard` 토크나이저가 하이픈에서도 끊어 `vanguard`·`1` 두 토큰이 되고, `buildTextClause`의 기본 `multi_match` OR(`best_fields`)는 둘 중 하나만 맞아도 매치를 인정하기 때문이다 — 흔한 토큰 `1`이 본문·브랜치명 어딘가에 있기만 해도 걸렸다. 오늘 다른 세션이 같은 증상을 `operator:"and"`로 고치려다 `FR-SRCH-011 AC-4`(한글·영문 혼용 질의는 한쪽 언어 토큰만 맞아도 매치)를 깨 main CI를 실패시켰고, 그 한 줄을 되돌린 경위는 CR-103/104 cascade의 병합 판정 문단에 있다. **정정: 판단 기준을 분석 후 토큰 개수가 아니라 사용자가 입력한 원문의 공백 여부로 바꾼다.** 공백 없는 한 단어(`VANGUARD-1`)는 `type:"phrase"`로 서브토큰이 인접해야만 매치되게 묶고, 공백 있는 여러 단어(`결제 retry`)는 기존 `best_fields`(OR)를 그대로 유지해 AC-4를 지킨다. 요구사항 문장은 바꾸지 않는다 — AC-4는 "혼용 질의"를 다루지 "하이픈이 든 한 단어"를 다룬 적이 없었고, 이번 정정은 AC-4가 애초에 규정하지 않던 빈틈을 채운다. 실제 Elasticsearch로 검증했다: `_analyze`로 토큰 분리·위치를 확인하고, `.partial` edge_ngram 서브필드와의 상호작용(1자 토큰은 `min_gram` 미달로 색인 자체가 안 됨)도 실측했으며, 되돌린 코드에서 새 시험이 실제로 실패하는 것을 먼저 확인한 뒤(변이 규율) 재적용했다. DEV-722. 상태: closed — main `5b2a24f` push(PR 없이, 사용자 지시), CI(run `35229686599`) verify success·integration은 무관한 `apps/ingest-gateway` 부하 시험(p95 지연 예산 간헐 초과, `facets.test.ts` 자체는 첫 시도에 40/40 통과)만으로 1차 실패해 재실행 후 success(2026-09-17).
...
## CR-103 — Diff 분할 보기·History 안내·사이드바 고정 레이아웃 정정 (2026-09-17)

사용자가 프런트엔드 결함/개선 네 건을 보고했다(2026-09-17). 이 CR은 그중 SRS 문장 변경이 필요 없는 세 건을 묶는다. (1) split diff 뷰의 두 소스 컬럼이 `.source-code-scroll` 하나를 공유해, 한쪽의 긴 줄을 읽으려 스크롤하면 반대쪽 컬럼도 함께 밀려나 화면 밖으로 사라졌다(DEV-720) — `table-layout:fixed`와 `<colgroup>`로 두 컬럼을 44px/`calc(50% - 44px)`로 고정하고 각자 독립적으로 스크롤하게 정정했다. (2) Commit history 탭에 경로를 먼저 선택하지 않고 진입하면(가장 흔한 진입 경로) 체크박스·Compare selected가 비활성화되는 이유가 화면에 드러나지 않았다(DEV-721) — 게이팅 로직 자체는 FR-SRC-002 AC-3대로 정확했으므로 바꾸지 않고, 안내 배너를 추가했다. 같은 자리에 기존 `CopyButton` 컴포넌트로 SHA 복사도 채웠다(요구사항 밖 일관성 보완). (3) 좌측 사이드바(Browse repositories·Files & folders)와 결과 표가 높이 제한 없이 페이지 전체를 늘렸다 — SRS·파생 UI 어디에도 정확한 치수 규정이 없던 레이아웃 디테일이라 **WP-090**(신설)으로 사이드바·필터·결과 표를 뷰포트 높이에 고정하고 내부 스크롤로 바꿨다. 독립 코드 리뷰(code-review 스킬, high)가 10건을 지적했고, 반영 과정에서 구현이 11번째 결함(파일 간 CSS 특이도 충돌로 압축 수정 상당수가 무력화되던 문제)을 추가로 찾아 함께 고쳤다. **모바일 대응은 사용자 결정으로 이번 범위에서 제외한다** — 시도했다가 되돌렸다(대응 없음이 최종 상태). WP-085에 정정 주석, WP-090 신설. DEV-720, DEV-721. 상태: closed — main `d0e74c7`(코드)·`e6db5f1`(문서) push 완료(PR 없이, 사용자 지시), 최종 CI(run `35225253617`, CR-104와 합류) verify·integration 모두 success.
...
- Cascade: 문서 인덱스·정의서 → feature·PRD(후보/OD만) → 별도 draft 구현 계획 → 로드맵·검증 계획·WP 문서·원장 → 실행 브리프. 정식 UI/API/데이터 계약 갱신은 R0 승인 cascade에 구체적으로 배정했다. 현재 상위 계약을 덮는 파생 명세는 만들지 않는다.
- 시작 검사: `--report` Phase 2, ERROR 2/WARN 2; `--strict` ERROR 4/WARN 2. 기존 정의되지 않은 요구사항·화면 참조, risks 파일 경로 두 곳, 역사 원장 자리표시자 두 건이다. 신규 계획을 구현 handoff 통과로 선언하지 않는다.
- 완료 검사: 아래 검증 기록을 따른다. 범위 결정과 DEV-719 요구사항 정정이 남아 있으므로 CR은 닫지 않는다.

### CR-102 검증 기록

- 문서 `--report`: Phase 2 / 오류 2·경고 2. `--strict`: 오류 4·경고 2. 편집 전후 같은 issue이며 신규 issue 0건. 기존 오류를 해소하거나 handoff gate를 통과했다고 주장하지 않는다.
- HTML 검사: `validate_html_artifact.py docs/40_delivery/deep-research-report.html --require-visual --require-interactive` 통과, 경고 0.
- 실제 Chromium: file URL에서 1440px/375px 본문 가로 넘침 없음, 테마 전환·예시 복사·목차 이동·원문 다운로드 확인. 브라우저 JavaScript 오류 0, 외부 요청 0. 원문 8개 절을 수록했고 다운로드 bytes와 입력 Markdown 일치를 확인했다(SHA-256 `04441c4ef3d50c4adf468b21f1a6194f834ae5a87dd1c0161ad9b2b2902b96d3`).
- 앱·DB·사내 장비 시험은 이번 문서 요청에서 수행하지 않았다. 코드 변경 없음. 기존 query-builder 사용자 변경은 보존했다.

## CR-101 — 병합된 PR의 state 파생 보정 (2026-09-17)

사용자가 `0.1.0-pilot.12` 사내 화면에서 Status를 Merged로 두면 결과가 없고 「My merged PRs」 탭도 비어 있다고 보고했다(결정자 2026-09-17). 원인은 질의가 아니라 문서였다: 투영(`documents.ts`)이 GitHub의 원시 `state`(`open`·`closed`)를 그대로 색인하고 병합을 `merged`로 파생하지 않아, `is:merged`/`state:merged`(`term state=merged`)와 맞는 문서가 하나도 없었다. 같은 전제(`state === 'merged'`)를 쓰는 화면 배지·PR 상세·M 번호 조회(`not_merged` 판정)·늦은 PR 스냅숏의 채번 재개 관문(FR-SEQ-008 AC-11)·스냅숏 근거도 병합을 보지 못하고 있었다. 시험은 손으로 `state:'merged'`를 심은 문서로만 통과했다. 정정: 파생 규칙을 도메인 헬퍼 `derivePullRequestState`(병합 신호가 있으면 `merged`) 한 곳에 두고 투영이 쓴다. 이미 저장된 스냅숏은 마이그레이션 032가 같은 규칙으로 바로잡고, 재색인(ADR-004)이 그 문서를 색인하므로 업그레이드 뒤 `prs-pull-requests` 재색인이 필요하다. 질의·매핑·SRS 문장은 바꾸지 않으며 API 문서의 「state는 GitHub이 준 값 그대로」 문장만 사실에 맞게 고친다. DEV-718, WP-089. 상태: closed — PR #208 squash 병합 `3ba1c4b`, main CI `35179018409` success (2026-09-17). `0.1.0-pilot.13`으로 발행했다(태그 `3ba1c4b`) — 사내는 업그레이드 뒤 `prs-pull-requests` 재색인이 필요하다. 사내 실데이터(재색인 뒤 Merged 필터·탭)는 NOT RUN.

## CR-100 — M 번호 운영자 확인서와 확정 근거 보존 (2026-09-17)

`0.1.0-pilot.11` 사내 반입에서 `MNUMBER_ENABLED=true`로 M 채번을 켜자 세 저장소 모두 `merge_seq=1`의 직접 푸시 초기 커밋에서 `negative_evidence_unavailable`로 멈췄고, 2-parent 머지 커밋은 `unsupported_merge_profile`로 같은 결과였으며, 운영자가 SQL로 넣은 `direct_confirmed` 근거를 워커가 `unresolved`로 덮었다(Upstream Feedback 2026-09-17). 사용자는 제안대로 처리하도록 승인했다. (1) **운영자 확인서**(FR-SEQ-008 AC-15, ENT-SEQ-008, 마이그레이션 031, `prsctl mnumber`): 시퀀스 공간·에폭 단위로 운영자가 「PR 근거가 끝내 없는 항목과 프로파일 밖 항목은 번호 없이 지나간다」고 행위자·사유·범위·유예와 함께 결정한다. 이것이 WP-074 설계 2.2가 열어 둔 사내 승인 증거 소스이며 DEV-581 게이트를 닫는다. 무조건 자동 스킵과 시간만으로의 확정은 AC-9·AC-10 위반이라 채택하지 않는다. (2) **확정 근거 보존**(DEV-715): 트랜잭션 밖 읽기와 트랜잭션 안 저장 사이의 경합을 잠금 재검증과 SQL 방어선으로 막는다. (3) 요청 3(unresolved를 건너뛰며 진행)은 AC-2·AC-3 위반으로 **기각**한다. (4) 문서 검사기가 SRS·작업 패키지를 draft로 읽던 회귀(DEV-716, 상태 줄 매몰)를 함께 고친다. WP-088. 상태: closed — PR #207 squash 병합 `08fbc3a`, main CI `35178340678` success (2026-09-17). `0.1.0-pilot.13`으로 발행했다(태그 `3ba1c4b`, 원장 머리 절). 사내 실데이터 검증은 NOT RUN.
...
## CR-096 — 전체 화면 Radix·영문 UI·사용자 테마 (2026-09-16)

사용자 승인: CR-095의 일반 검색 전환을 운영·분석·상세·저장된 검색 등 전체 화면으로 확장한다. Conductor 런타임 의존성을 제거하고 Radix 기반 제품 프리미티브로 교체한다. 제품 제공 문구·접근성 라벨·오류 안내는 영어로 통일하며 저장소의 PR 제목·본문·사용자 데이터는 번역하지 않는다. 다크/라이트 전환 버튼과 저장된 선호도를 제공한다. `template.html` 통합 검색 배치는 유지하며, 동일한 SaaS 10곳의 정보 위계·표·폼·내비게이션 패턴을 전 화면에 적용한다. NFR-007·FR-SRCH-003·FR-ADMIN-001 관련 표현 변경이며 기존 권한·서버 동작은 보존한다. WP-084. 상태: 구현·검증·`0.1.0-pilot.10` 발행 완료. 기존 문서 gate 오류와 실 GHE 검증은 별도다.

## CR-095 — 일반 검색 UI Radix 전환 (2026-09-16)

사용자 승인: 일반 사용자 화면에서 Conductor 사용을 중단하고 `@radix-ui`로 재구현한다. 통합 검색의 배치는 `artifacts/template.html`의 헤더 → 300px 저장소 탐색 + 탭/필터/결과를 따른다. operator 화면과 기존 컴포넌트는 보존한다. 공식 SaaS 사이트 10곳 조사 결과를 표현 설계에 반영한다. FR-SRCH-003·005~008, NFR-007의 표현 계층 변경이며 새 API·DB·수집 기능은 추가하지 않는다. WP-083으로 추적한다. 상태: 구현·검증·`0.1.0-pilot.10` 발행 완료. 기존 문서 gate 오류는 별도다.

## CR-094 — 기본 저장소 작업 공간과 operator 전용 기존 UI (2026-09-16)

사용자 승인: `artifacts/template.html` 및 두 분석 문서의 레이아웃을 Conductor로 재구현한다. 기존 컴포넌트와 운영 도구는 삭제하지 않고 operator 전용으로 보존한다. 일반 사용자의 저장소 등록 검토 요청·승인 절차를 검색 진입에서 폐지하고, 접근 가능한 현재 저장소의 PR 목록을 바로 표시한다. 접근 범위 필터와 기존 API 쓰기 권한은 유지한다. `FR-SRCH-003`, `FR-SRCH-005`~`FR-SRCH-008`, `FR-ING-009`의 화면 계약 정정이며 WP-082로 추적한다. upstream 워커 smoke와 컨테이너 네트워크 제한 빌드 안내도 포함한다. 검증은 사용자 지시에 따라 변경 경로 중심으로 최소화한다. 상태: implementing. 릴리스 발행 결과는 구현 원장에 기록한다.
...
| CR-115 | 2026-09-22 | scope | 사용자 지시 2026-09-22 — 사내 pilot.17 확정 요건(M 번호를 Git revision으로 쓰기 위한 원격 lightweight 태그) | **확정된 M 번호마다 `refs/tags/M-<코드>-<번호>`를 머지 커밋에 원격으로 만든다 — durable work·멱등·충돌은 보고만(이동·삭제 경로 없음)·전용 App·감사 `merge_number.tag`·커밋 문서 M 값 투영·대조 잡·`prsctl mnumber tags`.** 기본 꺼짐. 다중 시퀀스 브랜치는 OD-015 | FR-SEQ-012 · OD-015 · ADR-026 · ADR-022 Amendment · JOB-SEQ-007 · ENT-SEQ-001 · ENT-SEQ-006 · ENT-CORE-001 · API-ADM-001 · API-ADM-002 · THR-061 · RB-28 · WP-100 · DEV-740~743 | SRS · PRD · 용어집 · 매트릭스 · UI 컴포넌트 · 실행 지시서 · 백엔드 · API 계약 · 데이터 모델 · 비동기 · 보안 · 인프라 · 관측성 · ADR · WP · 원장 · RUNBOOK · k8s | closed — main `83d30fb`(PR #226), 원장 6.106 |
| CR-114 | 2026-09-22 | scope | 사용자 지시 2026-09-22 — 사내 pilot.17 보고(검색창 `M-1900-1450`이 전문 검색으로 떨어짐, `mnum:` 범위·바인딩 부담) | **M 번호 표기 문자열을 검색창 식별자로 정본에서 해석하고, `mnum:` 단일 값과 유일 추적 브랜치의 `base:` 생략을 연다.** 후보에 M 세 키·정본 시퀀스 값, 기능 꺼짐은 `merge_number_disabled`, 전 저장소 무바인딩은 OD-014 | FR-SRCH-001 AC-7 · FR-SRCH-005 AC-9 · OD-014 · API-SRCH-001 · WP-099 · DEV-738 · DEV-739 | SRS · PRD · 용어집 · 매트릭스 · 화면 흐름 · 실행 지시서 · 백엔드 · API 계약 · PIPE handoff · WP · 원장 | closed — main `9782ba9`(PR #225), 원장 6.105 |
...
| CR-090 | 2026-09-14 | scope | `REL-007` 검증된 레지스트리 운영 승인·R0 실행 정책 수직 (결정자 지시 2026-09-14) | **관리자의 명시적 승인과 차단 결정이 실제 실행을 통제하고, 그 근거와 변경 이력이 보존되게 한다 — 새로 실행 가능한 명령은 없다.** 운영자가 A-006에서 현재 적재된 gh capability 정의와 실행기 검증 근거를 미리 보고 운영 승인·철회하며, A-005(최소)에서 `pr.list`를 사유와 함께 차단·재개한다. 결정은 배포 범위(`GHE_BASE_URL`의 호스트)마다 하나인 정책 revision으로 남는다 — 기대 revision 충돌 검출, 중복 방지 키, append-only 이력, 같은 트랜잭션의 감사(`FR-AUTH-004 AC-6` 예외). API의 요청 수락과 실행기의 claim이 같은 판정 함수로 확인한다: 승인이 없으면 `admin_action_required`, 운영자 차단이면 `policy_blocked`, 수락 뒤 revision이 바뀐 대기 요청은 과거 승인을 승계하지 않고 닫힌다. DB 변경은 `SECURITY DEFINER` 함수(소유자 `prs_admin`, 고정 `search_path`, `prs_app`에는 EXECUTE만)와 advisory lock 직렬화로 하고, claim 가드 트리거가 이전 앱 버전으로 되돌린 뒤에도 승인 없는 실행권 확정을 막는다. 스냅숏 활성화 조건에 결과 계약 차원(보고서 판 해석기·재현 해시·게이트)을 반영하고(`DEV-685`), 현재 WP-045 상태 문구를 정정한다. 운영 승인은 196개 명령의 실행 허용·사내 GHES 확인·`GATE-GH-01e`·`06`·`08`의 증거·`REL-007` 완료가 아니다 | FR-GH-009 · FR-GH-011 · FR-AUTH-004 · API-GH-001 · API-GH-002 · API-GH-008 · API-GH-013 · ENT-GH-006 · ENT-GH-012 · ENT-GH-013 · ENT-GH-014 · JOB-GH-001 · JOB-GH-003 · A-005 · A-006 · W-010 · REL-007 · WP-045 · WP-059 · WP-080 · DEV-685 | 요구사항(SRS v2.28 · 용어집 · 추적 매트릭스) · 파생 UI(와이어프레임 A-005·A-006·W-010 · 상태 매트릭스 · 흐름 · QA 체크리스트 · IA · 에이전트 브리프) · 아키텍처(API 계약 · 데이터 모델 · 보안 · 비동기 · 백엔드 · 프런트엔드 · 인프라 · 관측성 · ADR) · 로드맵 · 작업 패키지 · 검증 계획 · 런북 · 코드 · 구현 원장 | closed — cascade·검사기 결과·병합 뒤 main CI를 기록하고 닫는다 (2026-09-14) |
| CR-091 | 2026-09-15 | correction | `0.1.0-pilot.6` 사내 반입 Upstream Feedback (ADR-021) · 결정자 지시 2026-09-15 | **반입 뒤 정상 기동한 배포에서 로그인·운영 권한·업그레이드가 막힌 세 자리를 고친다 — 요구사항 문장은 바꾸지 않는다.** (1) **TLS 없는 파일럿에서 GHE 로그인을 시험할 방법이 없었다**(`DEV-694`). `CR-083`의 면제는 인증을 끈 배포뿐이었다. `ALLOW_INSECURE_COOKIES=true`를 `SESSION_COOKIE_SECURE=false`와 **함께 명시한** 배포만 인증을 켠 채 서게 한다(사용자 결정). 구현 중 하나를 더 찾았다 — 세션·왕복 쿠키가 `__Host-` 접두라 `Secure`만 풀면 브라우저가 쿠키를 버린다. `Secure`가 없으면 접두 없는 이름으로 발급하고 `search-api`로는 정본 이름으로 다시 조립한다. (2) **세션 인증으로 바꾼 뒤 누구도 `operator`가 될 수 없었다**(`DEV-695`). 역할 합집합(`CR-015`)이 문서에만 있었고 코드에서는 로그인 콜백이 관리자 지정 자리에 빈 배열을 넘겼으며 관리자 지정을 쓰는 운영 경로가 없었다. 사내 제안 둘(팀 매핑에 `operator` 허용, 운영 화면을 `manager`로 하향)은 `CR-015`의 경계를 넓히므로 기각하고(사용자 결정), 요청마다 합성하는 실효 역할과 `prsctl role`을 둔다(`FR-AUTH-001` AC-10, `FR-AUTH-004` 표 두 행과 AC-6 예외). (3) **토큰이 남은 `.env`로 인증을 켠 업그레이드가 컨테이너 교체 뒤에야 crash-loop로 드러났다**(`DEV-696`). `prsctl`이 compose 렌더를 `search-api`와 같은 규칙으로 판정해 먼저 멈춘다. 결정자 지시로 병합 뒤 `0.1.0-pilot.7`까지 발행한다 | FR-AUTH-001 · FR-AUTH-004 · API-AUTH-001 · ENT-CORE-005 · ENT-CORE-007 · WP-012 · WP-076 · WP-070 · THR-052 · THR-053 · DEV-048 · DEV-049 · DEV-694 · DEV-695 · DEV-696 | 요구사항(SRS v2.29 · 용어집) · 아키텍처(보안 · API 계약 · 백엔드 · 프런트엔드 · 데이터 모델 · 인프라) · 작업 패키지 · 런북 · 배포 정의 · 코드 · 구현 원장 | closed — cascade·검사기 결과·병합 뒤 main CI를 기록하고 닫는다 (2026-09-15). `0.1.0-pilot.7` 발행은 별도 기록 |
| CR-092 | 2026-09-15 | correction | `0.1.0-pilot.7` 사내 반입 Upstream Feedback (ADR-021) · 결정자 지시 2026-09-15 | **pilot.7 반입 뒤 인증을 켠 배포에서 저장소 목록이 비고, 로그인 뒤 `localhost:3000`으로 떨어지고, 로그아웃할 방법이 없고, 스모크가 정상 서비스를 실패로 보고한 네 자리를 고친다 — 요구사항 문장은 바꾸지 않는다.** (1) **쓰이지 않는 조직·팀 조회 하나가 접근 범위 전체를 503으로 만들었다**(`DEV-698`). 볼 수 있는 저장소가 500개 이하인 범위는 필터가 저장소 ID만 쓰는데 출처가 조직·팀까지 늘 조회했고, App에 조직 `Members` 권한이 없으면 그 조회가 실패했다. 조직·팀은 그 표현(`org_team`)을 쓰는 범위에서만 읽고, 실패는 단계·오류 분류·상태 코드·필요 권한으로 운영 로그에 남긴다(응답은 사유를 싣지 않는다). `/me`의 `org_count`·`team_count`는 읽지 않은 모드에서 `null`이다. 사내 제안 둘 가운데 App 권한 추가는 런북의 권한 표로 받고, DB 기반 출처 전환은 `OD-002`와 `FR-AUTH-002` AC-1에 어긋나 택하지 않았다(사용자 결정). (2) **로그인·계정 연결 콜백이 서버가 들은 출처로 복귀 주소를 만들었다**(`DEV-699`). 경로만 담은 `Location`으로 바꿔 `Host`를 믿지도 새 설정값을 두지도 않는다(`THR-054`). (3) **화면에 로그아웃이 없었다**(`DEV-700`). C-001의 사용자 메뉴에 「로그아웃」 폼 제출을 두고 문서 요청에는 303으로 공개 완료 화면을 보인다(사용자 결정 — 로그인으로 곧장 보내면 IdP 세션으로 다시 로그인된다). (4) **`prsctl smoke`가 본문과 헤더를 한 파이프로 읽어 정상 `search-api`를 간헐적으로 실패시켰다**(`DEV-697`). **코드를 바꾸지 않은 피드백 셋**: 로그인 때 팀 동기화(원인이 아니며 `FR-AUTH-003`이 요구하지 않는다), `smp*` 저장소의 시퀀스 대상 브랜치(저장소마다 운영자가 적는 값 — `FR-ING-009` AC-1, 런북 안내, 사용자 결정), 디자인 시스템 개선(별도 트랙, 사용자 결정). 릴리스는 발행하지 않는다(결정자 지시) | FR-AUTH-001 · FR-AUTH-002 · FR-AUTH-003 · FR-ING-009 · OD-002 · API-AUTH-001 · C-001 · FLOW-000 · WP-012 · WP-015 · WP-070 · THR-054 · DEV-691 · DEV-697 · DEV-698 · DEV-699 · DEV-700 | 아키텍처(보안 · API 계약 · 백엔드 · 프런트엔드) · 파생 UI(컴포넌트 명세 C-001 · 화면 흐름 FLOW-000) · 작업 패키지 · 런북 · 배포 정의 · 코드 · 구현 원장 | closed — cascade·검사기 결과·병합 뒤 main CI를 기록하고 닫는다 (2026-09-15). 릴리스는 발행하지 않았다(결정자 지시) |
| CR-093 | 2026-09-16 | implementation | 최신 Conductor 적용 및 Shell·W-001 UI 개선(결정자 지시, 이전 세션 재개) | react/css 0.4.1·tokens 0.4.0 exact 및 lucide-react 1.46.0. 공통 셸·탭·필터 칩·접기·분할 미리보기·복사·경로·로딩을 Conductor로 전환하고 제품 ResultTable 계약을 유지한다. SRS 의미·API·인가·DB·ES 불변 | FR-SRCH-001 · FR-SRCH-006 · FR-SRCH-008 · NFR-007 · W-001 · ADR-006 · WP-081 · DEV-701 · DEV-702 | 파생 UI · 토큰 · QA · 프런트엔드 · ADR · WP · 원장 · 에이전트 브리프 | closed — PR #196 squash `a8796de`, PR CI `35061974889`·main CI `35062529329` success, `0.1.0-pilot.8` 발행 (2026-09-16) |
| CR-100 | 2026-09-17 | scope | `0.1.0-pilot.11` 사내 반입 Upstream Feedback(M-번호 채번 차단, 2026-09-17) · 결정자 승인 2026-09-17 | **PR 근거가 끝내 없는 항목의 판단을 시스템의 추정이 아니라 운영자의 확인서로 만든다.** `FR-SEQ-008`에 AC-15를 더하고 AC-10에 예외를 적는다. 확정 근거를 미확정으로 덮던 경합을 막는다(DEV-715). 「unresolved를 건너뛰며 진행」은 기각. 문서 검사기 회귀(DEV-716) 정정 | FR-SEQ-008 AC-15 · ENT-SEQ-008 · WP-088 · DEV-715 · DEV-716 · DEV-717 · ADR-023 | SRS · PRD · 용어집 · WP-074 설계 · ADR · 데이터 · 비동기 · 보안 · 인프라 · RUNBOOK · WP · 원장 | closed — PR #207 squash `08fbc3a`, PR CI `35177846020`·main CI `35178340678` success, 독립 검토 minor 3 반영 (2026-09-17) · `0.1.0-pilot.13` 발행 |
...
| CR-105 | 2026-09-17 | correction | 사용자 보고 2026-09-17 — Search 제목에 `VANGUARD-1` 검색 시 숫자 1만 겹치는 무관한 결과가 섞임 | **하이픈으로 쪼개지는 식별자의 전문 검색 과매칭.** 판단 기준을 분석 후 토큰 개수가 아니라 원문 공백 여부로 바꿔, 공백 없는 한 단어는 `type:"phrase"`로 인접 매칭, 공백 있는 여러 단어는 기존 OR을 유지(AC-4 보존). 요구사항 문장 변경 없음 | DEV-722 · FR-SRCH-011 AC-4 | 원장 | closed — main `5b2a24f`, CI(run `35229686599`) 재실행 뒤 success(2026-09-17) |
...
### CR-091 cascade — `0.1.0-pilot.6` 반입 피드백: 평문 HTTP 파일럿 로그인 · 관리자 지정 역할 · 토큰 공존 사전 감지

시작 `origin/main`은 `a95e4d9`(사내 피드백을 `agent-context/upstream-feedback.md`에 올린 커밋, 코드 변경 없음), 브랜치는 `fix/cr091-pilot6-auth-feedback`(작업 트리 격리, 격리 서비스 `prs-cr091-*`)이다. **`srs_final.md`가 baseline이므로 이 CR을 먼저 등록하고 요구사항을 고쳤다.**

- [x] 요구사항(SRS v2.29): `FR-AUTH-001` AC-2에 두 번째 면제(`ALLOW_INSECURE_COOKIES`)와 `Secure` 없는 쿠키의 이름 규칙을, **AC-10**(요청마다 판정하는 실효 역할, 관리자 지정의 호스트 운영 도구 경로, 감사)을 더했다. 6장 권한 판정 규칙에 역할을 받는 세 경로를 적었다. `FR-AUTH-004` 감사 대상 표에 `user_role.grant`·`user_role.revoke`, AC-6에 같은 트랜잭션 예외를 더했다. **신규 FR·NFR 없음, 안정 ID 재번호화 0건.**
- [x] 확인 뒤 바꾸지 않은 것: `prd.md`(FR 목록과 기능 정의가 그대로다), 요구사항-화면 매트릭스(`FR-AUTH-001`·`FR-AUTH-004`의 화면 대응이 그대로다 — 역할 지정은 화면이 아니라 호스트 명령이다), 파생 UI 문서(내비게이션 규칙은 이미 역할 기반이고 그 역할의 출처만 정정됐다).
...
- [x] 구현 원장: `DEV-694`~`DEV-696`을 5장에, 4장 `FR-AUTH-001`·`FR-AUTH-004` 행을, 검증 결과(독립 검토 두 관점·변이·배터리·이미지·게이트)를 6.89장에 적었다. PR CI(run `34926323705` attempt 1 success(verify 4m20s · integration 8m02s — 단위 2,760 · a11y 424 · e2e 196 · 통합 1,805 · 회귀 496))·squash 병합 `f8db374`·main CI(run `34926885455` attempt 1 success(verify 4m37s · integration 7m32s — 단위 2,760 · a11y 424 · e2e 196 · 통합 1,805 · 회귀 496))를 후속 기록 PR이 6.89장에 더했다.

**사내 제안을 그대로 구현하지 않은 자리 둘.** 둘 다 `CR-015`가 세운 경계를 지키기 위해서이며 사용자가 결정했다(2026-09-15).

| 사내 제안 | 실제 | 왜 |
| --- | --- | --- |
| `GHE_TEAM_ROLE_MAP`에서 `operator` 매핑 허용 | 팀 매핑은 여전히 `manager`·`qa`만. `operator`는 `prsctl role grant` | GHE 팀을 만들 수 있는 사람이 운영 권한을 발급하게 된다(`DEV-049`). 막힌 원인은 경계가 아니라 **관리자 지정 경로가 코드에 없었던 것**이다(`DEV-695`) |
| `/ops/*`의 역할 요구를 `manager`로 하향 | 운영 화면은 `operator` 그대로 | `FR-ADMIN-001` AC-4가 승인한 계약이며, 하향하면 `manager` 팀 매핑 하나로 운영 콘솔 쓰기가 열린다 |
| `ALLOW_INSECURE_COOKIES=true` 명시 플래그 | 그대로 받되 `SESSION_COOKIE_SECURE=false`와 **함께** 적어야 선다. 오타는 기동 거부, 기동·health 경고, `Secure`가 없으면 접두 없는 쿠키 이름 | `CR-078`의 우려(「열어 두면 열린 채로 켜진다」)에 대한 답을 「적어 낸 선언」으로 유지한다. 이름 규칙이 없으면 플래그가 기동만 통과시키고 로그인은 여전히 실패한다 |
...
**병합 판정.** 이 CR의 PR은 필수 check green과 독립 검토의 blocker·major 해소 뒤 squash 병합하고, 병합 커밋의 main CI를 실측해 후속 기록 PR에서 닫는다. 결정자 지시(2026-09-15)에 따라 그 뒤 `0.1.0-pilot.7`을 발행한다. 사내 반입과 실제 GHE 확인은 `NOT RUN`이다.

### CR-092 cascade — `0.1.0-pilot.7` 반입 피드백: 접근 범위의 쓰이지 않는 조회 · 프록시 뒤 복귀 주소 · 로그아웃 · 스모크 파싱

시작 `origin/main`은 `0a83797`(사내 피드백 일곱 건을 `agent-context/upstream-feedback.md`에 올린 커밋, 코드 변경 없음), 브랜치는 `fix/cr092-pilot7-feedback`(작업 트리 격리, 격리 서비스 `prs-cr091-*` 재사용)이다. **착수 전에 일곱 건 전부를 코드·문서와 실측으로 대조했고, 넷은 사내가 적은 원인이나 전제가 실제와 달랐다.** 사용자 결정 넷(2026-09-15): 필요한 조회만 + 진단 로그, 사용자 메뉴 + 로그아웃 완료 화면, `smp*` 브랜치는 운영 조치 안내, 디자인 시스템은 이 PR에서 제외.

- [x] 확인 뒤 바꾸지 않은 것: `srs_final.md`(요구사항 문장이 그대로 성립한다 — `FR-AUTH-002` AC-1·AC-6은 저장소 ID 집합과 500개 전환만 정하고, `FR-AUTH-001` AC-5는 로그아웃의 서버 세션 무효화를 이미 요구한다), `prd.md`, 용어집, 요구사항-화면 매트릭스(`/auth/signed-out`은 `/auth/callback`처럼 화면 ID가 없는 인증 라우트의 결과다), 데이터 모델·마이그레이션(없음).
- [x] 아키텍처: 보안 v1.12(4장 로그아웃 행, 복귀 주소·로그아웃의 경계, 5.2의 조직·팀 조회 조건과 실패 로그, `THR-054`), API 계약 v0.32(API-AUTH-001 `org_count`·`team_count`의 `null`과 예시), 백엔드 v0.12(6.1 2단계), 프런트엔드 v0.9(라우트 표 `/auth/logout`·`/auth/signed-out`, 10장의 경로 리다이렉트 규칙).
- [x] 파생 UI: 컴포넌트 명세 v0.16(C-001 사용자 메뉴), 화면 흐름 v0.9(FLOW-000 6단계의 경로 복귀와 7단계 로그아웃).
- [x] 코드: `packages/authz`(`GheAccessScopeSource`가 `org_team` 범위에서만 조직·팀을 읽고 단계를 붙인 `AccessScopeLookupError`를 던진다, `SCOPE_STAGE_PERMISSION`, `AccessScopeResolver`의 `log`와 `describeScopeFailure`), `apps/search-api`(`createAuthContext`가 로그를 넘긴다, `/me` 요약의 `null`), `apps/web`(`lib/redirect.ts`의 `redirectToPath`와 콜백 둘, 로그아웃 라우트의 303, `components/UserMenu.tsx`·`SignedOutView.tsx`, `app/auth/signed-out/page.tsx`, `lib/auth-paths.ts`), `deploy/single-host/prsctl`(`http_status`).
- [x] 런북: 2.C 「수집용 GHE App에 줄 권한」(권한 표·503 진단·`permission_cache` 손 조작 금지)과 「시퀀스 대상 브랜치 정하기·바꾸기」, 2.C 2단계의 화면 ID 정정(`A-001` → `A-002`), 6장 「역방향 프록시 뒤에서」·「로그아웃」, 8장 증상 표(고친 하나, 새 다섯).
- [x] 작업 패키지 v2.35: `WP-012`·`WP-015`·`WP-070`에 정정 주석. **새 WP를 만들지 않았다** — 각 WP의 계약을 코드가 이행하지 못한 자리다.
- [x] 구현 원장: `DEV-697`~`DEV-700`을 5장에, 4장 `FR-AUTH-001`·`FR-AUTH-002` 행을, 검증을 6.91장에 적었다. PR CI(run `34971448100` attempt 1 success(verify 4m31s · integration 8m27s — 단위 2,790 · a11y 429 · e2e 199 · 통합 1,807 · 회귀 503))·squash 병합 `ae9bf27`·main CI(run `34972449347` attempt 1 success(verify 4m32s · integration 7m22s — 단위 2,790 · a11y 429 · e2e 199 · 통합 1,807 · 회귀 503))를 후속 기록 PR이 6.91장에 더했다.

**사내 제안과 실제.**

| 사내 피드백 | 실측 | 처분 |
| --- | --- | --- |
| scope 503 — `members:read` 부족 추정, DB 기반 출처 전환 제안 | 흐름은 사실이다. 다만 **쓰이지 않는 조회가 원인의 절반**이었고(저장소 셋이면 조직·팀 값은 어디에도 쓰이지 않는다), 사유가 로그에 없어 원인은 확인할 수 없었다. 협업자 권한 조회는 `Metadata`, 조직·팀 조회는 `Members`다(GitHub REST 문서) | 필요한 조회만, 실패 단계 로그, 런북 권한 표(`DEV-698`). DB 출처는 `OD-002`(GHE 협업자·팀 API로 확정)와 AC-1(실효 권한)에 어긋나고 `team_member`는 저장소 권한이 아니어서 기각(사용자 결정) |
| 로그인 시 팀 멤버십 동기화 | 현상(버전 0, `team_member` 빔)은 사실이지만 **정상 상태다.** 로그인은 권한 변경이 아니고(`FR-AUTH-003`은 웹훅 무효화와 TTL만 요구), 500개 이하 범위의 필터는 `team_member`·`allowed_team_ids`를 쓰지 않는다. 저장소가 안 보인 원인은 위의 503이다 | 코드 변경 없음. 런북 8장에 「정상이다」 행 |
| 로그아웃 뒤 `localhost:3000` 리다이렉트 | 로그아웃은 리다이렉트하지 않았다(JSON 200). **로그인 콜백과 Operations 콜백**이 `request.nextUrl.origin`으로 복귀 주소를 만들었고, pilot.7 web 이미지에 `Host`를 넘겨도 `https://localhost:3000/…`이었다 | 경로만 담은 `Location`(`DEV-699`). `WEB_EXTERNAL_URL`·`X-Forwarded-Host` 신뢰는 택하지 않았다 — 맞출 값이 늘거나 열린 리다이렉트가 된다 |
| 로그아웃 버튼 없음 | 사실. C-001이 「사용자 메뉴」를 적었지만 항목이 없었고 구현도 없었다 | 사용자 메뉴의 「로그아웃」 폼 제출과 공개 완료 화면(`DEV-700`, 사용자 결정) |
| smoke healthz 오탐 — 「pilot.7에서 파싱 방식이 바뀌었다」 | 그 파이프는 `CR-060`(pilot.2)부터 같다. `docker exec`가 stdout·stderr를 따로 날라 개행 없는 본문이 헤더 앞에 붙으면 `$2`가 `HTTP/1.1`이다 — pilot.7 이미지로 20회 중 8회 재현, 간헐이라 이전 반입에서 보이지 않았다 | 상태 코드만 읽는 `http_status`(`DEV-697`) |
| `smp*` 저장소의 시퀀스 대상 브랜치는 `dev` | 코드에 `main` 기본값이 없다. 브랜치는 저장소마다 운영자가 적고(`FR-ING-009` AC-1) 편집하면 채번을 요청한다(AC-12) | 코드 변경 없음(사용자 결정). 런북 2.C 절차 — 이름 규칙 자동 적용은 새 범위다 |
| 디자인 시스템 개선 | Conductor는 npm 패키지(0.3.1 고정)이고, 원본 저장소에서 Badge·Button 작업이 진행 중이다 | 이 CR에서 제외, 별도 트랙(사용자 결정) |

**구현이 스스로 고른 것.** 조직·팀을 늘 읽던 이유(「임계를 넘나들 때 다시 조회하지 않도록」)는 성립하지 않았다 — 한 범위의 표현은 그 범위의 저장소 수가 정하므로 한 캐시 값 안에서 넘나듦이 없다. 실패 로그는 메시지를 싣지 않는다 — `@prs/github`의 전송 오류 메시지가 응답 본문 앞 200자를 담는다. `redirectToPath`는 규칙을 어긴 경로를 `/`로 고치지 않고 던진다 — 호출부가 걸렀어야 할 값을 조용히 고치면 결함이 가려진다. 로그아웃 라우트는 `Accept`로 응답을 가른다 — 스크립트 호출자와 기존 e2e의 JSON 계약을 깨지 않는다. 사용자 메뉴의 폼은 메뉴 밖에 두고 항목 선택이 제출한다 — 메뉴 내용이 포털로 그려져 폼 안에 들어갈 수 없다.

**검증.** 원장 6.91장이 정본이다.

**병합 판정.** 이 CR의 PR은 필수 check green과 독립 검토의 blocker·major 해소 뒤 squash 병합하고, 병합 커밋의 main CI를 실측해 후속 기록 PR에서 닫는다. **릴리스는 발행하지 않는다**(결정자 지시 2026-09-15). 사내 반입과 실제 GHE 확인은 `NOT RUN`이다.

### CR-093 cascade — 최신 Conductor와 Shell·W-001 표현 계층

기준 `8c567b7`, 브랜치 `feature/cr093-conductor-w001`. `srs_final.md`·PRD·용어집·요구사항-화면 매트릭스의 기능 의미와 대응은 유지한다. 신규 FR·API·DB 마이그레이션은 없다. IA·흐름·상태 판정은 기존 계약을 유지하고, 와이어프레임·컴포넌트·토큰·QA·프런트엔드·ADR-006·WP-081·원장 6.92장·실행 브리프를 갱신했다. PR #196 squash `a8796de`, PR CI `35061974889`·main CI `35062529329` success, `0.1.0-pilot.8`의 런타임 검사·시크릿 검사·자산 SHA-256 대조를 기록해 CR을 닫는다.

이전 지시의 pilot.8 미발행은 **2026-09-16 재개 시 사용자의 새 릴리스 발행 지시로 대체**됐다. 발행 여부·태그·자산 digest는 실제 실행 후 원장에 기록한다.

### CR-100 cascade — M 번호 운영자 확인서와 확정 근거 보존

기준 `dcaad12`, 브랜치 `fix/cr100-mnumber-attestation`. 결정자 승인(2026-09-17): 요청 1·2는 운영자 확인서, 관찰 4는 결함 수정, 요청 3은 기각, 문서 검사기 회귀 정정 포함.

- [x] 요구사항(SRS v2.35): `FR-SEQ-008`에 AC-15(운영자 확인서)를 더하고 AC-10에 그 예외와 「확정 → 미확정 금지」를 적었으며 「증거 가용성 제한」에 CR-100 보완을 달았다. 감사 대상 액션 표에 `mnumber_attestation.create`·`revoke`. 상태 줄을 제목 바로 아래로 되돌렸다(DEV-716).
- [x] PRD: CR-100 제품 계약 한 문단.
- [x] 용어집: 「M 번호 운영자 확인서」.
- [x] 요구사항-화면 매트릭스·파생 UI·API 계약: 변경 없음 — 화면·API가 없다(CLI만).
- [x] 기술 아키텍처: WP-074 설계 2.2 보완·3절 판정 행·6.2 source_kind/proof·6.5 ENT-SEQ-008·12절 C7, ADR-023 accepted + Amendment, 데이터 모델 ENT-SEQ-008, 비동기 JOB-SEQ-004 트리거, 보안(prsctl·감사), 인프라(prsctl 목록), RUNBOOK 7.D.
- [x] 전달: 작업 패키지 WP-088(상태 줄 복귀 포함), 원장 3장·DEV-715~717·6.94장.
- [x] 에이전트 브리프: 변경 없음.
- [x] Upstream Feedback 회신: `agent-context/upstream-feedback.md` 각 요청 아래.
- [x] 검증기 `--strict` ERROR 6→4·WARN 4→2(남은 것은 기존) · PR #207 CI `35175836037`(verify attempt 2 success)·`35177846020` success · squash 병합 `08fbc3a` · main CI `35178340678` success (2026-09-17). closed.

### CR-101 cascade — 병합된 PR의 state 파생 보정

기준 CR-100 브랜치(`fix/cr100-mnumber-attestation`) 위 `fix/cr101-merged-state`. 결정자 보고 2026-09-17. correction — 요구사항 문장은 바꾸지 않는다.

- [x] 요구사항(SRS): 변경 없음 — `FR-SRCH-005 AC-1`의 `state`·`is` 키와 뜻은 그대로다. PRD·용어집·매트릭스: 변경 없음.
- [x] 파생 UI: 변경 없음 — Status 선택지(Merged/Open/Closed)는 이미 세 값을 전제한다.
- [x] 기술 아키텍처: API 계약 v0.36(「state는 GitHub 값 그대로」 문장을 파생 규칙으로 정정), 데이터 모델 v0.27(ENT-CORE-002 `state` 값), RUNBOOK 업그레이드 절(032 뒤 재색인)·문제 해결 행.
- [x] 전달: 작업 패키지 WP-089, 원장 3장·DEV-718·6.95장.
- [x] Upstream Feedback: `agent-context/upstream-feedback.md`에 항목과 회신(반입 뒤 재색인 절차).
- [x] 검증기 `--strict` ERROR 4·WARN 2(기존과 동일) · PR #208 CI `35178504451` success · squash 병합 `3ba1c4b` · main CI `35179018409` success (2026-09-17). closed. 사내 확인 및 기존 미통과 릴리스 게이트를 통과로 바꾸지 않는다.

### CR-103 cascade — Diff 분할 보기·History 안내·사이드바 고정 레이아웃 정정

기준 `2a4f37e`, 브랜치 `fix/cr102-frontend-fixes`(worktree `/home/roqkf/pr-search-wt/cr102-frontend-fixes` 격리; 브랜치명은 실측 전 임시로 붙인 이름이고 문서상 채번은 CR-103이다 — 같은 날 공유 checkout에서 다른 세션이 Regression 확장 제안 작업으로 CR-102·DEV-719·OD-010~013을 먼저 점유해 실측 후 밀렸다, 그 작업은 별도이며 이 CR은 손대지 않는다). 사용자가 프런트엔드 결함/개선 네 건을 보고했고(2026-09-17), 그중 SRS 문장 변경이 필요 없는 세 건을 이 CR로 묶는다. correction — 요구사항 문장은 바꾸지 않는다.

- [x] 요구사항(SRS): 변경 없음 — `FR-SRC-003` AC-2(split/unified 표현)와 `FR-SRC-002` AC-3(리비전 둘 선택 비교)은 이미 승인되어 있고 구현 결함·안내 부재만 정정한다. PRD·용어집·요구사항-화면 매트릭스: 변경 없음.
- [x] 파생 UI: 변경 없음 — 사이드바·필터·결과 표의 정확한 치수를 규정한 문서가 애초에 없었다(레이아웃 디테일).
- [x] 기술 아키텍처: 변경 없음.
- [x] 전달: `WP-085`에 정정 주석(DEV-720·DEV-721), `WP-090` 신설(검색 좌측 패널·결과 표 고정 레이아웃). 원장 3장·5장·6.96장.

**정정 1 — split diff 뷰, 두 소스 컬럼이 하나의 가로 스크롤을 공유했다(DEV-720).** `apps/web/app/source-workspace.css`의 `.source-diff-table--split`이 `min-width:260px; width:50%`를 두 소스 컬럼(`nth-child(2)`·`nth-child(4)`)에 걸었는데, 테이블 자체는 `table-layout` 지정이 없어 기본값 `auto`였다. `auto` 레이아웃에서 `min-width`가 `width:50%`와 싸우며 컬럼 폭이 렌더마다 흔들렸고, 무엇보다 두 컬럼이 바깥의 `.source-code-scroll` 하나를 공유해 스크롤했다 — 한쪽(예: before)에 긴 줄이 하나만 있어도 `auto` 레이아웃이 그 컬럼을 내용에 맞춰 넓히면서 테이블 전체 폭이 늘고, 그 긴 줄을 읽으려 오른쪽으로 스크롤하면 반대쪽(after) 컬럼도 같은 만큼 밀려 화면 밖으로 사라졌다. 사용자가 "코드가 누락되는 경우가 있다"고 본 것은 실제 데이터 손실이 아니라 이 스크롤 결합이었다 — `compareLines`(`apps/web/lib/source-analysis.ts`)의 diff 정렬 자체는 `diff@9.0.0`의 `diffLines`가 이 버전에서 항상 removed를 added보다 먼저 내보낸다는 사실을 실측 스크립트로 확인했고, 3제거+1추가 같은 비대칭 교체·서로 떨어진 여러 hunk를 모두 정확히 짝짓는 것을 확인해 손대지 않았다.
정정: `.source-diff-table--split`에 `table-layout:fixed`를 걸고 두 소스 컬럼을 `width:calc(50% - 44px)`(44px는 `.source-line-number`의 폭)로 고정해 두 컬럼 + 두 줄번호 거터의 합이 정확히 100%가 되게 했다. 각 컬럼에 `overflow-x:auto`를 걸어 긴 줄이 그 컬럼 안에서만 스크롤되고 반대쪽 컬럼 위치에 영향을 주지 않게 했다. `min-width`는 셀이 아니라 테이블 자체에 `388px`(150px×2 + 44px×2)로 걸었다 — 브라우저 실측 결과 `table-layout:fixed`에서는 셀 자체의 `min-width`가 폭 계산에 반영되지 않는 것을 확인했기 때문이다(`getComputedStyle`은 150px를 읽지만 실제 렌더 폭은 93px). 이 최소폭 아래(예: 375px 뷰포트)에서는 테이블 전체가 다시 `.source-code-scroll`의 가로 스크롤로 넘어가는데, 이는 원래 버그(정상 폭에서도 스크롤이 결합)와 달리 좁은 뷰포트에서만 나타나는 용인 가능한 예외다. `<colgroup>`(44/`calc(50% - 44px)`/44/`calc(50% - 44px)`, split 모드에서만 렌더)을 추가해 컬럼 폭 산정이 `ui-sr-only`(`position:absolute`로 테이블 레이아웃 밖) thead나 첫 렌더 행이 접힌-컨텍스트 `colSpan=4` gap 행인지에 좌우되지 않게 했다 — 이 시나리오를 실제로 재현해(`getBoundingClientRect`로 9개 행의 컬럼 폭이 모두 일치함을 확인) `<colgroup>` 없이도 문제가 없었음을 먼저 확인했지만, 강건성을 위해 유지했다.

**정정 2 — Commit history 진입 시 비교 불가 사유가 보이지 않았다(DEV-721).** `apps/web/components/source/SourceHistory.tsx`의 `isFile = Boolean(path) && kind !== 'directory'`가 체크박스·Compare selected 버튼을 게이팅하는데, 이는 `FR-SRC-002` AC-3("파일의 리비전 둘을 선택해 비교")대로 정확하다 — 문제는 게이팅이 아니라 **안내 부재**였다. `RepositoryWorkspace.tsx`에서 "Commit history" 탭에 경로 선택 없이(가장 흔한 진입 경로) 바로 들어가면 `path`가 빈 문자열이라 `isFile`이 `false`가 되고, 모든 체크박스가 비활성화되는데 그 이유는 툴바 위 캡션 한 줄뿐이었다. 정정: `isFile`이 거짓일 때 `Banner`(`tone="info"`, 기존 컴포넌트 재사용)로 "파일을 선택해야 비교할 수 있다"를 명확히 보이고, 파일을 선택했을 때는 기존 캡션을 그대로 유지한다. `Banner`의 기본 `role="status"`는 `HistoryBody`가 경로마다 완전히 리마운트되는 구조(`key={repository|path|revision|branch}`)와 만나 폴더를 여러 번 클릭할 때마다 반복 알림을 낼 수 있어 `role="note"`로 재정의했다(정적 설명이지 실시간 상태가 아니라는 판단, 컴포넌트를 포크하지 않고 prop만 덮어씀). 게이팅 로직 자체는 한 글자도 바꾸지 않았다.

**보완 — SHA 복사(요구사항 밖, 일관성 보완).** 같은 History 행의 SHA가 `<code>`로만 표시되고 복사 수단이 없었다. `FR-SRC-002` AC-1이 요구하지 않는 항목이지만, 이미 승인된 컴포넌트 패턴(`ui_component_spec.md`의 `CopyButton`, 40자 전체 복사 + 실패 폴백)이 이 자리에만 빠져 있던 일관성 결함이라 CR 없이 안전하게 채운다. 기존 `CopyButton`을 그대로 재사용했다(새 클립보드 로직을 만들지 않았다).

**보완 — WP-090 신설, 검색 좌측 패널·필터·결과 표 고정 레이아웃.** `.repo-sidebar`(Browse repositories + Files & folders)가 높이 제한 없이 쌓여 페이지 전체를 늘렸다. `position:sticky`를 유지한 채 `display:flex; flex-direction:column; height:calc(100dvh - 48px); overflow:hidden`으로 바꾸고(`max-height`가 아니라 `height`가 필요하다 — `max-height`만으로는 콘텐츠가 짧을 때 높이가 비확정 상태가 되어 `flex-grow`가 분배할 대상이 없어진다), 두 카드(`repo-repositories`/`source-tree`)에 `flex:1 1 0`과 서로 다른 `min-height` 바닥(72px/185px, 후자는 자체 헤더·필터 입력 체감분을 더한 값)을 줬다. 48px는 `.repo-sidebar`의 `top:24px` sticky 기준이며, 헤더가 sticky가 아니라서 스크롤 전(페이지 최상단, 121px 아래에서 시작)과 스크롤 후(24px에 고정) 두 상태가 다르다는 것을 실제로 두 상태 모두 스크롤해 측정했다(고정 상태 뷰포트 900px에서 바닥 876px, 넘치지 않음; 비고정 상태는 접힘 없이 살짝 아래로 내려가는 정상적인 sticky-사이드바 동작임을 스크린샷으로 확인). 결과 표는 `.repo-results-scroll`로 감싸 `max-height:calc(100dvh - 689px); min-height:320px`로 자체 스크롤을 갖게 했다(기존 "More changes" 버튼은 그대로 그 안에 있다 — 페이지네이션 방식은 이 CR에서 바꾸지 않는다). 저장소 이름은 `overflow-wrap:anywhere`(여러 줄 줄바꿈) 대신 말줄임 + `title` 툴팁으로 압축했다. 700px 높이 같은 짧은 화면에서 두 카드의 바닥이 그 여백을 다 못 채우는 문제는 `.repo-sidebar-note`를 `@media (max-height:750px)`에서 숨겨 해결했다. **모바일(`max-width:800px`) 대응은 구현·검증까지 했으나 사용자가 "모바일은 고려할 필요 없다"고 결정해 되돌렸다** — 되돌린 뒤에도 finding #3의 `max-height:750px` 규칙과는 서로 무관함을 재확인했다.

**독립 검토(2026-09-17, code-review 스킬, high, worktree 대상).** 10건 지적, 최고 심각도부터: (1) 사이드바 높이가 실제로 쓰이지 않는 `.repo-topbar`/`.repo-shell-main`을 근거로 계산됐다(전체 `.tsx` grep으로 실측 확인 — `RepositoryWorkspace`는 항상 `ReaderShell`의 `.reader-header-inner`(88px)+`.reader-container`(32px 상단 패딩) 안에서 렌더된다) — 재계산해 반영. (2) 모바일 브레이크포인트 미반영 — 구현했다가 사용자 결정으로 도로 제거(위 참조). (3) 700px 높이에서 두 카드 바닥이 형제 요소(제목·Branch select·경로 폼·note) 예산을 안 챙겨 note가 클리핑 — 반영. (4) diff 컬럼 최소폭 소실(260px→0) — 388px 테이블 최소폭 + `<colgroup>`로 반영(위 참조). (5) `table-layout:fixed`가 접힌-컨텍스트 첫 행에서 컬럼폭을 잘못 셀 수 있다는 우려 — 실제로 재현해 문제 없음을 확인(위 참조). (6) `Banner`의 `role="status"` 반복 알림 우려 — `role="note"`로 정정(위 참조). (7) 저장소 이름 말줄임이 `title`에만 의존해 터치/키보드 전용 사용자가 못 읽는다 — 이 코드베이스에 이미 있는 기존 패턴과 동일 수준이라 판단, 조치 안 함. (8) `LegacyRepositoryWorkspace.tsx`(범위 밖 4번째 파일)가 같은 미스코프 셀렉터를 공유하는데 검증 안 됨 — 임시 라우트를 만들어 실제 렌더로 확인(정상 동작, 라우트는 검증 뒤 삭제해 빌드로 재확인) 뒤 삭제. (9) CR 미등록(이 문서 작성으로 해소). (10) 매직 넘버 산재 — 낮은 우선순위, 정확한 근거 주석으로 대체. 반영 과정에서 구현이 스스로 11번째 결함(9.5번, `reader-workspace.css`의 `.reader-ui .repo-*` 병렬 재테마가 `repository-workspace.css`의 압축 수정 상당수와 같은 특이도(0,2,0)로 묶여 로드 순서상 이겼다 — `getComputedStyle`로 실측 확인)를 찾아 두 파일의 값을 맞춰 정정했다.

**검증.** 워크트리(Node 22, `pnpm install` 별도 실행) 기준: `pnpm typecheck` 0 오류, `pnpm lint` 0, `pnpm run lint:deps` 위반 0(패키지 16개), `pnpm exec vitest run apps/web/lib/source-analysis.test.ts` 5/5(미변경 파일 sanity check), `pnpm --filter @prs/web run build` 성공(라우트 목록 변경 없음, 두 차례 재확인). `SourceHistory`·`SourceDialogs`·`RepositoryWorkspace`·`SourceTree`를 직접 겨냥한 단위·a11y·e2e 시험은 이 저장소에 없음을 확인했다(기존 a11y/e2e는 legacy `SearchView` 경로만 건다). 실제 백엔드·GHE 접속이 이 환경에 없어, `next start`(HMR 이슈로 `next dev` 대신 사용) 프로덕션 서버에 `@prs/contracts` 타입과 일치하는 fixture로 `window.fetch`를 모킹해 실제 브라우저(1440×900·1280×800·1280×700·375×812·390×844)로 전체 흐름(저장소→사이드바 독립 스크롤→결과 표 스크롤/페이지네이션→Commit history 배너/체크박스/SHA 복사→트리 파일 선택→diff modal split/unified/토글/컬럼 독립 스크롤)을 확인했다 — `getBoundingClientRect`/`getComputedStyle`/`scrollLeft`/`scrollTop` 등 실측 기반이며 스크린샷도 남겼다. 클립보드 "Copied" 확인만 headless sandbox 제약으로 못 봤다(실패 폴백 메시지는 정상 렌더 확인, 컴포넌트 자체는 다른 화면에서 이미 쓰이는 기존 코드).

**병합 판정.** 코드는 main에 직접 커밋했다(`d0e74c7`, 문서 3종은 다른 세션의 미커밋 CR-102 작업과 같은 파일이라 수동으로 병합해 `e6db5f1`로 커밋). 사용자 지시(2026-09-17)로 PR 없이 main에 직접 push했다. `packages/es/src/query-builder.ts`(다른 세션의 별도 미커밋 변경, 이 CR과 무관)도 같은 지시로 `2639c65`에 함께 커밋·push했는데, main CI(run `35223670264`)의 integration 잡이 `facets.test.ts`에서 3건 실패했다 — 원인은 그 커밋이 `buildTextClause`에 더한 `operator:"and"`가 FR-SRCH-011 AC-4("한글·영문 혼용 질의가 두 토큰을 모두 본다", OR 의미)와 정면으로 충돌한 것이었다. 죽은 주석과 함께 그 한 줄만 되돌려 `1bc7a27`로 재커밋·재push했다(되돌린 뒤 단위 64/64 통과, `2a4f37e` 시점 동작과 포맷 외 차이 없음을 diff로 확인 — `operator:"and"`가 있던 상태에서 단위 시험을 따로 돌리지는 않았다, 단위 시험은 원래도 이 회귀를 못 잡았을 가능성이 있다). 재push(`1bc7a27`) 직후 CR-104 커밋(`246fb79`)을 이어 push해 같은 ref의 CI 동시 실행 제한으로 `1bc7a27`의 CI(run `35224521242`)는 `cancelled`(실패 아님 — "높은 우선순위 대기 요청 존재")로 끝났다. 최종적으로 검증하는 것은 `246fb79`의 CI(run `35225253617`)이며 verify·integration 모두 success(2026-09-17) — `operator:"and"` 되돌리기가 실제로 통합 시험을 통과시켰음을 확정한다. 사내 실제 GHE 데이터로의 확인은 `NOT RUN`.

### CR-104 cascade — 검색 결과 무한 스크롤

기준 CR-103 브랜치 위 worktree `/home/roqkf/pr-search-wt/cr103-infinite-scroll`(브랜치 `fix/cr103-infinite-scroll`, main `d0e74c7` 기준). 사용자가 프런트엔드 결함/개선 네 건 중 남은 하나(무한 스크롤)를 결정했다(2026-09-17): 버튼 제거, 완전 자동 로드. scope — `W-001-PAGER`(WP-016)가 세운 버튼 선택 자체를 뒤집지만 FR-SRCH-008은 트리거 방식에 중립적이라 SRS 요구사항 문장은 바뀌지 않는다.

- [x] 요구사항(SRS): 변경 없음 — `FR-SRCH-008`(커서 페이지네이션)은 트리거 방식(버튼/스크롤)을 규정하지 않는다.
- [x] 파생 UI: `pr_search_wireframe_spec.md` v0.25, `W-001-PAGER` 행을 스크롤 자동 로드로 갱신(커서 거절 시 자동 재시도 금지 원칙은 그대로 인용).
- [x] 기술 아키텍처: 변경 없음.
- [x] 전달: `WP-091` 신설. 원장 3장·6.97장.

**구현.** `apps/web/components/RepositoryWorkspace.tsx` 한 파일. `IntersectionObserver`(`rootMargin:200px`) sentinel은 마운트 시 한 번만 생성한다 — 의존성 배열에 `[nextCursor, loading, error]`를 넣어 매번 재생성하면, `.observe()` 호출 시 스펙상 즉시 콜백이 재발화해 새로 추가된 행이 sentinel을 아직 밀어내지 못한 상황(작은 페이지 크기·큰 뷰포트)에서 한 settle에 여러 페이지가 연쇄 로드될 위험이 있다. 대신 콜백은 `useRef`로 최신 `{nextCursor, loading, error}`를 읽고, sentinel의 실제 교차 전환에서만 재평가한다. **`error`가 있으면 콜백이 아무 것도 하지 않는다** — CR-043의 "커서 거절 시 사유를 구분해 알리고 첫 페이지로, 자동 재시도 없음" 규칙이 그대로 유지된다(기존 에러 배너·"Reload first page" 버튼은 손대지 않았다). 결과 카운트(`{count} shown`)는 `aria-live="polite"`로 바꿔 스크린리더에 자동 로드를 알린다. 로딩 표시는 `Spinner`(`aria-hidden` 래퍼 안, 이중 알림 방지) — `CursorPager.tsx`(이 코드베이스의 기존 커서 페이지네이션 컴포넌트)의 `Spinner`+`aria-live` 패턴을 그대로 따랐다(이 저장소에 `IntersectionObserver` 사용례가 없어 새로 만들되, 로딩/알림 표현은 기존 것을 재사용). 키보드 전용 사용자는 결과 영역의 네이티브 스크롤(PageDown·End)로 sentinel에 닿아 같은 경로로 더 불러온다 — 버튼이 없어졌다고 별도 키보드 트리거를 만들지 않았다.

**알려진 경미한 한계(수정하지 않음, 범위 밖으로 판단).** 목록이 이미 맨 아래로 스크롤된 상태에서 스크롤 없이 데이터만 초기화되면(예: "Reload first page"를 맨 아래에서 클릭) sentinel의 기하학적 교차 상태가 전환되지 않아 관찰자가 재발화하지 않는다. 실제로 재현했다: 맨 아래에서 reload → 자동 로드 안 됨; 살짝 위로 스크롤했다가 다시 아래로 → 다음 스크롤에서 정상 로드. 실제 사용자의 어떤 스크롤 제스처든 즉시 자기 교정되므로 실전에서 고착 상태에 머무를 일은 드물다. 트리거 방식 교체라는 이번 범위를 넘는 스크롤 위치 관리가 필요해 고치지 않았다.
...
기준: main `7179794` 위 worktree `/home/roqkf/pr-search-wt/upgrade-blockers`(브랜치 `fix/cr122-cr123-upgrade-blockers`). correction — 요구사항 문장은 바꾸지 않고 구현을 FR-REL-003 AC-3에 맞추며, 운영 절차의 순서를 고친다. ID는 `docs/`와 열린 PR(없음)을 실측해 정했다 — CR-122·CR-123·DEV-774~DEV-776. 새 ADR·JOB·EVT·RB 번호는 쓰지 않는다.

- [x] 요구사항: 해당 없음 — FR-REL-003 AC-3이 이미 「대상이 아직 색인되지 않았으면 미해결」을 정한다. PRD·용어집·매트릭스도 바뀌지 않는다.
- [x] 파생 UI: 해당 없음 — W-002는 간선의 `resolved`를 그대로 읽는다.
...
기준: CR-123과 같은 브랜치. correction — CLI 배선 하나와 시험.
...
기준: CR-119·CR-120 병합 기록 뒤의 main `ee72de7` 위 worktree `/home/roqkf/pr-search-wt/links-reindex`(브랜치 `feature/cr121-links-reindex`). correction — 기존 FR 문장과 AC는 바꾸지 않고 AC를 둘 더하며, 사용자 결정 하나를 OD로 남긴다. ID는 `docs/`와 원격 브랜치, 열린 PR을 실측해 정했다 — CR-121·WP-104·DEV-766~DEV-773·OD-017·ENT-REL-003, 마이그레이션 037. 새 ADR·JOB·EVT·RB 번호는 쓰지 않는다: 스택의 정본을 PostgreSQL로 옮기는 것은 ADR-004를 지키는 일이고, 잡은 JOB-REL-004·JOB-REL-006·JOB-ING-006을 보완하며, 운영 절차는 RUNBOOK 7.I다.

- [x] 요구사항: SRS v2.45 → v2.46 — `FR-ING-008` AC-11, `FR-REL-006` AC-6 신설, `FR-REL-006`의 관련 데이터에 ENT-REL-003, 14장 `OD-017`(resolved)과 확인 문장, v2.46 주석. PRD v1.23 → v1.24(제품 계약 문단). 용어집은 바꾸지 않는다(새 용어가 없다 — 「해제」는 FR-REL-006 AC-3의 말 그대로다). 매트릭스 v1.19 → v1.20(CR-121 표 2행).
- [x] 파생 UI: 해당 없음 — 화면이 바뀌지 않는다(A-003은 잡 사유를 그대로 보이고, W-002의 스택 표시는 간선의 `detached`를 그대로 읽는다).
- [x] 기술 아키텍처: 데이터 모델 v0.32 → v0.33(머리 주석, 엔티티 카탈로그 ENT-REL-003, 3.1장 `pull_request_stack`·`reindex_link_pending`, 4.3장 스택 간선의 정본), 비동기 v0.19 → v0.20(머리 주석, 3.4장 스택의 정본·역방향 재평가, 3.5장 이중 쓰기 대상 15개·재구축 정본 표·「간선 재구축의 미처리와 기대 간선」 절·전환 전 검증 세 항목·실패 처분 두 행·보관의 대기열 정리), API 계약 v0.45 → v0.46(머리 주석 — 경로·응답 변경 없음, `prsctl links import-stacks`), 관측성 v0.12 → v0.13(재색인 로그 세 줄, `link_relations_total`이 해제 간선의 재쓰기를 세지 않음), 백엔드 아키텍처 v0.18 → v0.19(머리 주석 — 스택 정본의 소유 모듈, 미처리 경로, 재구축 포트, ADR-008 우회 조회 허용 목록).
- [x] 전달: 작업 패키지 v2.66 → v2.67(WP-104 절·상태 표), 원장 v6.111 → v6.112(머리 절, 3장 WP-104, 4장 FR-ING-008 AC-11·FR-REL-006 AC-6, 5장 DEV-766~DEV-773, 6.112장). `deploy/single-host/RUNBOOK.md` 7.I(스택 가져오기와 prs-links 재색인 확인)와 8장 세 행, `deploy/single-host/prsctl` 사용법 주석의 `links import-stacks`. `agent-context/upstream-feedback.md` 셋째 항목에 상류 반영 주석.
- [x] 코드·시험: 원장 6.112장.
- 문서 검증기: 원장 6.112장.
- 독립 리뷰: 원장 6.112장 「독립 리뷰」.
- 상태: **closed**(2026-09-25).

**병합 판정.** PR #237(base `main`, head `6235bf7`)의 CI(run 36085206961)는 verify·integration 모두 success다. 최종 트리(`4484e0f`)의 전 계층 게이트, 변이 32종(30종 사멸, 둘은 등가·도달 불가), 코드 독립 리뷰 두 번(상·중 지적 0건)과 문서 독립 리뷰([중] 1건·[정보] 3건 — 모두 `6235bf7`에서 반영)는 원장 6.112장에 있다. 사용자 승인(2026-09-25 「스쿼시 병합해」 — 이 세션에서 PR #237에 대해 다시 받았다)으로 squash 병합했다 — main `df3d8ef`(2026-09-25), 트리는 PR head와 같다. 병합 커밋의 main CI(run 36100117388)는 verify·integration 모두 success다. 스택 가져오기와 사내 적용은 NOT RUN이다(RUNBOOK 7.I).
...
기준: main `3d831a9` 위의 worktree `/home/roqkf/pr-search-wt/p0-flow003`(브랜치 `fix/flow003-back-hydration`). correction — 시험 파일 하나와 기록만 바꾼다. ID는 `docs/`와 원격 브랜치·열린 PR(0건)을 실측해 정했다 — CR-118·DEV-758.

- [x] 요구사항·파생 UI·기술 아키텍처·운영 문서: 해당 없음 — 제품 동작과 계약이 바뀌지 않는다.
- [x] 전달: 원장 v6.106 → v6.107(머리 절, 5장 DEV-758과 DEV-377·DEV-424·DEV-689·DEV-708의 재진단 표기, 6.109장).
- [x] 코드·시험: `apps/web/e2e/flow-003.spec.ts` — 원장 6.109장.
- 상태: **closed**(2026-09-25).
...
### CR-117 cascade — `git merge dev`로 받아 온 dev 체인 커밋의 PR 연결: 원본 커밋은 「그 PR이 새로 가져온 커밋」이다

기준: main `ea59bb6` 위의 worktree `/home/roqkf/pr-search-wt/cr117-source-commits`(브랜치 `feature/cr117-source-commits`). 착수는 `5c52aa2`에서 했고, 작업 중 main에 올라온 사용자의 `agent-context/upstream-feedback.md` 정리 커밋 `ea59bb6`(그 파일 하나만 바꾸고 코드 변경은 없다) 위로 rebase했다. 그 파일은 main 쪽을 그대로 채택하고 첫 항목의 상류 반영 주석만 다시 얹었으며, 코드 diff(`apps`·`packages`·`regression`·`handoff`·`deploy`)는 rebase 전후 sha256이 같다. correction — 기존 FR 문장과 AC는 바꾸지 않고 AC를 둘 더한다. ID는 `docs/`와 로컬·원격 브랜치, 열린 PR(0건)을 실측해 정했다 — CR-117·WP-102·DEV-754·DEV-755·OD-016, PSI 계약 차이 D-22. 새 ADR·ENT·JOB·RB 번호는 쓰지 않는다: 스키마 변경이 없고, ADR-004 Amendment(관계 정본은 PostgreSQL) 안에서 읽는 규칙을 정한 것이며, JOB-REL-008과 RB-29를 보완한다.

- [x] 요구사항: SRS v2.43 → v2.44 — `FR-SRCH-002` AC-7 신설(예외 칸에 first-parent 한계, 추적하지 않는 base, 늦게 채워지는 PR 대응), `FR-SRCH-003` AC-5 신설, `OD-016` 신설과 같은 날 결정(resolved — 기각한 「PR의 base 브랜치 기준」과 대가를 함께 적었다), v2.44 주석. PRD v1.21 → v1.22(제품 계약 문단), 용어집 v0.18 → v0.19(「원본 커밋」을 한 뜻으로 고치고 유효 연결·체인 소속 어휘와 금지 동의어를 더했으며, 「관계 정본」 행에 원시 관측을 적었다), 매트릭스 v1.17 → v1.18(CR-117 표 3행).
- [x] 파생 UI: UI 컴포넌트 명세 v0.27 → v0.28(C-018 `excludedCount` prop과 `excluded_commits` 상태, 작업 공간 상세 창과 같은 문구), 화면 상태 매트릭스 v0.21 → v0.22(W-002 `excluded_commits`), 화면 QA 체크리스트 v0.27 → v0.28(QA-W002-30), AI 에이전트 실행 지시서 v0.16 → v0.17(관계를 읽는 질의는 술어 하나, 체인 정의 하나, 재투영 의도, `chainShas`, 역할 직접 쓰기는 복구 하나). IA·와이어프레임·화면 흐름은 바꾸지 않았다 — 새 화면·새 흐름이 없다.
- [x] 기술 아키텍처: API 계약 v0.43 → v0.44(머리 주석, API-SRCH-003의 `source_commits_excluded`와 뺀 뒤의 `source_commits_total`), 데이터 모델 v0.31 → v0.32(스키마 변경 없음 — 원시 관측과 유효 연결), 비동기 v0.17 → v0.18(JOB-REL-008의 읽기 규칙, 채번·강제 푸시 재채번·복구 재채번의 재투영 의도, 재색인 replay가 비추는 값), 백엔드 v0.17 → v0.18(유효 연결 술어와 체인 소속 조회의 자리), 관측성 v0.10 → v0.11(RB-29 — 계획의 두 줄, 체인 규칙 몫에는 `refetch`가 필요 없음, `apply`의 역할 되돌리기 예외, 바뀌는 대상). API 계약과 백엔드는 본문을 먼저 고치고 머리글을 빠뜨렸던 것을 같은 CR 안에서 올렸다. ADR은 바꾸지 않는다.
- [x] 계약 handoff: `handoff/pipe-search-integration/v1/` — OpenAPI(`PullRequestDetailResponse`의 필수 키 `source_commits_excluded`, `source_commits`·`source_commits_total`의 뜻과 근거 줄), captured 예시 `examples/read.pull_request.200.json`, `handoff/pipe-search-integration/v1/CONTRACT_DIFF.md` D-22, manifest 재생성(계약 checksum `c9a800d8…` → `967857d6…`). `handoff/pipe-search-port`(2026-09-21 고정 스냅숏)는 CR-114 때처럼 건드리지 않는다.
- [x] 전달: 작업 패키지 v2.63 → v2.64(WP-102 절·상태 표), 원장 v6.105 → v6.106(머리 절, 3장 WP-102, 4장 FR 매핑 2행, 5장 DEV-754·DEV-755와 DEV-752 일부 해소, 6.108장). `deploy/single-host/RUNBOOK.md` 7.G의 CR-117 절(계획의 두 줄, `refetch` 불필요, 역할 되돌리기, 배포 주의, 반입 뒤 순서)과 8장 문제 해결 첫 행. `agent-context/upstream-feedback.md` 첫 항목에 상류 반영 주석.
- [x] 코드·시험: 원장 6.108장.
- 문서 검증기(`validate_srs_prd_env.py`): 원장 6.108장에 기준선 대비 결과를 적는다.
- 독립 리뷰: 원장 6.108장 「독립 리뷰」.
- 상태: **closed**(2026-09-24).

**병합 판정.** PR #230(base `main`, head `d70898f`)의 CI(run 35983177086)는 verify·integration 모두 success다. 코드·문서 전량을 담은 head `ecd839c`의 CI(run 35981825964)도 같고, 그 뒤 두 커밋은 문서와 인계 묶음만 바꿨다. 중간 head `da473c7`의 run 35982831584은 다음 push로 `cancel-in-progress` 취소됐다 — 취소된 실행을 근거로 쓰지 않는다. 구현 지시에는 병합 승인이 없었으므로 먼저 물었고, 사용자 지시(2026-09-24 「origin/main에 병합해라」)로 squash 병합했다. 병합 직전 `origin/main`이 기준 `ea59bb6` 그대로이고 `mergeStateStatus: CLEAN`임을 확인했다. 병합 커밋 `65acf83`의 부모는 `ea59bb6`이고 트리가 `d70898f`와 같으며, main CI(run 35993248300)도 verify·integration 모두 success다(취소되지 않았다). 병합 기록은 별도 PR(브랜치 `docs/cr117-merge-record`)로 했다. 문서 검증기는 이 기록 뒤에도 기준선과 오류·경고 목록이 같다.

### CR-116 cascade — 커밋에 남는 과거 PR 번호: 관계 정본과 전용 투영기

기준: main `83d30fb`(CR-115 PR #226 병합분) 위의 worktree `/home/roqkf/pr-search-wt/cr116-pr-links`(브랜치 `feature/cr116-pr-links`). correction/reliability — 기존 FR 문장과 AC-1~AC-5는 바꾸지 않고 AC를 셋 더한다. ID는 `docs/`와 로컬·원격 브랜치 전부, 그리고 열린 PR #227을 실측해 정했다 — CR-116·WP-101·DEV-744~753·마이그레이션 036·ENT-CORE-009~011·JOB-REL-008·RB-29·ADR-004 Amendment(새 ADR 번호를 쓰지 않는다 — CR-113과 같은 결함 계열이다).

- [x] 요구사항: SRS v2.42 → v2.43 — `FR-SRCH-002` AC-6 신설, `FR-ING-004` AC-6 신설, `FR-ING-008` AC-9 신설, v2.43 주석. PRD·용어집·매트릭스는 관계 정본 어휘와 CR-116 표만 더한다.
- [x] 기술 아키텍처: 데이터 모델 v0.30 → v0.31(ENT-CORE-009~011, 036 DDL, 5장 합집합 문단 대체와 소유권 문단, 커밋 문서 매핑 두 필드), 비동기 v0.16 → v0.17(JOB-REL-008, 3.5장 「PR 연결은 replay로 복원한다」와 전환 전 검증 항목), API 계약(`/history`의 `pull_request_numbers` 의미 보완), 관측성 v0.9 → v0.10(경보 1행, RB-29), API 계약 v0.42 → v0.43(API-SRC-002 의미 보완), ADR **v0.19 → v0.20**(ADR-004 Amendment), 백엔드 **v0.16 → v0.17**(조인 경로의 정본 문장). 뒤 둘의 시작 번호가 CR-115 cascade가 적은 것과 다른 이유: CR-115가 「ADR v0.19 → v0.20」·「백엔드 v0.16 → v0.17」을 적어 두고 **머리글에는 반영하지 않았다.** 없던 상향을 있었던 것으로 이어 쓰지 않고 실제 파일 값에서 한 단계 올린다.
- [x] 전달: 작업 패키지 v2.62 → v2.63(WP-101 절·상태 표, WP-008의 합집합 문장 승계 주석), 원장 v6.103 → v6.104(머리 절, 3장 WP-101, 4장 FR 매핑, 5장 DEV-744~753과 DEV-019·DEV-726 승계, 6.107장). `deploy/single-host/RUNBOOK.md` 7.G(전체 대조·복구 절차, 구버전 워커 배제, 롤백 위험). `agent-context/upstream-feedback.md` 넷째 항목에 상류 반영 주석.
- [x] 코드·시험: 원장 6.107장.
- 문서 검증기(`validate_srs_prd_env.py`): 원장 6.107장에 기준선 대비 결과를 적는다.
- 독립 리뷰: 원장 6.107장 「독립 리뷰」.
- 상태: **closed**(2026-09-23).

**병합 판정.** PR #228(base `main`, head `b20d861`) CI(run 35760961156)는 verify·integration 모두 success다. 그 직전 `49c5b49`(코드·문서 전량)의 CI(run 35759802933)도 같고, 두 커밋의 차이는 CI 기록 세 줄뿐이라 코드 트리가 같다. 첫 push `96a3bc0`의 run 35759017065은 force-push로 `cancel-in-progress` 취소됐다 — 취소된 실행을 근거로 쓰지 않는다. 사용자 지시서(2026-09-23)가 「필수 리뷰·CI를 통과한 정상 main 병합」을 명시 승인했고 `main`에는 브랜치 보호 규칙이 없어(404) CI만이 게이트이므로, `mergeStateStatus: CLEAN`을 확인하고 squash 병합했다 — 병합 커밋 `f9cda82`, main CI(run 35762117662)도 verify·integration 모두 success다. 문서 검증기는 병합 전 기준선(`main 83d30fb`)과 오류 3·경고 12로 목록까지 동일했다. 병합 기록은 CR-114·CR-115와 한 PR(브랜치 `docs/cr114-cr115-merge-record`)로 묶었다.

### CR-115 cascade — 확정된 M 번호를 원격 저장소의 lightweight 태그로 굳힌다

기준: CR-114 브랜치 `feature/cr114-mnumber-search`의 `7f980be`(main `13074f8` + CR-114 두 커밋) 위에 stacked, worktree `/home/roqkf/pr-search-wt/cr115-mnumber-tags`(브랜치 `feature/cr115-mnumber-tags`). scope — 기존 FR 문장·AC는 바꾸지 않고 `FR-SEQ-012`를 신설한다. ID는 `docs/`와 로컬·원격 브랜치 전부를 실측해 정했다 — CR-115·WP-100·FR-SEQ-012(010·011은 미병합 브랜치 `docs/regression-first-slice`가 선점)·JOB-SEQ-007·ADR-026(024는 같은 브랜치가 선점)·THR-061·RB-28·OD-015·마이그레이션 035·DEV-740~743(739는 CR-114 독립 검토가 썼다).

- [x] 요구사항: SRS v2.41 → v2.42 — `FR-SEQ-012` 신설(AC-1~AC-11, 예외/실패 처리), 2장 「셋뿐이다」·3.2절 「예외 둘」, `FR-AUTH-004` AC-1 감사 표 `merge_number.tag` 행(행위자가 사람이 아닌 두 번째 행), `OD-015`(open, 권고안 포함), v2.42 주석. PRD v1.19 → v1.20 — 제품 계약 문단, 비목표·범위·범위 밖의 「유일한」 정정, FR 표 행. 용어집 v0.16 → v0.17 — M 번호 lightweight 태그·태그 대조 어휘와 금지 동의어. 매트릭스 v1.15 → v1.16 — CR-115 표.
- [x] 파생 UI: UI 컴포넌트 명세 v0.26 → v0.27(C-045 JobRunForm의 `mnumber_tag_reconcile` 규칙), AI 에이전트 실행 지시서 v0.15 → v0.16(쓰기 반경·자격·정본 재확인·감사 규율). IA·와이어프레임·화면 흐름·상태 매트릭스는 바뀌지 않았다 — 새 화면이 없다.
- [x] 기술 아키텍처: 백엔드 v0.16 → v0.17(채번 트랜잭션의 `tag` 의도·`tag` 역할), API 계약 v0.41 → v0.42(API-ADM-002 `mnumber_tag_reconcile` 행·규칙, API-ADM-001 `tag_enabled`), 데이터 모델 v0.29 → v0.30(ENT-CORE-001·ENT-SEQ-001·ENT-SEQ-006, 035 DDL, 커밋 문서 M 필드 소유권), 비동기 v0.15 → v0.16(JOB-SEQ-007 행, 스케줄, 지표 — 빠져 있던 표기 지표 두 행도 메움 DEV-743), 보안 v1.14 → v1.15(13.5절, 6장 시크릿 행, THR-061, 원칙 6), 인프라 v0.23 → v0.24(배포 단위 표 `pipeline-worker:tag`, `prsctl mnumber tags`), 관측성 v0.8 → v0.9(경보 1행, RB-28, 커버리지 21종), ADR v0.19 → v0.20(ADR-026, ADR-022 Amendment).
- [x] 전달: 작업 패키지 v2.61 → v2.62(WP-100 절·상태 표), 원장 v6.102 → v6.103(머리 절, 3장 WP-100, 4장 FR-SEQ-012, 5장 DEV-740~743, 6.106장). `deploy/single-host/RUNBOOK.md` 7.F(App 등록·ruleset·`.env`·upgrade·한 저장소부터·확인·상태 사전·충돌 시 절차·권한 차단·에폭 상향·끄기). `deploy/k8s/pipeline-worker-annotate.yaml`(`annotate,tag`, `prs-tag-secrets`)·`tag-secret.example.yaml`. `agent-context/upstream-feedback.md` 셋째 항목에 상류 반영 주석.
- [x] 코드·시험: 원장 6.106장.
- 문서 검증기(`validate_srs_prd_env.py`): 원장 6.106장에 기준선 대비 결과를 적는다.
- [x] 독립 리뷰(`deep-reasoner` 둘 — 첫 검토자는 사용량 한도로 끊겼고 남긴 질문 둘은 코드·시험으로 닫았다, 둘째는 보고했다): 수정 권고 3건(중저 2·하중 1 — AC-4 쓰기 직전 재확인 범위, AC-6 회차 내 관측의 감사, AC-10 잘린 목록의 재개) — 모두 같은 CR에서 수정(`b4c8799`), 시험 +4, 변이 5종. 원장 6.106장 「독립 리뷰」.
- 상태: **closed**(2026-09-22).

**병합 판정.** PR #226(base `main`, head `4aa1268`) CI(run 35737470156)는 verify·integration 모두 success다. 사용자 지시(2026-09-22 「병합했다」)로 squash 병합했다 — 병합 커밋 `83d30fb`(23:13 KST), main CI(run 35738760726)도 verify·integration 모두 success다(취소되지 않았다). 상세 검증·독립 리뷰 재확인은 원장 6.106장 「병합」. 병합 기록은 CR-114와 한 PR(브랜치 `docs/cr114-cr115-merge-record`)로 묶었다.
...
**병합 판정.** PR #225(head `7793caa`) CI(run 35719530750)는 verify·integration 모두 success다. 사용자 지시(2026-09-22 「#225를 먼저 병합했어」)로 squash 병합했다 — 병합 커밋 `9782ba9`(22:59 KST), main CI(run 35737130985)도 success다. 병합 뒤 CR-115의 세 커밋을 `rebase --onto origin/main`으로 다시 올려 PR #226의 base를 `main`으로 옮겼다(코드 트리 동일을 `git diff --stat`으로 실측). 상세 검증·독립 리뷰 재검토는 원장 6.105장 「병합」. 병합 기록은 CR-115와 한 PR(브랜치 `docs/cr114-cr115-merge-record`)로 묶었다.

### CR-113 cascade — 머지 시퀀스의 Elasticsearch 투영 수렴

기준 main `2c3004f`(착수 시 `origin/main`과 같음), worktree `/home/roqkf/pr-search-wt/cr113-seq-projection`(브랜치 `feature/cr113-sequence-projection`). correction/reliability — 기존 AC 문장은 바꾸지 않고 AC를 더한다. ID는 `docs/`와 로컬·원격 브랜치 전부(`git grep` 45 ref)를 실측해 정했다 — CR-113·WP-098·DEV-733~737·마이그레이션 034·JOB-SEQ-006. 새 ADR·OD·감사 액션은 만들지 않았다(ADR-004 Amendment, 감사는 `job.run` 재사용).

- [x] 요구사항: SRS v2.39 → v2.40 — `FR-SEQ-001` AC-7·AC-8, `FR-ING-008` AC-8, `FR-ADMIN-003` AC-6, v2.40 주석. PRD v1.17 → v1.18 — 제품 계약 문단. 용어집 v0.14 → v0.15 — 시퀀스 투영·재투영·durable 투영 작업·문서별 판정 어휘와 금지 동의어. 매트릭스 v1.13 → v1.14 — CR-113 표(4행).
- [x] 파생 UI: UI 컴포넌트 명세 v0.25 → v0.26(C-045 JobRunForm의 `sequence_reproject` 규칙), AI 에이전트 실행 지시서 v0.13 → v0.14(한 해석·한 쓰기, 격리 가드). IA·와이어프레임·화면 흐름·상태 매트릭스는 바뀌지 않았다 — 정렬 UI는 그대로다.
- [x] 기술 아키텍처: 백엔드 v0.14 → v0.15(4.3 의사코드·주의점), API 계약 v0.39 → v0.40(API-ADM-002 `sequence_reproject` 행·규칙, API-ADM-007 `consistent` 색인 보고), 데이터 모델 v0.28 → v0.29(ENT-SEQ-006 `project`·`progress`, `job_type_chk`, 시퀀스 필드 소유권, 6장 정렬·`seq:` 단락), 비동기 v0.14 → v0.15(JOB-SEQ-006 행, 3.5장 이중 쓰기 표 행 교체·replay 절·전환 전 검증 8항목, 10장 지표 2행), 인프라 v0.22 → v0.23(`prsctl sequence`), 관측성 v0.7 → v0.8(경보 1행, RB-27, 커버리지 20종), ADR v0.18 → v0.19(ADR-004 Amendment — CR-113).
- [x] 전달: 작업 패키지 v2.58 → v2.59(WP-098 절·상태 표), 원장 v6.99 → v6.100(머리 절, 3장 WP-098, 4장 FR-SEQ-001 AC-7·AC-8/FR-ING-008/FR-ADMIN-003 AC-6, 5장 DEV-733~737, 6.104장). `deploy/single-host/RUNBOOK.md` 7.E(배포 SHA·endpoint·별칭 확인, dry-run, 제한 범위 재투영, 원시 필드 대조, 새 검색 요청 검증, 예외 목록). `agent-context/upstream-feedback.md`에 상류 반영 주석.
- [x] 코드·시험: 원장 6.104장.
- 문서 검증기(`validate_srs_prd_env.py`): 기준선 `2c3004f`은 `--report` 오류 3·경고 12, `--strict` 오류 6·경고 12이며 이 CR 뒤에도 **두 모드 모두 오류·경고 목록이 완전히 같다** — 새 issue 0.
- 상태: **closed**(2026-09-22). 내부망 적용은 NOT RUN, 사내 배포 SHA는 NOT VERIFIED.

**병합 판정.** 독립 리뷰(`deep-reasoner` 둘 — 첫 검토자는 세션 한도로 끊겨 새로 띄웠고 둘 다 보고했다)가 정본 불변·누락 은폐 방지·브랜치/에폭 가드·락 배치를 견고하다고 판정했고, 지적 셋(높음: 전환 전 검증의 대표 범위 정렬이 `other_space` 커밋을 기대치에 넣어 다중 시퀀스 브랜치 저장소의 `prs-commits` 재색인이 전환되지 않음 / 중간: replay 판정의 shadow 결과 폴백 / 낮음: 대기 조회 비용·보고 비대칭·기록 크기·격리 가드)을 수정해 변이 2종(10종 누적)과 시험 2건(2-브랜치 재색인, shadow 실패 판정)을 더했다(원장 6.104). 최종 head `36f0444`에서 로컬 전량(typecheck·lint·lint:deps·단위 3,205·통합 132 파일/2,093·회귀 512·build) 통과, PR CI(run 35677604997) verify·integration success. 사용자 지시(2026-09-22)로 PR #223을 squash 병합했다 — 병합 커밋 `a6ea00d`(11:07 KST), 트리가 `36f0444`와 같다. 병합 커밋의 main CI: run 35678283958 — verify·integration 모두 success(취소되지 않았다; 병합 뒤 main에 다른 push가 없었다). 병합 기록은 별도 브랜치 `docs/cr113-merge-record`로 했다(작업 패키지 v2.60·원장 v6.101·이 대장). 문서 검증기는 이 기록 뒤에도 두 모드 모두 오류·경고 목록이 기준선과 같다. 릴리스·태그는 발행하지 않았다. closed.
