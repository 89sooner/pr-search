...
- 유형: correction(요구사항 공백 — 용어집의 「원본 커밋」 정의가 두 문장으로 갈려 있었고, 구현은 GitHub의 PR 커밋 목록 전체를 원본 커밋으로 읽었다). `FR-SRCH-002` AC-7 신설, `FR-SRCH-003` AC-5 신설, 용어집 「원본 커밋」 정의 보완, `OD-016` 신설과 동시 결정(resolved). **기존 FR의 요구사항 문장과 AC 문장은 바꾸지 않는다. 안정 ID 재번호화 0건.** 상태: **open** — 구현 중. 기준 main `ea59bb6`(착수는 `5c52aa2`에서 했고, 그 뒤 main에 올라온 사용자의 `agent-context/upstream-feedback.md` 정리 커밋 위로 rebase했다 — 코드 diff는 rebase 전후 같다), worktree `/home/roqkf/pr-search-wt/cr117-source-commits`(브랜치 `feature/cr117-source-commits`).
...
- 제외: 같은 반입에서 함께 보고된 prs-commits 재색인 verify 과대 계산과 prs-links 재색인 shadow 부분 갱신 실패 — 별도 CR로 연다. 운영 서버 변경, 사내 배포·적용, Release 발행. `role` 소유권의 나머지(원본 목록에서 빠진 체인 밖 커밋의 `source_commit`, DEV-752 잔여).
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| CR-117 | 2026-09-24 | correction | 사용자 지시 2026-09-24 — 사내 pilot.18 운영 보고(`git merge dev`로 받아 온 dev 체인 커밋에 그 PR 번호가 붙음, `ebc781d` = [983, 1671, 1855]) | **원본 커밋을 「그 PR이 새로 가져온 커밋」으로 정의하고, 추적 브랜치의 현재 체인에 이미 오른 커밋은 그 커밋을 올린 PR에만 속하게 한다.** 원시 관측과 CR-116의 삭제 규율은 그대로 두고 연결을 읽는 모든 자리가 같은 술어를 쓴다. 체인이 바뀌면 관계를 다시 투영하고, PR 투영은 체인 커밋의 역할을 덮지 않으며, PR 상세도 같은 정의로 원본 커밋을 낸다 | FR-SRCH-002 AC-7 · FR-SRCH-003 AC-5 · OD-016 · JOB-REL-008 · RB-29 · WP-102 · DEV-754 · DEV-755 · PSI D-22 | SRS · PRD · 용어집 · 매트릭스 · 화면 명세 · 실행 지시서 · 백엔드 · API 계약 · 데이터 모델 · 비동기 · 관측성 · PSI handoff · WP · 원장 · RUNBOOK | open — 구현 중 |
