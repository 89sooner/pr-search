...
**배포는 프로파일 둘이다** (ADR-021 / CR-059). **Profile A**는 단일 호스트 Docker Compose이며 **첫 사내 반입의 현재 기본**이고, **Profile B**는 다중 호스트 Kubernetes로 다중 서버·고가용이 필요해질 때 쓴다. 첫 사내 반입은 **오프라인 번들 하나**로 이루어지며 사내에서 `git clone`·`pnpm install`·레지스트리 접근을 요구하지 않는다. 반입된 형상은 **외부로 되돌아오지 않는다** — 외부 저장소가 upstream 릴리스 생산자이고 내부 저장소가 영구 다운스트림 제품 라인이며, **내부 수정을 외부에 병합하는 절차는 설계하지 않는다.**
