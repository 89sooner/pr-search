 ...
 *
 * 관용하는 공백은 **`\r`·`\n`을 뺀 모든 공백**이다. 칸 문자와 탭만 보면 사용자가
 * 붙여 넣은 non-breaking space 뒤의 접두를 못 알아보고 하나 더 붙인다. `\r`·`\n`을
 * 빼는 것은 `[` 뒤 아무 데나 있는 `]`까지 삼켜 본문 일부를 접두로 오인하지 않기
 * 위해서다. **U+2028·U+2029 같은 유니코드 줄 구분자는 여기서 공백으로 취급된다** —
 * 관용은 탐지에만 쓰이고 넓을수록 이중 접두를 더 잘 막으므로 판정을 틀리게 하지
 * 않는다.
 */
