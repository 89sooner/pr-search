/**
 * 시간 예산과 변경 요청 간격 (WP-075 안전성 보강 → CR-115에서 `@prs/github`로 옮김).
 *
 * ## 왜 순수 모듈로 떼는가
 *
 ...
 *
 * ## 왜 `@prs/github`에 있는가 (CR-115)
 *
 * 표기(`@prs/github-annotate`)와 태그(`@prs/github-tag`)가 같은 간격 규율을 쓴다 — 공식
 * 문서의 「변경 요청 사이 최소 1초, 직렬」은 App이 아니라 GHE의 계약이다. 두 쓰기
 * 패키지가 서로를 의존하면 자격 경계가 흐려지므로, 신원을 모르는 이 유틸리티를 아래
 * 계층에 둔다. **자격·토큰·경로는 여기 없다.**
 */
