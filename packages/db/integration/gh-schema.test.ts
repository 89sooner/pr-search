...
describe('028 왕복', () => {
  it('027 → 028 → 027 → 028 — 내려가면 세 표가 사라지고 올라오면 돌아온다', async () => {
    ...
    const reverted = await migrateDown(pool, 10);
    expect(reverted).toEqual(['037', '036', '035', '034', '033', '032', '031', '030', '029', '028']);
    ...
    expect(applied).toEqual(['028', '029', '030', '031', '032', '033', '034', '035', '036', '037']);
