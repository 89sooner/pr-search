...
describe('029 왕복', () => {
  it('028 → 029 → 028 → 029 — 내려가면 두 표와 함수가 사라지고 028의 표는 남는다', async () => {
    ...
    const reverted = await migrateDown(pool, 9);
    expect(reverted).toEqual(['037', '036', '035', '034', '033', '032', '031', '030', '029']);
    ...
    expect(await migrateUp(pool)).toEqual(['029', '030', '031', '032', '033', '034', '035', '036', '037']);
