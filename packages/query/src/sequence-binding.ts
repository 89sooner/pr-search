 ...
 * ## 범위 조건만 대상이다
 *
 * `seq:1234`(스칼라)는 여기서 보지 않는다. SRS AC-2가 승인한 것은 범위뿐이고
 * 스칼라는 질의 빌더에서 `MATCH_NONE`이 되어 언제나 0건이다 — 이 규칙을
 * 적용하면 그 동작이 400으로 바뀌는데, 그것은 이 CR이 승인받은 범위 밖의 제품
 * 결정이다 (DEV-364, 원장 5장에 판정을 남겼다).
 */
