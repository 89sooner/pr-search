...
async function mock(page) {
  await page.route('**/api/**', async route => {
    ...
    else if (path === '/api/search') body = { items: [{ kind: 'pull_request', repository: repo, pr_number: 7, title: 'Improve retry policy', author: 'minji', state: 'merged', merge_seq: 2901, seq_epoch: 3, sequence_space: `${repo}@main`, merge_number: 'M-API-77', merge_number_state: 'assigned', merge_number_epoch: 3, merged_at: '2026-09-16T09:30:00Z', changed_files_count: 2, additions: 1, deletions: 1, url: `/pr/${repo}/7` }], total: { value: 1, relation: 'eq' }, facets: { label: [{ value: 'bug', count: 1 }] }, facets_omitted: false, facets_status: 'ready' };
...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.getByRole('button', { name: 'Merged after' }).textContent(), 'YYYY-MM-DD');
    assert.equal(await page.getByRole('button', { name: 'Merged before' }).textContent(), 'YYYY-MM-DD');
    assert.ok(calls.some(url => url.pathname === '/api/search' && url.searchParams.get('sort') === 'pr_number' && url.searchParams.get('order') === 'desc' && url.searchParams.get('facets') === 'true'));
    await page.getByRole('button', { name: 'Merged after' }).click();
    await page.screenshot({ path: `${output}/calendar-light.png`, fullPage: true, animations: 'disabled' });
    await page.getByRole('button', { name: 'Today', exact: true }).click();
    assert.match(await page.getByRole('button', { name: 'Merged after' }).textContent(), /^\d{4}-\d{2}-\d{2}$/);
    await page.getByRole('button', { name: 'Merged before' }).click(); await page.getByRole('button', { name: 'Today', exact: true }).click();
    assert.equal(await page.getByRole('columnheader', { name: '#' }).count(), 1);
    assert.equal(await page.getByRole('columnheader', { name: /M number/ }).count(), 1);
    await page.getByRole('combobox', { name: 'Base branch' }).last().click(); await page.getByRole('option', { name: 'main', exact: true }).click();
    await page.getByRole('combobox', { name: 'Label' }).click(); await page.getByRole('option', { name: 'bug', exact: true }).click();
    await page.getByRole('combobox', { name: 'Status' }).click(); await page.getByRole('option', { name: 'Merged', exact: true }).click();
    const filteredRequest = page.waitForRequest(request => request.url().includes('/api/search') && new URL(request.url()).searchParams.get('q')?.includes('is:merged'));
    await page.locator('.repo-filter-form button[type="submit"]').click(); const filtered = new URL((await filteredRequest).url()).searchParams.get('q');
    assert.ok(filtered?.includes('is:merged') && filtered.includes('base:"main"') && filtered.includes('label:"bug"'), `Observed filtered query: ${filtered}`);
    await page.getByText('M-API-77', { exact: true }).waitFor();
    assert.equal(await page.locator('.repo-title-button').first().textContent(), 'Improve retry policy');
    assert.equal(await page.getByRole('link', { name: '#7 in GitHub Enterprise' }).getAttribute('href'), `https://ghe.example.com/${repo}/pull/7`);
    await page.screenshot({ path: `${output}/search-pr-table-light.png`, fullPage: true, animations: 'disabled' });
