...
if [ "$RELEASE" -eq 1 ]; then
  ...
  undo_note() {
    if [ "$UNDONE" -eq 1 ]; then
      ...
      # 삭제 실패 뒤 A→B→A로 돌아온 태그는 같은 SHA여도 다른 주체의 것일 수 있다 (DEV-559).
      printf '릴리스는 지웠으나 태그 %s를 지우지 못했다 (옮겨졌거나 삭제가 거부됐다) — 커밋 일치는 소유를 증명하지 않는다. 독립적으로 소유가 확인되기 전에는 태그를 보존하고 새 버전으로 실행한다' "$VERSION"
    else
