...
| `… 릴리스는 지웠으나 태그 <version>를 지우지 못했다 (옮겨졌거나 삭제가 거부됐다)` | 이 실행이 만든 태그를 지우지 못했다. **옮겨졌을 수도 있다** | **대상을 먼저 본다** — 아래 「남은 태그를 지우기 전에」 |
...
**기대값 없는 삭제를 쓰지 마라.** 스크립트가 "지우지 못했다"고 말하는 경우는 둘을 함께 담는다 — 이 실행이 만든 태그가 **그대로 남은 것**과, 그 사이 **다른 커밋으로 옮겨진 것**이다. 후자를 기대값 없이 지우면 다른 발행자가 옮긴 태그를 지우게 된다(`DEV-541`이 코드에서 막은 것을 손으로 하는 셈이다).
...
| 대상이 | 무엇을 하는가 |
| --- | --- |
| **이 실행의 커밋**(스크립트 메시지가 그 값을 함께 적는다) | 기대값을 건 lease로 지우고 **같은 버전으로** 다시 실행한다 |
| **다른 커밋** | **지우지 않는다.** 이 실행의 것이 아니다. 누가 발행 중인지 확인한 뒤 **새 버전으로** 실행한다 |

```bash
# 대상이 이 실행의 커밋일 때만 — 그 사이 옮겨졌으면 lease가 거부한다
git push --force-with-lease="refs/tags/<version>:<이 실행의 커밋>" origin ":refs/tags/<version>"
```
