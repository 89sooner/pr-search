...
- 반입 뒤 순서: `./prsctl upgrade` → 저장소마다 `./prsctl links plan --repository <owner/name>`으로 두
  ...
  **이 `apply`는 반입마다 저장소별로 한 번 꼭 돌린다.** 배포만으로는 이미 색인에 있는 번호가 바뀌지
  않는다. PR 상세의 제외 판정도 커밋 문서의 연결을 재료로 쓰므로, `apply` 뒤 러너가 수렴하기 전에는
  커밋 화면과 PR 화면이 둘 다 옛 값을 보인다.
- **추적 브랜치 목록을 바꾼 뒤에도 `apply`를 한 번 돌린다**(DEV-756). 브랜치를 빼거나 예전에 추적하던
  브랜치를 다시 넣으면 체인 소속이 바뀌지만, 등록·설정 변경 경로는 새 브랜치의 채번만 요청하고 관계를
  다시 투영하지 않는다. 사내 dev 단일 추적 프로파일에서는 생기지 않는다.
- 역할 판정은 두 갈래다(DEV-757). `apply`는 병합 근거까지 보고 `merge_commit`으로 되돌리지만, 커밋 보강과
  재구축은 체인 행의 PR 대응만 본다. 그래서 대응이 아직 비어 있는 커밋은 나중 보강에서 `direct_push`로
  다시 바뀔 수 있다. 대응이 채워지면 둘 다 `merge_commit`으로 수렴하므로 자동 조치는 두지 않았다.
...
### 7.I prs-links 재색인 — 스택 간선 가져오기와 간선 재색인 확인 (WP-104 / FR-ING-008 AC-11 · FR-REL-006 AC-6, `CR-121`)

`0.1.0-pilot.18`에서 prs-links v1 재색인이 shadow 쓰기의 `document_missing_exception`으로 실패했고, 보고에
따르면 v1이 계속 서비스 중이다(반입 뒤 2번으로 확인한다). 원인은 두 겹이었다 — 재구축 도중 기존 간선의
부분 갱신(참조의 해결 상태·스택의 해제 표식)이 새 인덱스에서 아직 만들어지지 않은 간선을 찾지 못했고
(그래서 잡이 실패했다), 그 뒤에 숨어 **해제된 스택 간선은 색인에만 있어서 재색인이 성공하면 새
인덱스에서 조용히 사라졌다.** `CR-121`부터는 부분 갱신의 문서 없음을 잡의 미처리로 남겨 전환 전에 소유
source를 다시 파생하고, 검증이 기대 간선을 정본에서 계획해 간선마다 대조하며, 스택의 성립·해제를
PostgreSQL(`pull_request_stack`)에 남긴다.

**다시 재색인하기 전에 스택 간선을 옮긴다.** 배포 전부터 서비스 인덱스에만 있는 스택 간선(특히 해제된
것)은 정본에 없다. 옮기지 않은 채 재색인하면 전환 전 검증이 막는다(`스택 정본에 없는 서비스 stacks_on
간선 N건`) — 막지 않으면 그 이력이 사라지기 때문이다. 아래 명령은 이미 있는 것만 쓴다.

1. **어느 형상이 떠 있는가.** `./prsctl lineage`로 배포 SHA와 모든 워커 이미지가 같은 판인지 본다.
   `CR-121` 이후 코드가 아니면 가져오기 명령이 없고 재색인이 같은 실패를 되풀이한다 — 업그레이드가
   먼저다. 업그레이드가 마이그레이션 037(`pull_request_stack`·`reindex_link_pending`)을 적용한다.

2. **별칭과 구체 인덱스, 잡 기록을 적어 둔다.** 7.H의 2·3번 명령에서 `prs-commits`를 `prs-links`로 바꿔
   쓴다. 실패한 잡의 대상 인덱스가 별칭에 붙지 않았는지 본다 — 붙어 있으면 누군가 손으로 전환한 것이고,
   그 인덱스에는 해제된 스택 간선이나 해결 상태가 빠져 있을 수 있다.

3. **스택 간선을 저장소마다 한 번 옮긴다 — 배포 직후, 첫 간선 재색인 전에.** 먼저 `--dry-run`으로 몇
   개가 옮겨질지 본다. 이때는 아무것도 쓰지 않는다.

   ```bash
   ./prsctl links import-stacks --repository <owner/name> --dry-run
   ./prsctl links import-stacks --repository <owner/name>
   ```

   출력은 읽은 서비스 `stacks_on` 간선 수, 넣은(넣을) 행, 이미 있는 행, 형식 오류로 두는 간선 수다. 두 번
   돌려도 같은 표다 — 이미 있는 관계는 덮지 않는다(파생이 먼저 만든 행이 이긴다). `links apply`처럼
   ...
   형식 오류(`from_id`·`to_id`가 `<repository_id>:<PR 번호>`가 아니거나 두 끝의 저장소가 다름)로 둔
...
4. **수정된 빌드에서 새 인덱스를 정본으로 다시 만든다.** 7.H 5번과 같은 CLI에서 별칭만 바꾼다.
   ...
   검증」 두 줄의 시각을 적어 둔다(사내 규모에서는 NOT MEASURED).

   잡이 `completed`면 별칭은 새 인덱스로 옮겨졌다. `failed`면 별칭은 그대로이고 사유가 잡의 `error`에
   남는다(500자에서 잘린다 — 수가 먼저 오고 표본이 뒤따른다).

   - `스택 정본에 없는 서비스 stacks_on 간선 N건` — 3번을 돌리지 않았거나 빠뜨린 저장소가 있다. 3번을
     돌리고 다시 실행한다.
   - `회수되지 않은 간선 미처리 N건`·`link_pending_at_cutover` — 검증이나 전환 직전에 새 부분 갱신이
     들어왔다. 쓰기가 드문 시간에 다시 실행한다.
   - `link_pending_unrecovered` — 다섯 라운드의 회수로도 끝나지 않은 source가 있다. 사유의 표본(source
...
5. **새 검색 요청으로 확인한다.** 3번의 출력에 스택이 있던 저장소에서 하위 PR을 열어 관계
   화면(W-002)에 스택과 해제 표시가 그대로 보이는지, 참조가 있는 PR에서 참조 간선과 해결 상태가
   보이는지 본다.

6. **옛 인덱스는 확인이 끝난 뒤에 정리한다.** 4번의 잡이 `completed`면 그 잡의 원본 인덱스(v1)는 7일 보관
   뒤 정리 스윕이 지운다. 실패한 잡의 대상 인덱스는 어느 잡의 보관 대상도 아니어서 남는다 — 별칭에 붙지
   않았음을 확인한 뒤에만 손으로 지운다.

**하지 않는 것.** 실패한 잡을 `completed`로 바꾸지 않는다 — 보관 정리가 그 잡의 원본 인덱스를 지운다.
별칭을 손으로 새 인덱스에 옮기지 않는다 — 검증을 지나지 않은 인덱스에는 해제된 스택 간선이나 해결
상태가 빠져 있을 수 있다. `document_missing_exception`을 무시하거나 `scripted_upsert`로 없는 간선을 만드는
우회를 하지 않는다 — 근거·접근 범위 필드가 없는 간선이 생긴다. 서비스 인덱스를 직접 고치지 않는다 —
정본은 PostgreSQL이다(ADR-004).

**`links apply`(7.G)와 역할이 다르다.** `links apply`는 커밋 문서의 PR 연결 번호를 고치며 간선
