...
| `… — 발행하지 않았다. 릴리스는 지웠으나 태그가 남았…` / `… 이 실행이 만든 것이 아니므로 지우지 않았다` | 발행 전에 멈췄고 **일부만** 치웠다 | 메시지가 지시하는 대로 태그를 확인·정리한 뒤 같은 버전으로 다시 실행한다 |
...
```bash
# 1) 실제 상태를 읽는다 — draft · 태그 대상 · 자산을 한 번에 본다
gh api repos/<owner>/<repo>/releases/tags/<version> \
  --jq '{draft, immutable, target_commitish, assets: [.assets[] | {name, size, digest}]}'
...
**`draft`가 `false`이고 이름·크기·digest가 로컬과 같으면 그 릴리스는 온전하다.** 스크립트가 멈춘 것은 확인 조회의 문제였을 뿐이므로 **지우지 않고 그대로 쓴다.** 다만 성공 출력이 나오지 않았으니, 사내 운영자에게 전달할 셋 중 **버전과 자산 SHA-256은 위 두 명령의 결과에서 읽는다**(2.A 「사내 운영자에게 전달할 것은 셋이다」).

**어긋나면 새 버전으로 다시 발행한다.** 잠긴 저장소에서는 그 릴리스를 지워도 같은 이름을 회수하지 못한다. 지우기 전에 위 두 명령의 출력을 기록해 둔다 — 무엇이 어긋났는지는 그것으로만 확인할 수 있다.
