...
export POSTGRES_HOST=127.0.0.1 POSTGRES_PORT=55439 POSTGRES_USER=prs POSTGRES_PASSWORD=prs POSTGRES_TEST_DB=prs_test \
  REDIS_URL=redis://127.0.0.1:56384 ELASTICSEARCH_NODE=http://127.0.0.1:59205 GH_PINNED_BIN=/tmp/prs-pinned-gh/2.97.0/gh
docker compose -f $SP/compose.cr091.yml up -d     # postgres:16-alpine·redis:7-alpine·elasticsearch:8.19.0 (CI와 같은 이미지)
```

### 배터리·변이·게이트

```bash
$SP/battery.sh $SP/logs/battery-3                 # CI 순서, 단계마다 종료 코드와 코드 트리 해시
node $SP/mutation/run.mjs A1 C1 …                 # 원문 보관→정확히 한 번 치환→시험→복원·바이트 대조
for t in migrate search-api pipeline-worker gh-executor web; do docker build --target $t -t prs/<img>:cr091-final .; done
(cd deploy/single-host && ./smoke-images.sh cr091-final)
docker run --rm --network host -e POSTGRES_HOST=127.0.0.1 -e POSTGRES_PORT=55439 -e POSTGRES_USER=prs -e POSTGRES_PASSWORD=prs -e POSTGRES_DB=prs_test prs/search-api:cr091-final node dist/role-cli.js grant <login> operator --actor image-smoke
docker compose … config --format json             # 회귀가 prsctl 추출·판정을 대조하는 기준(YAML 렌더가 아니다)
```

### 실패했던 명령과 원인

- 배터리 1회차 회귀 2건: 런북 anchor 절 시험이 낱말의 첫 등장부터 잘라 새 `bash` 블록을 읽음 / 기동 검증 시험이 `if (failure === null) return;` 표지를 찾음.
- 배터리 1회차 e2e `flow-003:176` 1건(부하 민감, 단독 3/3).
- 배터리 2회차 단위 3건: DEV-615 파서가 인라인 `-e`만 읽음 / 회귀 10건: release-tag fake-docker가 새 게이트를 흉내 내지 않음.
- 통합 시험 첫 실행: `beforeEach`가 `app_user` 행을 지워 프로세스 등록 캐시와 어긋남 / `audit_record.correlation_id`는 UUID라 가짜 문자열 거부.
- Write 도구로 만든 파일은 LF, 기존 파일은 CRLF — 커밋이 LF로 정규화하므로 무해.

## 2026-09-14 (6차) 라운드에서 쓴 것 (CR-090)

### 환경

```bash
export PATH="$HOME/.nvm/versions/node/v22.23.2/bin:$PATH"
export POSTGRES_HOST=127.0.0.1 POSTGRES_PORT=55438 POSTGRES_USER=prs POSTGRES_PASSWORD=prs POSTGRES_TEST_DB=prs_test \
  REDIS_URL=redis://127.0.0.1:56383 ELASTICSEARCH_NODE=http://127.0.0.1:59204 GH_PINNED_BIN=/tmp/prs-pinned-gh/2.97.0/gh
# 통합 시험 전에 CI처럼 빈 DB·빈 Redis
docker exec prs-approval-postgres psql -U prs -d prs -c "DROP DATABASE IF EXISTS prs_test WITH (FORCE)" -c "CREATE DATABASE prs_test"
docker exec prs-approval-redis redis-cli FLUSHALL
```

### 필수 배터리 (CI 순서, 단계마다 종료 코드·코드 트리 해시)

```bash
# verify: typecheck → lint → lint:deps → test → build → test:a11y → test:contrast → test:e2e
# integration: test:integration → test:regression
codehash() { git ls-tree -r HEAD | grep -vE "\s(docs/|agent-context/|deploy/single-host/RUNBOOK.md)" | sha256sum | cut -c1-16; }
```

### 변이 (원문 보관 → 정확히 한 번 치환 → 시험 → 원문 복원·바이트 대조, SQL 변이는 새 DB)

```bash
node $ACD5/scratchpad/mutation/run.mjs S9 S10 U1 T15 T16   # ACD5=/tmp/claude-1000/-home-roqkf-pr-search/acd5a0d8-…
```

### 옛 앱 롤백 실측 (A 옛 코드 029 + 대기 요청 → B 새 migrate 이미지 030 → C 옛 insert·claim)

```bash
git archive 26e2627 | tar -x -C $SP/oldapp && (cd $SP/oldapp && pnpm install --frozen-lockfile)
POSTGRES_TEST_DB=prs_rollback pnpm exec vitest run --config vitest.integration.config.ts packages/db/integration/rollback-phase-a.test.ts
docker run --rm --network host -e POSTGRES_HOST=127.0.0.1 -e POSTGRES_PORT=55438 -e POSTGRES_DB=prs_rollback -e POSTGRES_USER=prs -e POSTGRES_PASSWORD=prs prs/db:cr090-final
POSTGRES_TEST_DB=prs_rollback pnpm exec vitest run --config vitest.integration.config.ts packages/db/integration/rollback-phase-c.test.ts
```

### 이미지 재검증 (release·tag 없음)

```bash
for t in migrate search-api pipeline-worker gh-executor web; do DOCKER_BUILDKIT=1 docker build --target $t -t prs/<img>:cr090-final .; done
(cd deploy/single-host && ./smoke-images.sh cr090-final)
docker run -d --network none --read-only --tmpfs /tmp -e NODE_ENV=production -e WEB_PORT=3000 -e SEARCH_API_URL=http://127.0.0.1:9 -e AUTH_ENABLED=false -e SESSION_COOKIE_SECURE=true prs/web:cr090-final   # /ops/gh-policy·/ops/gh-registry·/gh SSR
```

### 백업 복원 스모크 (prsctl restore와 같은 경로)

```bash
POSTGRES_TEST_DB=prs_restore_src pnpm exec vitest run --config vitest.integration.config.ts packages/db/integration/gh-policy.test.ts -t '왕복'
POSTGRES_TEST_DB=prs_restore_src pnpm exec vitest run --config vitest.integration.config.ts packages/db/integration/gh-policy.test.ts -t '^(?!.*왕복)'
docker exec prs-approval-postgres pg_dump -U prs -d prs_restore_src -Fc > src.dump
docker exec -i prs-approval-postgres pg_restore -U prs -d prs_restore_dst --exit-on-error < src.dump
POSTGRES_DB=prs_restore_dst node packages/db/dist/cli.js migrate     # 「적용할 마이그레이션이 없다」
# 표마다 count + md5(string_agg(t::text, '|' ORDER BY t::text)) 비교, 카탈로그는 tgenabled::text처럼 형 변환
```

### 문서 편집·검사

```bash
node $SP/edit.mjs <spec.mjs>          # dry run — 항목마다 기대 횟수와 정확히 같아야 한다
node $SP/edit.mjs <spec.mjs> --apply  # 개행을 감지해 CRLF·LF를 따른다, 하나라도 틀리면 아무것도 쓰지 않는다
python3 ~/.claude/skills/build-srs-prd-env/scripts/validate_srs_prd_env.py --root <worktree> --strict
pnpm exec vitest run packages/contracts/src/error-codes.test.ts && pnpm test:regression   # 문서를 읽는 시험 — 환경 변수와 함께
```

### PR·CI·병합 (gh 2.4.0)

```bash
gh pr create --base main --head feature/rel007-registry-approval --title "…" --body-file $SP/pr-body.md
gh api -X PATCH repos/89sooner/pr-search/pulls/188 -F body=@$SP/pr-body.md     # gh pr edit는 본문 갱신 실패
gh api "repos/89sooner/pr-search/actions/runs?branch=<branch>&per_page=5" --jq '.workflow_runs[] | select(.head_sha=="<sha>") | "\(.id) \(.status) \(.conclusion)"'
gh api repos/89sooner/pr-search/actions/jobs/<job>/logs | sed -E 's/\x1b\[[0-9;]*[A-Za-z]//g' | grep -E "Run pnpm |Tests  |passed \("
gh pr merge 188 --squash --subject "… (#188)" --body-file $SP/merge-body.txt    # --delete-branch는 로컬 브랜치도 지운다
git worktree add -b docs/cr-090-merge-record <path> origin/main && git -C <path> branch --unset-upstream
```

### 실패했던 명령과 원인

- 문서 커밋 뒤 `pnpm exec vitest run --config vitest.regression.config.ts`를 환경 변수 없이 → `range-vs-git`·`releases-vs-git` 실패(19 skip). 실행 조건 누락이며, 환경 변수와 함께 `pnpm test:regression`은 484 통과.
- 복원 스모크 1회차: `'…' || tgenabled` → `operator is not unique: text || "char"` — 카탈로그 비교가 양쪽 모두 비어 PASS가 무효였다. `::text`로 고침.
- 복원 스모크 2회차: 가드 트리거 개수 기대를 1로 적음 → 부모 + 월 파티션 셋으로 4. 스크립트 오류.
- 배터리 1회차(`11254b3`) `pnpm lint` 2건 — `import('@prs/db').PoolClient` 형 표기(`consistent-type-imports`).
- `sleep 45; cat …` — 하네스가 막는다. 기다림은 `run_in_background`의 `until` 루프로.
- `git worktree remove --force /home/roqkf/pr-search-wt/contracts`(5차 자원 정리) — 권한 시스템 거절, 다시 시도하지 않음.
...
**번들은 풀어서 그 안의 것을 돌려 본다.** 고침이 담겼는지는 파일을 읽어서가 아니라 실행해서 안다.
...
**발행 값은 API로 다시 읽는다.** 스크립트 출력은 스크립트의 주장이다.
...
**발행 전제는 미리 잰다** (스크립트도 빌드 앞에서 검사하지만 무엇이 나올지 알고 시작한다).
...
**스택 PR은 병합 전에 시뮬레이션한다.** 자식이 부모를 조상으로 담고 둘 다 base=main이면 순서대로 병합하면 충돌한다.
...
**격리 시험 — 셸 함수의 경계.** `compose`가 필요한 부분 앞까지만 떼어 와 표로 돌린다.
...
**SQL은 실제 PostgreSQL로 실증한다.** 멱등인지까지 본다.
...
- `pkill -f "next start"`가 **자기 셸까지 잡는다.** 백그라운드 PID를 `kill "$PID"`로 끝내고, 여러 단계는 **스크립트 파일로 분리**한다 — 인라인 힙독에서는 출력이 통째로 사라졌다.
- `toContain`은 **부분 문자열로 통과한다.** `ADMIN_DATABASE_URL_X`가 검사를 지나갔다. 키 이름은 줄 첫머리 정규식으로 좁힌다.
