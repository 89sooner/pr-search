# 다음 작업 · 미해결 항목 · 확인할 사항
최신 기준 (**2026-09-17 10차 · CR-103(WP-085 정정·WP-090)·CR-104(WP-091 무한스크롤)·CR-105(DEV-722 검색 과매칭) main 반영, 최종 커밋 `750fb91`, CI green · 컨텍스트 한도로 아래 두 항목은 다음 세션으로 넘김**)

**main은 `750fb91`이다. 착수 전 `git status`로 공유 checkout에 다른 세션의 미커밋 작업이 있는지 먼저 확인하라** — 2026-09-17 10차 세션 중 다른 세션의 CR-102(Regression 확장 제안) 미커밋 문서·`packages/es/src/query-builder.ts` 변경과 동시에 작업한 이력이 있다(상세: `session-notes.md`의 "10차" 절 Risks/gotchas).

## 먼저 할 것 — 2026-09-17 10차에서 이어받을 것 (사용자 방향은 이미 확정, 다시 안 물어도 됨)

1. [ ] **범위 검색·M번호 필터.** `FR-SRCH-005`/`FR-SRCH-006` 개정(PR번호 범위 키 신설, M번호 범위 키 신설, SHA 범위는 프런트엔드가 `seq:`로 변환하는 것으로 문서화) → `packages/query/src/keys.ts` → `packages/es/src/query-builder.ts` → `RepositoryWorkspace.tsx` 필터 폼. 착수 전 CR/DEV/WP 번호 재실측 필수. 상세 설계 메모는 `session-notes.md` 10차 절 Next steps 1번.
2. [ ] **Commit history PR번호 병기.** `FR-SRC-002` AC-1 개정 → `packages/contracts/src/source.ts`의 `SourceCommit`에 `pr_number` 추가 → `/history` API가 기존 SHA→PR 역해석 재사용 → `SourceHistory.tsx` 표시+카피. 상세는 `session-notes.md` 10차 절 Next steps 2번.
3. [ ] worktree 3개(`cr102-frontend-fixes`·`cr103-infinite-scroll`·`cr105-search-fix`) 정리 여부를 사용자에게 물을 것(병합 완료, 삭제는 `[Git Destructive]`로 막힐 수 있음).

## 이전 (8차 이하, 아래는 2026-09-15 시점 기준 — 이후 진행 여부 미확인, 착수 전 실제 상태 재확인)
...
**워크트리가 없을 때**: `/tmp`는 휘발이다. 미커밋 코드가 사라졌으면 이 문서의 files.md 절과 decisions.md로 다시 만든다 — 그래서 **첫 일은 커밋**이다(아래 0번).

## 0. 가장 먼저 — 미커밋 코드를 브랜치에 커밋해 `/tmp` 휘발에서 구한다

`git add`는 **경로를 명시**한다(공유 체크아웃 규율). 개행 경고(LF→CRLF)는 core.autocrlf 때문이며 HEAD는 LF다 — `git diff --cached --shortstat`으로 규모를 본 뒤 커밋한다.
...
`agent-context/`는 **메인 체크아웃**(`/home/roqkf/pr-search`)에 최신본이 있다(이 파일). 워크트리의 `agent-context/`는 지난 판 그대로이므로, 커밋 전에 `cp -r /home/roqkf/pr-search/agent-context/* /tmp/pr-search-rel007/agent-context/`로 맞춘다.
...
## 2. 웹 화면을 끝낸다 (W-010 최소 + W-021 최소) — 컴포넌트는 있고 **페이지가 없다**

만들 것 (기존 `apps/web/app/ops/jobs/page.tsx` + `components/OpsJobsView.tsx` 모양을 따른다):

- `apps/web/app/gh/page.tsx` — `GuardedPage title="GitHub 작업" returnTo="/gh"` + `<GhCommandCenterView />`
- `apps/web/components/GhCommandCenterView.tsx` ('use client') — 순서: `GET /api/gh/identity`·`/api/gh/capabilities`·`/api/gh/contexts/repositories` 병렬 → `GhIdentityBanner`(연결: `POST /api/gh/identity {return_to:'/gh'}` → `authorize_url`로 `window.location.assign`; 해제: `DELETE`) → `GhCapabilityList`(선택 `pr.list`) → `GhPrListForm`(즉시 검증 `validateForm`) → 폼이 유효하면 debounce로 `POST /api/gh/executions/preview` → `GhExecutionPreview` → 실행 버튼은 `canExecute(violations, preview)`일 때만 → `POST /api/gh/executions` (헤더 `Idempotency-Key: newIdempotencyKey()`, 결과 오면 키 재생성) → `GET /api/gh/executions/{id}/stream`(EventSource, `state`·`done` 이벤트; 실패 시 1초 폴링 `GET /api/gh/executions/{id}`로 대체) → `GhExecutionPanel`(취소 `POST …/cancel`). 404(`GH_*` 경로 없음)는 「이 배포에서는 GitHub 작업이 열리지 않았다」로. 401 `GH_IDENTITY_REQUIRED`는 연결 안내.
- `apps/web/app/gh/history/page.tsx` + `components/GhHistoryView.tsx` — `GET /api/gh/executions?limit=50[&before=]`, 표(실행 ID·capability·저장소·상태·요청·종료), 행 선택 시 `GhExecutionPanel`, 「같은 구성으로 다시 실행」은 `invocation`을 `/gh?prefill=`로 넘겨 **새 미리보기·새 실행**(과거 승인 승계 없음, FR-GH-012 AC-4).
- `apps/web/app/gh/identity/callback/route.ts` — GHE가 보낸 `?code&state`를 받아, `lib/proxy.ts`의 `resolveProxyAuth`·`buildProxyHeaders`로 세션 쿠키를 붙여 `search-api`의 `POST /api/v1/gh/identity/callback`에 넘기고, 응답의 `return_to`로 `redirect`. 실패면 `/gh?identity=failed`로. **code·state를 로그에 남기지 않는다.** `web`이 IdP 왕복을 소유한다는 인프라 규칙과 같은 자리(`app/auth/callback/route.ts` 참고).
- `components/LeftNavPanel.tsx`의 `ICONS`에 `'gh-command-center': 'branch'`, `'gh-history': 'jobs'` 추가(없으면 `repository` 기본 아이콘).
- `lib/nav.test.ts`에 github 섹션이 모든 역할에 보인다는 시험, `lib/gh.test.ts`(`validateForm`이 서버와 같은 위반을 내는지·`resultKind` 표·`describeError`), `a11y/gh.test.tsx`(폼 라벨·live region·표 caption·axe 0건), `e2e/gh.spec.ts`(API 대역: 연결 없음 → 연결 버튼 → 폼 → 미리보기 argv 표시 → 실행 202 → 스트림 → 결과 표; 위반 시 실행 버튼 비활성; 404 안내). `architecture.test.ts`가 `dangerouslySetInnerHTML` 0건을 이미 거는지 확인하고 없으면 gh 경로에 건다 (QA-GH-24).

## 3. 배포·회귀를 잇는다

- `deploy/single-host/build-bundle.sh`: `APP_TARGETS`에 `gh-executor`, `IMAGE_NAME[gh-executor]="prs/gh-executor"`; `prsctl`의 적재 확인 목록에 `prs/gh-executor`; `smoke-images.sh`에 executor 기동 검사(꺼진 상태로 `/healthz` 200 + `execution: disabled`).
- `deploy/single-host/.env.example`에 「GitHub Operations Plane」절: `GH_OPERATIONS_ENABLED=false`, `GHE_OPS_CLIENT_ID/SECRET/REDIRECT_URI`, `GH_IDENTITY_VAULT_KEY`(`openssl rand -hex 32`), `GH_EXECUTOR_MAX_CONCURRENT`. 자격 분리 설명(Data App·로그인 OAuth App·표기 App·**Operations App** 넷).
- `docker build --target gh-executor -t prs/gh-executor:dev .`을 실제로 돌려 본다 — 내려받기·해시·`USER node`·`/healthz`. 그다음 compose의 `read_only`·tmpfs 조합으로 기동해 `execution: disabled`를 본다.
- `regression/runtime-reachability.test.ts`: (a) `spawn(`이 저장소에서 `apps/gh-executor/src/spawn.ts` 한 곳뿐이고 `shell: true`·`exec(`·`sh -c`가 0건, (b) `buildArgv(` 정의가 하나, (c) `Dockerfile`의 `GH_VERSION`·두 해시가 `pin.ts`와 같음, (d) `redact` 패턴이 `@prs/github`의 것과 같음, (e) compose에서 `gh-executor`만 `GH_IDENTITY_VAULT_KEY`를 `search-api`와 나눠 갖고 `GHE_APP_*`·`GHE_ANNOTATE_*`·`GHE_OPS_CLIENT_SECRET`은 받지 않음, (f) `server.ts`가 `registerGhRoutes(`를 부르고 `runtime.ts`가 `buildGhDeps(`를 넘김, (g) 실행기 `index.ts`가 `startSweeper(`·구독을 세우고 종료에서 닫음. `CAPABILITIES` 표 형식을 쓸지는 decisions.md 「아직 정하지 않은 것」.
- `deploy/k8s/`: 지시는 K8s를 주력으로 만들지 말라고 했다. 최소 `gh-executor.yaml`(replicas 1, `GH_OPERATIONS_ENABLED` configmap, 전용 시크릿 `prs-gh-ops-secrets`)과 README 한 줄을 둘지 결정 — 인프라 3.1 표가 「Profile A 미포함」이었던 것을 이번에 뒤집으므로 표를 고친다.

## 4. 전체 배터리 (순차, 격리 서비스에서)

`typecheck → lint → lint:deps → test → test:regression → test:integration → build → test:a11y → test:contrast → web build → test:e2e`. **지금까지 돌린 것은 부분뿐이다** (commands.md). `pnpm lint`는 새 코드에서 오류 1건을 고친 뒤 전체는 안 돌렸다. `pnpm typecheck`(web 포함) 미실행. 변이 시험 목록(최소): 제약 우회(`--web` 허용) · argv 빌더 둘째 생성 · 재검증 중 argv 대조 제거 · 봉인 AAD 제거 · 멱등 보조 표 제거 · SIGKILL 유예 제거 · 환경 허용 목록에 `GH_TOKEN` 추가 · `sanitizeText` 무력화 · 이력 소유자 검사 제거 · 만료 검사 제거. **skip은 통과가 아니다** — 통과 건수를 파싱한다.

## 5. 문서 cascade (CR-086) — 순서는 CLAUDE.md의 Update Cascade

1. `docs/00_governance/change_control.md`: CR-086 행(`scope`, 트리거 「결정자 REL-007 착수 승인 2026-09-13」) + `### CR-086 cascade` 절. CR-085 cascade 절(1663행~) 모양을 따른다.
2. `srs_final.md`: FR 문장 변경 **없음**을 확인해 적는다. v2.27로 올릴지는 감사 표에 `gh.execution.*`가 없으므로 「실행 감사는 `gh_execution`이 축」이라는 한 줄만 필요할 수 있다 — 필요 없으면 「바꿀 것이 없다는 사실을 확인한 뒤 적는다」.
3. 데이터 모델 3.5장: 마이그레이션 번호 006/007/009 → **028**로 정정, `gh_identity_secret`·`gh_execution_idempotency` 추가, `gh_execution_idem_uk`가 성립하지 않는 이유, 만들지 않은 표와 이유. 비동기 9.1: `prs:gh:executions` 파티션 4·소비자 `gh-executor`·JOB-GH-007 구현 상태. 백엔드 12.1: 모듈이 `apps/search-api/src/gh/*`와 `apps/gh-executor`에 있음. 인프라 3.1: `gh-executor` Profile A **포함**(1/1), 12장 값 확정(상한·workspace tmpfs 256m). 보안 10.2: 봉인 최소 구현과 대가; 6장 시크릿 표에 `GH_IDENTITY_VAULT_KEY`·Operations App client secret 행. API 계약: API-GH-002 미리보기·`Idempotency-Key` 헤더, API-GH-007 callback, API-GH-010 상세·`before` 키셋, API-GH-005 「상태만 스트리밍」, API-GH-003 경로가 `/gh/contexts/repositories`. 프런트엔드 9.1 라우트 표는 `/gh`·`/gh/history`만 실재.
4. 로드맵: REL-007 `in_progress`, 4.2장 끝에 「2026-09-13 착수」. 작업 패키지: `### WP-077 REL-007 R0 PR 목록 조회 첫 수직 구현` 신설(WP-076 모양, DoD는 QA-GH-01·02·03·04·05·10·11·12·14·17·18·21·22·23·24·25 인용), 3장 표에 WP-077 `in_progress`, WP-045/046/047/048/061/062/066을 `in_progress`로 **부분**임을 적는다(각각 무엇이 들어왔는지).
5. 원장: 3장 행 갱신, 4장 FR-GH-001/002/003/006/008/011/012·NFR-009/010/011 `partial`, 5장 DEV-650~(파티션 유니크 불가 · SSE 출력 미스트리밍 · 봉인이 비밀 저장소 대체 · `workflow run`의 `--json` 필드 절 부재 · `extension exec --help` 인증 요구 · `--state closed`→CLOSED+MERGED · 앱→앱 시험 import · 멱등 표 TTL 없음 · 분류 195건 unknown), 6.83장 검증 기록(외부/사내 표, 배터리, 변이, CI run·attempt).
6. `RUNBOOK.md` 7.C 「GitHub 작업(Operations App) 켜기」: Operations App 등록(권한 `Pull requests: read`, callback `<web>/gh/identity/callback`, **User-to-server token expiration 옵트인**), `.env` 채우기, `prsctl upgrade`, 한 사용자로 연결·실행 확인, 끄는 법(`GH_OPERATIONS_ENABLED=false`)과 028 down은 별개라는 것.
7. validator: `python3 /home/roqkf/.claude/skills/build-srs-prd-env/scripts/validate_srs_prd_env.py --root /tmp/pr-search-rel007 --strict` — **기준선은 main에서 6건(오류 4·경고 2)**: `FR-CSS-005` 미정의, `D-002` 미정의, `risks.md` 경로 2건, placeholder(change_control 12·원장 8). 신규 0건이 목표이며 strict 전체 통과라고 적지 않는다.

## 6. 독립 검토 → 수정 재검토 → PR → CI → 병합

검토 둘: (가) 권한·자격·입력·출력·공개 CI 경계, (나) 실행 수명주기·취소·중복·결과·회귀. 검토자의 「미해소」는 코드로 확인한다. PR 본문에 `Refs: WP-077 FR-GH-…`와 attribution. CI는 **필수 check green이 원칙**(이번에는 예외 병합 승인이 없다). 병합 뒤 `origin/main`에서 재검증하고 원장 3장·`agent-context` 갱신.

## 7. 최종 보고 형식 (지시서 18장)

앞머리 다섯 답: ① CI가 실제로 돌았는가 → **예, run 34705991448 attempt 2, 두 잡 success, runs-on 불변, 코드 변경 0** ② 고친 것 → 없음 ③ 새로 할 수 있는 일 ④ 기존 검색과의 차이(실시간 CLI 조회 vs 수집 색인) ⑤ 사내 미확인(실제 GHE·Operations App·GHES rate limit·`SSL_CERT_FILE` 사내 CA). 그다음 기술 근거(SHA·run·argv·manifest 해시 `ad00027d…`·gh 해시·migration·이미지·리뷰·PR), 별도 장 「능동 자가 연구 결과」(session-summary.md 2절), 별도 장 「Agent-Initiated Decisions」(decisions.md의 C 표 전부, 항목마다 근거·대안·영향·되돌리기·관련 CR/DEV/시험).

## 열린 편차 (건드리지 않는다 — 지시 15장)

DEV-581·588·603·618·642·647·649. 이번 흐름을 막지 않는 한 손대지 않는다. 원장 6.82장의 변이 16/18 불일치도 실행 로그 없이는 고치지 않는다.

## 사내가 나중에 할 것 (NOT RUN)

실제 GHE에 Operations App 등록·인가·`gh pr list` 실행, GHES의 user-to-server 토큰 만료 설정, 사내 CA가 `SSL_CERT_FILE`로 gh에 닿는지, 028 운영 적용, `gh-executor` 이미지 반입.

---

최신 기준 (**2026-09-13 · `WP-075` 안전성 보강 — `CR-085` closed, 병합 완료**)

**`main = 65caf0c`. 실측하라.** PR #177(코드·문서)과 #178(번들 검증 기록)이 squash 병합된 결과다.
브랜치 `fix/wp075-annotate-safety`·`docs/cr085-pilot6-candidate`는 원격에 남아 있다.
...
**원격 브랜치 전체로 재실측하라.** main에서만 재면 열린 PR이나 병합되지 않은 브랜치가
선점한 번호를 놓친다. 병렬 세션이 있으면 채번 현황을 먼저 묻는다.

## 가장 앞에 있는 것 — CI가 코드를 실행하지 못한다 (원인은 둘이다)

`steps=0`으로 두 잡이 2초 만에 끝난다. **시험이 돌아서 실패한 것이 아니다.**

**「결제만 풀면 된다」가 아니다.** 이 저장소에는 self-hosted 러너 `DESKTOP-HMAIKJ2`가
**이미 등록돼 있고 online**인데(`self-hosted,Linux,X64`, `busy=false`), 워크플로 두 잡이
모두 `runs-on: ubuntu-latest`라 **그것을 부르지 않는다.** 과금 대상인 호스팅 러너를
요구했고 그것이 막힌 것이다. `design-system`이 멀쩡했던 것은 그 저장소가 **public**이라
호스팅 러너가 무료이기 때문이며, `pr-search`는 **private**이다.

선택지는 둘이고 **결정자가 「지금은 바꾸지 않는다」를 골랐다**(2026-09-13).

1. 결제·한도를 푼다 → `12189b1`에서 다시 돌려 확인한다.
2. `runs-on`을 `[self-hosted, Linux, X64]`로 바꾼다. 저장소가 private이라 fork PR 위험이
   없고 러너가 `docker` 그룹에 있어 `services:`도 뜬다. **바꾸면 포트를 먼저 본다** —
   `integration`의 서비스 컨테이너가 `5432`·`9200`·`6379`를 호스트에 바인딩하므로
   개발용 `prs-postgres` 등이 떠 있으면 그 잡이 실패한다. 러너가 한 대라 두 잡은 순차로 돈다.

- **그 전까지 `0.1.0-pilot.6`을 발행하지 않는다.** 후보 번들은 만들어 검증까지 마쳤다.
- 후보는 `/tmp/pr-search-bundle-pilot6/`에 있으나 **임시 경로라 재부팅에 사라진다.**
  발행할 때는 그때의 `main`에서 다시 만든다.

## 사내가 표기를 켜려면 (런북 7.B가 정본 — **0단계가 새로 생겼다**)

1. **먼저 읽기 전용으로 본다.** `docker compose run --rm --no-deps worker-annotate
   node dist/annotate-preview-cli.js --repository <org>/<저장소>`.
   「다음 회차 대상」이 켰을 때 제목이 바뀔 건수이고, 「확인하지 못한 것」도 함께 읽는다.
2. 표기 전용 App 등록 → `.env` 채움 → `prsctl upgrade` → 한 저장소에서 먼저.
3. **차단은 조회 성공으로 풀리지 않는다** — 실제 쓰기 성공이나 `annotate_resume`이다.
4. 제목이 기대와 다르게 저장되면 `body_changed`로 멈춘다. 확인한 뒤 `annotate_resume`으로 연다.

## 열린 편차 (표기 관련)

| 편차 | 무엇 | 왜 열려 있나 |
| --- | --- | --- |
| `DEV-618` | 제목 read-then-write 경합 | 공식 API가 비안전 메서드의 조건부 요청을 **지원하지 않는다고 명시한다** |
| `DEV-647` | 지속 실패 시 이벤트 무한 유예와 파티션 head-of-line | 유예 상한은 `DEV-627`의 판단을 뒤집는 일이라 별도 설계가 필요하다. 일일 스윕이 backstop한다 |
| `DEV-649` | 토큰 발급이 중단 신호를 안 받음 | `@prs/github`의 계약을 넓혀야 한다. 영향은 비변경 요청의 지연뿐 |
| `DEV-581` | 직접 푸시의 영구 부재 확정 근거 | 결정자의 판단이 필요하다 |
| `DEV-603` | planner의 멱등 갈래가 도달 불가 | 어느 쪽이 정본인지 정해야 한다 |
| `DEV-588` | 조정 스캔 취소 시험의 경합 | `FR-ADMIN-002`를 소유한 WP의 몫 |
| `DEV-642` | 사내 GHES의 실제 한도 설정 미확인 | 사내가 확인해 주면 런북에 적는다 |

## 확인할 사항

- [ ] CI 결제가 풀렸는가. 풀리면 `65caf0c`의 체크를 돌려 green을 확인한다.
- [ ] `0.1.0-pilot.6`을 발행할 것인가 — **발행하려면 그때의 main에서 번들을 다시 만든다.**
- [ ] 원격 브랜치 정리 여부 — 병합된 것이 계속 쌓이고 있다.
- [ ] 병합된 PR의 미해결 리뷰 스레드는 GraphQL `isResolved`로 직접 세라.
      PR #177·#178은 0건이지만 저장소 전체로는 이전 라운드의 것이 남아 있다.
- [ ] 개발 백킹 서비스(`prs-postgres` 등)는 이 세션 시작 시점에 이미 종료돼 있었고
      **내가 올리지 않았다.** 시험은 `prs-wp075safety` 프로젝트로 따로 띄웠다가 내렸다.

## 그다음 후보

M 넘버 계열(`WP-074`·`WP-075`)이 안전성까지 끝났다. 로드맵 4.2장은 여기서 끝나고
그 뒤는 `REL-007` 이후 GitHub Operations Plane이다. **자동으로 시작하지 않는다** —
결정자의 지시를 기다린다.
...
## 0. 시작하기 전에 — 이 인계의 값을 실측하라

**여기 적힌 수는 전부 적은 순간의 값이다.** 이전 세션이 그것을 실증했다 — 인계 팩이 "백엔드는 이미 전부 서 있다"고 적었는데 틀렸고, 이번 세션은 그 팩이 "26건"이라 적은 미해결 리뷰가 실제로는 22건인 것을 다시 확인했다.
...
> **REL-004 8/8. REL-005 3/4** — WP-037·038·039 done, 남은 것은 **WP-040 하나**
> **REL-003 릴리스 게이트는 여전히 미통과.** Gate 5 성능은 NOT RUN(1,000만 문서 합성 데이터셋 부재)
> 열린 PR **0건**(양 저장소)
> **미해결 리뷰는 0건이 아니다** — 병합된 PR **열**에 **24건**이 남아 있다(`DEV-414`). 아래 C절
> open DEV **13건** — `DEV-001·006·010·016·026·058·061·304·305·395·399·414·427`
> 다음 빈 ID: **CR-055 · DEV-428** (측정값. 쓰기 전 다시 잰다)
> **`DEV-427` 열려 있다** — CI 통합의 `link-rebuild` 되먹임 시험이 한 번 실패했고 세 번
> 재현하지 못했다. 재시도 통과를 해소로 적지 않는다
> **`DEV-377`(flow-003 뒤로가기)이 닫혔다** — 진단이 달랐고 CI가 그것을 드러냈다(`DEV-424`)
...
## A. 지금 당장 — WP-040 A-002·A-003 운영 콘솔 (REL-005 마지막)

- **관련 요구사항은 작업 패키지가 정본이다** — `FR-ING-009`·`FR-ADMIN-002`·`FR-ADMIN-003`·`FR-ING-006`·`FR-ING-008`
  (`pr_search_work_packages.md` WP-040). 여기 옮겨 적은 값이 아니라 그 문서를 읽는다.
  선행 WP-010·019·028·035(전부 done).
- **화면 셋이 남았다**: `A-001` 파이프라인(아카이브 섹션 포함), `A-002` 저장소 등록, `A-003` 잡 운영.
- **`WP-039`가 진입점을 이미 올바르게 갈랐다** — `nav.ts`의 `allowedRoles`가 `operator`에게
  파이프라인·저장소 등록을, `security_officer`에게 감사 기록과 파이프라인(`A-001-ARCHIVE`만)을
  보인다. **화면 본체가 그 규칙을 다시 만들지 않게 하라** — 서버가 403을 내고 화면은
  `no_permission`을 그린다(A-004 선례).
- 착수 전 계약 감사(레포 관례, `CR-054`가 열다섯 번째였다): `QA-A001-01~14`가 실제로 셋인지,
  `A-001-ARCHIVE`의 `archive_only` 상태가 구현 가능한지, `API-ADM-002`의 `pause`/`resume`가
  화면에 있는지.

### WP-039가 깔아 둔 자리 — 다시 만들지 말 것

- `packages/domain/src/audit.ts` — **감사 액션의 정본 어휘**다. 활성·미활성·legacy 셋을 가르며
  `@prs/domain/audit` 서브패스로 브라우저도 쓴다. **개수를 세지 않는다.**
- `apps/search-api/src/audit/recorder.ts` — **공용 실패 격리 경계 하나**다. 호출부마다
  `try`/`catch`를 두지 않는다. 회귀가 `auditRepo.recordAudit(`의 직접 호출을 다섯 파일에서 금지한다.
- `apps/search-api/src/audit/cursor.ts` — PostgreSQL 키셋. 지문에 **접근 범위를 넣지 않는다**
  (감사는 행위의 기록이라 저장소 권한이 답을 바꾸지 않는다).
- `packages/db/src/partitions.ts` — `runPartitionRetention`이 **만들고 나서 지운다**. 경계는
  **카탈로그에서 읽고 이름으로 추측하지 않는다.**
- `apps/web/lib/nav.ts` — 항목마다 `allowedRoles`. **새 항목·새 역할의 기본값은 보이지 않음이다.**
- `AuditRecordTable` — `C-013`을 쓰지 않는다(`RangeResultTable` 선례, `DEV-419`).

## B. 그 뒤 — REL-006 (WP-041·042·044)

- **`WP-041`(안전 구간 표식)과 `WP-044`(내보내기)는 감사 액션을 함께 활성화한다.**
  정본 표(`srs_final.md` FR-AUTH-004 AC-1)의 `safe_marker.set`·`export.create`를 `활성`으로
  바꾸고 배선을 넣는다. **지금 그것이 기록되지 않는 것은 누락이 아니다**(`DEV-403`).
- `WP-043`은 **착수하지 않는다** — `ACC-06`이 NOT RUN이라 활성화 조건이 성립하지 않는다.

## C. 미해결 리뷰 24건 (DEV-414)

**세어 보니 0건이 아니었다. 그리고 처음 센 수도 잘려 있었다.**
...
**처음 쓴 계수 명령이 `last:25`라 `#32`·`#49`를 조용히 빼고 22건이라 답했다.** `last:100`으로
다시 세니 24건이다 — `commands.md`의 명령이 **두 `pageInfo`를 읽어 잘림을 드러낸다.**
design-system 저장소에도 9건이 있다(같은 방식으로 다시 세야 한다).

**지적 자체는 대부분 후속 PR에서 이미 정정된 것으로 보이나 실측 없이 그렇게 단정하지
않는다.** 스레드마다 실측해 정정된 것은 답변·resolve하고, 아직 고쳐지지 않은 지적이 있으면
`DEV`로 등록한다.

## D. 배포에서 해야 할 것

- **`ADMIN_DATABASE_URL`** (WP-039 신규) — 없으면 `JOB-AUD-001`만 서지 않는다. 값은
  `prs_admin`이 **아니라** 그 롤의 멤버십을 가진 로그인 주체를 가리킨다(`DEV-416`).
  `CREATE ROLE prs_retention LOGIN ...; GRANT prs_admin TO prs_retention;`
- **마이그레이션 018·019** — `pnpm run db:migrate`. 019는 파티션 표의 소유권을 `prs_admin`으로
  옮긴다. **`prs_app`의 `INSERT`·`SELECT`는 그대로다**(실측 확인).
- `pnpm es:apply-mappings`(changed_lines 백필), 마이그레이션 017 — WP-037이 남긴 것, 아직 미적용.
- `SEARCH_CURSOR_HMAC_KEY`(32자 이상, 복제본 공통), `pnpm es:reindex` — WP-032가 남긴 것.
- **실제 Prometheus 경보 규칙 적용은 NOT RUN이다.** 지표(`audit_record_failed_total`)와 계약은
  섰고 환경 적용만 남았다.
...
Gate 4 보안(일부 가능) · Gate 5 성능(**여전히 불가** — `test:perf` harness는 있으나 1,000만 문서
합성 데이터셋이 없다) · Gate 6 운영(불가 — 실제 K8s 없음). `ACC-06`은 NOT RUN이고
`W-007`·`WP-043`은 NOT ACTIVATED다.

## G. 미뤄 둔 항목

- ~~`DEV-377` flow-003 뒤로가기~~ → **닫혔다**(`DEV-424`). 진단이 "5초 초과"였는데 실제 원인은
  **히스토리 경합**이었다 — 첫 뒤로가기의 전이가 끝나기 전에 두 번째를 불렀다. `WP-039`가
  e2e를 9건 더해 부하가 커지자 CI를 실제로 막았고 **그 로그가 원인을 드러냈다.**
  두 번째 `goBack()` 전에 URL을 먼저 확인하도록 고친 뒤 전량 4회 연속 통과.
  **증상으로 적어 둔 판정은 오래 열려 있는다**
- `DEV-399` 원장 5장 아홉 행의 파이프 이스케이프 + **칸 수를 세는 회귀 시험**
- `DEV-395` 자기 리뷰 소급 — 재투영이 필요하고 그것은 백필·조정 스캔의 기계다
- `DEV-058` test:perf — harness는 있고 릴리스 규모 실측만 남았다 (Gate 5)
- `DEV-061` direct_push — AC-3 충족을 실측하지 못해 열어 둔다
- 문서 검증기 `--strict` 자기참조 오탐 — **문서를 고쳐 게이트를 통과시키지 않는다.**
  판정은 언제나 `main` 대비 diff다

# 이전 세션 기록 (보존)

## (2026-08-29 2차 · CR-036 · WP-038 종료 시점)

## A. 지금 당장 — WP-039 감사 기록과 A-004 (REL-005)

- FR-AUTH-004, NFR-006. 감사 대상 13종 기록(보안 문서 7장), `security_officer` 한정 조회, A-004 화면(C-013 재사용).
- 선행 WP-012·WP-002(done). API-ADM-005, ENT-CORE-007. `JOB-AUD-001` 보존 만료 파티션 드롭.
- 착수 전 계약 감사(레포 관례): 감사 액션 목록·비동기 기록의 실패 격리·감사 화면 진입 자체 기록.

## B. 그 다음 — WP-040 A-002·A-003 운영 콘솔 (REL-005 마지막)

## C. 배포에서 해야 할 것 (WP-037·CR-053이 남긴 것 — 아직 실환경 미적용)

- `pnpm es:apply-mappings`(changed_lines 백필), 마이그레이션 017.
- WP-032가 남긴 `SEARCH_CURSOR_HMAC_KEY`, `pnpm es:reindex`.

## D. 릴리스 게이트 4·5·6 (변화 없음)

Gate 5 성능은 여전히 불가 — test:perf harness는 있으나 1000만 문서 합성 데이터셋이 없다. ACC-06 NOT RUN(W-007·WP-043 NOT ACTIVATED).

---

## (완료) WP-038 W-006 통계 대시보드 — 2026-08-29 (2차) done (원장 6.49장, PR #80 `b5ae933`)

아래는 착수 시점 기록이다. DEV-380은 CR-036 dataviz 0.2.0 배포로 해소됐고 이 WP가 소비한다.
...
- [x] PR #228 병합 — main `f9cda82`(2026-09-23). PR CI·main CI 모두 success.
- [ ] PR #227(병합 기록) 병합 — CR-114·CR-115·CR-116 셋의 `closed`·`done` 반영. CR-116 병합으로 생긴 충돌은 rebase로 풀었다.
...
- [x] **PR #230 병합** — main `65acf83`(2026-09-24, 사용자 지시 「origin/main에 병합해라」). PR CI·main CI 모두 success.
- [x] **병합 기록 PR #231 병합** — main `3d831a9`(2026-09-24). 브랜치 `docs/cr117-merge-record`(CR-117 `closed`, WP-102 `done`, 원장 3·4장 상태, 원장 4장에 「병합 전」으로 남아 있던 CR-113~116 행 11개 정정, 17차 마감 인계). 병합은 사용자 결정이다.
- [x] **다음 세션 첫 일: 나머지 두 보고를 새 CR로 연다** — 18차에 열었다: 2번은 CR-119(main `e94cb4f`), 3번은 작업 B로 진행 중(아래 「18차 뒤 남은 것」).(사용자 지시 2026-09-24 「새 CR은 다음 세션에서」). **둘 다 사내 반입 없이 외부에서 고칠 수 있다** — 원인이 코드에 있어 격리 인프라에서 재현 시험·수정·PR까지 된다. 사내가 필요한 일은 반입 뒤 재색인 재실행과, 손으로 전환한 v4 인덱스의 상태 확인뿐이다(아래 2번 항목). 권고 순서는 2번(prs-commits) 먼저 — 숨은 메타데이터 누락 가능성이 있고 범위가 더 크다. 순서는 사용자가 아직 확정하지 않았으니 착수 때 확인한다. CR 번호는 `docs/` grep과 `gh pr list --state open`으로 실측한다.
...
- [x] **같은 반입의 2번 보고 — prs-commits 재색인 verify 과대 계산(별도 CR).** → CR-119 / WP-103, main `e94cb4f`(2026-09-25). 숨은 메타데이터 누락(DEV-760)은 재현됐고 함께 고쳤다. 사내 수동 v4 전환 확인은 RUNBOOK 7.H(NOT RUN). 원인(17차 전반 분석, 코드로만 확인): `rebuildCommits`가 쓰기 결과와 무관하게 스냅숏 행마다 문서 ID를 기대 건수에 넣는다(`apps/pipeline-worker/src/reindex.ts`의 `tally.documentIds`). 체인 밖 스냅숏 행(PR 브랜치에 있다가 rebase·force-push로 PR에서 빠진 옛 커밋)은 `createWith`가 없어 새 인덱스에 문서를 만들지 않고, 404는 `packages/es/src/commit-metadata.ts`에서 조용히 noop이다. 보고서와 다른 점: 집계는 Set이라 「중복 집계」가 아니고, 문제 커밋은 「직접 push」가 아니다. 「ES `_count`와 비교」는 이미 그렇게 한다 — 틀린 것은 기대 건수다. **숨은 문제**: PR 브랜치 커밋도 커밋 보강 경로로 정본에 들어오는데(`commit-enrich.ts`), 재구축 때는 메타데이터 쓰기가 문서 생성보다 먼저라 새 인덱스에서 메시지·작성자가 비어 있을 가능성이 높다 — 기대 건수만 고치면 이것이 가려진다. **사내가 손으로 v4로 전환했다면 먼저 확인**: 잡이 failed가 된 뒤의 쓰기는 v4에 없다(이중 쓰기는 running 잡에만, `packages/db/src/repositories/reindex.ts`), 정상 전환 뒤 돌았을 시퀀스 full sweep도 돌지 않았다, v3는 자동 삭제되지 않는다(보관 정리는 completed 잡만).
- [ ] **같은 반입의 3번 보고 — prs-links 재색인 shadow_write_failed(별도 CR).** → 18차 작업 B로 진행 중. 요청 1(shadow의 문서 없음 무시)도 그대로 받지 않는다 — 해제된 스택 간선이 조용히 사라지는 것을 재현했다(아래 「18차 뒤 남은 것」). 원인: 재구축 포트 → `resolveReferencesTo`(`apps/pipeline-worker/src/link.ts`) → 간선 부분 갱신(`packages/es/src/links.ts`)에서, 간선의 주인이 아직 처리되지 않았으면 shadow에 문서가 없어 `document_missing_exception`이 난다. `sendLinkBulk`는 shadow 쪽 오류를 모두 실패로 올리지만 같은 패키지의 커밋 메타데이터와 관계 요약은 shadow에 아직 없는 문서를 실패로 세지 않는다. 처리 순서 때문이라 참조가 얽힌 저장소에서는 매번 날 가능성이 높다. 요청 1(shadow의 문서 없음 무시)은 선례와 같고 안전해 보인다. 요청 2(`scripted_upsert: true`)는 접근 범위 필드 없는 간선 문서를 만들어 fail-closed(ADR-008, DEV-213)를 어기므로 받지 않는다. 이 경로를 끝까지 도는 통합 시험이 없다(`link-reindex.test.ts`는 이름과 달리 PR 연결 복원만 본다).
...
- [ ] 격리 컨테이너 `prs-cr117-postgres`(55447)·`prs-cr117-es`(59213, `cluster.name=prs-cr117-isolated`)·`prs-cr117-redis`(56392) — 17차 마감 때 떠 있다. 다음 CR이 그대로 재사용할 수 있고(DB는 새 CR용으로 따로 만든다), 정리한다면 다른 세션이 쓰지 않는지 먼저 확인한다.
- [ ] worktree `cr117-source-commits` — 지금은 기록 브랜치 `docs/cr117-merge-record`가 올라가 있다. PR #231 병합 뒤 기존 정리 대기 목록에 합류한다.

## 18차 뒤 남은 것 (2026-09-25, 진행 중)

- [x] **P0 = CR-118** — main CI의 FLOW-002 뒤로가기 e2e 실패는 시험의 대기 문제였다. main `0dd7734`, 기록 `9dd2644`.
- [x] **CR-120** — CR-118 기록 PR의 CI에서 드러난 `link-rebuild.test.ts` 정적 대기 경합. main `e181751`.
- [x] **작업 A = CR-119 / WP-103** — main `e94cb4f`, main CI(run 36055350099) success.
- [ ] **병합 기록 PR(CR-119·CR-120)** — 브랜치 `docs/cr119-cr120-merge-record`. 병합됐는지 먼저 실측한다.
- [ ] **작업 B — prs-links 재색인(CR-121·WP-104·FR-ING-008 AC-11 예정, 등록 직전에 다시 잰다).** 지시서 6~7장 그대로다. 사용자 결정(2026-09-25): 해제된 `stacks_on` 이력을 PostgreSQL에 저장하고, 스택 간선은 PG 상태의 전체 쓰기로 바꾸며, 배포 전부터 서비스 인덱스에만 있는 해제 간선은 일회성 가져오기 명령으로 옮긴다. 재현 시험(워크트리 `links-reindex`, 미추적) 두 사례가 수정 전 코드에서 실패한다. 결함 경로와 설계 뼈대는 이어가기 세션 scratchpad의 `b-design.md`.
- [ ] **B의 시험 준비** — `upsertRepository`는 `allowed_team_ids`를 쓰지 않는다. 재현 시험이 `setAllowedTeams`를 불러야 범위 필드 판정이 뜻을 갖는다.
- [ ] **사내(NOT RUN)** — CR-119 RUNBOOK 7.H(수동 v4 전환 확인, 새 빌드에서 prs-commits 재색인, 검증 단계 시간 기록), CR-117 저장소마다 `links apply`.
