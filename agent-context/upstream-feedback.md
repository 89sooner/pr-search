...
## 검색 Merged 필터 · My merged PRs 빈 결과 (버그)

**발견**: 0.1.0-pilot.12 사내 화면, Status=Merged와 「My merged PRs」 탭 (2026-09-17, 결정자 보고)
**현상**: 검색 화면에서 Status 필터를 Merged로 두면 아무것도 나오지 않고, 「My merged PRs」 탭도 결과가 없다고 잘못 나온다.

> **상류 반영 (`CR-101` / `DEV-718`) — 원인은 질의가 아니라 문서였다.** 색인 투영이 GitHub의 원시 `state`(`open`·`closed`)를 그대로 저장하고 병합을 `merged`로 파생하지 않아, `is:merged`(`state = merged`)와 맞는 문서가 하나도 없었다. 같은 전제를 쓰는 「Merged」 배지·PR 상세·M 번호 조회(`not_merged`)·늦은 PR 스냅숏의 채번 재개도 병합을 보지 못하고 있었다. 이제 투영이 병합 신호(`merged`·`merged_at`)가 있으면 `merged`로 파생하고, 마이그레이션 032가 이미 저장된 스냅숏을 같은 규칙으로 바로잡는다.
>
