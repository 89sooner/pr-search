...
export function WorkspaceEntityPage({ repository, prNumber, commitSha, gheBaseUrl }: { repository: string; prNumber?: number; commitSha?: string; gheBaseUrl?: string }): ReactNode {
  ...
  return <section className="repo-main"><a href={`/search?${new URLSearchParams({ repository })}`}>← 통합 검색</a><header className="repo-heading"><h1>{repository} · {prNumber ? `#${prNumber}` : commitSha?.slice(0, 12)}</h1></header>
