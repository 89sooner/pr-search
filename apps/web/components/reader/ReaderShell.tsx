...
export function ReaderShell({ children, user, operator = false, roles = [] }: { children: ReactNode; user: { login: string; email: string | null } | null; operator?: boolean; roles?: readonly Role[] }): ReactNode {
  ...
  return <div className="reader-ui reader-shell">
    ...
    <header className="reader-header"><div className="reader-header-inner">
      ...
      <Dropdown.Root><Dropdown.Trigger className="reader-workspace-menu">Workspace<ChevronDown size={13} /></Dropdown.Trigger><Dropdown.Portal><Dropdown.Content align="end" sideOffset={10} className="reader-ui reader-user-menu">{visibleNavEntries(roles).map((entry, index, entries) => <div key={entry.id}>{entry.section !== entries[index - 1]?.section ? <Dropdown.Label className="reader-menu-section">{SECTION_LABELS[entry.section]}</Dropdown.Label> : null}<Dropdown.Item asChild><Link href={entry.href}>{entry.label}</Link></Dropdown.Item></div>)}</Dropdown.Content></Dropdown.Portal></Dropdown.Root>
      <ThemeToggle />
