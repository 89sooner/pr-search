...
export function RepositoryWorkspace({ login = '', loginPath, gheBaseUrl }: { login?: string; loginPath: string; gheBaseUrl?: string }): ReactNode {
  ...
  function field(key: string, label: string, placeholder = '', type = 'text'): ReactNode {
    return <label className={`repo-field repo-field--${key}`}><span>{label}</span><input type={type} data-reader-search={key === 'q' ? '' : undefined} value={draft[key] ?? ''} placeholder={placeholder} onChange={event => { setDraft(current => ({ ...current, [key]: event.target.value })); }} /></label>;
  }
