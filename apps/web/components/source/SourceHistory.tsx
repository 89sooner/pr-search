...
import { Badge, Banner, Button, CopyButton } from '../ui';
...
function HistoryBody({ repository, path, kind, revision, branch }: { repository: string; path: string; kind: string; revision: string; branch: string }) {
  const [page, setPage] = useState(1); const [pinned, setPinned] = useState(revision); const [commits, setCommits] = useState<SourceHistoryCommit[]>([]);
  ...
  return <section className="source-history" aria-label="Path history" onKeyDown={event => {
    ...
    {isFile ? <p className="source-caption">Select two revisions to compare this file. Folder history includes changes under the selected path. History is path-based; renamed files can be followed from their previous path in Diff.</p> : <Banner tone="info" title="Select a file to enable comparison" role="note">Comparing revisions is only available for a single file, not a folder or the repository root. Select a file in the tree on the left.</Banner>}
    ...
    <div className="source-history-table"><table><thead><tr><th scope="col"><span className="ui-sr-only">Compare</span></th><th scope="col">Revision</th><th scope="col">Change</th><th scope="col">Linked PRs</th><th scope="col">Author</th><th scope="col">Committed</th><th scope="col">Inspect</th></tr></thead><tbody>{commits.map(commit => <tr key={commit.sha} onMouseEnter={() => { setActive(commit.sha); }} onFocusCapture={() => { setActive(commit.sha); }}><td><input type="checkbox" aria-label={`Select ${commit.sha.slice(0, 7)} for comparison`} checked={selected.includes(commit.sha)} disabled={!isFile || (selected.length >= 2 && !selected.includes(commit.sha))} onChange={() => { setSelected(previous => previous.includes(commit.sha) ? previous.filter(sha => sha !== commit.sha) : [...previous, commit.sha]); }} /></td><td><code title={commit.sha}>{commit.sha.slice(0, 9)}</code><CopyButton value={commit.sha} label="Copy SHA" /></td><td><button type="button" className="source-history-title" onClick={() => { setDiff({ repository, commit: commit.sha }); }}>{commit.message.split('\n')[0]}</button></td><td>{commit.pull_request_numbers === null ? <span className="source-pr-pending">Pending</span> : commit.pull_request_numbers.length === 0 ? <span className="source-pr-empty">—</span> : <span className="source-pr-list">{commit.pull_request_numbers.map(number => <span key={number} className="source-pr-chip"><Badge tone="neutral">#{number}</Badge><CopyButton value={String(number)} label={`Copy PR #${number}`} /></span>)}</span>}</td><td>{commit.author}</td><td><time dateTime={commit.date ?? undefined}>{commit.date ? new Date(commit.date).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }) : 'Unknown'}</time></td><td><Button size="sm" variant="ghost" aria-label={`View diff for ${commit.sha.slice(0, 7)}`} onClick={() => { setDiff({ repository, commit: commit.sha }); }}><GitCompareArrows size={15} />Diff</Button></td></tr>)}</tbody></table></div>
