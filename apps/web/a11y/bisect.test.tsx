...
it('C-029 announces candidate count and passes axe in progress', async () => {
  ...
  expect(await screen.findByText('남은 후보 4건 · 예상 잔여 검사 2회')).toHaveAttribute('aria-live', 'polite');
...
it('epoch conflict on mark immediately invalidates the displayed next point', async () => {
  ...
  const good = await screen.findByRole('button', { name: /^정상$/ });
  await userEvent.setup().click(good);
  expect(await screen.findByText('탐색 상태 무효')).toBeVisible();
  expect(screen.queryByTestId('bisect-next')).toBeNull();
  expect(screen.getByRole('button', { name: '탐색 초기화' })).toBeEnabled();
});
