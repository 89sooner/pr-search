...
/**
 * 식별자 해석·상세 조회의 실행 재료 (CR-112).
 *
 * 일반 경로와 PIPE 연동 경로가 **같은 값**으로 세 실행 함수를 부른다. 세션 컨텍스트(`auth`)는 여기
 * 없다 — 주체와 접근 범위는 `ReadInvocation`이 준다.
 */
export interface ResolveExecution {
  readonly deps: ResolveDeps & DetailDeps;
  readonly pool: Pool;
  readonly loginPath: string;
  readonly gheBaseUrl: string | null;
}

export function resolveExecution(options: Omit<ResolveRouteOptions, 'auth'>): ResolveExecution {
  const { loginPath, gheBaseUrl, pool, mergeNumberEnabled = false, ...rest } = options;
  return {
    deps: { ...rest, mergeNumbers: { pool, enabled: mergeNumberEnabled } },
    pool,
    loginPath,
    gheBaseUrl,
  };
...
/** 주체를 확인한다. 실패하면 응답을 보내고 `null` (일반 경로: 세션 검증). */
async function authenticate(
  invocation: ReadInvocation,
  reply: FastifyReply,
  loginPath: string,
): Promise<ReadPrincipal | null> {
  try {
    return await invocation.identify();
  } catch (error) {
    const shape = toAuthError(error, { correlationId: invocation.correlationId, loginPath });
    if (shape !== null) {
...
export function registerResolveRoutes(app: FastifyInstance, options: ResolveRouteOptions): void {
  ...
  const execution = resolveExecution(rest);

  app.get(RESOLVE_PATH, async (request, reply) =>
    executeResolve((request.query ?? {}) as Record<string, unknown>, reply, sessionInvocation(request, auth), execution),
  );
  app.get(COMMIT_PATH, async (request, reply) =>
    executeCommitDetail(
      request.params as { repository?: string; commit_sha?: string },
      reply,
  ...
  app.get(PULL_REQUEST_PATH, async (request, reply) =>
    executePullRequestDetail(
      request.params as { repository?: string; pr_number?: string },
      reply,
      sessionInvocation(request, auth),
      execution,
    ),
...
export async function executeResolve(
  ...
  const { deps, loginPath, gheBaseUrl } = execution;

  const principal = await authenticate(invocation, reply, loginPath);
...
// ---------------------------------------------------------------- API-SRCH-002
export async function executeCommitDetail(
  params: { readonly repository?: string; readonly commit_sha?: string },
  reply: FastifyReply,
  invocation: ReadInvocation,
  execution: ResolveExecution,
): Promise<FastifyReply> {
  const { correlationId } = invocation;
  const { deps, loginPath, pool } = execution;
  ...
  try {
    ...
    await recordView(pool, userId, 'commit', repository, commitSha, detail !== null, correlationId);
    if (detail === null) {
...
// ---------------------------------------------------------------- API-SRCH-003
export async function executePullRequestDetail(
  params: { readonly repository?: string; readonly pr_number?: string },
  reply: FastifyReply,
  invocation: ReadInvocation,
  execution: ResolveExecution,
): Promise<FastifyReply> {
  ...
  try {
    ...
    await recordView(pool, userId, 'pull_request', repository, rawNumber, detail !== null, correlationId);
    if (detail === null) {
      return notFound(reply, correlationId, 'PR을 찾을 수 없습니다');
    }
    return reply.send({ ...detail, correlation_id: correlationId });
