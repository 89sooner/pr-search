 ...
 * 끝난 순간을 멈춤으로 읽는다(DEV-765). 두 소비자가 같은 신호를 다 받은 상태를 `until`로 묻고,
 * 루프는 신호가 멎지 않아 시한에서 던지는 것으로 잡는다.
 */
...
describe('직접 푸시 종단과 전량 재파생 (WP-029 / CR-039)', () => {
  ...
  it('**되먹임이 없다** — 커밋 보강이 자기 신호를 되받아 다시 내지 않는다 (DEV-216)', async () => {
    ...
    try {
      ...
      /*
       * **두 소비자가 같은 신호를 다 받은 상태까지 기다린다** (CR-120, DEV-765).
       *
       ...
       * 멎지 않아 시한에서 던지고, 유한한 되먹임이 있으면 아래 상한 판정이 잡는다.
       */
      await settle(seen, {
        until: async () =>
